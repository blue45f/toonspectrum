...
export async function capturePageEvidence(page: Page, testInfo: TestInfo, name: string): Promise<void> {
  ...
  try {
    for (let index = 0; index < 500; index += 1) {
      ...
      if (frame.y > coveredHeight + 1 || frame.width < documentWidth - 2) {
        throw new Error(`Screenshot coverage gap at ${frame.x}, ${frame.y}`);
      }
      ...
      coveredHeight = Math.min(documentHeight, Math.max(coveredHeight, frame.y + frame.height));
      if (coveredHeight >= documentHeight) break;
      const next = frame.y + frame.height - Math.min(160, Math.floor(frame.height / 3));
      if (next <= targetY) throw new Error("Screenshot scroll made no forward progress");
      targetY = next;
    }
