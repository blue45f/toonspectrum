...
- **플랫폼 커버리지(18개 · 각 ≥100편)**: 공개 카탈로그 크롤러로 수집하는 18개 플랫폼 — 네이버웹툰·네이버시리즈·카카오웹툰·카카오페이지·레진·리디·문피아·조아라·노벨피아·봄툰·탑툰·포스타입·미스터블루·투믹스·북큐브·원스토리·교보문고·예스24. **모든 플랫폼이 최소 100편 이상 등록**됩니다(`scripts/crawlers/<id>.mjs`, `lib/server/catalog-sources.ts`의 `IMPLEMENTED_CRAWLERS`). 로그인/성인 인증/약관을 우회하지 않고 공개 목록 메타데이터만 사용하며, 도달 불가 플랫폼은 빈 슬롯으로 남기지 않고 목록에서 제외했습니다 — **피너툰**(도메인 연결 종료)·**버프툰**(서비스 종료, nc.com 리다이렉트)·**코미코**(서비스는 운영 중이나 크롤 egress에서 IP 차단; 크롤러는 보존, 도달 가능한 egress에서 재활성화 가능).
- **DB 주기 갱신**:
