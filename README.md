...
작품 데이터는 하드코딩 seed가 아니라 크롤러가 만든 검증 스냅샷을 운영 소스로 사용합니다. `lib/data/` seed 모듈은 제거했고, 기본 사용자 경로는 `apps/api/data/catalog.json.gz`를 빌드 시 `public/data/*.json`으로 변환해 Vercel/CDN에서 제공하는 정적 카탈로그입니다. Nest API는 번들 스냅샷을 먼저 로드하고, `WEBDEX_CATALOG_FORCE_DB=1`이거나 번들 파일이 없을 때 DB `catalog_snapshot`으로 폴백합니다. 스냅샷이 없으면 빈 런타임 카탈로그로 시작해 잘못된 하드코딩 데이터가 노출되지 않게 합니다.

```bash
pnpm crawl                       # 크롤러 JSON을 stdout으로 출력(서버 스케줄러용)
pnpm ingest                      # 크롤 후 로컬 DB 스냅샷에 적재(로컬/폴백 API 경로)
...
> DB는 **PostgreSQL**입니다(`lib/db`가 `DATABASE_URL`로 연결, 미설정 시 로컬 docker `:55432` 폴백). 리뷰·커뮤니티·인증 같은 동적 데이터와 카탈로그 ingest/폴백 경로에 사용합니다. 설정은 아래 [실행](#실행)의 "DB 준비"를 참고하세요.
...
- **표지 썸네일**: 핫링크/CORS 회피를 위해 Nest API의 `/api/cover` 프록시를 경유해 표시. 허용 호스트는 플랫폼별 표지 CDN으로 한정합니다 — pstatic·kakaopagecdn·kakaocdn·lezhin·ridicdn·munpia·joara·cloudfront(포스타입)·mrblue·bookcube·onestore·yes24·novelpia·balcony(봄툰)·toptoon·toomics·kyobobook.
...
- **랭킹 갱신성**: 현재 활성 사용자 경로는 `scripts/build-static-catalog.ts`와 `src/catalog-static.ts`가 `disableLive=true`로 계산하는 스냅샷 산식 랭킹입니다. `/api/ranking`도 기본적으로 같은 스냅샷 산식 모드로 동작합니다. `lib/server/live.ts`의 실시간 어댑터와 `WEBTOON_LIVE_*` 환경변수는 보존되어 있지만, 별도 운영 경로로 다시 연결하기 전까지 기본 랭킹에는 외부 실시간 호출을 반영하지 않습니다.
...
> 데이터 갱신: 정적 운영에서는 `pnpm catalog:gen`으로 `public/data/*.json`을 재생성하고 재배포합니다. API 폴백/DB 운영 경로에서는 폴링(`CATALOG_REFRESH_POLL_SECONDS`, 기본 60s)으로 새 스냅샷을 **무중단 핫 리로드**하거나, `POST /api/catalog/refresh`로 즉시 반영합니다. 전체 흐름은 [`docs/data-pipeline.md`](docs/data-pipeline.md) 참고.
