...
작품 데이터는 개발 seed 파일이 아니라 서버 DB 스냅샷을 운영 소스로 사용합니다. `lib/data/titles.ts`는 초기 폴백이며, 운영에서는 Nest API가 크롤러 JSON 결과를 `catalog_snapshot`에 저장하고 메모리 카탈로그를 갱신합니다.

```bash
node scripts/crawl.mjs                 # 개발 seed 파일 재생성
...
lib/                 데이터 모델 · 시드 · 검색/랭킹/추천 로직 · 스토어
  data/              seed/fallback 데이터 (운영 카탈로그는 DB 스냅샷)
