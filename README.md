...
작품 데이터는 파일 seed가 아니라 서버 DB 스냅샷을 운영 소스로 사용합니다. `lib/data/titles.ts`와 `lib/data/reviews.ts`는 제거했고, Nest API가 크롤러 JSON 결과를 `catalog_snapshot`에 저장한 뒤 메모리 카탈로그를 갱신합니다. DB 스냅샷이 없으면 빈 런타임 카탈로그로 시작해 잘못된 하드코딩 데이터가 노출되지 않게 합니다.
