...
const api = {
  ...
  async loseContext() {
    ...
    if (gpu.getContext("webgl2")?.isContextLost()) throw new Error("GPU context was lost before injection");
    await new Promise<void>((resolve, reject) => {
      const canvas = gpu;
      const done = () => { clearTimeout(timer); canvas.removeEventListener("webglcontextlost", done); resolve(); };
      const timer = setTimeout(() => { canvas.removeEventListener("webglcontextlost", done); reject(new Error("GPU loss notification timed out")); }, 5000);
      canvas.addEventListener("webglcontextlost", done, { once: true }); extension.loseContext();
    });
    return present();
  },
  async recover() {
