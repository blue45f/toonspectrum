...
export function hasMeaningfulRouteContent(root: HTMLElement): boolean {
  for (const candidate of root.querySelectorAll(MEANINGFUL_SELECTOR)) {
    if (!candidate.closest("[data-route-semantic-heading], [data-route-recovery], [data-route-loading-fallback]")) return true;
  }
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT);
  let textLength = 0;
  for (let node = walker.nextNode(); node; node = walker.nextNode()) {
    const parent = node.parentElement;
    if (parent?.closest("[data-route-semantic-heading], [data-route-recovery], [data-route-loading-fallback]")) continue;
