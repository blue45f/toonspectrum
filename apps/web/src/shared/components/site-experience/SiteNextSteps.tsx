  ...
  return (
    <section className="site-next-steps" aria-labelledby={headingId} data-testid="site-next-steps">
      ...
      <div className="site-next-steps__heading">
        <div>
          <p className="site-next-steps__eyebrow"><Sparkles size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("shared.components.site.experience.SiteNextSteps", "en", "YOUR NEXT CHAPTER")}</p>
          <h2 id={headingId}>{translateBilingualValueForLocale(locale, "shared.components.site.experience.SiteNextSteps", "영감을, 다음 장면으로.", "Turn inspiration into your next scene.")}</h2>
          <p>{translateBilingualValueForLocale(locale, "shared.components.site.experience.SiteNextSteps", "지금의 발견을 학습과 재료, 새로운 작품으로 이어가세요.", "Connect your discoveries with techniques, materials and new stories.")}</p>
        </div>
        <button type="button" className="site-experience-toggle" aria-pressed={settings.mode === "calm"}
          ...
          title={translateBilingualValueForLocale(locale, "shared.components.site.experience.SiteNextSteps", "배경과 장식 효과를 줄입니다. 시스템의 모션 감소 설정도 존중합니다.", "Reduce decorative effects. System reduced-motion settings are always respected.")}>
          <Sparkles size={15} aria-hidden="true" />{translateBilingualValueForLocale(locale, "shared.components.site.experience.SiteNextSteps", "차분한 화면", "Calm appearance")}
        </button>
      </div>
      <nav className="site-next-steps__grid" aria-label={translateBilingualValueForLocale(locale, "shared.components.site.experience.SiteNextSteps", "다음 활동 추천", "Suggested next destinations")}>
        {destinations.map((id, index) => {
          const destination = EXPERIENCE_DESTINATIONS[id];
          const Icon = ICONS[destination.icon];
          const [title, description] = translateLocaleBranchForLocale(locale, "shared.components.site.experience.SiteNextSteps", destination);
          return (
      ...
      <div className="site-next-steps__drawing">
        <div><span>{translateCurrentStaticSourceText("shared.components.site.experience.SiteNextSteps", "en", "MADE FOR PEOPLE WHO DRAW.")}</span><p>{translateBilingualValueForLocale(locale, "shared.components.site.experience.SiteNextSteps", "영감을 모았다면, 이제 당신의 선으로 그려 보세요.", "You have the inspiration. Make your own mark.")}</p></div>
        <Link href="/studio"><PenTool size={17} aria-hidden="true" />{translateBilingualValueForLocale(locale, "shared.components.site.experience.SiteNextSteps", "전문 드로잉 작업실 열기", "Open the drawing studio")}<ArrowUpRight size={17} aria-hidden="true" /></Link>
      </div>
    </section>
  );
}
