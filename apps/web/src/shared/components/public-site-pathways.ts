...
export const PUBLIC_JOURNEY = [
  { id: "discover", href: "/discover", ko: "영감 찾기", en: "Discover", paths: [...DISCOVER_PURPOSE_PREFIXES, "/research", "/references", "/now", "/news", "/insights", "/opportunities"] },
  { id: "learn", href: "/learn", ko: "기법 익히기", en: "Learn", paths: ["/learn", "/help", "/guide"] },
  { id: "resources", href: "/market", ko: "재료 고르기", en: "Resources", paths: ["/market"] },
  { id: "create", href: "/make", ko: "장면 그리기", en: "Create", paths: ["/make", "/design", "/story-lab"] },
  { id: "share", href: "/showcase", ko: "작품 나누기", en: "Share", paths: ["/showcase", "/create", "/community", "/reviews", "/pencafe"] },
] as const;

/** Segment boundaries prevent /marketplace or /learning from claiming a step. */
export function publicJourneyPhase(pathname: string): PublicJourneyPhase | undefined {
  const normalized = pathname.replace(/\/+$/u, "") || "/";
  return PUBLIC_JOURNEY.find((step) => step.paths.some((prefix) => normalized === prefix || normalized.startsWith(`${prefix}/`)))?.id;
}
