...
export function ComicPortrait({ cast, large = false }: { cast: ComicCastId; large?: boolean }) {
  ...
  return <span className={formatI18nTemplate(translateCurrentStaticSourceText("shared.components.comic.ComicCast", "en", "comic-portrait{v0}"), { v0: String(large ? " comic-portrait-large" : "") })} aria-hidden="true">
...
export function ComicCastPicker({ value, onChange }: { value: ComicCastId; onChange: (id: ComicCastId) => void }) {
  return <fieldset className="comic-cast-picker"><legend>{translateCurrentStaticSourceText("shared.components.comic.ComicCast", "ko", "오늘 함께할 캐릭터")}</legend><div>
