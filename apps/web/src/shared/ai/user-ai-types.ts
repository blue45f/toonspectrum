...
/** One configuration owns all user-supplied or zero-cost AI connections. Never export credentials with works. */
...
function normalizedCostPolicy(
  ...
  if (
    typeof value === "string"
    && USER_AI_COST_POLICIES.includes(value as UserAiCostPolicy)
  ) {
...
export function normalizeUserAiConfiguration(value: unknown): UserAiConfiguration {
  ...
  const connections = raw.connections.map((item): UserAiConnection => {
    if (!item || typeof item !== "object" || Array.isArray(item)) {
      throw new Error("잘못된 AI 연결입니다.");
    }
    const source = item as Record<string, unknown>;
    for (const field of fields) {
      if (
        typeof source[field] !== "string"
        || source[field].length > (field === "apiKey" ? 4096 : 300)
      ) {
        throw new Error("AI 연결 필드를 확인하세요.");
      }
    }
    const stringValues = Object.fromEntries(
      fields.map((field) => [field, (source[field] as string).trim()]),
    ) as unknown as Omit<UserAiConnection, "costPolicy">;
    const connection: UserAiConnection = {
      ...stringValues,
      costPolicy: normalizedCostPolicy(source, stringValues),
    };
    const apiKeyHasControlCharacter = Array.from(connection.apiKey).some((character) => {
      const code = character.charCodeAt(0);
      return code < 32 || code === 127;
    });
    if (
      !/^[A-Za-z0-9_-]{1,80}$/u.test(connection.id)
      || !connection.label
      || apiKeyHasControlCharacter
      || (!connection.apiKey && !isLoopbackAiUrl(connection.baseUrl))
    ) {
      throw new Error("연결 이름과 유효한 사용자 API 키를 입력하세요. 로컬 연결은 API 키를 비워둘 수 있습니다.");
    }
  ...
  for (const capability of USER_AI_CAPABILITIES) {
    ...
    if (
      id !== null
      && (typeof id !== "string" || !connections.some((item) => item.id === id))
    ) {
