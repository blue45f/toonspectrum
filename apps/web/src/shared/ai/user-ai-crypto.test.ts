...
const configuration = {
  ...
  assignments: { text: "personal", image: "personal", inference: null, "three-d": null },
};

describe("encrypted user AI vault", () => {
  it("round-trips credentials without storing the passphrase", async () => {
    const encrypted = await encryptUserAiVault(configuration, "correct horse battery staple");
    expect(encrypted).not.toContain("secret-key");
    expect(encrypted).not.toContain("correct horse battery staple");
    await expect(decryptUserAiVault(encrypted, "correct horse battery staple")).resolves.toEqual(configuration);
  });

  it("fails closed for a wrong password", async () => {
    const encrypted = await encryptUserAiVault(configuration, "correct horse battery staple");
    await expect(decryptUserAiVault(encrypted, "another password value")).rejects.toThrow(/비밀번호/u);
  });
});
