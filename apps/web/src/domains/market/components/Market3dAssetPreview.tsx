...
export function Market3dAssetPreview({ recipe, className }: {
  ...
  return (
    <section aria-labelledby={headingId} aria-describedby={noteId} className={formatI18nTemplate(translateCurrentStaticSourceText("domains.market.components.Market3dAssetPreview", "en", "overflow-hidden rounded-xl border border-line bg-card {v0}"), { v0: String(className ?? "") })}>
      <h2 id={headingId} className="border-b border-line px-4 py-3 text-sm font-semibold text-fg">
        {translateCurrentStaticSourceText("domains.market.components.Market3dAssetPreview", "ko", "3D 에셋 미리보기 (")}{recipe.name})
      </h2>
      ...
      <div className="space-y-2 px-4 py-3 text-xs text-fg-2">
        <p>{translateCurrentStaticSourceText("domains.market.components.Market3dAssetPreview", "ko", "3D 에셋")}</p>
        <p>{Object.keys(recipe.parameters ?? {}).length}{translateCurrentStaticSourceText("domains.market.components.Market3dAssetPreview", "ko", "개 파라미터")}</p>
        <p className="max-w-full break-all font-mono">{recipe.recipeId}</p>
        <p id={noteId} className="leading-relaxed text-fg-3">
          {translateCurrentStaticSourceText("domains.market.components.Market3dAssetPreview", "ko", "검증된 모델에만 실제 렌더 이미지를 표시합니다. 미리보기가 없는 모델을 임의의 도형으로 대체하지 않습니다. Studio에서 선택한 모델을 검증·가져온 뒤 회전·확대하여 확인하세요.")}</p>
      </div>
    </section>
  );
}
