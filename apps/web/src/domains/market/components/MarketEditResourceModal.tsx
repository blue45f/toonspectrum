      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-line bg-card shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between border-b border-line px-5 py-4">
          <h2 id="market-edit-title" className="text-base font-bold text-fg flex items-center gap-2">
            ...
            <span>{translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "에셋 정보 수정 및 새 버전 릴리즈")}</span>
          </h2>
          <button
            type="button"
            onClick={onClose}
          ...
          >
            <X className="size-4" />
            <span className="sr-only">{translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "닫기")}</span>
          </button>
        </div>

        <form onSubmit={handleSubmit} className="max-h-[80vh] overflow-y-auto p-5 space-y-4">
          <div>
            <label htmlFor="edit-asset-name" className="block text-xs font-semibold text-fg">{translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "에셋명")}</label>
            <input
              id="edit-asset-name"
              type="text"
              value={name}
          ...
          <div>
            <label htmlFor="edit-asset-desc" className="block text-xs font-semibold text-fg">{translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "상세 설명")}</label>
            <textarea
              id="edit-asset-desc"
              rows={3}
              value={description}
          ...
          <div>
            <label htmlFor="edit-asset-tags" className="block text-xs font-semibold text-fg">{translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "태그 (쉼표로 구분, 최대 8개)")}</label>
            <input
              id="edit-asset-tags"
              type="text"
              value={tagInput}
              onChange={(e) => setTagInput(e.target.value)}
              placeholder={translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "선화, 펜터치, 3D, 웹툰, 로판")}
              className="mt-1 h-8 w-full rounded-lg border border-line bg-panel px-2.5 text-xs text-fg focus:border-accent focus:outline-none"
            />
          </div>

          <div>
            <label htmlFor="edit-asset-license" className="block text-xs font-semibold text-fg">{translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "사용권 라이선스")}</label>
            <select
          ...
          <div className="rounded-xl border border-accent/40 bg-accent/5 p-3.5 space-y-3">
            <label className="flex items-center gap-2 text-xs font-bold text-fg cursor-pointer select-none">
              ...
              />
              <span>{translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "새 버전으로 판올림 릴리즈 (SemVer Update)")}</span>
            </label>

            {bumpVersion ? (
              <div className="space-y-2.5 pt-1">
                <div className="flex items-center gap-2">
                  <span className="text-xs text-fg-3">{translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "현재 v")}{record.resourceVersion} →</span>
                  <input
                    type="text"
                    value={newVersion}
                    onChange={(e) => setNewVersion(e.target.value)}
                    placeholder="1.1.0"
                    aria-label={translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "새 버전 번호")}
                ...
                <div>
                  <label htmlFor="edit-asset-release-notes" className="block text-[0.7rem] font-semibold text-fg">
                    {translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "릴리즈 노트 (업데이트 변경사항)")}</label>
                  <textarea
                    id="edit-asset-release-notes"
                    rows={2}
                    value={releaseNotes}
                    onChange={(e) => setReleaseNotes(e.target.value)}
                    placeholder={translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "예: 필압 보정 곡선 최적화 및 3D 뷰포트 은선 렌더링 성능 향상")}
          ...
          <div className="flex items-center justify-end gap-2 border-t border-line pt-4">
            ...
            >
              {translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "취소")}</button>
            <button
              type="submit"
            ...
            >
              {translateCurrentStaticSourceText("domains.market.components.MarketEditResourceModal", "ko", "수정 사항 저장")}</button>
          </div>
        </form>
      </div>
    </div>
