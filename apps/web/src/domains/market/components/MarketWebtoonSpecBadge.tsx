    ...
    >
      ...
      {polycountGrade === "ultra-light" ? (
        <span className="inline-flex items-center gap-1 rounded-md border border-emerald-500/30 bg-emerald-500/15 px-2 py-0.5 text-emerald-500">
          ...
          <span>{translateCurrentStaticSourceText("domains.market.components.MarketWebtoonSpecBadge", "ko", "초경량 3D")}</span>
        </span>
      ) : null}
      {polycountGrade === "optimal-webtoon" ? (
        <span className="inline-flex items-center gap-1 rounded-md border border-accent/30 bg-accent/15 px-2 py-0.5 text-accent">
          ...
          <span>{translateCurrentStaticSourceText("domains.market.components.MarketWebtoonSpecBadge", "ko", "웹툰 최적화")}</span>
        </span>
      ) : null}
      {polycountGrade === "mid-poly" ? (
        <span className="inline-flex items-center gap-1 rounded-md border border-line bg-raised px-2 py-0.5 text-fg-2">
          ...
          <span>{translateCurrentStaticSourceText("domains.market.components.MarketWebtoonSpecBadge", "ko", "중밀도 3D")}</span>
        </span>
      ) : null}
      {polycountGrade === "heavy-warning" ? (
        <span className="inline-flex items-center gap-1 rounded-md border border-amber-500/30 bg-amber-500/15 px-2 py-0.5 text-amber-500">
          <AlertTriangle className="size-3" aria-hidden="true" />
          <span>{translateCurrentStaticSourceText("domains.market.components.MarketWebtoonSpecBadge", "ko", "고밀도 (LOD 권장)")}</span>
        </span>
      ) : null}

      {hasLineExtraction ? (
        <span className="inline-flex items-center gap-1 rounded-md border border-line bg-raised px-2 py-0.5 text-fg-2">
          <span>{translateCurrentStaticSourceText("domains.market.components.MarketWebtoonSpecBadge", "ko", "은선 렌더링 지원")}</span>
        </span>
      ) : null}
      ...
      {isNoAiProtected ? (
        <span className="inline-flex items-center gap-1 rounded-md border border-purple-500/30 bg-purple-500/10 px-2 py-0.5 text-purple-500">
          ...
          <span>{translateCurrentStaticSourceText("domains.market.components.MarketWebtoonSpecBadge", "ko", "NoAI 조건 공개")}</span>
        </span>
      ) : null}

      {licenseTier ? (
        <span className="inline-flex items-center rounded-md border border-line/60 bg-panel px-2 py-0.5 text-fg-3">
          {licenseTier === "solo-creator" && translateCurrentStaticSourceText("domains.market.components.MarketWebtoonSpecBadge", "ko", "1인 작가 상업")}
          {licenseTier === "studio-team" && translateCurrentStaticSourceText("domains.market.components.MarketWebtoonSpecBadge", "ko", "스튜디오 팀(5인)")}
          {licenseTier === "corporate-agency" && translateCurrentStaticSourceText("domains.market.components.MarketWebtoonSpecBadge", "ko", "에이전시 법인")}
          {licenseTier === "open-cc0" && translateCurrentStaticSourceText("domains.market.components.MarketWebtoonSpecBadge", "ko", "CC0 공개")}
        </span>
      ) : null}
    </div>
  );
