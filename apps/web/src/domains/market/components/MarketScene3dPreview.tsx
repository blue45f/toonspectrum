...
export function MarketScene3dPreview({ recipe, className }: MarketScene3dPreviewProps) {
  ...
  return (
    <div
      ...
      className={formatI18nTemplate(translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "en", "overflow-hidden rounded-xl border border-line bg-card {v0}"), { v0: String(className ?? "") })}
    >
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line bg-panel/50 px-4 py-2.5">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <h2 id="market-3d-heading" className="min-w-0 break-words text-xs font-semibold text-fg">
            {translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "3D 프리셋 참고 일러스트 (")}{recipe.name})
          </h2>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <span className="inline-flex min-h-6 min-w-0 max-w-full items-center break-all rounded bg-raised px-2 text-[0.65rem] text-fg-3">
            {translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "레시피: ")}{recipe.recipeId}
          </span>
          <div role="radiogroup" aria-label={translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "조명 프리셋")} className="flex rounded-lg border border-line bg-raised/80 p-0.5">
            <button
              type="button"
              role="radio"
              aria-checked={lightPreset === "day"}
              onClick={() => setLightPreset("day")}
              className={formatI18nTemplate(translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "en", "rounded-md px-2 py-1 text-[0.68rem] font-medium transition-colors {v0}"), { v0: String(lightPreset === "day"
                  ? "bg-accent text-on-accent shadow-sm"
                  : "text-fg-3 hover:text-fg") })}
            >
              {translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "주간")}</button>
            <button
              type="button"
              role="radio"
              aria-checked={lightPreset === "sunset"}
              onClick={() => setLightPreset("sunset")}
              className={formatI18nTemplate(translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "en", "rounded-md px-2 py-1 text-[0.68rem] font-medium transition-colors {v0}"), { v0: String(lightPreset === "sunset"
                  ? "bg-accent text-on-accent shadow-sm"
                  : "text-fg-3 hover:text-fg") })}
            >
              {translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "노을")}</button>
            <button
              type="button"
              role="radio"
              aria-checked={lightPreset === "night"}
              onClick={() => setLightPreset("night")}
              className={formatI18nTemplate(translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "en", "rounded-md px-2 py-1 text-[0.68rem] font-medium transition-colors {v0}"), { v0: String(lightPreset === "night"
                  ? "bg-accent text-on-accent shadow-sm"
                  : "text-fg-3 hover:text-fg") })}
            >
              {translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "야간")}</button>
          </div>
        </div>
      </div>
      ...
      <div className="relative flex aspect-[16/9] w-full items-center justify-center bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-raised via-canvas to-panel p-6">
        <svg aria-hidden="true" className="h-full w-full max-w-[360px]" viewBox="0 0 400 300" xmlns="http://www.w3.org/2000/svg">
          <defs>
            ...
            <linearGradient id="wallRight" x1="0" y1="0" x2="1" y2="1">
              <stop
                offset="0%"
                stopColor={lightPreset === "sunset" ? translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "en", "#fbbf24") : lightPreset === "night" ? "#312e81" : "#64748b"}
                stopOpacity="0.4"
              />
              <stop
                offset="100%"
            ...
            <linearGradient id="floor" x1="0" y1="0" x2="1" y2="1">
              ...
              <stop
                offset="100%"
                stopColor={lightPreset === "sunset" ? "#b45309" : lightPreset === "night" ? "#0369a1" : translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "en", "#1d4ed8")}
                stopOpacity="0.4"
              />
            </linearGradient>
          </defs>
          ...
          <polygon points="180,180 230,158 260,172 210,194" fill={lightPreset === "sunset" ? translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "en", "#fb923c") : lightPreset === "night" ? translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "en", "#38bdf8") : "#f59e0b"} opacity="0.8" />
          <polygon points="180,180 210,194 210,218 180,204" fill={lightPreset === "sunset" ? translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "en", "#ea580c") : lightPreset === "night" ? "#0284c7" : "#d97706"} opacity="0.9" />
        ...
        <div className="absolute bottom-3 left-3 flex min-h-6 items-center gap-2 rounded bg-canvas/90 px-2 py-0.5 text-[0.65rem] text-fg shadow-sm backdrop-blur-sm">
          ...
          <span>{translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "자유 카메라 시점")}</span>
        </div>
        <div className="absolute bottom-3 right-3 flex min-h-6 items-center gap-2 rounded bg-canvas/90 px-2 py-0.5 text-[0.65rem] text-fg shadow-sm backdrop-blur-sm">
          ...
          <span>{lightPreset === "day" ? translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "주간 햇살 조명") : lightPreset === "sunset" ? translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "노을빛 골든아워") : translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "네온 야간 조명")}</span>
        </div>
        ...
        {recipe.parameters ? (
          <div className="absolute right-3 top-3 flex min-h-6 items-center gap-1.5 rounded-md bg-canvas/90 px-2.5 py-1 text-[0.68rem] font-medium text-fg shadow-sm backdrop-blur-sm">
            ...
            <span>{Object.keys(recipe.parameters).length}{translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "개 씬 환경변수")}</span>
          </div>
        ) : null}
      </div>

      <p id="market-3d-preview-note" className="border-t border-line bg-panel/30 px-4 py-2 text-[0.68rem] leading-relaxed text-fg-3">
        {translateCurrentStaticSourceText("domains.market.components.MarketScene3dPreview", "ko", "레시피 메타데이터를 설명하기 위한 단순화된 일러스트이며 실제 Studio 렌더 결과가 아닙니다.")}</p>
    </div>
  );
}
