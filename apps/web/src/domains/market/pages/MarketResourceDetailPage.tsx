  ...
  return (
    <Container size="wide" className="py-7 sm:py-10">
      ...
      >
        ...
        {translateCurrentStaticSourceText("domains.market.pages.MarketResourceDetailPage", "ko", "탐색 결과로 돌아가기")}</Link>

      {loading ? (
        <div className="mt-6">
          <p role="status" className="sr-only">{translateCurrentStaticSourceText("domains.market.pages.MarketResourceDetailPage", "ko", "마켓 리소스 상세 정보를 불러오는 중입니다.")}</p>
        ...
        <div className="mt-8 rounded-xl border border-dashed border-line bg-panel p-12 text-center">
          <p className="text-sm font-medium text-fg">{translateCurrentStaticSourceText("domains.market.pages.MarketResourceDetailPage", "ko", "리소스를 찾을 수 없어요")}</p>
          <p className="mx-auto mt-1.5 max-w-sm text-sm text-fg-2">{translateCurrentStaticSourceText("domains.market.pages.MarketResourceDetailPage", "ko", "배포자가 비공개로 전환했거나 주소가 잘못되었을 수 있어요. 마켓에서 비슷한 리소스를 찾아보세요.")}</p>
          <Link href="/market/browse" className={buttonClass({ variant: "outline", size: "sm", className: "mt-4" })}>{translateCurrentStaticSourceText("domains.market.pages.MarketResourceDetailPage", "ko", "다른 리소스 찾아보기")}</Link>
        </div>
      ) : error || !record ? (
        <div role="status" className="mt-8 rounded-xl border border-warn/40 bg-warn/10 p-10 text-center">
          <p className="text-sm font-medium text-fg">{translateCurrentStaticSourceText("domains.market.pages.MarketResourceDetailPage", "ko", "지금은 리소스 정보를 불러올 수 없어요")}</p>
          <p className="mx-auto mt-1.5 max-w-sm text-sm text-fg-2">{translateCurrentStaticSourceText("domains.market.pages.MarketResourceDetailPage", "ko", "현재 작업이나 내 리소스에는 영향을 주지 않습니다. 네트워크 상태를 확인한 뒤 다시 시도해 주세요.")}</p>
          <button type="button" onClick={reload} className={buttonClass({ variant: "outline", size: "sm", className: "mt-4" })}>{translateCurrentStaticSourceText("domains.market.pages.MarketResourceDetailPage", "ko", "다시 시도")}</button>
        </div>
      ) : (
        <div className="mt-6 space-y-6">
          <FriendlyQuickGuide
            title={translateCurrentStaticSourceText("domains.market.pages.MarketResourceDetailPage", "ko", "이 리소스를 쓰기 전에 3가지만 확인하세요")}
            description={translateCurrentStaticSourceText("domains.market.pages.MarketResourceDetailPage", "ko", "좋아 보이는 리소스라도 프로젝트 조건과 사용권이 맞아야 안전하게 사용할 수 있습니다.")}
            steps={[
