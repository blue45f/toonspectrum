    ...
    <div>
      <section className="border-b border-line bg-ledger">
        <Container size="wide" className="py-7 sm:py-10">
          ...
          <div className="mt-6 grid gap-7 xl:grid-cols-[minmax(0,1.08fr)_minmax(24rem,0.92fr)] xl:items-center">
            <div>
              <p className="eyebrow text-accent">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "en", "TOONSTUDIO / WEBTOON MATERIALS")}</p>
              <h1 className="mt-4 text-pretty text-[clamp(2.4rem,5vw,4.5rem)] font-bold leading-[1.15] tracking-[-0.055em] text-fg">
                {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "웹툰의 한 컷을,")}<br />{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "더 깊게 만드는 재료.")}</h1>
              <p className="mt-5 max-w-xl text-pretty text-base leading-relaxed text-fg-2 sm:text-lg">
                {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "소재 마켓에서 내 작품의 선, 색, 배경을 찾아보세요.")}</p>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-fg-3">
                {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "템플릿으로 장면을 시작하고, 2D·3D 에셋을 배치하고, 브러시와 색·보정 리소스로 마무리하세요. 파일 형식보다 지금 만들고 싶은 결과에서 시작합니다.")}</p>
              <div className="mt-5 flex flex-wrap items-center gap-2.5 border-t border-line pt-4 sm:mt-7 sm:pt-5">
                <Link href="/market/browse" className={buttonClass({ variant: "solid", size: "md" })}>
                  ...
                  {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "소재 찾기")}</Link>
                <Link
                  href="/market/publish"
                ...
                >
                  ...
                  {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "소재 공유하기")}</Link>
                <Link
                  href="/studio?assetMarket=community&communityView=share"
                  className={buttonClass({ variant: "outline", size: "md" })}
                >
                  <Upload className="h-4 w-4" aria-hidden="true" />
                  {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "Studio에서 공유")}</Link>
                <span className="rounded-full bg-good/15 px-2.5 py-1 text-xs font-medium text-good">
                  {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "현재 모든 소재 무료")}</span>
              </div>
            </div>
          ...
          <FriendlyQuickGuide
            className="mt-5"
            title={translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "처음이라면 종류보다 하고 싶은 작업부터 고르세요")}
            description={translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "템플릿·에셋·브러시는 적용 방식이 서로 다릅니다. 상세 화면에서 실제 사용 위치와 호환성을 먼저 보여드립니다.")}
            steps={[
            ...
            actionLabel={translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "전체 소재 둘러보기")}
      ...
      <section className="border-y border-line bg-card/40 py-8 sm:py-10">
        <Container size="wide">
          <div className="flex items-baseline justify-between gap-3">
            <div>
              <p className="eyebrow text-accent">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "en", "Webtoon Collections")}</p>
              <h2 className="mt-1 text-lg font-bold text-fg sm:text-xl">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "장르·제작 목적 컬렉션")}</h2>
              <p className="mt-1 text-xs leading-5 text-fg-3">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "장면 예시에서 출발해 관련 태그의 리소스를 탐색하세요. 이미지는 테마를 설명하기 위한 예시입니다.")}</p>
            </div>
            <Link
              href="/market/browse"
              className="inline-flex min-h-11 items-center text-xs font-semibold text-accent hover:text-accent-2"
            >
              {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "전체 보기 →")}</Link>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-line/60 pt-2.5 text-[0.7rem] font-medium text-fg-2">
                    <span className="font-semibold text-accent">#{theme.tag} {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "소재 찾기")}</span>
                    <span className="flex items-center gap-1 text-fg-3 transition-transform group-hover:translate-x-1">
                      {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "보러가기 ")}<ArrowRight className="size-3" aria-hidden="true" />
                    </span>
                  </div>
                </Link>
              );
          ...
          <div className="flex items-baseline justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold text-fg sm:text-lg">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "3D 배경·데생 추천")}</h2>
              </div>
              <p className="mt-1 text-xs leading-5 text-fg-3">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "카메라를 돌려 구도를 잡고 캔버스로 가져올 수 있는 3D 리소스입니다.")}</p>
            </div>
            <Link href="/market/browse?kind=3d-asset" className="inline-flex min-h-11 items-center text-xs font-semibold text-accent hover:text-accent-2">
              {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "3D 전체 보기 →")}</Link>
          </div>
      ...
      {popularTags.length >= 3 ? (
        <Container size="wide" className="pb-10 sm:pb-12">
          <h2 className="eyebrow text-fg-3">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "지금 많이 쓰는 키워드")}</h2>
          <ul className="mt-3 flex flex-wrap gap-1.5">
            {popularTags.map((tag) => (
              <li key={tag}>
                <Link
                  href={formatI18nTemplate(translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "en", "/market/browse?tag={v0}"), { v0: String(encodeURIComponent(tag)) })}
      ...
      <Container size="wide" className="pb-10 sm:pb-12">
        <div className="flex items-baseline justify-between gap-3">
          <div>
            <h2 className="eyebrow text-fg-3">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "최근 공유")}</h2>
            <p className="mt-1 text-xs leading-5 text-fg-3">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "최근 공개된 리소스를 실제 미리보기와 함께 확인합니다.")}</p>
          </div>
        ...
        {hasFatalLatestError ? (
          <div role="alert" className="mt-6 rounded-2xl border border-warn/30 bg-warn/5 p-8 text-center sm:p-10">
            ...
            <h3 className="mt-3 text-base font-bold text-fg">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "최근 공유 리소스를 불러올 수 없어요")}</h3>
            <p className="mx-auto mt-1.5 max-w-sm text-sm text-fg-2">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "일시적인 네트워크 문제이거나 서버 장애일 수 있어요. 다시 시도해도 다른 작업에는 영향을 주지 않습니다.")}</p>
            <button type="button" onClick={latest.reload} className={buttonClass({ variant: "outline", size: "sm", className: "mt-4" })}>
              <RefreshCw className="mr-1.5 size-3.5" aria-hidden="true" />
              {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "다시 시도")}</button>
          </div>
        ) : null}
        {latest.stale ? (
          <StaleNoticeBar
        ...
        {hasFatalLatestError ? null : (
          <>
            {latest.loading ? <p role="status" className="sr-only">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "최근 공유된 마켓 리소스를 불러오는 중입니다.")}</p> : null}
            ...
            {!latest.loading && latest.items.length === 0 ? (
              <div className="mt-6 rounded-2xl border border-dashed border-line bg-panel p-8 text-center sm:p-10">
                ...
                <h3 className="mt-3 text-base font-bold text-fg">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "아직 공유된 리소스가 없어요")}</h3>
                <p className="mx-auto mt-1.5 max-w-md text-sm text-fg-2">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "Studio에서 만든 템플릿, 브러시, 팔레트와 에셋을 가장 먼저 공유해 보세요.")}</p>
                <div className="mt-5 flex justify-center">
                  <Link href="/studio?assetMarket=community&communityView=share" className={buttonClass({ variant: "solid", size: "sm" })}>
                    ...
                    {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "Studio에서 첫 리소스 공유하기")}</Link>
                </div>
              </div>
            ) : null}
          </>
      ...
      <Container size="wide" className="pb-14">
        <section className="market-production-route mb-12" aria-labelledby="market-next-step-title">
          <div><span className="eyebrow text-accent">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "en", "MATERIALS INTO YOUR NEXT PANEL")}</span><h2 id="market-next-step-title" className="mt-3">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "재료를 골랐다면,")}<br />{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "이제 내 원고에 맞춰보세요.")}</h2><p>{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "마음에 드는 소재를 모으고 제작 조건을 비교하세요. 선화·채색이 막히는 순간에는 학습 과정을, 장면의 근거가 필요할 때에는 리서치 데스크를 이어서 활용할 수 있습니다.")}</p></div>
          <nav aria-label={translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "리소스 선택 다음 작업")}><Link href="/market/library">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "내 리소스에서 작업 재료 정리 ")}<ArrowRight size={16} aria-hidden="true" /></Link><Link href="/learn/paths/visual-finish">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "선과 색의 완성도를 높이는 실습 ")}<ArrowRight size={16} aria-hidden="true" /></Link><Link href="/research/assets">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "복식·소품·배경 레퍼런스 찾기 ")}<ArrowRight size={16} aria-hidden="true" /></Link><Link href="/studio">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "ToonStudio에서 다음 컷 그리기 ")}<ArrowRight size={16} aria-hidden="true" /></Link></nav>
        </section>
        <div>
          <h2 className="eyebrow text-fg-3">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "사용권 안내")}</h2>
          <p className="mt-1 text-xs leading-5 text-fg-3">{translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "무료 여부와 별개로 상업 이용, 수정, 출처 표기 조건을 확인하세요.")}</p>
        </div>
          ...
          {MARKET_LICENSES.map((license) => (
            <li key={license.license} className="rounded-xl border border-line bg-card p-4 transition-colors hover:border-accent/30 hover:bg-raised/60">
              ...
              <a
                ...
                target={license.url ? translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "en", "_blank") : undefined}
                rel={license.url ? translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "en", "noreferrer") : undefined}
              ...
              >
                {translateCurrentStaticSourceText("domains.market.pages.MarketHomePage", "ko", "사용권 전문 보기")}{license.url ? " ↗" : ""}
              </a>
            </li>
          ))}
        </ul>
