...
const calendarSchema = z.object({ kind: z.literal("calendar"), month: z.string(), status: statusSchema, reason: reasonSchema.optional(), source: z.enum(["local", "kasi"]), checkedAt: z.string().optional(), policyRevision: z.string().max(100),
...
export function FortuneEnrichment({ reading }: { reading: FortuneReading }) {
  ...
  const cancel = () => { sequence.current += 1; abort.current?.abort(); setBusy(false); setNotice(""); };
  const load = async () => {
    if (busy) return;
    abort.current?.abort(); const controller = new AbortController(); abort.current = controller;
    const request = ++sequence.current; setBusy(true); setNotice("");
    ...
    try {
      ...
      const response = await fetch(`/api/fortune/${month ? "calendar" : "horoscope"}?${query}`, { signal: controller.signal, cache: "no-store", credentials: "omit" });
  ...
  const external = horoscope && (horoscope.status === "external" || horoscope.status === "external-cache");
  return <section className="fo-calendar-section" aria-label={month ? "달력 데이터 대조" : "별자리 기간별 읽기"} aria-busy={busy}>
    ...
    <button type="button" className="fo-button" disabled={busy || Boolean(external) || calendar?.source === "kasi"} onClick={() => { void load(); }}>{busy ? "추가 데이터 확인 중…" : month ? "공공 데이터로 대조하기" : "외부 추가 읽기 확인"}</button>
    {busy && <button type="button" className="fo-button" onClick={cancel}>확인 취소</button>}
    <div role="status" aria-live="polite">{notice || (reason ? `${reasons[reason]} 기본 로컬 해석은 계속 사용할 수 있어요.` : "")}</div>
    {calendar?.source === "kasi" && <div><p>{mismatches.length ? `${mismatches.length}일에 차이가 있어요. 기존 달력은 변경하지 않았습니다.` : `${calendar.checks.length}일의 음력 날짜·윤달·요일 대조 일치`}</p>
