...
export function FortuneInteractiveDeck({ value, onChange, three, size = 22 }: FortuneInteractiveDeckProps) {
  const [order, setOrder] = useState(() => Array.from({ length: size }, (_, i) => i));
  const [shuffleCount, setShuffleCount] = useState(0);
  const shuffle = () => { setOrder(shuffleFortuneDeck(Math.random, size)); setShuffleCount((count) => count + 1); onChange(null); };
  return <fieldset className="fo-tarot-picker fo-deck">
    <legend>마음이 가는 카드 뒷면을 골라 주세요</legend>
    <div className="fo-deck-actions"><p>{three ? (size === 78 ? "지금의 장면·다른 관점·작은 실천의 세 장이 펼쳐져요." : "한 장을 고르면 과거·현재·미래의 세 장이 펼쳐져요.") : `${size}장 중, 오늘 마음에 닿는 한 장.`}<small>카드 번호가 해석을 결정해요. 같은 날·같은 번호는 같은 결과입니다.</small></p><button type="button" className="fo-button" onClick={shuffle}><Shuffle size={16} />카드 섞기</button></div>
    <div className="fo-deck-track" key={shuffleCount} aria-label={`${size}장의 카드 선택`}>
