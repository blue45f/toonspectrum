...
export function FortuneReadingView({ reading }: { reading: FortuneReading }) {
  const [mode, setMode] = useState<"story" | "report">(reading.calendar || reading.terms ? "report" : "story");
  ...
  return <article className="fo-report" aria-label={`${reading.title} 결과`}>
    ...
    {mode === "story" && <FortuneStoryReader reading={reading} />}
    ...
    <FortuneCreativeMission reading={reading} />
