...
export function readFortunePreferences(): FortunePreferences {
  try {
...
export function writeFortunePreferences(value: FortunePreferences): boolean {
  try { localStorage.setItem(KEY, JSON.stringify({ favorites: value.favorites.slice(0, 29), notebook: value.notebook.slice(0, 12) })); return true; } catch { return false; }
}
export function clearFortunePreferences(): boolean {
