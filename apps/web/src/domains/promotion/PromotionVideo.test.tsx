...
describe("promotion video privacy boundary", () => {
  it("does not contact the provider before an explicit click", () => {
    const { container } = render(<PromotionVideo url="https://youtu.be/dQw4w9WgXc" title="작품" />);
    expect(container.querySelector("iframe")).toBeNull();
    fireEvent.click(screen.getByRole("button", { name: /홍보 영상 보기/u }));
    const iframe = container.querySelector("iframe");
    expect(iframe?.getAttribute("src")).toBe("https://www.youtube-nocookie.com/embed/dQw4w9WgXc");
  ...
  it("does not render arbitrary iframe HTML or lookalike provider hosts", () => {
    const { container } = render(<PromotionVideo url="https://youtube.com.evil.example/watch?v=dQw4w9WgXc" title="작품" />);
    expect(container.childElementCount).toBe(0);
  });
  it("requires fresh consent when the video changes", () => {
    const view = render(<PromotionVideo url="https://youtu.be/dQw4w9WgXc" title="작품" />);
    fireEvent.click(screen.getByRole("button", { name: /홍보 영상 보기/u }));
