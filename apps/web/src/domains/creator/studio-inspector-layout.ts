...
function action(
  ...
  return { id, label, description, keywords, route, kind, path, ...(focusTarget ? { focusTarget } : {}) };
}
...
const ALWAYS_AVAILABLE_ACTIONS: readonly StudioInspectorAction[] = [
  ...
  action("canvas", "캔버스 설정", "배경, 높이, 여백, 그리드와 가이드를 조절합니다.", ["canvas", "배경", "높이", "gutter", "그리드", "가이드"], { primary: "document", document: "canvas" }, "panel", "문서 › 캔버스"),
  action("grade", "페이지 색보정", "페이지 전체의 밝기, 대비, 채도와 무드를 조절합니다.", ["grade", "color", "밝기", "대비", "채도", "무드", "비네트"], { primary: "document", document: "grade" }, "panel", "문서 › 색보정"),
  action("navigator", "미니맵·탐색", "긴 웹툰 페이지의 현재 위치를 확인하고 이동합니다.", ["navigator", "minimap", "미니맵", "이동", "스크롤"], { primary: "document", document: "navigator" }, "panel", "문서 › 미니맵"),
  action("publish", "작품 정보", "초안 저장과 게시에 공통으로 쓰는 제목, 설명과 태그를 입력합니다.", ["publish", "게시", "작품", "제목", "설명", "태그", "업로드"], { primary: "publish" }, "panel", "게시 준비 › 작품 정보"),
  action("canvas-resize", "캔버스 크기", "페이지 높이와 여백, 크기 변경 방식을 조절합니다.", ["canvas", "resize", "height", "캔버스", "높이", "크기", "여백"], { primary: "document", document: "canvas" }, "property", "문서 › 캔버스 › 크기", "canvas.resize"),
  action("canvas-guides", "가이드와 스냅", "사용자 가이드, 웹툰 가이드와 맞춤 동작을 설정합니다.", ["guide", "snap", "grid", "가이드", "스냅", "그리드", "맞춤"], { primary: "document", document: "canvas" }, "property", "문서 › 캔버스 › 가이드", "canvas.guide-lines"),
  action("canvas-style", "용지와 캔버스 스타일", "배경색, 종이 질감과 웹툰 테마를 조절합니다.", ["paper", "style", "background", "용지", "종이", "질감", "배경", "테마"], { primary: "document", document: "canvas" }, "property", "문서 › 캔버스 › 스타일", "canvas.style"),
];

const IMAGE_ACTIONS: readonly StudioInspectorAction[] = [
  action("image-quick", "이미지 빠른 수정", "레이어 복원, 배경 제거, AI 채색, 팔레트와 기본 보정을 엽니다.", ["image", "quick", "레이어 복원", "분리", "배경 제거", "ai", "채색", "팔레트", "보정"], { primary: "properties", image: "quick" }, "tool", "대상 › 이미지 › 빠른 수정"),
  action("image-fill", "채우기·선화", "참조 레이어 채우기와 선화 정리를 엽니다.", ["fill", "bucket", "paint", "채우기", "선화", "틈 닫기", "참조"], { primary: "properties", image: "fill" }, "tool", "대상 › 이미지 › 채우기·선화"),
  ...
  action("image-mask", "원본 유지하고 가리기", "레이어 마스크", ["mask", "레이어 마스크", "마스크", "비파괴", "반전", "페인팅", "가리기"], { primary: "properties", image: "mask" }, "tool", "선택 항목 › 이미지 › 원본 유지하고 가리기"),
];

export function studioInspectorActions(context: StudioInspectorActionContext): readonly StudioInspectorAction[] {
  const contextual: StudioInspectorAction[] = [];
  if (context.hasSelection) {
    contextual.push(action("selection-properties", "선택 요소 속성", "선택한 요소의 기본, 배치와 스타일 설정을 엽니다.", ["properties", "inspector", "속성", "선택", "배치", "스타일"], { primary: "properties" }, "property", "대상 › 선택 요소"));
    if (context.selectedType === "text") {
      contextual.push(action("text-fill", "글자 채우기 스타일", "글자색과 그라디언트, 패턴 채우기를 설정합니다.", ["text fill", "font color", "글자색", "채우기", "그라디언트", "패턴"], { primary: "properties" }, "property", "대상 › 글자 › 채우기", "element.text-fill"));
    }
    if (context.selectedType === "text" || context.selectedType === "bubble") {
      contextual.push(
        action("typography", "글꼴", "글꼴, 크기, 굵기와 기울임을 고릅니다. 외곽선·그림자는 외형, 곡선 텍스트는 고급 조판에 있습니다.", ["typography", "font", "타이포그래피", "글꼴", "폰트", "크기", "굵게", "기울임"], { primary: "properties" }, "property", "대상 › 글자 › 글꼴", "element.typography"),
        action("text-align", "문단 · 정렬과 자간·행간", "가로 정렬, 세로 쓰기, 자간·행간과 말풍선 맞춤을 설정합니다.", ["align", "vertical", "letter spacing", "line height", "정렬", "왼쪽", "가운데", "오른쪽", "세로 쓰기", "자간", "행간", "문단"], { primary: "properties" }, "property", "대상 › 글자 › 문단", "element.text-align"),
      );
    }
    contextual.push(action("selection-layout", "위치·크기", "선택 요소의 위치, 크기, 회전과 배치 제약을 편집합니다.", ["layout", "position", "size", "rotation", "x", "y", "width", "height", "위치", "크기", "회전", "배치"], { primary: "properties" }, "property", "대상 › 배치", "selection.geometry"));
    if (context.selectedType !== null) {
      contextual.push(action("selection-order-align", "순서·캔버스 정렬", "앞뒤 순서, 페이지 정렬, 복제와 삭제를 관리합니다.", ["order", "align", "arrange", "캔버스 정렬", "정렬", "순서", "앞으로", "뒤로", "복제"], { primary: "properties" }, "property", "대상 › 순서·캔버스 정렬", "element.order-align"));
    }
  ...
  if (context.imageToolsAvailable === true || context.selectedType === "image" || context.selectedType === "draw") contextual.push(...IMAGE_ACTIONS);
...
export function filterStudioInspectorActions(
  ...
  return actions.filter((entry) => studioSearchTextMatches(query, [entry.label, entry.description, ...entry.keywords]));
}
