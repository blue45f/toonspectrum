  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between gap-2">
        <p className="text-[0.66rem] font-semibold text-fg-3 uppercase tracking-wider">{translateCurrentStaticSourceText("domains.creator.StudioGrainPanel", "ko", "그레인/텍스처 (Grain)")}</p>
        <button
          type="button"
          onClick={onReset}
          disabled={isIdentity}
          className={buttonClass({ size: "sm", variant: "quiet" })}
          title={translateCurrentStaticSourceText("domains.creator.StudioGrainPanel", "ko", "그레인/텍스처를 제거하고 원본으로 되돌립니다.")}
        >
          <RotateCcw className="size-3.5" />
          {translateCurrentStaticSourceText("domains.creator.StudioGrainPanel", "ko", "원본으로")}</button>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {GRAIN_TYPES.map((t) => (
          <StudioPanelChip
            ...
            title={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.StudioGrainPanel", "ko", "질감을 \"{v0}\"로 바꿉니다."), { v0: String(t.label) })}
          >
            {t.label}
          </StudioPanelChip>
        ))}
      ...
      <div className="space-y-2">
        ...
        <StudioSliderRow
          label={translateCurrentStaticSourceText("domains.creator.StudioGrainPanel", "ko", "크로마")}
      ...
      <div className={PANEL_LABEL_ROW}>
        {translateCurrentStaticSourceText("domains.creator.StudioGrainPanel", "ko", "시드(Seed)")}<span className="flex items-center gap-1.5">
          <StudioPanelChip
            onClick={() => onPatch({ seed: nextSeed(value.seed) })}
            title={translateCurrentStaticSourceText("domains.creator.StudioGrainPanel", "ko", "노이즈 패턴을 결정적으로 다시 뽑습니다(같은 시드는 같은 무늬).")}
          >
            {translateCurrentStaticSourceText("domains.creator.StudioGrainPanel", "ko", "새 시드")}</StudioPanelChip>
