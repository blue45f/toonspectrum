...
export function CreatorEcosystemPage() {
  ...
  return (
    <Container size="wide" className="py-7 sm:py-10">
      ...
      <section className="mt-8"><SectionHeading icon={BookOpen} eyebrow="STARTER STORIES" title="완성형 샘플 작품" description="자체 제작한 이야기 구조와 장면 조합입니다. 원작 복제가 아닌 편집 가능한 연습 출발점으로 사용합니다." />
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{SAMPLE_WORKS.map((work) => <article key={work.id} className={CARD}><span className="text-[0.65rem] font-black text-accent">{work.genre}</span><h3 className="mt-2 font-black text-fg">{work.title}</h3><p className="mt-2 text-xs leading-5 text-fg-3">{work.summary}</p><div className="mt-3 flex flex-wrap gap-1">{work.scenePackIds.map((id) => <span key={id} className="rounded-full bg-raised px-2 py-1 text-[0.62rem] text-fg-2">{SCENE_PACKS.find((pack) => pack.id === id)?.title}</span>)}</div><button type="button" className={`${BUTTON} mt-4 w-full`} onClick={() => { setState((current) => ({ ...current, installedScenePackIds: [...new Set([...current.installedScenePackIds, ...work.scenePackIds])] })); setNotice(`${work.title}에 필요한 장면 팩을 작업대에 추가했습니다.`); }}><PackagePlus size={15} /> 연습 구성 추가</button></article>)}</div>
      </section>

      <section className="mt-10"><SectionHeading icon={PackagePlus} eyebrow="SCENE RECIPES" title="장면·연기·배경 팩" description="낱개 소재 수가 아니라 바로 수정 가능한 장면 목적, 카메라, 포즈, 표정, 소품을 묶어서 제공합니다." />
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{SCENE_PACKS.map((pack) => { const installed = state.installedScenePackIds.includes(pack.id); return <article key={pack.id} className={CARD}><h3 className="font-bold text-fg">{pack.title}</h3><ul className="mt-2 space-y-1 text-xs text-fg-3">{pack.includes.map((item) => <li key={item}>· {item}</li>)}</ul><button type="button" aria-pressed={installed} className={`${BUTTON} mt-4 w-full ${installed ? "border-good/40 bg-good/10 text-good" : ""}`} onClick={() => togglePack(pack.id)}>{installed ? <CheckCircle2 size={15} /> : <PackagePlus size={15} />}{installed ? "작업대에 추가됨" : "작업대에 추가"}</button></article>; })}</div>
      </section>

      <section className="mt-10"><SectionHeading icon={WandSparkles} eyebrow="GUIDED PRACTICE" title="실제 완료를 기준으로 하는 안내형 실습" description="화면을 읽었다고 끝내지 않고 저장·내보내기·재검사까지 체크합니다." />
        <div className="grid gap-3 lg:grid-cols-3">{GUIDED_LESSONS.map((lesson) => { const completed = state.lessonSteps[lesson.id] ?? []; return <article key={lesson.id} className={CARD}><h3 className="font-bold text-fg">{lesson.title}</h3><p className="mt-1 text-xs text-fg-3">{completed.length}/{lesson.steps.length} 완료</p><div className="mt-3 grid gap-1">{lesson.steps.map((step) => <label key={step} className="flex min-h-11 items-center gap-2 rounded-lg px-2 text-sm text-fg-2 hover:bg-raised"><input type="checkbox" checked={completed.includes(step)} onChange={() => toggleLessonStep(lesson.id, step)} />{step}</label>)}</div></article>; })}</div>
      </section>

      <section className="mt-10"><SectionHeading icon={ScanSearch} eyebrow="FIXABLE PREFLIGHT" title="검수 → 위치 → 안전 수정 → 재검사" description="형식 오류와 편집 제안을 구분합니다. 제목 공백·태그 중복만 자동 수정하고 에셋 누락·권리·승인은 그대로 차단합니다." />
        <div className="grid gap-4 lg:grid-cols-2"><div className={CARD}><label className="grid gap-2 text-sm font-bold text-fg">원고 점검 JSON<textarea className={`${INPUT} min-h-72 font-mono text-xs`} value={preflightSource} onChange={(event) => setPreflightSource(event.target.value)} /></label><div className="mt-3 flex flex-wrap gap-2"><button type="button" className={BUTTON} onClick={runPreflightFromSource}><ClipboardCheck size={15} /> 점검</button><button type="button" className={BUTTON} onClick={applySafeFixes} disabled={!preflightDocument}>안전 수정 적용</button><button type="button" className={BUTTON} onClick={() => preflightDocument && downloadJson("toonstudio-preflight-fixed.json", preflightDocument)} disabled={!preflightDocument}><Download size={15} /> 수정본</button></div></div><div className={CARD}><h3 className="font-bold text-fg">점검 결과</h3>{!findings.length ? <p className="mt-3 text-sm text-fg-3">점검을 실행하거나 현재 오류가 없습니다.</p> : <ul className="mt-3 grid gap-2">{findings.map((finding, index) => <li key={`${finding.code}-${index}`} className={`rounded-lg border p-3 text-xs ${finding.severity === "error" ? "border-bad/40 bg-bad/10" : "border-warn/40 bg-warn/10"}`}><strong>{finding.code}</strong><p className="mt-1">{finding.message}</p><code className="mt-1 block text-fg-3">{finding.path}</code></li>)}</ul>}</div></div>
      </section>

      <section className="mt-10"><SectionHeading icon={Sparkles} eyebrow="STORY CONTINUITY" title="회차별 설정 변경 영향" description="캐릭터·의상·소품·장소 값을 회차 순서로 비교하고, 설명 없는 변경만 경고합니다." />
        <div className="grid gap-4 lg:grid-cols-[0.8fr_1.2fr]"><div className={`${CARD} grid gap-2`}><input className={INPUT} inputMode="numeric" value={continuityDraft.episode} onChange={(event) => setContinuityDraft((current) => ({ ...current, episode: event.target.value }))} aria-label="회차" placeholder="회차" /><input className={INPUT} value={continuityDraft.entity} onChange={(event) => setContinuityDraft((current) => ({ ...current, entity: event.target.value }))} aria-label="대상" /><input className={INPUT} value={continuityDraft.field} onChange={(event) => setContinuityDraft((current) => ({ ...current, field: event.target.value }))} aria-label="설정 필드" /><input className={INPUT} value={continuityDraft.value} onChange={(event) => setContinuityDraft((current) => ({ ...current, value: event.target.value }))} aria-label="값" /><textarea className={INPUT} value={continuityDraft.transitionReason} onChange={(event) => setContinuityDraft((current) => ({ ...current, transitionReason: event.target.value }))} aria-label="변경 이유" placeholder="변경 이유가 있으면 입력" /><button type="button" className={BUTTON} onClick={addContinuityFact}>설정 기록 추가</button></div><div className={CARD}><div className="max-h-64 overflow-auto"><table className="w-full text-left text-xs"><thead><tr className="text-fg-3"><th>회차</th><th>대상</th><th>필드</th><th>값</th><th>설명</th></tr></thead><tbody>{state.continuityFacts.map((fact) => <tr key={fact.id} className="border-t border-line"><td className="py-2">{fact.episode}</td><td>{fact.entity}</td><td>{fact.field}</td><td>{fact.value}</td><td>{fact.transitionReason || "—"}</td></tr>)}</tbody></table></div>{continuityIssues.length ? <ul className="mt-3 grid gap-2">{continuityIssues.map((issue) => <li key={`${issue.previousId}-${issue.currentId}`} className="rounded-lg border border-warn/40 bg-warn/10 p-2 text-xs">{issue.message}</li>)}</ul> : <p className="mt-3 text-xs text-good">설명 없는 설정 충돌이 없습니다.</p>}</div></div>
      </section>

      <section className="mt-10"><SectionHeading icon={Languages} eyebrow="LOCALIZATION" title="원문·번역·승인·오래된 번역 관리" description="원문이 바뀌면 기존 번역을 자동으로 오래된 상태로 표시합니다. AI는 사용자 키로 초안만 제안하며 승인 전에는 배포 데이터가 아닙니다." />
        <div className="grid gap-4 lg:grid-cols-2"><div className={`${CARD} grid gap-2`}><input className={INPUT} value={dialogueDraft.id} onChange={(event) => setDialogueDraft((current) => ({ ...current, id: event.target.value }))} aria-label="대사 ID" /><textarea className={INPUT} value={dialogueDraft.source} onChange={(event) => setDialogueDraft((current) => ({ ...current, source: event.target.value }))} aria-label="원문" /><div className="grid grid-cols-[7rem_1fr] gap-2"><input className={INPUT} value={dialogueDraft.locale} onChange={(event) => setDialogueDraft((current) => ({ ...current, locale: event.target.value }))} aria-label="번역 로케일" /><textarea className={INPUT} value={dialogueDraft.translated} onChange={(event) => setDialogueDraft((current) => ({ ...current, translated: event.target.value }))} aria-label="번역문" /></div><div className="flex flex-wrap gap-2"><button type="button" className={BUTTON} onClick={saveDialogue}>원문 저장</button><button type="button" className={BUTTON} onClick={() => void suggestTranslation()} disabled={aiBusy}>{aiBusy ? "제안 중…" : "내 AI로 번역 초안"}</button><button type="button" className={BUTTON} onClick={() => saveTranslation(false)}>초안 저장</button><button type="button" className={`${BUTTON} border-good/40 text-good`} onClick={() => saveTranslation(true)}>검토 승인</button></div></div><div className={CARD}><table className="w-full text-left text-xs"><thead><tr className="text-fg-3"><th>ID</th><th>원문</th><th>{dialogueDraft.locale}</th><th>상태</th></tr></thead><tbody>{state.dialogue.map((row) => { const status = translationStatus(row, dialogueDraft.locale); return <tr key={row.id} className="border-t border-line"><td className="py-2">{row.id}</td><td className="max-w-40 truncate">{row.source}</td><td className="max-w-40 truncate">{row.translations[dialogueDraft.locale]?.text ?? "—"}</td><td className={status === "approved" ? "text-good" : status === "stale" ? "text-warn" : "text-fg-3"}>{status}</td></tr>; })}</tbody></table></div></div>
      </section>

      <section className="mt-10"><SectionHeading icon={Users} eyebrow="BETA READERS" title="제작자 검토와 독자 반응을 분리" description="이해도·가독성·다음 장면 기대를 묻는 가벼운 오프라인 패키지를 내보내고 응답을 다시 가져옵니다." />
        <div className="grid gap-4 lg:grid-cols-2"><div className={`${CARD} grid gap-2`}><input className={INPUT} value={betaTitle} onChange={(event) => setBetaTitle(event.target.value)} aria-label="베타 패키지 작품명" /><textarea className={`${INPUT} min-h-32`} value={betaPages} onChange={(event) => setBetaPages(event.target.value)} aria-label="페이지 이름 목록" /><div className="flex flex-wrap gap-2"><button type="button" className={BUTTON} onClick={exportBetaPackage}><Download size={15} /> 검토 패키지</button><label className={`${BUTTON} cursor-pointer`}><Upload size={15} /> 응답 가져오기<input className="sr-only" type="file" accept="application/json" onChange={(event) => void importFeedback(event.target.files?.[0])} /></label></div></div><div className={CARD}><h3 className="font-bold text-fg">누적 응답 {state.betaFeedback.length}</h3>{state.betaFeedback.length ? <ul className="mt-3 grid max-h-56 gap-2 overflow-auto">{state.betaFeedback.map((feedback) => <li key={feedback.id} className="rounded-lg border border-line p-2 text-xs"><strong>{feedback.pageId}</strong> · 이해 {feedback.clarity}/5 · 가독성 {feedback.readability}/5 · 기대 {feedback.curiosity}/5<p className="mt-1 text-fg-3">{feedback.comment || "의견 없음"}</p></li>)}</ul> : <p className="mt-2 text-sm text-fg-3">아직 가져온 응답이 없습니다.</p>}</div></div>
      </section>

      <section className="mt-10"><SectionHeading icon={Upload} eyebrow="PROCESS GALLERY" title="완성 이미지에서 제작 방법으로" description="콘티·선화·채색·완성 단계를 사용자 선택으로 묶어 포트폴리오 패키지로 내보냅니다. 원본 공개 여부는 작가가 결정합니다." />
        <div className={CARD}><label className={`${BUTTON} cursor-pointer`}><Upload size={15} /> 과정 이미지 추가<input className="sr-only" type="file" multiple accept="image/png,image/jpeg,image/webp" onChange={(event) => void addProcessImages(event.target.files)} /></label><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{processImages.map((image, index) => <figure key={`${image.label}-${index}`} className="overflow-hidden rounded-xl border border-line bg-panel"><img src={image.dataUrl} alt={`${image.label} 단계`} className="aspect-[3/4] w-full object-cover" /><figcaption className="p-2 text-xs font-bold text-fg">{image.label}</figcaption></figure>)}</div><button type="button" className={`${BUTTON} mt-4`} disabled={!processImages.length} onClick={() => downloadJson("toonstudio-process-gallery.json", { kind: "toonstudio-process-gallery", version: 1, exportedAt: new Date().toISOString(), credit: "사용자 제공 이미지", stages: processImages })}><Download size={15} /> 과정 패키지 내보내기</button></div>
      </section>

      <section className="mt-10" aria-labelledby="ecosystem-detailed-workbench-heading">
        <h2 id="ecosystem-detailed-workbench-heading" className="sr-only">상세 제작 워크벤치</h2>
        <CreatorEcosystemWorkbench />
      </section>

      <section className="mb-12 mt-10"><SectionHeading icon={Sparkles} eyebrow="READ → LEARN → CREATE" title="감상에서 창작 연습으로 연결" description="특정 작품을 복제하지 않고 장르에서 관찰한 감정·구도 원리를 자체 장면으로 다시 연습합니다." />
        <div className={CARD}><label className="grid max-w-xs gap-1 text-sm font-bold text-fg">관심 장르<select className={INPUT} value={readerGenre} onChange={(event) => setReaderGenre(event.target.value)}>{["로맨스", "일상", "액션", "미스터리", "판타지", "공포"].map((genre) => <option key={genre}>{genre}</option>)}</select></label><blockquote className="mt-4 border-l-2 border-accent pl-4 text-sm leading-7 text-fg-2">{readerPrompt}</blockquote><div className="mt-4 flex flex-wrap gap-2"><Link to="/explore" className={BUTTON}>작품 탐색</Link><Link to="/learn" className={BUTTON}>관련 학습</Link><Link to="/studio/templates" className={`${BUTTON} bg-accent text-on-accent`}>자체 예제로 제작 시작</Link></div></div>
      </section>
    </Container>
  );
}
