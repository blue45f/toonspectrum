  ...
  render(): ReactNode {
    ...
    return (
      <aside className="fixed bottom-[calc(var(--studio-canvas-bottom-inset,7rem)+7.5rem)] right-3 z-40 hidden max-w-[min(24rem,calc(100vw-1.5rem))] rounded-xl border border-line bg-panel p-3 text-xs text-fg lg:block"
        aria-label={translateCurrentStaticSourceText("domains.creator.offline.StudioOfflinePanelBoundary", "ko", "오프라인 안내 로딩 실패")}>
        <p role="status">{translateCurrentStaticSourceText("domains.creator.offline.StudioOfflinePanelBoundary", "ko", "오프라인 준비 안내를 열지 못했습니다. 편집기는 유지됩니다. 원고 저장 상태는 저장센터에서 확인해 주세요.")}</p>
        <button type="button" className="mt-2 min-h-11 rounded-lg border border-line px-3"
          onClick={() => { this.setState({ dismissed: true }); }}>{translateCurrentStaticSourceText("domains.creator.offline.StudioOfflinePanelBoundary", "ko", "안내 닫기")}</button>
      </aside>
    );
  }
}
