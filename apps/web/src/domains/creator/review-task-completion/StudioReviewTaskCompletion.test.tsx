...
it("keeps the task label stable independently of available option titles", async () => {
  draw(); await open();
  const select = screen.getByLabelText("이 의견에 연결된 작업", { exact: true }) as HTMLSelectElement;
  expect(select.labels?.[0]?.textContent?.trim()).toBe("이 의견에 연결된 작업");
