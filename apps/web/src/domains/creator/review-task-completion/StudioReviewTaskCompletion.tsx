...
const StudioHandoffEnvelopeComposer = lazy(() => import("../handoff-envelope/StudioHandoffEnvelope").then((module) => ({ default: module.StudioHandoffEnvelopeComposer })));
...
function Criteria({ request, actorId }: { readonly request: StudioReviewTaskCompletionRequest; readonly actorId: string }) {
  ...
  return <section className="mt-3 rounded-lg border border-line p-3" aria-label={bt("완료 기준 검토", "Review completion criteria")}>
    ...
    {current ? <Suspense fallback={null}><StudioHandoffEnvelopeComposer workId={request.subject.workId} taskId={request.taskId} /></Suspense> : null}
...
function CompletionChoices({ request, actorId }: { readonly request: StudioReviewProductionRequest; readonly actorId: string }) {
  ...
  return <div className="mt-3">
    <label htmlFor={taskSelectId} className="block text-sm">{bt("이 의견에 연결된 작업", "Task linked to this comment")}</label>
      <select id={taskSelectId} className="mt-1 min-h-11 w-full rounded-lg border border-line bg-card px-2" value={selected?.id ?? ""} onChange={(event) => setTaskId(event.target.value)}>
