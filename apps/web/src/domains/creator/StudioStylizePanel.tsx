  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between gap-2">
        <p className="text-[0.66rem] font-semibold text-fg-3 uppercase tracking-wider">{translateCurrentStaticSourceText("domains.creator.StudioStylizePanel", "ko", "스타일라이즈 (Stylize)")}</p>
        <button
          type="button"
          onClick={onReset}
          disabled={isIdentity}
          className={buttonClass({ size: "sm", variant: "quiet" })}
          title={translateCurrentStaticSourceText("domains.creator.StudioStylizePanel", "ko", "스타일 효과를 제거하고 원본으로 되돌립니다.")}
        >
          <RotateCcw className="size-3.5" />
          {translateCurrentStaticSourceText("domains.creator.StudioStylizePanel", "ko", "원본으로")}</button>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {STYLIZE_TYPES.map((t) => (
          <StudioPanelChip
            ...
            title={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.StudioStylizePanel", "ko", "효과를 \"{v0}\"로 바꿉니다."), { v0: String(t.label) })}
          >
            {t.label}
          </StudioPanelChip>
        ))}
