  ...
  return (
    <div className="space-y-2 border-t border-line/35 pt-2.5">
      <div className="flex items-center justify-between gap-2">
        <div>
          <p className="text-xs font-semibold text-fg-3">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "고급 자")}</p>
          <p className="text-[0.58rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "곡선·어안·평행선·동심원·방사선 가이드는 여러 개 표시하고 하나만 스냅합니다.")}</p>
        </div>
      ...
      {document.rulers.length === 0 ? (
        <p className="rounded-md border border-dashed border-line px-2 py-3 text-center text-[0.63rem] leading-relaxed text-fg-3">
          {translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "의상 곡선, 차량 곡면, 초광각 배경을 따라 그릴 자를 추가해 보세요.")}</p>
      ) : (
        <div role="list" aria-label={translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "고급 자 목록")} className="space-y-1">
          {document.rulers.map((ruler) => {
            ...
            return (
              <div
                key={ruler.id}
                role="listitem"
                className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "en", "flex items-center gap-1 rounded-md border px-1.5 py-1 {v0}"), { v0: String(isSelected ? "border-accent/60 bg-accent/8" : "border-line bg-card") })}
              >
                <button
                  type="button"
                ...
                <button
                  ...
                  className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "en", "grid size-8 place-items-center rounded hover:bg-raised disabled:opacity-45 {v0}"), { v0: String(isActive ? "text-accent" : "text-fg-3") })}
                ...
                <button
                  type="button"
                  aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "{v0} 삭제"), { v0: String(ruler.name) })}
                  disabled={disabled}
      ...
      {selected ? (
        <div className="space-y-2 border-l border-line/50 py-1 pl-2">
          <label className="block space-y-1 text-[0.65rem] font-semibold text-fg-3">
            {translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "적용 범위")}<select
              value={selected.scope.kind === "page" ? translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "en", "page") : formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "en", "group:{v0}"), { v0: String(selected.scope.groupId) })}
              disabled={disabled}
            ...
            >
              <option value="page">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "현재 페이지 전체")}</option>
              {groups.map((group) => (
                <option key={group.id} value={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "en", "group:{v0}"), { v0: String(group.id) })}>{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "그룹 · ")}{group.name}</option>
              ))}
            </select>
          </label>
          ...
          <div className="flex flex-wrap gap-1">
            <StudioPanelChip
              ...
              title={disabled ? disabledTitle : translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "이 자의 스냅 사용 가능 여부")}
            >
              {selected.enabled ? translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "스냅 허용") : translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "스냅 잠금")}
            </StudioPanelChip>
          </div>
          ...
          {selected.type === "curve" ? (
            <>
              <label className="block space-y-1 text-[0.65rem] font-semibold text-fg-3">
                {translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "평행선 방식")}<select
                ...
                >
                  <option value="through-start">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "획 시작점을 지나는 평행선")}</option>
                  <option value="on-curve">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "원래 곡선")}</option>
                  <option value="fixed">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "고정 오프셋")}</option>
                </select>
              </label>
              {selected.snapMode === "fixed" ? (
                <StudioSliderRow
                  label={translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "오프셋")}
              ...
              {(["p0", "p1", "p2", "p3"] as const).map((key, index) => (
                <CoordinatePair
                  key={key}
                  label={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "제어점 {v0}"), { v0: String(index + 1) })}
                  x={selected[key].x}
                  y={selected[key].y}
                  disabled={disabled}
          ...
          {selected.type === "fisheye" ? (
            <>
              <CoordinatePair
                label={translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "렌즈 중심")}
                x={selected.centerX}
                y={selected.centerY}
                disabled={disabled}
              ...
              />
              <StudioSliderRow
                label={translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "반경")}
              ...
              />
              <StudioSliderRow
                label={translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "회전")}
                min={0}
                max={359}
                step={1}
              ...
              />
              <StudioSliderRow
                label={translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "시야각")}
              ...
              />
              <StudioSliderRow
                label={translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "렌즈 강도")}
              ...
              />
              <label className="block space-y-1 text-[0.65rem] font-semibold text-fg-3">
                {translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "가이드 계열")}<select
                ...
                >
                  <option value="auto">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "자동")}</option>
                  <option value="radial">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "방사형")}</option>
                  <option value="spherical">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "구면형")}</option>
                </select>
              </label>
              <label className="block space-y-1 text-[0.65rem] font-semibold text-fg-3">
                {translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "렌즈 밖 입력")}<select
                ...
                >
                  <option value="clamp">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "가장자리에 고정")}</option>
                  <option value="passthrough">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "보정 없이 통과")}</option>
                  <option value="reject">{translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "입력 거부")}</option>
                </select>
              </label>
            </>
          ) : null}
          ...
          {selected.type === "parallel" ? (
            <>
              <StudioSliderRow
                label={translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "각도")}
                min={0}
                max={179}
                step={1}
              ...
              />
              <StudioSliderRow
                label={translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "가이드 간격")}
              ...
              />
              <CoordinatePair
                label={translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "기준점")}
                x={selected.originX}
                y={selected.originY}
                disabled={disabled}
              ...
              <p className="text-[0.58rem] leading-relaxed text-fg-3">
                {translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "획은 시작점을 지나는 같은 각도의 평행선에 스냅됩니다. 가이드 간격은 표시 전용입니다.")}</p>
            </>
          ) : null}
          ...
          {selected.type === "concentric" ? (
            <>
              <CoordinatePair
                label={translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "중심점")}
                x={selected.centerX}
                y={selected.centerY}
                disabled={disabled}
                onCommit={(centerX, centerY) => onPatch(selected.id, { centerX, centerY })}
              ...
              />
              <p className="text-[0.58rem] leading-relaxed text-fg-3">
                {translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "획은 시작점을 지나는 중심 기준 원에 스냅됩니다. 가이드 간격은 표시 전용입니다.")}</p>
            </>
          ) : null}
          ...
          {selected.type === "radial" ? (
            <>
              ...
              />
              <p className="text-[0.58rem] leading-relaxed text-fg-3">
                {translateCurrentStaticSourceText("domains.creator.StudioAdvancedRulerPanel", "ko", "획은 중심에서 시작점을 지나는 방사선에 스냅되고 중심 반대쪽으로는 넘어가지 않습니다.")}</p>
            </>
          ) : null}
        </div>
      ) : null}
