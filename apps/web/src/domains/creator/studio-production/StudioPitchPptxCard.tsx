  ...
  return (
    <section className="rounded-2xl border border-line bg-card p-4 shadow-sm" data-studio-pptx-export>
      <header className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-bold text-fg">{translateCurrentStaticSourceText("domains.creator.studio.production.StudioPitchPptxCard", "ko", "PowerPoint 편집본")}</h2>
          <p className="mt-1 max-w-3xl text-xs leading-relaxed text-fg-2">
            {translateCurrentStaticSourceText("domains.creator.studio.production.StudioPitchPptxCard", "ko", "제목과 본문을 PowerPoint의 편집 가능한 텍스트 상자로 내보냅니다. 글꼴은 수신 환경에서 대체될 수 있으며 복잡한 원고 그래픽은 이 모드에 자동 포함되지 않습니다.")}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button type="button" className={buttonClass({ variant: "outline", size: "sm" })} onClick={onAddSlide}>
            <Plus className="size-4" aria-hidden="true" />
            {translateCurrentStaticSourceText("domains.creator.studio.production.StudioPitchPptxCard", "ko", "슬라이드 추가")}</button>
          <button type="button" className={buttonClass({ size: "sm" })} onClick={download} disabled={slides.length === 0}>
            <Download className="size-4" aria-hidden="true" />
            {translateCurrentStaticSourceText("domains.creator.studio.production.StudioPitchPptxCard", "ko", "PPTX 저장")}</button>
        </div>
      </header>
      <div className="space-y-3">
        {slides.map((slide, index) => (
          <article key={slide.id} className="grid gap-2 rounded-xl border border-line bg-panel p-3 md:grid-cols-[7rem_minmax(0,1fr)]">
            <div className="text-xs font-bold text-fg-3">{translateCurrentStaticSourceText("domains.creator.studio.production.StudioPitchPptxCard", "en", "SLIDE ")}{index + 1}</div>
            <div className="space-y-2">
              <label className="block text-xs font-semibold text-fg-2">
                {translateCurrentStaticSourceText("domains.creator.studio.production.StudioPitchPptxCard", "ko", "제목")}<input
                  defaultValue={slide.title}
                  key={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.studio.production.StudioPitchPptxCard", "en", "{v0}:title:{v1}"), { v0: String(slide.id), v1: String(slide.title) })}
              ...
              <label className="block text-xs font-semibold text-fg-2">
                {translateCurrentStaticSourceText("domains.creator.studio.production.StudioPitchPptxCard", "ko", "본문")}<textarea
                  defaultValue={slide.body}
                  key={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.studio.production.StudioPitchPptxCard", "en", "{v0}:body:{v1}"), { v0: String(slide.id), v1: String(slide.body) })}
