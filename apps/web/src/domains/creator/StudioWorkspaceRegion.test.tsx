...
async function edit() { fireEvent.click(screen.getByRole("button", { name: "배치 편집" })); await screen.findByRole("button", { name: "배치 완료" }); }

describe("StudioWorkspaceRegion", () => {
  it("suspends a hidden authored sidebar without forgetting its arrangement", async () => {
  ...
  it("keeps editing controls out of the default workspace until explicitly requested", async () => {
  ...
  it("offers click-only placement, position locks, and a restore action", async () => {
    render(<Harness />); await edit(); fireEvent.click(screen.getByRole("button", { name: "테스트 도구 배치 설정" })); await screen.findByText("드래그 없이 배치");
  ...
  it("does not float on a click or canceled drag and cleans its preview", async () => {
    render(<Harness />); await edit(); const handle = screen.getByRole("button", { name: "테스트 도구 이동" });
  ...
  it("commits drag placement, preserves child identity, and ignores secondary pointers", async () => {
    render(<Harness />); await edit(); const child = screen.getByRole("button", { name: "도구 실행" }); const handle = screen.getByRole("button", { name: "테스트 도구 이동" });
  ...
  it("suspends desktop placement on a phone without erasing its desktop preference", async () => {
  ...
  it("collapses without unmounting content and restores its size", async () => {
  ...
  it("resizes by numeric entry and clamps oversized dimensions", async () => {
    render(<Harness />); await edit(); fireEvent.click(screen.getByRole("button", { name: "테스트 도구 배치 설정" })); await screen.findByText("드래그 없이 배치");
  ...
  it("restores a saved region arrangement using the toolbar", async () => {
    render(<Harness />); await edit(); fireEvent.click(screen.getByRole("button", { name: "영역 분리" }));
