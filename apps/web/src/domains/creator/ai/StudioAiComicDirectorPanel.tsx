    ...
    <div className="space-y-2">
      <div className="relative mx-auto aspect-video max-h-72 overflow-hidden rounded-xl border border-line bg-canvas">
        <img
          ref={imageRef}
          src={source}
          alt={translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "수리 마스크를 그릴 현재 후보")}
          onLoad={configure}
          className="absolute inset-0 size-full object-contain"
        />
        <canvas
          ref={canvasRef}
          aria-label={translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "수리하거나 전경으로 분리할 영역을 칠하는 마스크")}
          onPointerDown={(event) => {
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        <label className="text-[0.67rem] font-semibold text-fg-2">
          {translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "브러시")}<input
            type="range"
            min={12}
            max={180}
            value={brush}
        ...
        <button type="button" onClick={() => fillPreset("center")} className="min-h-11 rounded-lg border border-line bg-card px-2 text-[0.65rem] text-fg-2 hover:bg-raised">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "가운데 선택")}</button>
        <button type="button" onClick={() => fillPreset("lower")} className="min-h-11 rounded-lg border border-line bg-card px-2 text-[0.65rem] text-fg-2 hover:bg-raised">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "아래쪽 선택")}</button>
        <button type="button" onClick={() => fillPreset("all")} className="min-h-11 rounded-lg border border-line bg-card px-2 text-[0.65rem] text-fg-2 hover:bg-raised">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "전체 선택")}</button>
        <button type="button" onClick={() => fillPreset("clear")} className="min-h-11 rounded-lg border border-line bg-card px-2 text-[0.65rem] text-fg-2 hover:bg-raised">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "마스크 지우기")}</button>
      </div>
    </div>
  );
}
  ...
  const modal = (
    <div className="fixed inset-0 z-[90] bg-[oklch(0.08_0.01_70/0.82)] p-2 text-fg backdrop-blur-sm sm:p-4">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "AI 코믹 디렉터")}
        tabIndex={-1}
      ...
      >
        <header className="flex min-h-16 flex-wrap items-center gap-2 border-b border-line bg-panel px-3 py-2 sm:px-4">
          ...
          <div className="min-w-0">
            <h2 className="text-sm font-black">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "AI 코믹 디렉터")}</h2>
            <p className="text-[0.65rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "이야기·기준 → 컷 연출 → 제작·수리 → 마감·추가")}</p>
          </div>
          {sessionId ? <span className="rounded-full border border-good/35 bg-good/10 px-2 py-1 text-[0.62rem] font-semibold text-good">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "세션 자동 저장")}</span> : null}
          <button type="button" onClick={onClose} aria-label={translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "닫기")} title={translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "닫기 (Esc)")} className={cn("ml-auto grid size-11 place-items-center rounded-lg border border-line bg-card text-fg-3 hover:bg-raised hover:text-fg", STUDIO_FOCUS_RING)}><X size={16} aria-hidden /></button>
        </header>

        <div className="grid min-h-0 md:grid-cols-[12.5rem_minmax(0,1fr)]">
          <nav aria-label={translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "AI 코믹 디렉터 제작 단계")} className="overflow-x-auto border-b border-line bg-panel p-2 md:overflow-y-auto md:border-b-0 md:border-r">
          ...
          <div className="min-h-0 overflow-y-auto px-3 py-3 [scroll-padding-block:5rem] sm:px-5 sm:py-4">
            {stage === "brief" ? (
              <div className="mx-auto max-w-5xl space-y-4">
                <section className="border-b border-line pb-4">
                  <div className="mb-2 flex items-start justify-between gap-2"><div><h3 className="text-sm font-bold">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "이야기와 제작 기준")}</h3><p className="text-[0.68rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "이미지 비용이 발생하기 전에 대본과 반복 기준을 정합니다.")}</p></div><span className="rounded-full border border-line bg-card px-2 py-1 text-[0.62rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "이미지 생성 없음")}</span></div>
                  <label className="block text-xs font-semibold text-fg-2" htmlFor="comic-director-story">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "이야기 또는 회차 대본")}</label>
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2 text-[0.65rem] text-fg-3"><span>{storyText.length.toLocaleString("ko-KR")} / {STORY_TEXT_MAX.toLocaleString("ko-KR")}{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "자")}</span><label className="ml-auto flex items-center gap-1.5">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "컷 수")}<select value={sceneCountHint ?? ""} onChange={(event) => onSceneCountHintChange(event.target.value ? Number(event.target.value) : undefined)} disabled={busy} className="min-h-11 rounded-lg border border-line bg-card px-2"><option value="">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "자동")}</option>{Array.from({ length: 9 }, (_, index) => index + 2).map((count) => <option key={count} value={count}>{count}{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "컷")}</option>)}</select></label><button type="button" onClick={onGenerate} disabled={!textConfigured || busy || !storyText.trim()} className="min-h-11 rounded-lg bg-accent px-4 text-xs font-bold text-on-accent disabled:opacity-45"><Sparkles size={13} className="mr-1 inline" aria-hidden />{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "컷 구성 만들기")}</button></div>
                </section>
                <section>
                  <h3 className="text-xs font-bold">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "작품 바이블 · AI 이미지 참조 팩")}</h3>
                  <p className="mb-2 text-[0.65rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "캐릭터 정체성, 구도·연출, 화풍 참조를 역할별로 버전 관리합니다.")}</p>
                  ...
                  {imageReferenceMissingCount > 0 ? <p role="alert" className="mt-2 rounded-lg border border-bad/30 bg-bad/10 p-2 text-xs text-bad">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "참조 에셋 ")}{imageReferenceMissingCount}{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "개를 찾을 수 없습니다.")}</p> : null}
                  {sessionId && api ? <button type="button" onClick={() => void api.appendVisualBibleRevision(sessionId, session.visualBible).then((result) => setMediaMessage(result.ok ? `작품 바이블 revision ${result.data.revision} 저장 완료` : result.message))} className="mt-2 min-h-11 rounded-lg border border-line bg-card px-3 text-xs font-semibold hover:bg-raised">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "현재 작품 바이블 revision 저장")}</button> : null}
                </section>
              </div>
            ) : null}
            ...
            {stage === "direction" ? (
              <div className="mx-auto max-w-6xl">
                <div className="mb-3 flex flex-wrap items-center gap-2"><div><h3 className="text-sm font-bold">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "AI가 이해한 컷과 연출 확인")}</h3><p className="text-[0.65rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "체크박스가 실제 제작 대상을 결정합니다.")}</p></div><button type="button" onClick={() => setSelectedIndexes(items.map((_, index) => index))} disabled={busy || !items.length} className="ml-auto min-h-11 rounded-lg border border-line bg-card px-3 text-xs hover:bg-raised">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "전체 선택")}</button><button type="button" onClick={() => setSelectedIndexes([])} disabled={busy || !selectedIndexes.length} className="min-h-11 rounded-lg border border-line bg-card px-3 text-xs hover:bg-raised">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "선택 해제")}</button></div>
                {!items.length ? <div className="rounded-xl border border-dashed border-line p-8 text-center"><p className="text-sm font-semibold">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "아직 컷 구성이 없습니다.")}</p><button type="button" onClick={() => setStage("brief")} className="mt-3 min-h-11 rounded-lg bg-accent px-4 text-xs font-bold text-on-accent">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "이야기 입력으로 이동")}</button></div> : (
                  <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_23rem]">
                    <ul aria-label={translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "컷 구성")} className="grid gap-2 sm:grid-cols-2">
                      {items.map((item, index) => (
                        <li key={index} className={cn("rounded-xl border bg-card p-2", activeIndex === index ? "border-accent" : "border-line")}>
                          <div className="flex gap-2"><label className="grid size-11 shrink-0 place-items-center rounded-lg border border-line bg-panel"><span className="sr-only">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "컷 ")}{index + 1} {translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "제작 선택")}</span><input type="checkbox" checked={selected.has(index)} onChange={() => setSelectedIndexes((current) => current.includes(index) ? current.filter((value) => value !== index) : [...current, index].sort((left, right) => left - right))} disabled={busy} className="size-4 accent-accent" /></label><button type="button" onClick={() => setActiveIndex(index)} className={cn("min-h-11 min-w-0 flex-1 rounded-lg text-left", STUDIO_FOCUS_RING)}><strong className="block truncate text-xs">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "컷 ")}{index + 1} · {item.summary}</strong><span className="block truncate text-[0.62rem] text-fg-3">{SCENARIO_BEAT_LABELS[item.beatType]} {translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "· 대사 ")}{item.bubbles.length}{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "개")}</span></button><div className="h-16 w-20 shrink-0 overflow-hidden rounded-lg bg-raised"><StageThumbnail item={item} /></div></div>
                          <label className="mt-2 block text-[0.64rem] font-semibold text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "제작 방향")}<select aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "{v0}번 장면 제작 방향"), { v0: String(index + 1) })} value={directions[index] ?? translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "en", "faithful")} onChange={(event) => updateDirection(index, event.target.value as DirectorDirection)} disabled={busy} className="mt-1 min-h-11 w-full rounded-lg border border-line bg-panel px-2 text-xs text-fg">{Object.entries(DIRECTIONS).map(([id, meta]) => <option key={id} value={id}>{meta.label}</option>)}</select></label>
                        </li>
                      ))}
                    </ul>
            ...
            {stage === "production" ? (
              <div className="mx-auto max-w-6xl space-y-3">
                <div className="flex flex-wrap items-center gap-2"><div><h3 className="text-sm font-bold">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "후보를 만들고 사용할 결과 선택")}</h3><p className="text-[0.65rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "완료된 컷부터 검토하고, 틀린 영역만 수리하거나 레이어로 분리합니다.")}</p></div>{busy ? <button type="button" onClick={onCancel} className="ml-auto min-h-11 rounded-lg border border-line bg-card px-3 text-xs font-semibold hover:bg-raised">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "취소")}</button> : null}</div>
                {busy ? <div role="status" aria-live="polite" className="flex items-center gap-2 rounded-xl border border-accent/30 bg-accent-soft p-3 text-xs"><Loader2 size={15} className="animate-spin motion-reduce:animate-none" aria-hidden />{stageLabel ?? translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "제작 중…")}{progress ? ` (${progress.done}/${progress.total})` : ""}</div> : null}
                <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_25rem]">
                  <section><div className="mb-2 flex gap-1 overflow-x-auto pb-1">{items.map((item, index) => <button key={index} type="button" onClick={() => { setActiveIndex(index); setMaskDataUrl(""); }} className={cn("min-h-11 shrink-0 rounded-lg border px-3 text-xs", activeIndex === index ? "border-accent bg-accent-soft text-accent" : "border-line bg-card text-fg-3")}>{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "컷 ")}{index + 1}{item.imageCandidates?.length ? ` · ${item.imageCandidates.length}` : ""}</button>)}</div>{activeItem ? <><div className="grid gap-2 sm:grid-cols-2">{scenarioImageCandidates(activeItem).map((candidate, candidateIndex) => { const chosen = candidate.id === activeCandidate?.id; return <label key={candidate.id} className={cn("overflow-hidden rounded-xl border bg-card", chosen ? "border-accent" : "border-line")}><span className="block aspect-[4/3] bg-raised"><img src={candidate.imageDataUrl} alt={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "컷 {v0} 후보 {v1}"), { v0: String(activeIndex + 1), v1: String(candidateIndex + 1) })} className="size-full object-cover" /></span><span className="flex min-h-11 items-center gap-2 px-2"><input type="radio" name={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "en", "candidate-{v0}"), { v0: String(activeIndex) })} checked={chosen} onChange={() => chooseCandidate(candidate.id)} className="size-4 accent-accent" /><strong className="text-xs">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "후보 ")}{candidateIndex + 1}</strong>{candidate.repair ? <span className="rounded-full bg-good/10 px-2 py-0.5 text-[0.58rem] text-good">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "부분 수리")}</span> : null}{candidate.qualityReport ? <span className="ml-auto text-[0.58rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "분석됨")}</span> : null}</span></label>; })}</div>{!scenarioImageCandidates(activeItem).length ? <div className="rounded-xl border border-dashed border-line p-8 text-center"><p className="text-sm font-semibold">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "아직 이미지 후보가 없습니다.")}</p><button type="button" onClick={() => onRegenerateScene(activeIndex)} disabled={!imageConfigured || busy || regeneratingIndex !== null} className="mt-3 min-h-11 rounded-lg bg-accent px-4 text-xs font-bold text-on-accent disabled:opacity-45">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "이 컷 이미지 생성")}</button></div> : null}</> : null}</section>
                  <aside className="space-y-3 rounded-xl border border-line bg-panel p-3"><section><h3 className="flex items-center gap-1.5 text-xs font-bold"><ScanSearch size={14} className="text-accent" aria-hidden />{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "후보 품질 분석")}</h3><p className="mt-1 text-[0.62rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "해상도·투명도·가장자리·명암을 실제 픽셀로 측정합니다. 정체성과 해부는 모르는 상태로 남깁니다.")}</p><button type="button" onClick={() => void analyzeCandidate()} disabled={!activeCandidate || mediaBusy !== null} className="mt-2 min-h-11 w-full rounded-lg border border-line bg-card text-xs font-semibold hover:bg-raised disabled:opacity-45">{mediaBusy === "quality" ? translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "분석 중…") : translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "현재 후보 픽셀 품질 분석")}</button>{activeCandidate?.qualityReport ? <ul className="mt-2 space-y-1 text-[0.62rem] text-fg-2">{activeCandidate.qualityReport.findings.map((finding) => <li key={finding.id} className={cn("rounded-lg border p-2", finding.severity === "review" ? "border-warn/30 bg-warn/10" : "border-line bg-card")}>{finding.message}</li>)}</ul> : null}</section><section className="border-t border-line pt-3"><h3 className="flex items-center gap-1.5 text-xs font-bold"><WandSparkles size={14} className="text-accent" aria-hidden />{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "마스크 영역 수리")}</h3>{!repairCapabilities.maskEdit ? <><p className="mt-1 rounded-lg border border-warn/30 bg-warn/10 p-2 text-[0.64rem] text-warn">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "마스크 영역 수리를 연결하지 않았습니다. ")}{repairCapabilities.reason}</p><button type="button" onClick={() => onRegenerateScene(activeIndex)} disabled={!activeItem || busy} className="mt-2 min-h-11 w-full rounded-lg border border-line bg-card text-xs font-semibold hover:bg-raised disabled:opacity-45">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "이 컷 전체 다시 제작")}</button></> : activeCandidate ? <><MaskEditor source={activeCandidate.imageDataUrl} onChange={setMaskDataUrl} /><label className="mt-2 block text-[0.64rem] font-semibold text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "수리 대상")}<select value={repairTarget} onChange={(event) => setRepairTarget(event.target.value as StudioAiComicRepairTarget)} className="mt-1 min-h-11 w-full rounded-lg border border-line bg-card px-2 text-xs">{REPAIR_TARGETS.map((target) => <option key={target.id} value={target.id}>{target.label}</option>)}</select></label><label className="mt-2 block text-[0.64rem] font-semibold text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "수리 지시")}<textarea value={repairPrompt} onChange={(event) => setRepairPrompt(event.target.value)} rows={3} className="mt-1 w-full rounded-lg border border-line bg-card p-2 text-xs" /></label><button type="button" onClick={() => void repairCandidate()} disabled={!maskDataUrl || mediaBusy !== null} className="mt-2 min-h-11 w-full rounded-lg bg-accent px-3 text-xs font-bold text-on-accent disabled:opacity-45">{mediaBusy === "repair" ? translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "선택 영역 수리 중…") : translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "선택 영역만 수리")}</button></> : <p className="mt-1 text-[0.64rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "먼저 사용할 후보를 선택하세요.")}</p>}</section><section className="border-t border-line pt-3"><h3 className="flex items-center gap-1.5 text-xs font-bold"><Layers3 size={14} className="text-accent" aria-hidden />{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "레이어 분리")}</h3><p className="mt-1 text-[0.62rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "검토한 마스크가 있으면 실제 전경·배경 PNG를 만들고, 없으면 단일 레이어 폴백을 명시합니다.")}</p><button type="button" onClick={() => void decomposeCandidate()} disabled={!activeCandidate || mediaBusy !== null} className="mt-2 min-h-11 w-full rounded-lg border border-line bg-card text-xs font-semibold hover:bg-raised disabled:opacity-45">{mediaBusy === "layers" ? translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "레이어 분리 중…") : maskDataUrl ? translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "마스크로 전경·배경 분리") : translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "단일 레이어로 유지")}</button>{activeItem?.layerManifest ? <p className={cn("mt-2 rounded-lg border p-2 text-[0.62rem]", activeItem.layerManifest.editable ? "border-good/30 bg-good/10 text-good" : "border-line bg-card text-fg-3")}>{activeItem.layerManifest.editable ? formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "{v0}개 편집 레이어 준비됨"), { v0: String(activeItem.layerManifest.layers.length) }) : activeItem.layerManifest.limitation}</p> : null}</section>{mediaMessage ? <p role="status" className="rounded-lg border border-line bg-card p-2 text-[0.65rem] text-fg-2">{mediaMessage}</p> : null}</aside>
                </div>
              </div>
            ) : null}

            {stage === "finish" ? (
              <div className="mx-auto grid max-w-6xl gap-4 lg:grid-cols-[minmax(0,1fr)_23rem]"><section><h3 className="text-sm font-bold">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "독자 흐름과 적용 차이")}</h3><p className="mb-3 text-[0.65rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "이미지, 네이티브 대사·말풍선과 준비된 전경·배경 레이어를 확인합니다.")}</p><div className="mx-auto max-w-md overflow-hidden rounded-2xl border border-line-strong bg-[oklch(0.94_0.012_83)] p-2">{items.map((item, index) => <div key={index} className="mb-6 overflow-hidden rounded-lg bg-raised"><div className="aspect-[4/3]"><StageThumbnail item={item} /></div>{item.dialogue ? <p className="m-3 rounded-full bg-[oklch(0.96_0.01_85)] px-4 py-2 text-center text-xs font-semibold text-[oklch(0.2_0.01_70)]">{item.dialogue.split("\n")[0]}</p> : null}</div>)}</div></section><aside className="rounded-xl border border-line bg-panel p-3"><h3 className="text-xs font-bold">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "추가 위치")}</h3><label className="mt-2 flex min-h-14 items-center gap-2 rounded-xl border border-line bg-card p-2"><input type="radio" name="apply-target" checked={applyTarget === "current-page"} onChange={() => onApplyTargetChange("current-page")} /><span><strong className="block text-xs">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "현재 페이지 아래")}</strong><span className="text-[0.6rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "기존 레이어를 바꾸지 않음")}</span></span></label><label className="mt-2 flex min-h-14 items-center gap-2 rounded-xl border border-line bg-card p-2"><input type="radio" name="apply-target" checked={applyTarget === "new-page"} onChange={() => onApplyTargetChange("new-page")} aria-label={translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "다음 새 페이지")} /><span><strong className="block text-xs">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "다음 새 페이지")}</strong><span className="text-[0.6rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "별도 세로 원고 페이지 생성")}</span></span></label><h3 className="mt-4 border-t border-line pt-3 text-xs font-bold">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "추가될 객체")}</h3><dl className="mt-2 space-y-1.5 text-[0.68rem]"><div className="flex justify-between"><dt>{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "컷 프레임")}</dt><dd className="font-bold">{applyDiff.panelCount}</dd></div><div className="flex justify-between"><dt>{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "장면 이미지")}</dt><dd className="font-bold">{applyDiff.imageCount}</dd></div><div className="flex justify-between"><dt>{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "네이티브 말풍선")}</dt><dd className="font-bold">{applyDiff.nativeBubbleCount}</dd></div><div className="flex justify-between"><dt>{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "분리 레이어")}</dt><dd className="font-bold">{applyDiff.decomposedLayerCount}</dd></div><div className="flex justify-between"><dt>{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "기존 레이어 변경")}</dt><dd className="font-bold text-good">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "없음")}</dd></div></dl>{revisionConflict ? <p role="alert" className="mt-3 rounded-lg border border-bad/30 bg-bad/10 p-2 text-[0.65rem] text-bad">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "세션 시작 뒤 원고 revision이 변경됐습니다. 최신 위치를 다시 확인해야 합니다.")}</p> : <p className="mt-3 rounded-lg border border-good/30 bg-good/10 p-2 text-[0.65rem] text-good"><ShieldCheck size={12} className="mr-1 inline" aria-hidden />{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "동일 operation ID 중복 적용 방지 · 한 번의 Undo")}</p>}<p className="mt-2 text-[0.62rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "후보 digest ")}{applyDiff.candidateDigest} · {sessionId ? translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "서버 검수 revision을 만든 뒤 적용") : translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "로컬 선택 기준으로 적용")}</p><button type="button" onClick={() => void approveAndApply()} disabled={busy || revisionConflict} className="mt-3 min-h-11 w-full rounded-lg bg-accent px-4 text-xs font-bold text-on-accent disabled:opacity-45">{translateCurrentStaticSourceText("domains.creator.ai.StudioAiComicDirectorPanel", "ko", "현재 페이지에 적용")}</button></aside></div>
            ) : null}
          </div>
        </div>
