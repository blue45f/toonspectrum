...
export function StudioAdvancedAiTools({
  ...
  return (
    <details className="shrink-0 rounded-xl border border-line bg-panel/40" data-studio-advanced-ai-tools="true">
      <summary className="flex min-h-11 cursor-pointer items-center justify-between gap-2 px-3 text-xs font-bold text-fg">
        <span>고급 공동 창작·3D</span><span className="text-[0.58rem] font-medium text-fg-3">획 제안 · 사용자 키 3D</span>
      </summary>
      <div className="grid gap-3 border-t border-line p-3">
        <div className="flex gap-1" role="tablist" aria-label="고급 AI 제작 도구">
          <button type="button" role="tab" aria-selected={active === "stroke"} onClick={() => setActive("stroke")} className="min-h-11 flex-1 rounded-lg border border-line text-xs font-bold aria-selected:border-accent aria-selected:bg-accent/10">획 제안</button>
          <button type="button" role="tab" aria-selected={active === "three-d"} onClick={() => setActive("three-d")} className="min-h-11 flex-1 rounded-lg border border-line text-xs font-bold aria-selected:border-accent aria-selected:bg-accent/10">AI 3D</button>
        </div>
