...
describe("pinned marketplace CC0 delivery", () => {
  it("pins 100 reviewed assets to the current catalog and real file hashes", () => {
  ...
  it("rejects unknown, path-traversal and cross-kind model references", () => {
    for (const ref of ["https://example.com/a.glb", "studio-3d-asset:../model", "studio-3d-asset:polyhaven-background-wide-street-01", "studio-asset:polyhaven-sofa-02"]) {
  ...
  it("loads the exact model file without redirects and rejects corrupt bytes", async () => {
    const asset = resolveStudioMarketplaceCc0Model("studio-3d-asset:polyhaven-painted-wooden-chair-01")!;
  ...
  it("does not fetch after cancellation or for an image reference", async () => {
    ...
    const model = resolveStudioMarketplaceCc0Model("studio-3d-asset:polyhaven-painted-wooden-chair-01")!;
  ...
  it("previews only the selected trusted image or model, never an arbitrary URL", () => {
    ...
    expect(resolveStudioMarketplaceAssetPreview("studio-3d-asset:polyhaven-painted-wooden-chair-01")?.src).toContain("polyhaven-painted-wooden-chair-01");
