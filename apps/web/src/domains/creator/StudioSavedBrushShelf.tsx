...
export function StudioSavedBrushShelf({
  ...
  return (
    <section className="mb-2.5" aria-label={translateCurrentStaticSourceText("domains.creator.StudioSavedBrushShelf", "ko", "고정 및 최근 브러시")}>
      <div className="mb-1 flex min-h-11 items-center justify-between gap-2">
        <p className="flex items-center gap-1 text-[0.7rem] font-medium text-fg-3">
          <Pin size={11} aria-hidden /> {translateCurrentStaticSourceText("domains.creator.StudioSavedBrushShelf", "ko", "고정·최근")}<span className="tabular-nums text-fg-2">{quickBrushes.length}/8</span>
        </p>
        {onManage ? (
          <button
            type="button"
          ...
          >
            <SlidersHorizontal size={13} aria-hidden /> {translateCurrentStaticSourceText("domains.creator.StudioSavedBrushShelf", "ko", "저장·관리")}</button>
        ) : null}
      </div>
      {quickBrushes.length === 0 ? (
        ...
        >
          {translateCurrentStaticSourceText("domains.creator.StudioSavedBrushShelf", "ko", "브러시를 저장하거나 고정하면 이 선반에서 바로 꺼내 쓸 수 있어요.")}</button>
      ) : (
        ...
        {quickBrushes.map((brush) => {
          ...
          return (
            <button
              ...
              aria-pressed={active}
              aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.StudioSavedBrushShelf", "ko", "{v0} 브러시 적용, {v1}px, {v2}퍼센트"), { v0: String(brush.name), v1: String(brush.strokeWidth), v2: String(Math.round(brush.brushOpacity * 100)) })}
              className={cx(
