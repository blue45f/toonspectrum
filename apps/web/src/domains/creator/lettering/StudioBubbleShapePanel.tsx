...
export function StudioBubbleShapePanel({
  ...
  return (
    <div className="mt-2.5 space-y-2 rounded-xl border border-line bg-card/45 p-2.5">
      <section aria-label={translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "말풍선 빠른 변형")} className="space-y-1.5">
        <p className="text-[0.72rem] font-semibold text-fg-3 uppercase tracking-wider">{translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "빠른 변형")}</p>
      ...
      {canCustomize && (
        ...
        <p className="text-[0.72rem] font-semibold text-fg-3 uppercase tracking-wider">{translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "커스텀 모양")}</p>
        {hasCustomShape && (
          <button
            type="button"
            onClick={onRevert}
            title={translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "기본 모양(테마·꼬리 설정 기반)으로 되돌립니다.")}
          ...
          >
            ...
            {translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "되돌리기")}</button>
        )}
      </div>
      ...
      {!hasCustomShape ? (
        <>
          <p className="text-[0.72rem] leading-relaxed text-fg-3">
            {translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "말풍선 외곽선을 촘촘한 점 배열로 바꿔 자유롭게 모양을 조각할 수 있습니다. 전환하면 현재 테두리 둥글기·꼬리 위치를 그대로 반영한 윤곽에서 시작하며, 별·하트 등 특수 모양은 조각의 출발점이 되지 않고 둥근사각형(+꼬리) 윤곽으로 대체됩니다.")}</p>
          <button
            type="button"
            onClick={onConvert}
          ...
          >
            ...
            {translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "커스텀 모양으로 전환")}</button>
        </>
      ) : (
        <>
          <div className="flex items-center justify-between gap-2">
            <span className="text-sm text-fg-2">{translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "점 편집")}</span>
            <StudioToggleChip
              active={active}
              onClick={onToggleEdit}
              title={active ? translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "점 편집을 끕니다.") : translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "점 편집을 켜고 폴리곤 위의 점을 조절합니다.")}
            >
              {active ? translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "편집 중") : translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "편집 시작")}
            </StudioToggleChip>
          </div>
                ...
                <button
                  ...
                  aria-label={
                    selectedPointIndex === null
                      ? translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "가장 긴 외곽선 선분에 점 추가")
                      : formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "선택한 {v0}번 점 다음 선분에 점 추가"), { v0: String(selectedPointIndex + 1) })
                  }
                  ...
                  title={
                    selectedPointIndex === null
                      ? translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "가장 긴 선분의 중점에 새 점을 추가합니다.")
                      : translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "선택한 점과 다음 점 사이의 중점에 새 점을 추가합니다.")
                  }
                >
                  <CirclePlus className="size-4" aria-hidden />
                  {translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "점 추가")}</button>
                <button
                  type="button"
                  aria-label={
                    selectedPointIndex === null
                      ? translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "삭제할 외곽선 점을 먼저 선택")
                      : formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "선택한 {v0}번 외곽선 점 삭제"), { v0: String(selectedPointIndex + 1) })
                  }
                  ...
                  title={
                    pointCount <= 3
                      ? translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "말풍선 외곽선에는 최소 3개의 점이 필요합니다.")
                      : selectedPointIndex === null
                        ? translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "캔버스에서 삭제할 점을 먼저 선택하세요.")
                        : formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "선택한 {v0}번 점을 삭제합니다."), { v0: String(selectedPointIndex + 1) })
                  }
                >
                  <CircleMinus className="size-4" aria-hidden />
                  {translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "선택 점 삭제")}</button>
              </div>
              <p role="status" className="text-[0.68rem] leading-relaxed text-fg-3">
                {selectedPointIndex === null
                  ? formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "선택된 점 없음 · 점 추가는 가장 긴 선분의 중점을 사용합니다. 현재 {v0}개"), { v0: String(pointCount) })
                  : formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "{v0}번 점 선택됨 · 점 추가는 다음 선분의 중점을 사용합니다. 현재 {v1}개"), { v0: String(selectedPointIndex + 1), v1: String(pointCount) })}
              </p>
              {pointCount < 3 ? (
                <p className="text-[0.72rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "편집할 점이 부족합니다.")}</p>
              ) : (
                <p className="text-[0.72rem] leading-relaxed text-fg-3">
                  {translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "점을 끌어 외곽선을 바꿉니다. 키보드에서는 Shift+외곽선 클릭으로 추가하고, Alt+점 클릭 또는 점 선택 후 Delete로 삭제할 수 있습니다.")}</p>
              )}
            </>
          ) : (
            <p className="text-[0.72rem] leading-relaxed text-fg-3">
              {translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "편집을 시작하면 폴리곤 위에 조절 핸들 ")}{pointCount}{translateCurrentStaticSourceText("domains.creator.lettering.StudioBubbleShapePanel", "ko", "개가 나타납니다.")}</p>
          )}
        </>
      )}
        </>
