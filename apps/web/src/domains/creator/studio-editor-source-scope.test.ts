...
describe("local manuscript source identity", () => {
  it.each(["ep01-new", "document-existing", "83000000-0000-4000-8000-000000000001"])(
    "preserves a positively resolved local document %s in any workspace",
    (documentId) => {
      for (const workspace of STUDIO_DOCUMENT_WORKSPACES) {
        ...
        const source = studioEditorSourceRoute({ ...route, workId: null });
...
it("does not turn an unregistered canonical ID into proof of a local document", () => {
  const route = routeFor("/studio/p/unknown/d/ep01-unknown");
  expect(studioEditorSourceRoute(route)).toBe(route);
  expect(isStudioSourceHydrationPending(route.workId, route.remixSourceWorkId, false)).toBe(true);
  expect(studioEditorPersistenceWorkId("local-document", "server-work")).toBe("server-work");
});
