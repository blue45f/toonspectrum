  ...
  return createPortal(<div ref={rootRef} className="fixed inset-0 z-[300] grid place-items-center bg-black/50 p-2 sm:p-4"
    ...
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={headingId} tabIndex={-1}
      ...
      <header className="flex shrink-0 items-center gap-3 border-b border-line px-4 py-2">
        <div className="min-w-0 flex-1"><p className="text-[0.65rem] font-medium text-fg-3">{translateCurrentStaticSourceText("domains.creator.catalog.StudioCatalogPreviewDialog", "ko", "소재 상세 · 적용 전 확인")}</p><h2 id={headingId} className="break-words text-base font-semibold text-fg">{title}</h2></div>
        <button type="button" data-preview-close aria-label={translateCurrentStaticSourceText("domains.creator.catalog.StudioCatalogPreviewDialog", "ko", "상세 미리보기 닫기")} onClick={onClose} className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.catalog.StudioCatalogPreviewDialog", "en", "{v0} w-11 shrink-0"), { v0: String(STUDIO_CATALOG_CONTROL) })}><X size={18} className="mx-auto" aria-hidden /></button>
      </header>
      <div className="min-h-0 overflow-y-auto overscroll-contain p-3 sm:p-4">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <div role="group" aria-label={translateCurrentStaticSourceText("domains.creator.catalog.StudioCatalogPreviewDialog", "ko", "미리보기 배경")} className="flex gap-1">
          ...
          <label className="flex items-center gap-2 text-xs text-fg-2">{translateCurrentStaticSourceText("domains.creator.catalog.StudioCatalogPreviewDialog", "ko", "확대 ")}<input type="range" aria-label={translateCurrentStaticSourceText("domains.creator.catalog.StudioCatalogPreviewDialog", "ko", "미리보기 확대")} min={50} max={200} step={25} value={zoom} onChange={(event) => setZoom(Number(event.target.value))} className="w-24 accent-accent" /><output className="w-10 tabular-nums">{zoom}%</output></label>
        </div>
