...
export function StudioSceneTemplateMap({ summary, label }: { summary: StudioSceneTemplateSummary; label: string }) {
  return <svg role="img" aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.catalog.StudioSceneTemplateMap", "ko", "{v0} 구성도. 실제 작화 렌더가 아닙니다."), { v0: String(label) })} viewBox={`0 0 ${summary.width} ${summary.height}`}
    ...
    {summary.seeds.map((seed, index) => {
      if (seed.type === "frame") return <rect key={index} x={seed.x} y={seed.y} width={seed.width} height={seed.height}
        fill={seed.bgColor ?? translateCurrentStaticSourceText("domains.creator.catalog.StudioSceneTemplateMap", "en", "#ffffff")} stroke={seed.stroke ?? "#282828"} strokeWidth={Math.max(2, seed.strokeWidth ?? 2)} />;
      ...
      return <g key={index} opacity={0.32}>
        ...
        <text x={seed.x + 12} y={seed.y + 28} fill={seed.stroke} fontSize={18}>{seed.type === "focusLines" ? translateCurrentStaticSourceText("domains.creator.catalog.StudioSceneTemplateMap", "ko", "집중선 영역") : translateCurrentStaticSourceText("domains.creator.catalog.StudioSceneTemplateMap", "ko", "속도선 영역")}</text>
      </g>;
    })}
  </svg>;
}
