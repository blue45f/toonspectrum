  ...
  return (
    <>
      <StudioMenuPopoverHeader
        icon={Boxes}
        title={translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "3D 장면")}
        description={translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "도구를 고르는 대신 장면에서 시작하세요. 캐릭터·배경·소품·포즈·카메라·조명을 한 흐름으로 완성합니다.")}
      />

      <section aria-label={translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "3D 장면 시작")} className="mb-3 rounded-2xl border border-accent/25 bg-accent-soft/10 p-2.5">
        <button
          type="button"
        ...
        >
          <span className="flex items-center gap-2 text-[0.72rem] font-bold text-fg">
            ...
            {translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "3D 장면 만들기")}</span>
          <span className="mt-1.5 block text-[0.61rem] leading-relaxed text-fg-3">
            {translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "배경과 캐릭터를 같은 카메라·조명·바닥에 놓고 포즈와 소품을 조정한 뒤 고해상도로 현재 컷에 넣습니다.")}</span>
        </button>

        <p className="mb-1.5 mt-3 px-0.5 text-[0.59rem] font-semibold text-fg-3">{translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "빠른 전문 편집")}</p>
        <div className="grid grid-cols-3 gap-1.5">
          <button type="button" className={QUICK_CARD} onClick={() => closeAnd(() => setCharacterShaperOpen(true))}>
            ...
            <span className="mt-1.5 block text-[0.61rem] font-bold text-fg">{translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "캐릭터 제작")}</span>
            <span className="mt-0.5 block text-[0.54rem] leading-snug text-fg-3">{translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "얼굴·헤어·체형·의상")}</span>
          </button>
          <button type="button" className={QUICK_CARD} onClick={() => closeAnd(() => setPoserVrmOpen(true))}>
            ...
            <span className="mt-1.5 block text-[0.61rem] font-bold text-fg">{translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "포즈·손")}</span>
            <span className="mt-0.5 block text-[0.54rem] leading-snug text-fg-3">{translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "표정·그립·소품 접촉")}</span>
          </button>
          <button type="button" className={QUICK_CARD} onClick={() => closeAnd(() => setMannequinPoserOpen(true))}>
            ...
            <span className="mt-1.5 block text-[0.61rem] font-bold text-fg">{translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "인체 참고")}</span>
            <span className="mt-0.5 block text-[0.54rem] leading-snug text-fg-3">{translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "빠른 데생 포즈")}</span>
          </button>
        </div>
      </section>

      <div className="mb-2 border-t border-line pt-2">
        <p className="mb-1.5 px-0.5 text-[0.62rem] font-semibold text-fg-3">{translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "2D 배경 도구")}</p>
        <StudioMenuSubtabs
          aria-label={translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "배경 메뉴 구역")}
          activeId={menu === "bgScene" || menu === "tone" || menu === "bgFill" ? menu : translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "en", "bgFill")}
      ...
      {menu === "bgFill" && (
        <Suspense fallback={<StudioPanelLoading label={translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "배경 편집기를 여는 중...")} />}>
          <StudioBackgroundPanel
            canvasW={CANVAS_W}
            canvasH={canvasH}
            currentBg={bg}
      ...
      {menu === "bgScene" && (
        <Suspense fallback={<StudioPanelLoading label={translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "2D 장면 라이브러리를 여는 중...")} />}>
          <Studio2dSceneBrowser
      ...
      {menu === "tone" && (
        <>
          <p className="mb-1.5 text-[0.66rem] font-medium text-fg-3">{translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "만화 스크린톤")}</p>
          <p className="mb-2 rounded-lg border border-line bg-card px-2 py-1.5 text-[0.66rem] leading-snug text-fg-3">
            {translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "톤을 누르면 캔버스에 깔려요. 패널을 먼저 선택하면 그 칸을 덮고, 망점 크기는 칸에 맞춰 일정하게 유지됩니다.")}</p>
          <div className="relative mb-2">
            <Search className="absolute left-2 top-1/2 h-3 w-3 -translate-y-1/2 text-fg-3" />
            <input
              type="text"
              placeholder={translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "톤 검색 (망점·선·교차선...)")}
            ...
            {toneSearchQuery && (
              <button
                ...
                aria-label={translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "검색어 지우기")}
          ...
          <div className="max-h-72 overflow-y-auto pr-1">
            <Suspense fallback={<StudioPanelLoading label={translateCurrentStaticSourceText("domains.creator.StudioSceneToolPopoverBody", "ko", "톤 패널을 여는 중...")} />}>
