  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between gap-2">
        <p className="text-[0.66rem] font-semibold uppercase tracking-wider text-fg-3">{translateCurrentStaticSourceText("domains.creator.StudioInkWashPanel", "ko", "수묵/번짐 재질 (Ink Wash)")}</p>
        <button
          type="button"
          onClick={onReset}
          disabled={isIdentity}
          className={buttonClass({ size: "sm", variant: "quiet" })}
          title={translateCurrentStaticSourceText("domains.creator.StudioInkWashPanel", "ko", "수묵·번짐·종이결 재질을 제거하고 원본으로 되돌립니다.")}
        >
          <RotateCcw className="size-3.5" />
          {translateCurrentStaticSourceText("domains.creator.StudioInkWashPanel", "ko", "원본으로")}</button>
      </div>

      <p className="text-[0.68rem] leading-relaxed text-fg-3">
        {translateCurrentStaticSourceText("domains.creator.StudioInkWashPanel", "ko", "원본을 바꾸지 않고 먹의 농담, 젖은 번짐, 한지 섬유와 안료 과립을 합성합니다. 사진·AI 소스·3D 배경 캡처에도 같은 결과로 적용돼요.")}</p>
      ...
      <label className={PANEL_LABEL_ROW}>
        {translateCurrentStaticSourceText("domains.creator.StudioInkWashPanel", "ko", "먹색")}<span className="flex items-center gap-1.5">
          <input
            type="color"
            value={value.inkColor}
            onChange={(event) => onPatch({ inkColor: event.target.value })}
            aria-label={translateCurrentStaticSourceText("domains.creator.StudioInkWashPanel", "ko", "수묵 안료색")}
      ...
      <div className={PANEL_LABEL_ROW}>
        <span>{translateCurrentStaticSourceText("domains.creator.StudioInkWashPanel", "ko", "재질 시드")}</span>
        <span className="flex items-center gap-1.5">
          <StudioPanelChip
            onClick={() => onPatch({ seed: nextSeed(value.seed) })}
            title={translateCurrentStaticSourceText("domains.creator.StudioInkWashPanel", "ko", "안료 과립과 한지 무늬를 결정적으로 다시 뽑습니다. 같은 시드는 언제나 같은 결과입니다.")}
          >
            <RefreshCw className="size-3" aria-hidden /> {translateCurrentStaticSourceText("domains.creator.StudioInkWashPanel", "ko", "새 무늬")}</StudioPanelChip>
