...
const MAX_TRACKS = 20;
const MAX_BASE64_LENGTH = Math.ceil(MUSIC_IMPORTED_MAX_BYTES / 3) * 4;
const MAX_ROW_LENGTH = MAX_BASE64_LENGTH + 40_000;
const PROVIDERS = new Set<MusicProviderId>([
  "elevenlabs",
  "ace-step-local",
  "adobe-firefly",
  "soundverse",
...
function optionalText(value: unknown, max: number): string | undefined {
  ...
  if (typeof value !== "string") throw new Error("음원 제작 정보가 올바르지 않습니다.");
  const clean = value.trim();
  if (!clean || clean.length > max || [...clean].some((character) => {
...
function safeTermsUrl(value: unknown): string {
  if (typeof value !== "string" || value.length > 500) throw new Error("음원 이용 조건 주소가 올바르지 않습니다.");
  try {
    const url = new URL(value);
    if (url.protocol !== "https:") throw new Error();
    return url.toString();
  } catch {
    throw new Error("음원 이용 조건 주소가 올바르지 않습니다.");
  }
}
...
function metadata(value: unknown): MusicTrackMetadata {
  ...
  const source = m.source ?? "generated";
  const generatedByElevenLabs = source === "generated"
    && m.provider === "elevenlabs"
    && ["music_v1", "music_v2_5"].includes(m.model)
    && [MUSIC_LEGACY_OUTPUT_FORMAT, MUSIC_OUTPUT_FORMAT].includes(m.format as typeof MUSIC_OUTPUT_FORMAT);
  const imported = source === "imported"
    && PROVIDERS.has(m.provider)
    && m.model === "external"
    && [MUSIC_IMPORTED_MP3_FORMAT, MUSIC_IMPORTED_WAV_FORMAT].includes(m.format as typeof MUSIC_IMPORTED_MP3_FORMAT);

  if (typeof m.id !== "string" || !/^[a-f0-9]{8}-[a-f0-9]{4}-[1-8][a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/iu.test(m.id)
    || (!generatedByElevenLabs && !imported)
  ...
  const termsUrl = generatedByElevenLabs ? MUSIC_TERMS_URL : safeTermsUrl(m.termsUrl);
  const sourceFilename = optionalText(m.sourceFilename, 255);
  const licenseNote = optionalText(m.licenseNote, 600);
  if (m.sha256 !== undefined && (typeof m.sha256 !== "string" || !/^[a-f0-9]{64}$/iu.test(m.sha256))) {
    throw new Error("음원 무결성 정보가 올바르지 않습니다.");
  }
  ...
  return {
    ...
    ...(typeof m.songId === "string" && /^[a-zA-Z0-9_-]{1,160}$/u.test(m.songId) ? { songId: m.songId } : {}),
    ...(sourceFilename ? { sourceFilename } : {}),
    ...(licenseNote ? { licenseNote } : {}),
    ...(m.sha256 ? { sha256: m.sha256.toLowerCase() } : {}),
    brief: parseMusicBrief(m.brief),
    termsUrl,
  };
}

function byteLimit(value: MusicTrackMetadata): number {
  return value.source === "imported" ? MUSIC_IMPORTED_MAX_BYTES : MUSIC_MAX_BYTES;
}

function isExpectedAudio(bytes: Uint8Array, value: MusicTrackMetadata): boolean {
  return value.format === MUSIC_IMPORTED_WAV_FORMAT ? isWav(bytes) : isMp3(bytes);
}
function parseRow(raw: string | null, ownerId: string): MusicRow | null {
  ...
  if (!row || row.version !== 1 || row.ownerId !== ownerId || typeof row.audioBase64 !== "string"
    || row.audioBase64.length > MAX_BASE64_LENGTH || !/^[A-Za-z0-9+/]+={0,2}$/u.test(row.audioBase64)) {
...
function decodeAudio(base64: string, value: MusicTrackMetadata): Blob {
  const bytes = Uint8Array.from(atob(base64), (character) => character.charCodeAt(0));
  if (bytes.byteLength > byteLimit(value) || !isExpectedAudio(bytes, value)) {
    throw new Error("보관함 음원 형식이 올바르지 않습니다.");
  }
  return new Blob([bytes], { type: musicMimeType(value.format) });
}
...
async function encodeTrack(track: LocalMusicTrack): Promise<string> {
  ...
  const expectedMime = musicMimeType(cleanMetadata.format);
  if (!(audio instanceof Blob) || audio.type !== expectedMime || audio.size > byteLimit(cleanMetadata) || audio.size <= 10) {
    throw new Error("저장할 음원 파일을 확인해 주세요.");
  }
  const bytes = new Uint8Array(await audio.arrayBuffer());
  if (!isExpectedAudio(bytes, cleanMetadata)) throw new Error("저장할 음원 파일을 확인해 주세요.");
  let binary = "";
...
const withProductLock: MusicLibraryLock = async (name, operation) => {
  if (typeof navigator === "undefined" || !navigator.locks) {
    throw new Error("이 환경에서는 안전한 음원 저장을 지원하지 않습니다. 음원 파일을 다운로드해 주세요.");
  }
...
export function createMusicLibrary(acquireDatabase: () => Promise<Database>, lock: MusicLibraryLock = withProductLock) {
  async function access<T>(
    ownerId: string,
    operation: (database: Database, rows: (MusicRow | null)[], keys: string[]) => Promise<T>,
  ): Promise<T> {
  ...
  return {
    load(ownerId: string): Promise<LocalMusicTrack[]> {
      return access(ownerId, async (_database, rows) => rows
        ...
        .map((row) => ({ metadata: row.metadata, ownerId, audio: decodeAudio(row.audioBase64, row.metadata) }))
    ...
    async save(track: LocalMusicTrack): Promise<void> {
      ...
      await access(row.ownerId, async (database, rows, keys) => {
        ...
        if (slot < 0) throw new Error("보관함은 계정당 20곡까지입니다. 음원을 다운로드하거나 기존 곡을 삭제해 주세요.");
