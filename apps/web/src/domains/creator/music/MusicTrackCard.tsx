...
import { MUSIC_ARCS, MUSIC_INTENSITIES, MUSIC_MOODS, musicFilename } from "@toonspectrum/core/studio-music";
  ...
  return (
    <article className="space-y-3 rounded-2xl border border-line bg-card p-4" aria-label={`${b.title} 음원`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0"><h3 className="break-words font-semibold">{b.title}</h3><p className="mt-1 text-xs text-fg-3">{MUSIC_MOODS.find((m) => m.id === b.mood)?.label} · {MUSIC_INTENSITIES.find((entry) => entry.id === b.intensity)?.label ?? "균형"} · {MUSIC_ARCS.find((entry) => entry.id === b.arc)?.label ?? "일정한 분위기"}</p><p className="mt-1 text-xs text-fg-3">요청 {b.seconds}초{duration === null ? "" : ` / 실제 ${duration.toFixed(1)}초`} · {b.vocals ? "보컬" : "연주곡"}{b.episodeId ? ` · ${b.episodeId}` : ""}</p></div>
