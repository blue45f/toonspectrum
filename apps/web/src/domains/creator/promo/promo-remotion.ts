...
export function promoRemotionFiles(input: PromoProject, sources: PromoRenderSources): Record<string, string | Uint8Array> {
  ...
  const panels = project.panels.map((panel, index) => {
    ...
    if (panel.foregroundSrc) {
      const foreground = extractMedia(panel.foregroundSrc);
      const foregroundPath = `foreground-${index + 1}.${foreground.extension}`;
      files[`public/${foregroundPath}`] = foreground.bytes;
      return { ...panel, src: path, foregroundSrc: foregroundPath };
    }
  ...
  let voiceover = project.voiceover;
  if (voiceover) {
    const media = extractMedia(voiceover.src);
    const path = `narration.${media.extension}`;
    files[`public/${path}`] = media.bytes;
    voiceover = { ...voiceover, src: path };
  }
...
const project = {...input, panels: input.panels.map(p => ({...p, src: staticFile(p.src), ...(p.foregroundSrc ? {foregroundSrc: staticFile(p.foregroundSrc)} : {})})), audio: input.audio ? {...input.audio, src: staticFile(input.audio.src)} : null, voiceover: input.voiceover ? {...input.voiceover, src: staticFile(input.voiceover.src)} : null};
...
import {AbsoluteFill, Html5Audio, Sequence, cancelRender, continueRender, delayRender, useCurrentFrame, useVideoConfig} from "remotion";
import {drawPromoFrame, loadPromoImages, type PromoImages} from "./promo-canvas";
import {PROMO_FPS, promoMusicGain, promoVoiceGain, type PromoProject} from "./promo-model";
export const Promo = ({project}: {project: PromoProject}) => {
  const frame = useCurrentFrame();
  const {width, height, durationInFrames} = useVideoConfig();
  const voiceStart = Math.round((project.voiceover?.startSec ?? 0) * PROMO_FPS);
  ...
  return <AbsoluteFill><canvas ref={canvas} width={width} height={height} />{project.audio ? <Html5Audio src={project.audio.src} loop loopVolumeCurveBehavior="extend" volume={f => promoMusicGain(project, f)} /> : null}{project.voiceover && voiceStart < durationInFrames ? <Sequence from={voiceStart} durationInFrames={Math.max(1, Math.min(durationInFrames - voiceStart, Math.ceil(project.voiceover.durationSec * PROMO_FPS)))} layout="none"><Html5Audio src={project.voiceover.src} volume={f => promoVoiceGain(project, f + voiceStart)} /></Sequence> : null}</AbsoluteFill>;
};
`;
...
This is motion-comic animation of supplied still images (camera moves, fades, captions), not generated character acting, lip sync, voice cloning or newly generated video frames. The ending card is included in the selected total duration. BGM is looped with a one-second fade at each end and ducked during the scheduled narration clip. Narration does not loop. Uploaded transparent foregrounds, transitions, procedural particles, focal crops and caption styling use the same renderer. The kit includes SRT, WebVTT and a frame-accurate generic shot-list JSON (not an editor-specific EDL).
