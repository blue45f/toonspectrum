...
const KIND_LABELS: Readonly<Record<StudioRevisionRecord["kind"], Readonly<Record<"ko" | "en", string>>>> = {
...
function formatDate(value: string, copyLocale: "ko" | "en"): string {
    ...
    try {
      ...
      const result = await restoreStudioRevision(
        ...
        currentHeadRevisionId,
        {
          ...
          createdAt: now,
          message: copyLocale === "ko"
            ? `${formatDate(revision.createdAt, copyLocale)} 버전에서 복원`
            : `Restored from ${formatDate(revision.createdAt, copyLocale)}`,
        },
      );
      ...
      setError(await getApiErrorMessage(
        nextError,
        copyLocale === "ko" ? "버전을 복원하지 못했습니다." : "The revision could not be restored.",
      ));
    } finally {
    ...
    return (
      <section className="rounded-3xl border border-line bg-card p-5 shadow-sm">
        <h2 className="text-xl font-black text-fg">
          {copyLocale === "ko" ? "버전 이력" : "Version history"}
        </h2>
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 id="studio-version-stack-title" className="mt-2 text-2xl font-black tracking-tight text-fg">
            {copyLocale === "ko" ? "작업본·검수본·승인본·게시본" : "Working, review, approved and release versions"}
          </h2>
        ...
        >
          ...
          {copyLocale === "ko" ? "이력 새로고침" : "Refresh history"}
        </button>
      </div>
      ...
      <div className="mt-5 space-y-3">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full border border-line bg-card px-2 py-1 text-[0.64rem] font-bold text-fg-2">
                      {KIND_LABELS[revision.kind][copyLocale]}
                    </span>
                    {isHead ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-accent px-2 py-1 text-[0.64rem] font-bold text-on-accent">
                        ...
                        {copyLocale === "ko" ? "현재 작업본" : "Current head"}
                      </span>
                    ) : null}
                    {isApproved ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-success-soft px-2 py-1 text-[0.64rem] font-bold text-success">
                        ...
                        {copyLocale === "ko" ? "승인 이력" : "Approval history"}
                      </span>
                    ) : null}
                  </div>
                  ...
                  <p className="mt-1 text-xs text-fg-3">
                    {formatDate(revision.createdAt, copyLocale)} · {revision.id}
                    {revision.parentIds.length > 0
                      ? ` · ${copyLocale === "ko" ? "부모" : "parent"} ${revision.parentIds.join(", ")}`
                      : ""}
                  </p>
                </div>
                  ...
                  {canRestore ? (
                    ...
                    >
                      ...
                      {confirmRevisionId === revision.id
                        ? copyLocale === "ko" ? "복원 확정" : "Confirm restore"
                        : copyLocale === "ko" ? "새 체크포인트로 복원" : "Restore as checkpoint"}
                    </button>
                  ) : null}
                </div>
              </div>
        ...
        {!loading && revisions.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-line p-6 text-center text-sm text-fg-3">
            {copyLocale === "ko" ? "표시할 클라우드 버전이 없습니다." : "No cloud revisions to display."}
          </p>
        ) : null}
      </div>
    </section>
