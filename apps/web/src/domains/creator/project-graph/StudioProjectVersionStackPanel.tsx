...
export function StudioProjectVersionStackPanel({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(await getApiErrorMessage(
        nextError,
        translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "버전 이력을 불러오지 못했습니다.", "Version history could not be loaded."),
      ));
    } finally {
      setLoading(false);
    }
  ...
  const restore = async (revision: StudioRevisionRecord) => {
    ...
    if (confirmRevisionId !== revision.id) {
      ...
      setNotice(translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "한 번 더 누르면 이 상태를 새 체크포인트로 복원합니다. 현재 이력은 삭제되지 않습니다.", "Press once more to restore this state as a new checkpoint. Current history is preserved."));
      return;
    }
    ...
    try {
      ...
      const result = await restoreStudioRevision(
        ...
        currentHeadRevisionId,
        {
          ...
          createdAt: now,
          message: translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", `${formatDate(revision.createdAt, locale)} 버전에서 복원`, `Restored from ${formatDate(revision.createdAt, locale)}`),
        },
      );
      setHeadOverride(result.headRevisionId);
      setConfirmRevisionId(null);
      setNotice(translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "새 복원 체크포인트를 만들었습니다. 과거와 현재 버전은 모두 보존됩니다.", "Created a restored checkpoint. Both previous and current versions are preserved."));
      await Promise.all([load(), graph.refresh()]);
    } catch (nextError) {
      setError(await getApiErrorMessage(
        nextError,
        translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "버전을 복원하지 못했습니다.", "The revision could not be restored."),
      ));
    } finally {
    ...
    return (
      <section className="rounded-3xl border border-line bg-card p-5 shadow-sm">
        <h2 className="text-xl font-black text-fg">
          {translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "버전 이력", "Version history")}
        </h2>
        <p className="mt-2 text-sm text-fg-2">
          {translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "ProjectGraph로 전환된 제작 문서가 아직 없습니다. 기존 로컬 원고는 계속 자동 저장됩니다.", "No production document has moved to ProjectGraph yet. Existing local documents continue to autosave.")}
        </p>
      </section>
    );
  }
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 id="studio-version-stack-title" className="mt-2 text-2xl font-black tracking-tight text-fg">
            {translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "작업본·검수본·승인본·게시본", "Working, review, approved and release versions")}
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-fg-2">
            {translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", `“${artifact.title}”의 고정된 버전 이력입니다. 복원은 기존 기록을 덮어쓰지 않고 새 체크포인트를 만듭니다.`, `Immutable history for “${artifact.title}”. Restore creates a new checkpoint without overwriting existing records.`)}
          </p>
        </div>
        <button
          type="button"
        ...
        >
          ...
          {translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "이력 새로고침", "Refresh history")}
        </button>
      </div>
      ...
      <div className="mt-5 space-y-3">
              ...
              <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full border border-line bg-card px-2 py-1 text-[0.64rem] font-bold text-fg-2">
                      {translateLocaleBranchForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", KIND_LABELS[revision.kind])}
                    </span>
                    {isHead ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-accent px-2 py-1 text-[0.64rem] font-bold text-on-accent">
                        ...
                        {translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "현재 작업본", "Current head")}
                      </span>
                    ) : null}
                    {isApproved ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-success-soft px-2 py-1 text-[0.64rem] font-bold text-success">
                        ...
                        {translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "승인 이력", "Approval history")}
                      </span>
                    ) : null}
                  </div>
                  ...
                  <p className="mt-1 text-xs text-fg-3">
                    ...
                    {revision.parentIds.length > 0
                      ? ` · ${translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "부모", "parent")} ${revision.parentIds.join(", ")}`
                      : ""}
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    ...
                    {translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "비교", "Compare")}
                  </Link>
                  {canRestore ? (
                    <button
                      type="button"
                    ...
                    >
                      ...
                      {confirmRevisionId === revision.id
                        ? translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "복원 확정", "Confirm restore")
                        : translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "새 체크포인트로 복원", "Restore as checkpoint")}
                    </button>
                  ) : null}
                </div>
              </div>
        ...
        {!loading && revisions.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-line p-6 text-center text-sm text-fg-3">
            {translateBilingualValueForLocale(locale, "domains.creator.project.graph.StudioProjectVersionStackPanel", "표시할 클라우드 버전이 없습니다.", "No cloud revisions to display.")}
          </p>
        ) : null}
      </div>
    </section>
