...
export function StudioBrushV6ProviderPlan({
  ...
  return (
    <section
      aria-label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6ProviderPlan", "ko", "무폴백 브러시 엔진 바인딩")}
      className="rounded-2xl border border-line bg-card/60 p-4 shadow-sm"
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-black text-fg">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6ProviderPlan", "ko", "무폴백 엔진 바인딩")}</h2>
          <p className="mt-1 text-xs leading-5 text-fg-3">
            {translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6ProviderPlan", "ko", "저장·협업·재생은 아래 프로바이더와 버전을 고정합니다. 실행 불가 시 다른 엔진으로 바꾸지 않습니다.")}</p>
        </div>
        <span className="rounded-full border border-accent/40 bg-accent/10 px-3 py-1 text-[0.68rem] font-black text-accent">
          {plan.licenseProfile} {translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6ProviderPlan", "en", "· fallback none")}</span>
      </div>
      ...
      {plan.blockedNodes.length > 0 ? (
        <div role="alert" className="mt-3 rounded-xl border border-danger/40 bg-danger/10 p-3 text-xs text-danger">
          <p className="font-black">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6ProviderPlan", "ko", "현재 저장 브러시 경로에서 실행할 수 없는 노드")}</p>
        ...
        {plan.bindings.map((binding) => (
          <article key={`${binding.providerId}:${binding.execution}`} className={ITEM}>
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <span className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6ProviderPlan", "en", "rounded-full border px-2 py-1 text-[0.62rem] font-black {v0}"), { v0: String(binding.execution === "native"
                  ? "border-accent/40 bg-accent/10 text-accent"
                  : "border-warning/40 bg-warning/10 text-warning") })}>
                {binding.execution === "native" ? translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6ProviderPlan", "ko", "제품 커널") : translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6ProviderPlan", "ko", "명시적 호환 어댑터")}
              </span>
            </div>
