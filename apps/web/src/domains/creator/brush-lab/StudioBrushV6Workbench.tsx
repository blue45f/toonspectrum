...
function materialNodeStatus(id: string, activeTuning: ReadonlySet<keyof BrushStudioV6Tuning>, topology = false): string {
  if (topology && !isBrushStudioV6TopologyNodeCompatible(id)) return "현재 획 구조와 미연결 · 실행하지 않음";
  ...
  return (
    <div className={`${SUB} ${inactive ? "opacity-55" : ""}`}>
      <div className="flex items-center justify-between gap-2 text-xs font-bold text-fg-2">
        ...
        <input type="number" aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "{v0} 직접 입력"), { v0: String(spec.label) })} min={spec.min} max={spec.max} step={spec.step} value={draft} disabled={inactive}
          ...
          className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "min-h-8 w-20 rounded-md border border-line bg-card px-1.5 text-right tabular-nums text-fg {v0}"), { v0: String(STUDIO_FOCUS_RING) })} />
      </div>
      ...
      {inactive ? <p className="mt-1 text-[0.65rem] leading-4 text-fg-3">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "현재 재료 조합에서 사용하지 않음")}</p> : null}
    </div>
  );
}
...
function PressureCurve({ program }: { readonly program: BrushStudioV6Program }) {
  ...
  return <figure className="mb-4 rounded-xl border border-line bg-bg-2/55 p-3">
    <figcaption className="text-xs font-bold text-fg-2">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "필압 반응 곡선")}</figcaption>
    <svg viewBox="0 0 240 165" role="img" aria-label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "가로축 입력 필압, 세로축 획에 적용되는 필압")} className="mt-2 h-40 w-full">
      ...
      <text x="20" y="156" className="fill-fg-3 text-[10px]">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "가볍게")}</text>
      <text x="190" y="156" className="fill-fg-3 text-[10px]">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "강하게")}</text>
    </svg>
...
function Select({ id, label, value, options, onChange }: { readonly id: string; readonly label: string; readonly value: string; readonly options: readonly { id: string; label: string; disabled?: boolean }[]; readonly onChange: (value: string) => void }) {
  return (
    <label htmlFor={id} className="block text-xs font-bold text-fg-2">{label}
      <select id={id} value={value} onChange={(event: ChangeEvent<HTMLSelectElement>) => onChange(event.currentTarget.value)} className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "{v0} mt-1.5"), { v0: String(INPUT) })}>
...
function ToggleNode({ node, selected, onChange, topology = false }: { readonly node: BrushStudioV6NodeDescriptor; readonly selected: boolean; readonly onChange: () => void; readonly topology?: boolean }) {
  ...
  return (
    <button type="button" aria-pressed={selected} disabled={!connected && !selected} onClick={onChange} className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "{v0} min-h-[92px] text-left transition-colors disabled:opacity-50 {v1} {v2}"), { v0: String(SUB), v1: String(selected ? "border-accent/60 bg-accent/10" : "hover:border-line-strong"), v2: String(STUDIO_FOCUS_RING) })}>
...
function Issue({ issue }: { readonly issue: BrushStudioV6Issue }) {
  ...
  return <article className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "rounded-xl border p-3 {v0}"), { v0: String(tone) })}><div className="flex justify-between gap-2"><h3 className="text-xs font-black text-fg">{issue.title}</h3><span className="text-[0.62rem] font-black uppercase text-fg-3">{issue.severity}</span></div><p className="mt-1 text-xs text-fg-3">{issue.detail}</p>{issue.fix ? <p className="mt-2 text-xs font-bold text-fg-2">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "해결: ")}{issue.fix}</p> : null}</article>;
}
    ...
    <section ref={workbenchRef} className="space-y-4">
      <div className={CARD}>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            <p className="text-[0.68rem] font-black uppercase tracking-[0.2em] text-accent">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "BRUSH STUDIO · FULL EDIT")}</p>
            <h2 className="mt-1 text-lg font-black text-fg">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "브러시 만들기·전체 편집")}</h2>
            <p className="mt-1 text-xs leading-relaxed text-fg-3">{BRUSH_STUDIO_V6_RECIPES.length}{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "개 시작 레시피에서 골라 실제 획을 비교하세요. 기본 편집은 결과 중심으로 단순하게, 전문가 설정은 엔진과 물리까지 단계적으로 엽니다.")}</p>
            <div role="group" aria-label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "브러시 편집 깊이")} className="mt-3 inline-flex rounded-xl border border-line bg-bg-2/55 p-1">
              <button type="button" aria-pressed={experience === "guided"} onClick={() => changeExperience("guided")} className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "min-h-[44px] rounded-lg px-3 text-xs font-black {v0} {v1}"), { v0: String(experience === "guided" ? "bg-accent text-on-accent" : "text-fg-2 hover:bg-raised"), v1: String(STUDIO_FOCUS_RING) })}>{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "기본 편집")}</button>
              <button type="button" aria-pressed={experience === "expert"} onClick={() => changeExperience("expert")} className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "min-h-[44px] rounded-lg px-3 text-xs font-black {v0} {v1}"), { v0: String(experience === "expert" ? "bg-accent text-on-accent" : "text-fg-2 hover:bg-raised"), v1: String(STUDIO_FOCUS_RING) })}>{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "전문가 설정")}</button>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <button type="button" className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "{v0} disabled:opacity-50"), { v0: String(PRIMARY) })} disabled={saving || !analysis.valid} title={analysis.valid ? undefined : translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "실행할 수 없는 엔진 조합은 저장하지 않습니다.")} onClick={() => { void saveToStudio(); }}>{saving ? translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "브러시 저장 중…") : translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "브러시로 저장")}</button>
            {savedHref ? <a href={savedHref} className={PRIMARY}>{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "원고에서 사용하기")}</a> : null}
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-4">
          <label htmlFor="brush-v6-name" className="text-xs font-bold text-fg-2">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "브러시 이름")}<input id="brush-v6-name" value={program.name} onChange={(event: ChangeEvent<HTMLInputElement>) => replace(patchBrushStudioV6Metadata(program, { name: event.currentTarget.value }), undefined, "name")} className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "{v0} mt-1.5"), { v0: String(INPUT) })} /></label>
          <Select id="brush-v6-goal" label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "품질 목표")} value={program.qualityGoal} options={[{ id: "responsive", label: "최저 지연" }, { id: "balanced", label: "균형" }, { id: "material", label: "재료 충실도" }, { id: "cinematic", label: "출력 극대화" }]} onChange={(value) => replace(patchBrushStudioV6Metadata(program, { qualityGoal: value as BrushStudioV6Program["qualityGoal"] }))} />
          <Select id="brush-v6-device" label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "대상 장치")} value={program.deviceProfile} options={[{ id: "desktop-pen", label: "데스크톱 펜" }, { id: "tablet-pen", label: "태블릿 펜" }, { id: "touch-hybrid", label: "펜+터치" }, { id: "mouse", label: "마우스" }]} onChange={(value) => replace(patchBrushStudioV6Metadata(program, { deviceProfile: value as BrushStudioV6Program["deviceProfile"] }))} />
          <div className={SUB}><p className="text-xs font-bold text-fg-2">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "조합 진단")}</p><p className="mt-1 text-sm font-black text-fg">{analysis.valid ? translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "조합 검사 통과") : translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "확인할 조합 있음")}</p><p className="mt-1 text-[0.68rem] text-fg-3">{analysis.issues.filter((issue) => issue.severity !== "info").length}{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "개 확인 사항 · 설정 자동 저장")}</p></div>
        </div>
        <p role="status" className="mt-3 rounded-xl border border-line bg-bg-2/55 px-3 py-2 text-xs text-fg-2">{status}</p>
      </div>

      <nav aria-label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "브러시 스튜디오 영역")} className="flex gap-2 overflow-x-auto rounded-2xl border border-line bg-card/45 p-2">
        {tabs.map((item) => <button key={item.id} type="button" aria-pressed={tab === item.id} onClick={() => setTab(item.id)} className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "min-h-[44px] shrink-0 rounded-xl px-3 text-xs font-black {v0} {v1}"), { v0: String(tab === item.id ? "bg-accent text-on-accent" : "text-fg-2 hover:bg-raised"), v1: String(STUDIO_FOCUS_RING) })}>{item.label}</button>)}
      </nav>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,0.75fr)]">
        <div className="space-y-4">
          ...
          {tab === "recipes" ? <Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "시그니처 레시피")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "이름·용도·영문 ID로 찾아 실제 질감과 조절값을 확인하세요.")}><StudioBrushV6RecipeShelf selectedId={program.id} onChoose={chooseRecipe} /></Panel> : null}

          {tab === "graph" ? <Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "Engine Graph")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "실제 획은 공통 CPU 접촉 계산기를 사용합니다. 회색 항목은 아직 연결되지 않은 설계 기록이며, 외부 엔진을 실행하지 않습니다.")}><div className="grid gap-4 lg:grid-cols-2">{SINGLE_SLOTS.map(({ slot, label, description }) => { const selected = program.slots[slot]; const node = brushStudioV6Node(selected); return <div key={slot} className={SUB}><Select id={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "brush-v6-slot-{v0}"), { v0: String(slot) })} label={label} value={selected} options={nodeOptions(slot, Boolean(topology))} onChange={(id) => choose(slot, id)} /><p className="mt-2 text-xs text-fg-3">{description}</p><p className="mt-1 text-[0.68rem] font-bold text-accent">{materialNodeStatus(node.id, activeTuning, Boolean(topology))}</p></div>; })}</div></Panel> : null}
          ...
          {tab === "material" ? <Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "재료·표면")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "종이 접촉, 도포, 안료 pickup과 높이 표현을 직접 조절합니다.")}><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{MATERIAL.map((spec) => <Slider key={spec.key} spec={spec} value={program.tuning[spec.key]} inactive={!activeTuning.has(spec.key)} onChange={(value) => patchTuning(spec.key, value)} />)}<label htmlFor="brush-v6-primary" className={SUB}><span className="text-xs font-bold text-fg-2">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "기본 색")}</span><input id="brush-v6-primary" type="color" value={program.tuning.primaryColor} onChange={(event: ChangeEvent<HTMLInputElement>) => replace(patchBrushStudioV6Tuning(program, { primaryColor: event.currentTarget.value }))} className="mt-2 h-10 w-full rounded-lg" /></label><label htmlFor="brush-v6-secondary" className={SUB}><span className="text-xs font-bold text-fg-2">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "혼합·패턴 색")}</span><input id="brush-v6-secondary" type="color" value={program.tuning.secondaryColor} disabled={!activeTuning.has("secondaryColor")} aria-describedby={!activeTuning.has("secondaryColor") ? translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "brush-v6-secondary-inactive") : undefined} onChange={(event: ChangeEvent<HTMLInputElement>) => replace(patchBrushStudioV6Tuning(program, { secondaryColor: event.currentTarget.value }))} className="mt-2 h-10 w-full rounded-lg disabled:opacity-55" />{!activeTuning.has("secondaryColor") ? <span id="brush-v6-secondary-inactive" className="mt-1 block text-[0.65rem] leading-4 text-fg-3">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "현재 재료 조합에서 사용하지 않음")}</span> : null}</label></div></Panel> : null}

          {tab === "physics" ? <><Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "물리 엔진")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "강모 접촉·안료 소모·입자·번짐 근사를 조합합니다. 실제 유체 격자나 캔버스 아래색 픽업은 실행하지 않습니다.")}><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{physicsNodes.map((node) => <ToggleNode key={node.id} topology={Boolean(topology)} node={node} selected={program.slots.physics.includes(node.id)} onChange={() => replace(toggleBrushStudioV6Node(program, "physics", node.id))} />)}</div></Panel><Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "물리 파라미터")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "현재 획에 실제로 쓰이는 설정만 조절할 수 있습니다. 회색 설정은 다른 재료 조합에서 활성화됩니다.")}><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{(topology?.controls ?? PHYSICS).map((spec) => <Slider key={spec.key} spec={spec} value={program.tuning[spec.key]} inactive={!activeTuning.has(spec.key)} onChange={(value) => patchTuning(spec.key, value)} />)}</div></Panel><Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "마감 패스")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "현재 재료 계산에 연결된 마감만 선택할 수 있습니다. 회색 항목은 실행되지 않는 설계 기록입니다.")}><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{finishNodes.map((node) => <ToggleNode key={node.id} topology={Boolean(topology)} node={node} selected={program.slots.finish.includes(node.id)} onChange={() => replace(toggleBrushStudioV6Node(program, "finish", node.id))} />)}</div></Panel></> : null}

          {tab === "pattern" ? <><Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "패턴·문양")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "반복 토폴로지와 문서 위상이 다른 패턴은 독립 브러시 정체성으로 유지합니다.")}><Select id="brush-v6-pattern" label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "패턴 프로그램")} value={program.slots.pattern} options={nodeOptions("pattern", Boolean(topology))} onChange={(id) => choose("pattern", id)} /><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{PATTERN.map((spec) => <Slider key={spec.key} spec={spec} value={program.tuning[spec.key]} inactive={!activeTuning.has(spec.key)} onChange={(value) => patchTuning(spec.key, value)} />)}</div><p className="mt-3 text-xs leading-5 text-fg-3">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "같은 개성 시드와 접촉 간격은 저장·다시 열기·내보내기에도 보존됩니다.")}</p></Panel></> : null}

          {tab === "runtime" ? <><StudioBrushV6ProviderPlan plan={analysis.providerPlan} /><Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "실행 패스")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "선택한 그래프의 실행 설계입니다. 아래 비용과 백엔드는 설계 추정이며 측정된 실행 결과가 아닙니다.")}><div className="space-y-2">{analysis.passes.map((pass) => <article key={pass.id} className={SUB}><div className="flex flex-wrap justify-between gap-2"><h3 className="text-xs font-black text-fg">{pass.label}</h3><span className="text-[0.65rem] font-black text-accent">{pass.phase} · {pass.domain} · {pass.budgetMs.toFixed(2)}ms</span></div><p className="mt-1 text-[0.68rem] text-fg-3">{pass.nodeIds.map((id) => brushStudioV6Node(id).provider).filter((value, index, values) => values.indexOf(value) === index).join(" + ")} · {pass.nodeIds.join(" → ") || translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "presentation")}</p></article>)}</div></Panel><Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "설계상 리소스 추정")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "그래프 모델에서 산출한 예상 필드와 메모리입니다. 현재 브라우저가 실제 할당한 메모리와 다를 수 있습니다.")}><div className="grid gap-2 sm:grid-cols-2">{analysis.resources.map((resource) => <div key={resource.id} className={SUB}><div className="flex justify-between text-xs font-black text-fg"><span>{resource.id}</span><span>{resource.format}</span></div><p className="mt-1 text-xs text-fg-3">{resource.owner} {translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "· scale ")}{resource.scale}</p><p className="mt-1 text-[0.68rem] font-bold text-fg-2">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "약 ")}{resource.estimatedMb.toFixed(1)}MB</p></div>)}</div></Panel><Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "브라우저 능력")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "브라우저가 제공하는 API 유무입니다. API 지원이 개별 외부 엔진의 연결이나 실행을 보장하지는 않습니다.")}><div className="flex flex-wrap gap-2">{capabilityRows(capabilities).map((item) => <span key={item.label} className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "rounded-full border px-3 py-1.5 text-[0.68rem] font-bold {v0}"), { v0: String(item.enabled ? "border-accent/40 bg-accent/10 text-accent" : "border-line bg-bg-2 text-fg-3") })}>{item.label} · {item.enabled ? translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "YES") : translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "NO")}</span>)}</div></Panel></> : null}
        </div>

        <aside className="space-y-4 xl:sticky xl:top-4 xl:self-start">
          <Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "결정적 질감 시험지")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "동일 seed로 종이·안료·입자·습식·강모·패턴 결과를 비교합니다.")}><canvas ref={previewRef} role="img" aria-label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "현재 브러시의 결정적 질감 시험지")} className="h-64 w-full rounded-xl border border-line bg-white" /></Panel>
          <Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "실제 입력 필기 패드")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "펜·마우스·허용된 터치로 필압·틸트·팜 차단과 재질 반응을 확인합니다.")}><canvas ref={liveRef} aria-label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "브러시 스튜디오 실제 입력 필기 패드")} className="h-72 w-full cursor-crosshair rounded-xl border border-line bg-white" /><div className="mt-3 grid grid-cols-3 gap-2 text-center text-[0.68rem]"><div className={SUB}><p className="font-bold text-fg-3">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "입력률")}</p><p className="mt-1 font-black text-fg">{telemetry.sampleRateHz}Hz</p></div><div className={SUB}><p className="font-bold text-fg-3">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "필압")}</p><p className="mt-1 font-black text-fg">{telemetry.pressure.toFixed(2)}</p></div><div className={SUB}><p className="font-bold text-fg-3">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "Tilt")}</p><p className="mt-1 font-black text-fg">{telemetry.tilt.toFixed(0)}°</p></div></div><div className="mt-2 flex items-center justify-between gap-2 text-[0.68rem] text-fg-3"><span>{telemetry.pointerType} {translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "· twist ")}{telemetry.twist.toFixed(0)}° · {telemetry.rejectedPalm ? translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "팜 차단") : translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "입력 수락")}</span><button type="button" className={BUTTON} onClick={() => controllerRef.current?.clear()}>{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "지우기")}</button></div></Panel>
          <Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "조합 진단·품질 추정")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "설정에서 계산한 참고 점수입니다. 오류와 경고는 조합의 제약을 알려주며 실제 속도와 결과는 직접 그려서 확인하세요.")}><div className="grid grid-cols-2 gap-2"><Metric label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "필기감")} value={analysis.metrics.handFeel} /><Metric label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "재료")} value={analysis.metrics.materialFidelity} /><Metric label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "색")} value={analysis.metrics.colorFidelity} /><Metric label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "시간 물리")} value={analysis.metrics.temporalFidelity} /><Metric label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "개성")} value={analysis.metrics.uniqueness} /><Metric label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "성능")} value={analysis.metrics.performance} /><Metric label={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "결정성")} value={analysis.metrics.determinism} /></div><div className="mt-3 space-y-2">{analysis.issues.length ? analysis.issues.map((entry) => <Issue key={entry.id} issue={entry} />) : <p className="rounded-xl border border-accent/35 bg-accent/10 p-3 text-xs font-black text-accent">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "그래프 조합 검사에서 확인할 사항이 없습니다.")}</p>}</div></Panel>
          {experience === "expert" ? <Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "선택한 재료·엔진 구성")} description={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "{v0}/{v1}개 타입 노드가 현재 프로그램을 구성합니다."), { v0: String(analysis.nodes.length), v1: String(BRUSH_STUDIO_V6_NODES.length) })}><div className="flex flex-wrap gap-1.5">{analysis.nodes.map((node) => <span key={node.id} title={node.description} className="rounded-full border border-line bg-bg-2 px-2.5 py-1 text-[0.65rem] font-bold text-fg-2">{node.label}</span>)}</div></Panel> : null}
        </aside>
      </div>
    </section>
  );
