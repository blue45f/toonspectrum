...
const SINGLE_SLOTS: readonly { slot: SingleSlot; label: string; description: string }[] = [
  ...
  { slot: "pigment", label: "안료", description: "Spectral.js·open-km 분광, ColorMix Lab 비교, 기존 WGM·Mixbox. 모델별 가정은 재료 탭에서 확인하세요." },
    ...
    <section ref={workbenchRef} className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,0.75fr)]">
        <div className="space-y-4">
          ...
          {tab === "material" ? <StudioPigmentComparison primary={program.tuning.primaryColor}
            secondary={program.tuning.secondaryColor} node={program.slots.pigment}
            secondaryActive={activeTuning.has("secondaryColor")} onSelect={(id) => choose("pigment", id)} /> : null}
          ...
          {tab === "runtime" ? <><StudioBrushV6ProviderPlan plan={analysis.providerPlan} /><Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "실행 패스")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "선택한 그래프의 실행 설계입니다. 아래 비용과 백엔드는 설계 추정이며 측정된 실행 결과가 아닙니다.")}><div className="space-y-2">{analysis.passes.map((pass) => <article key={pass.id} className={SUB}><div className="flex flex-wrap justify-between gap-2"><h3 className="text-xs font-black text-fg">{pass.label}</h3><span className="text-[0.65rem] font-black text-accent">{pass.phase} · {pass.domain} · {pass.budgetMs.toFixed(2)}ms</span></div><p className="mt-1 text-[0.68rem] text-fg-3">{pass.nodeIds.map((id) => brushStudioV6Node(id).provider).filter((value, index, values) => values.indexOf(value) === index).join(" + ")} · {pass.nodeIds.map((id) => brushStudioV6Node(id).label).join(" → ") || translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "presentation")}</p></article>)}</div></Panel><Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "설계상 리소스 추정")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "그래프 모델에서 산출한 예상 필드와 메모리입니다. 현재 브라우저가 실제 할당한 메모리와 다를 수 있습니다.")}><div className="grid gap-2 sm:grid-cols-2">{analysis.resources.map((resource) => <div key={resource.id} className={SUB}><div className="flex justify-between text-xs font-black text-fg"><span>{resource.id}</span><span>{resource.format}</span></div><p className="mt-1 text-xs text-fg-3">{resource.owner} {translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "· scale ")}{resource.scale}</p><p className="mt-1 text-[0.68rem] font-bold text-fg-2">{translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "약 ")}{resource.estimatedMb.toFixed(1)}MB</p></div>)}</div></Panel><Panel title={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "브라우저 능력")} description={translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "ko", "브라우저가 제공하는 API 유무입니다. API 지원이 개별 외부 엔진의 연결이나 실행을 보장하지는 않습니다.")}><div className="flex flex-wrap gap-2">{capabilityRows(capabilities).map((item) => <span key={item.label} className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "rounded-full border px-3 py-1.5 text-[0.68rem] font-bold {v0}"), { v0: String(item.enabled ? "border-accent/40 bg-accent/10 text-accent" : "border-line bg-bg-2 text-fg-3") })}>{item.label} · {item.enabled ? translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "YES") : translateCurrentStaticSourceText("domains.creator.brush.lab.StudioBrushV6Workbench", "en", "NO")}</span>)}</div></Panel></> : null}
        </div>
