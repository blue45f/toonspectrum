...
describe("Brush Studio V6 provider runtime", () => {
  ...
  it("blocks an unavailable p5 carrier instead of substituting another engine", () => {
    const plan = planBrushStudioV6ProviderRuntime(nodes(createBrushStudioV6Program("moss-flow")));
    expect(plan.valid).toBe(false);
    expect(plan.blockedNodeIds).toContain("carrier-p5-flow");
