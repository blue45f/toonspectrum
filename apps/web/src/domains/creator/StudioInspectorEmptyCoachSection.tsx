  ...
  return (
    <>
          {inspectorContentMode === "empty" && !unselectedImageToolsVisible && (
            ...
            >
              <p className="text-xs font-bold tracking-tight text-fg">{translateCurrentStaticSourceText("domains.creator.StudioInspectorEmptyCoachSection", "ko", "캔버스에서 바로 시작")}</p>
              <p className="mt-1 text-[0.6875rem] leading-snug text-fg-3">
                {translateCurrentStaticSourceText("domains.creator.StudioInspectorEmptyCoachSection", "ko", "빈 화면에 그려도 됩니다. V 로 선택, 모서리 핸들로 크기 조절, 레이어는 위 탭에서 바로 열려요.")}</p>
              {/*
              ...
              */}
              <div className="mt-2.5 grid grid-cols-2 gap-1.5">
                <button
                  type="button"
                  aria-label={translateCurrentStaticSourceText("domains.creator.StudioInspectorEmptyCoachSection", "ko", "펜으로 그리기")}
                ...
                >
                  {translateCurrentStaticSourceText("domains.creator.StudioInspectorEmptyCoachSection", "ko", "펜으로 그리기")}<span className="mt-0.5 block text-[0.6875rem] font-medium text-fg-3">{translateCurrentStaticSourceText("domains.creator.StudioInspectorEmptyCoachSection", "ko", "단축키 B")}</span>
                </button>
                <button
                  ref={imageEditButtonRef}
                  type="button"
                  aria-label={translateCurrentStaticSourceText("domains.creator.StudioInspectorEmptyCoachSection", "ko", "이미지 편집 · 전문 도구 열기")}
                  data-inspector-priority="essential"
                ...
                >
                  {translateCurrentStaticSourceText("domains.creator.StudioInspectorEmptyCoachSection", "ko", "이미지 편집")}<span className="mt-0.5 block text-[0.6875rem] font-medium text-fg-3">{translateCurrentStaticSourceText("domains.creator.StudioInspectorEmptyCoachSection", "ko", "가져오기·선택·합성본 준비")}</span>
                </button>
              </div>
              <button
                type="button"
                aria-label={translateCurrentStaticSourceText("domains.creator.StudioInspectorEmptyCoachSection", "ko", "스튜디오 사용법 따라 하기")}
                data-inspector-priority="essential"
              ...
              >
                {translateCurrentStaticSourceText("domains.creator.StudioInspectorEmptyCoachSection", "ko", "처음이라면 사용법 따라 하기")}<span className="mt-0.5 block text-[0.6875rem] font-medium text-fg-3">
                  {translateCurrentStaticSourceText("domains.creator.StudioInspectorEmptyCoachSection", "ko", "핵심 도구를 화면에서 차례로 안내해요")}</span>
              </button>
            </div>
          )}
    </>
