...
describe("StudioPageListPane", () => {
  ...
  it("keeps named page controls at 44px on desktop and mobile", () => {
    // The cross-browser soak found that desktop overrides shrank real hit areas to 24px.
    // Keep the same 44px control contract at every supported viewport.
    const props = createProps();
    render(<StudioPageListPane {...props} />);

    const collapse = screen.getByRole("button", { name: "페이지 목록 접기" });
    expect(collapse.getAttribute("aria-label")).toBe("페이지 목록 접기");
    ...
    for (const label of rowActionLabels) {
      ...
      expect(button.className, label).not.toContain("lg:size-6");
    ...
    expect(metaEdit.className).toContain("size-11");
    expect(metaEdit.className).not.toContain("lg:size-6");
