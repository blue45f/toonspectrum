...
export async function prepareStudioMarketplaceCc0ModelScene(
  runtimeRef: string,
  scopeOrCurrent: StudioMarketplaceModelImportScope | (() => boolean),
): Promise<{ scene: StudioBg3dSceneDocument; name: string; cancel: () => Promise<boolean> }> {
  const requested = typeof scopeOrCurrent === "function" ? { isCurrent: scopeOrCurrent } : scopeOrCurrent;
  const controller = new AbortController();
  const abort = () => controller.abort(requested.signal?.reason);
  requested.signal?.addEventListener("abort", abort, { once: true });
  if (requested.signal?.aborted) abort();
  const timeout = setTimeout(() => controller.abort(), 60_000);
  const scope = { isCurrent: requested.isCurrent, signal: controller.signal };
  try {
  assertCurrent(scope);
  ...
  try {
    ...
    assertCurrent(scope);
    return { scene, name: asset.name, cancel: () => library.compensateImportedBg3dModelsIfCreationMatchesV12(disposition) };
  } catch (error) {
    ...
    requested.signal?.removeEventListener("abort", abort);
  }
}
