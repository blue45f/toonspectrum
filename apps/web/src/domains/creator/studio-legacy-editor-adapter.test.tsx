...
describe("local manuscript editor entry", () => {
  it("preserves a positively resolved local source at the adapter boundary", () => {
    ...
    render(<LegacyStudioEditorAdapter remixId={null} studioRoute={{ ...route, workId: null }} />);
...
it("retains the fail-closed source for unresolved canonical document routes", () => {
  const route = parseStudioWorkspaceRoute({ pathname: "/studio/p/unknown/d/remote-document" });
  if (!route.valid) throw new Error("Invalid route fixture");
  render(<LegacyStudioEditorAdapter remixId={null} studioRoute={route} />);
  expect(screen.getByTestId("editor-source").dataset.workId).toBe("remote-document");
});
