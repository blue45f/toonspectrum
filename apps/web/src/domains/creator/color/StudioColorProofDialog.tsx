...
function PixelPreview({ pixels, width, height }: { pixels: Uint8ClampedArray; width: number; height: number }) {
  ...
  return <canvas ref={ref} width={width} height={height} role="img" aria-label={translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "선택한 ICC 프로필의 소프트 프루프")} className="max-h-[45dvh] max-w-full object-contain" style={{ background: "repeating-conic-gradient(#ddd 0% 25%, #fff 0% 50%) 50% / 16px 16px" }} />;
}
...
export default function StudioColorProofDialog({ host, onClose }: { host: StudioColorProofHost; onClose: () => void }) {
  ...
  return createPortal(<div className="fixed inset-0 z-[180] flex items-center justify-center bg-black/45 p-2 sm:p-4" data-studio-color-proof="true">
    <div ref={formRef} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1} data-studio-shortcut-boundary="true" className="max-h-[calc(100dvh-1rem)] w-full max-w-3xl overflow-y-auto rounded-2xl border border-line bg-card p-4 text-fg shadow-2xl">
      <h2 id={titleId} className="text-lg font-semibold">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "ICC 색상 확인·출력")}</h2>
      <p className="mt-2 text-sm text-fg-2">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "원고는 sRGB로 유지합니다. 선택한 RGB 프로필의 상대색도 변환 결과를 확인하고, 원본 ICC가 포함된 RGB 8비트 PNG로 내보냅니다.")}</p>
      <p className="mt-2 text-xs text-fg-3">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "현재 범위: 4KiB 이하 RGB matrix/TRC, 최대 16메가픽셀. CMYK LUT·인쇄용 잉크량·HDR 출력은 지원하지 않습니다. 일반 이미지 다운로드는 기존 sRGB입니다.")}</p>
      <label className="mt-3 flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" checked={authorized} onChange={event => setAuthorized(event.target.checked)} />{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "이 ICC 파일을 변환과 출력 파일에 포함할 권한이 있습니다.")}</label>
      <label className="block text-sm">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "ICC 프로필 선택")}<input type="file" accept=".icc,.icm,application/vnd.iccprofile" className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "en", "{v0} mt-1 w-full"), { v0: String(CONTROL) })} disabled={busy || host.disabled || !authorized} onChange={event => { const file = event.target.files?.[0]; event.target.value = ""; if (file) void importFile(file); }} /></label>
      {host.page.colorProof ? <div className="mt-2 flex flex-wrap items-center gap-2 text-sm"><span>{host.page.colorProof.profile.name} · {transform?.profile.description || translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "프로필 확인 중")}</span><button className={CONTROL} type="button" disabled={busy || host.disabled} onClick={() => { if (!host.commitProfile(host.page, undefined)) setError("현재 문서의 설정을 변경할 수 없습니다."); }}>{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "프로필 해제")}</button></div> : null}
      {selected ? <p className="mt-2 text-sm" role="status">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "현재 선택 색 ")}{host.color}: {selected.outOfGamut ? translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "대상 프로필 색역 밖 — 출력 시 색역 경계로 제한됩니다.") : translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "대상 프로필 색역 안")}</p> : null}
      <div className="mt-3 flex flex-wrap gap-2"><button type="button" className={CONTROL} disabled={busy || !transform} onClick={() => void preparePreview()}>{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "현재 페이지로 소프트 프루프")}</button>
        {preview ? <label className="text-sm">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "비교")}<select className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "en", "{v0} ml-2"), { v0: String(CONTROL) })} value={view} onChange={event => setView(event.target.value as typeof view)}><option value="original">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "sRGB 원본")}</option><option value="proof">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "ICC 소프트 프루프")}</option><option value="gamut">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "색역 경고 (자홍)")}</option></select></label> : null}</div>
      {preview ? <div className="mt-3"><PixelPreview pixels={view === "original" ? preview.original : preview[view]} width={preview.width} height={preview.height} /><p className="mt-2 text-sm">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "색역 밖 ")}{preview.outOfGamutPixels} {translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "/ 표시 픽셀 ")}{preview.visiblePixels} {translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "· 알파 유지")}</p></div> : null}
      {stale ? <p role="status" className="mt-2 text-sm">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "원고가 변경되었습니다. 소프트 프루프를 새로 만들어 주세요.")}</p> : null}
      {error ? <p role="alert" className="mt-2 text-sm text-bad">{error}</p> : null}
      {busy ? <p role="status" className="mt-2 text-sm">{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "색상 계산 중…")}</p> : null}
      ...
      })}>{translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "ICC 포함 PNG 다운로드")}</button><button type="button" className={CONTROL} onClick={dismiss}>{busy ? translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "취소하고 닫기") : translateCurrentStaticSourceText("domains.creator.color.StudioColorProofDialog", "ko", "닫기")}</button></div>
    </div>
  </div>, document.body);
}
