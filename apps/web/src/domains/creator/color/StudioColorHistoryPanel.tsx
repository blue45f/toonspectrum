    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 pb-2 border-b border-line/60">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className="font-semibold truncate">{translateCurrentStaticSourceText("domains.creator.color.StudioColorHistoryPanel", "ko", "컬러 히스토리 (Color History)")}</span>
          <span className="px-1 py-0.2 text-[10px] rounded font-medium bg-violet-500/20 text-violet-300 border border-violet-500/30 shrink-0">
            {translateCurrentStaticSourceText("domains.creator.color.StudioColorHistoryPanel", "en", "CSP")}</span>
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            title={translateCurrentStaticSourceText("domains.creator.color.StudioColorHistoryPanel", "ko", "현재 선택된 색상을 히스토리에 추가")}
          >
            <Plus size={11} aria-hidden />
            <span>{translateCurrentStaticSourceText("domains.creator.color.StudioColorHistoryPanel", "ko", "등록")}</span>
          </button>
          <button
            type="button"
            onClick={handleClearHistory}
            className="p-1 rounded text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 transition-colors"
            title={translateCurrentStaticSourceText("domains.creator.color.StudioColorHistoryPanel", "ko", "히스토리 전체 지우기")}
            aria-label={translateCurrentStaticSourceText("domains.creator.color.StudioColorHistoryPanel", "ko", "히스토리 전체 지우기")}
          >
      ...
      <div className="mt-2 min-h-16">
        {history.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-4 text-slate-500 text-[11px] gap-1">
            <span>{translateCurrentStaticSourceText("domains.creator.color.StudioColorHistoryPanel", "ko", "기록된 색상이 없습니다.")}</span>
            <span className="text-[10px] text-slate-600">{translateCurrentStaticSourceText("domains.creator.color.StudioColorHistoryPanel", "ko", "작업한 색상이 자동으로 저장됩니다.")}</span>
          </div>
        ) : (
            ...
            {history.map((colorHex, index) => {
              ...
              return (
                <button
                  ...
                  aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.color.StudioColorHistoryPanel", "ko", "{v0} 색상 선택"), { v0: String(colorHex) })}
                  title={colorHex}
                  className={cn(
      ...
      <div className="flex items-center justify-between mt-2 pt-1 border-t border-line/40 text-[10px] text-slate-400 font-mono">
        <span>{translateCurrentStaticSourceText("domains.creator.color.StudioColorHistoryPanel", "ko", "선택 색상: ")}{hoveredHex ?? activeColor}</span>
        <div className="flex items-center gap-1.5">
          <span className="text-[9px] text-slate-500">({history.length}{translateCurrentStaticSourceText("domains.creator.color.StudioColorHistoryPanel", "ko", "색 기록됨)")}</span>
          <span
