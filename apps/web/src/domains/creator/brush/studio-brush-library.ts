...
import { BRUSH_SOURCE_ARCHIVE_KIND, BRUSH_SOURCE_ARCHIVE_MAX_CHARACTERS, BRUSH_SOURCE_SETTINGS_MAX_CHARACTERS, requireStudioBrushOriginalSource, type StudioBrushOriginalSource } from "./studio-brush-original-source";
...
export function writeBrushJson(brush: StudioSavedBrush): string {
  ...
  const originalSource = requireStudioBrushOriginalSource(brush.originalSource);
  ...
  if (new TextEncoder().encode(archive).byteLength > BRUSH_SOURCE_ARCHIVE_MAX_CHARACTERS) {
...
export function importBrushFromJson(
  ...
  if (envelope?.kind === BRUSH_SOURCE_ARCHIVE_KIND) {
    ...
    originalSource = requireStudioBrushOriginalSource(envelope.originalSource);
  ...
  if (Object.hasOwn(obj, "originalSource")) originalSource = requireStudioBrushOriginalSource(obj.originalSource);
