    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <h3 id="brush-v5-composer-heading" className="text-sm font-bold text-fg">
            {translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "브러시 엔진 조합")}</h3>
          <p className="mt-0.5 text-[0.65rem] leading-relaxed text-fg-3">
            {translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "추천 조합에서 시작하거나 아래 엔진을 직접 바꿔 손맛·촉·종이·물성·색·출력을 원하는 만큼 조합하세요. 모든 설정은 하나의 커스텀 브러시로 저장할 수 있습니다.")}</p>
        </div>
        {customized ? (
          <button
            type="button"
          ...
          >
            ...
            {translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "시작 상태로")}</button>
        ) : null}
      </div>
      ...
      <div className="mt-3 rounded-xl border border-accent/25 bg-accent-soft/20 p-3">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[0.7rem] font-bold text-fg">{translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "추천 시작점")}</p>
            <p className="mt-0.5 text-[0.6rem] leading-relaxed text-fg-3">
              {translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "프리셋은 완성품이 아니라 출발점입니다. 선택한 뒤 촉, 필압, 입자, 듀얼 브러시, 물성 설정을 자유롭게 섞어 나만의 브러시로 저장할 수 있습니다.")}</p>
          </div>
          <span className="shrink-0 rounded-full border border-line bg-card px-2 py-1 text-[0.56rem] font-semibold text-fg-3">
            {translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "언제든 재조합")}</span>
        </div>
          ...
          {recipes.map(({ recipe, available }) => (
            ...
            >
              ...
              <span className="mt-0.5 block text-[0.58rem] leading-relaxed text-fg-3">
                {recipe.description}
                {!available ? translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", " · 현재 출력 미연결") : ""}
              </span>
            </button>
          ))}
        </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
        <div className="rounded-xl border border-line bg-bg-2/55 px-2.5 py-2">
          <span className="block text-[0.58rem] font-semibold text-fg-3">{translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "실행 부담")}</span>
          <strong className="mt-0.5 block text-xs text-fg">
            {COMPLEXITY_LABELS[plan.complexity]} · {plan.costScore}
          </strong>
        </div>
        <div className="rounded-xl border border-line bg-bg-2/55 px-2.5 py-2">
          <span className="block text-[0.58rem] font-semibold text-fg-3">{translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "즉시 적용 가능")}</span>
        ...
        <div className="rounded-xl border border-line bg-bg-2/55 px-2.5 py-2">
          <span className="block text-[0.58rem] font-semibold text-fg-3">{translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "사용 조건")}</span>
          <strong className="mt-0.5 block text-xs text-fg">{plan.rights.label}</strong>
        </div>
        <div className="rounded-xl border border-line bg-bg-2/55 px-2.5 py-2">
          <span className="block text-[0.58rem] font-semibold text-fg-3">{translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "조합 상태")}</span>
          <strong className={cn("mt-0.5 block text-xs", plan.canSave ? "text-good" : "text-bad")}>
            {plan.canSave ? translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "저장 가능") : translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "수정 필요")}
          </strong>
        </div>
      </div>
                    ...
                    <span className="flex items-center justify-between gap-2">
                      ...
                      {selected ? (
                        <span className={cn(
                          ...
                          {runtimeSelectable
                            ? translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "바로 적용")
                            : formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "{v0} · 출력 미연결"), { v0: String(INTEGRATION_LABELS[selected.integration]) })}
                        </span>
                      ) : null}
                    </span>
                    <select
                      aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "{v0} 선택"), { v0: String(STUDIO_BRUSH_COMPOSITION_SLOT_LABELS[slot]) })}
                      value={selectedId}
                      ...
                      {options.map((option) => {
                        ...
                        return (
                          <option key={option.id} value={option.id} disabled={!available}>
                            {option.label} · {available ? translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "바로 적용") : translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "현재 출력 미연결")}
                          </option>
                        );
                      })}
                    </select>
      ...
      {runtimePlan.unavailableSelections.length > 0 ? (
        ...
        >
          {translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "현재 렌더러가 직접 소비하지 않는 ")}{runtimePlan.unavailableSelections.length}{translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "개 슬롯은 읽기 전용입니다. 선택 가능한 항목만 실제 획·정착·내보내기에 반영됩니다.")}</p>
      ) : null}

      {plan.issues.length > 0 ? (
        <ul className="mt-3 space-y-1.5" aria-label={translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "브러시 엔진 조합 진단")}>
        ...
        <p className="mt-3 rounded-lg border border-good/30 bg-good/10 px-2.5 py-2 text-[0.62rem] font-semibold text-good">
          {translateCurrentStaticSourceText("domains.creator.brush.StudioBrushCompositionComposer", "ko", "충돌 없이 저장 가능한 엔진 조합입니다.")}</p>
      )}
    </section>
  );
}
