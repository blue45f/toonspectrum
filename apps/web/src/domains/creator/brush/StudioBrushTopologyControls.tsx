...
export function StudioBrushTopologyControls({ material, onChange }: {
  ...
  return <section aria-label={translateCurrentStaticSourceText("domains.creator.brush.StudioBrushTopologyControls", "ko", "획 구조와 재료 결합")} className="space-y-3 rounded-xl border border-accent/25 bg-accent-soft/20 p-3">
    <label htmlFor={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.StudioBrushTopologyControls", "en", "{v0}-carrier"), { v0: String(id) })} className="block text-xs font-bold text-fg">{translateCurrentStaticSourceText("domains.creator.brush.StudioBrushTopologyControls", "ko", "획 구조 엔진")}<select id={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.brush.StudioBrushTopologyControls", "en", "{v0}-carrier"), { v0: String(id) })} className={SELECT} value={topology?.id ?? ""}
        onChange={(event) => updateSlot("carrier", event.currentTarget.value || "carrier-webgpu-centerline")}>
        <option value="">{translateCurrentStaticSourceText("domains.creator.brush.StudioBrushTopologyControls", "ko", "기존 재료 접촉")}</option>
        {BRUSH_STUDIO_V6_TOPOLOGIES.map((entry) => <option key={entry.id} value={entry.id}>{entry.label}</option>)}
      </select>
    </label>
    <p className="text-xs leading-5 text-fg-3">{topology?.description ?? translateCurrentStaticSourceText("domains.creator.brush.StudioBrushTopologyControls", "ko", "서로 다른 획 구조와 현재 재료를 결합하세요.")} {translateCurrentStaticSourceText("domains.creator.brush.StudioBrushTopologyControls", "ko", "구조 전환 시 미지원 물리·패턴은 해제됩니다.")}</p>
    {topology ? <>
      {(["deposition", "surface"] as const).map((slot) => <label key={slot} htmlFor={`${id}-${slot}`} className="block text-xs font-bold text-fg">
        {slot === "deposition" ? translateCurrentStaticSourceText("domains.creator.brush.StudioBrushTopologyControls", "ko", "도포 재료") : translateCurrentStaticSourceText("domains.creator.brush.StudioBrushTopologyControls", "ko", "종이 표면")}
      ...
      <label className="flex min-h-11 items-center gap-2 text-xs font-bold text-fg">
        <input type="checkbox" checked={material.slots.finish.includes("finish-neon")} onChange={(event) => {
          ...
          if (next) onChange(next);
        }} /> {translateCurrentStaticSourceText("domains.creator.brush.StudioBrushTopologyControls", "ko", "발광 마감 결합")}</label>
      <p className="text-[0.65rem] leading-4 text-fg-3">{translateCurrentStaticSourceText("domains.creator.brush.StudioBrushTopologyControls", "ko", "서버·외부 엔진 없이 CPU에서 실행됩니다. 번짐은 도포 근사이며 유체 격자나 아래색 픽업은 아닙니다.")}</p>
    </> : null}
  </section>;
}
