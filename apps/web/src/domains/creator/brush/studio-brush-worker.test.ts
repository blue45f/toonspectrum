...
describe("studio-brush-worker-protocol & client", () => {
  ...
  it("retires a crashed worker, drains pending work, and creates a fresh generation", async () => {
    ...
    // Even if an already-delivered callback from the retired generation runs
    // late, it must not settle a request owned by the replacement worker.
    staleOnMessage?.({
      data: {
        version: STUDIO_BRUSH_WORKER_PROTOCOL_VERSION,
        id: secondRequest.id,
  ...
  it("retires the generation when postMessage throws synchronously", async () => {
    ...
    const first = FakeBrushWorker.instances[0];
    expect(first).toBeDefined();
    // The first request has already posted, so simulate a browser/provider
    // failure on the next send from the same cached Worker.
    ...
    // The original request was drained by retirement too, not leaked forever.
