    ...
    <article className="rounded-xl border border-line bg-panel p-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[0.6875rem] text-fg-3">
            {ownerName(aggregate, response)} {translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "· 기한 ")}{formatDate(response.dueAt)}
          </p>
        </div>
        <span className={cn(
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        <div className="rounded-lg border border-line bg-card p-2.5">
          <p className="text-[0.625rem] font-bold text-fg-3">{translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "기대 효과")}</p>
          <p className="mt-1 text-xs leading-5 text-fg-2">{response.expectedEffect || translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "기대 효과 미정")}</p>
        </div>
        <div className="rounded-lg border border-line bg-card p-2.5">
          <p className="text-[0.625rem] font-bold text-fg-3">{translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "실제 결과")}</p>
          <p className="mt-1 text-xs leading-5 text-fg-2">{response.actualEffect || translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "아직 기록되지 않음")}</p>
        </div>
      </div>
      {active ? (
        <label className="mt-3 block">
          <span className="text-[0.6875rem] font-bold text-fg-3">{translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "결과 또는 취소 사유")}</span>
          <textarea
            value={resultNote}
            ...
            placeholder={translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "완료 효과나 취소 사유를 기록하세요. 완료·취소 처리에는 기록이 필수입니다.")}
          />
        </label>
      ) : null}
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {response.status === "proposed" && canManage ? (
          <button type="button" className={buttonClass({ variant: "outline", size: "sm" })} disabled={busy} onClick={() => void transition("approved")}>
            <ShieldCheck className="size-4" aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "승인")}</button>
        ) : null}
        {(response.status === "proposed" || response.status === "approved") && canEdit ? (
          <button type="button" className={buttonClass({ size: "sm" })} disabled={busy} onClick={() => void transition("in-progress")}>
            <Play className="size-4" aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "대응 시작")}</button>
        ) : null}
        {response.status === "in-progress" && canEdit ? (
          <button type="button" className={buttonClass({ size: "sm" })} disabled={busy || !resultNote.trim()} onClick={() => void transition("completed")}>
            <CheckCircle2 className="size-4" aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "완료 처리")}</button>
        ) : null}
        {active && canEdit ? (
          <button type="button" className={buttonClass({ variant: "outline", size: "sm" })} disabled={busy || !resultNote.trim()} onClick={() => void transition("cancelled")}>
            <XCircle className="size-4" aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "취소")}</button>
        ) : null}
        {response.status === "cancelled" && canEdit ? (
          <button type="button" className={buttonClass({ variant: "outline", size: "sm" })} disabled={busy} onClick={() => void transition("proposed")}>
            <RotateCcw className="size-4" aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "다시 제안")}</button>
        ) : null}
      </div>
      {response.completedAt ? (
        <p className="mt-2 text-[0.6875rem] text-fg-3">{translateCurrentStaticSourceText("domains.creator.production.hub.ProductionRiskResponseCard", "ko", "완료 ")}{formatDate(response.completedAt)}</p>
      ) : null}
    </article>
  );
}
