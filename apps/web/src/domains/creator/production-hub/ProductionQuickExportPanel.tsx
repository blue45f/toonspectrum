...
export function ProductionQuickExportPanel({
  ...
  const reviewApproved = candidate?.review.status === "approved";
  const approvedForFinal = productionReviewMatchesApprovedFinal(candidate, process);
  ...
  const download = async () => {
    ...
    try {
      ...
      const result = await buildProductionPageManifestArchive({
        ...
        title: `${process.artifact.title}-${approvedForFinal ? "approved" : "review"}`,
        pages,
      ...
      setNotice(`${pages.length}페이지를 검증해 ${approvedForFinal ? "승인 FINAL" : "검수용"} CBZ로 저장했습니다.`);
    } catch (error) {
  ...
  return <section className="rounded-3xl border border-accent/30 bg-card p-4 sm:p-6" aria-labelledby="quick-export-title" data-production-quick-export="">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div>
        <div className="flex flex-wrap items-center gap-2"><span className="inline-flex min-h-7 items-center rounded-full border border-accent/35 bg-accent-soft px-2.5 text-[0.625rem] font-black text-accent">QUICK EXPORT</span>{approvedForFinal ? <span className="inline-flex min-h-7 items-center rounded-full border border-good/35 bg-good/10 px-2.5 text-[0.625rem] font-bold text-good"><ShieldCheck className="mr-1 size-3.5" aria-hidden="true" /> 승인 FINAL 일치</span> : <span className="inline-flex min-h-7 items-center rounded-full border border-warn/35 bg-warn/10 px-2.5 text-[0.625rem] font-bold text-warn">검수용 출력</span>}</div>
    ...
    {!candidate ? <div className="mt-4 rounded-xl border border-dashed border-line p-4 text-sm text-fg-2"><FileArchive className="mr-2 inline size-4" aria-hidden="true" /> 피드백 탭에서 고정 검수본을 선택하면 빠른 출력을 사용할 수 있습니다.</div> : <div className="mt-4 grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-line bg-panel p-3"><p className="text-[0.625rem] font-bold text-fg-3">원본</p><p className="mt-1 truncate text-xs font-black text-fg">{candidate.review.title}</p></div><div className="rounded-xl border border-line bg-panel p-3"><p className="text-[0.625rem] font-bold text-fg-3">상태</p><p className={cn("mt-1 text-xs font-black", reviewApproved ? "text-good" : "text-warn")}>{reviewApproved ? "검수 승인" : candidate.review.status === "changes-requested" ? "수정 요청" : "검수 중"}</p></div><div className="rounded-xl border border-line bg-panel p-3"><p className="text-[0.625rem] font-bold text-fg-3">용도</p><p className="mt-1 text-xs font-black text-fg">{approvedForFinal ? "승인 FINAL 빠른 출력" : "내부 검수·확인용"}</p></div></div>}
