...
export function StudioWorkSessionDetail({ view, actorId, controller, busy, saving = busy }: {
  ...
  return <article className="space-y-4" data-work-session-id={session.id}>
    ...
    {closing && host && joined && !terminal ? <form className="space-y-3 rounded-lg border border-line p-3" onSubmit={(event) => {
      event.preventDefault(); if (!host || !joined || !confirmed || busy || (closing === "close" && !summary.trim())) return;
