  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between gap-2">
        <p className="text-[0.66rem] font-semibold text-fg-3 uppercase tracking-wider">{translateCurrentStaticSourceText("domains.creator.StudioColorBalancePanel", "ko", "컬러 밸런스 (Color Balance)")}</p>
        <button
          type="button"
          onClick={onReset}
          disabled={isIdentity}
          className={buttonClass({ size: "sm", variant: "quiet" })}
          title={translateCurrentStaticSourceText("domains.creator.StudioColorBalancePanel", "ko", "컬러 밸런스를 제거하고 원본 색감으로 되돌립니다.")}
        >
          <RotateCcw className="size-3.5" />
          {translateCurrentStaticSourceText("domains.creator.StudioColorBalancePanel", "ko", "원본으로")}</button>
      </div>
