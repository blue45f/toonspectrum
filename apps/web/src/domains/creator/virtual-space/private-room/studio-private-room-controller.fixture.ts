...
export function fixture(count=2){
  let now=Date.now(),open=true,eligible=true,knockEligible=true,current=true;
  ...
  participants.forEach((p,i)=>{
    const api:StudioPrivateRoomApi={
      ...
      team:vi.fn(async()=>({workId:"work",viewer:{userId:`actor-${i}`,role:"owner" as const,status:"active" as const,capabilities:{view:true,comment:true,edit:true,manageMembers:true,respondInvite:false}},members:participants.map((_,index)=>({userId:`actor-${index}`,name:`Member ${index}`,role:(index===0?"owner":"editor") as "owner"|"editor",status:"active" as const,isOwner:index===0,image:""}))})),
    ...
    controllers.push(new StudioPrivateRoomController({workId:"work",actorId:`actor-${i}`,world,zoneId:"zone",doorId:"door",binding:leases[i]!.binding},{api,now:()=>now,current:()=>current,canCleanup:()=>current,eligible:()=>eligible,knockEligible:()=>knockEligible,direct:{getPeers:()=>participants.filter(peer=>peer!==p),subscribe(fn){receivers.set(p.sessionId,fn);return()=>receivers.delete(p.sessionId);},send(target,raw){receivers.get(target)?.(p,raw);return true;}}}));
  });
  return {controllers,apis,records,leases,participants,setEligible:(value:boolean)=>{eligible=value;},setKnockEligible:(value:boolean)=>{knockEligible=value;},setOpen:(value:boolean)=>{open=value;},setCurrent:(value:boolean)=>{current=value;},advance:(ms:number)=>{now+=ms;},
    ...
    sendRaw(from:number,to:number,raw:string){receivers.get(`client-${to}`)?.(participants[from]!,raw);},
