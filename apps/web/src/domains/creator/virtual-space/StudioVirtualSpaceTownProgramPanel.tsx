...
const TABS = ["quests", "events", "activities", "desks", "blueprints", "companion"] as const;
...
export function StudioVirtualSpaceTownProgramPanel({
  ...
  return <section className="vs2-panel studio-vspace-town-program" data-space-interactive="true">
    ...
    <div className="studio-vspace-town-tabs" role="tablist" aria-label={bt("마을 기능", "Town features")}>
      {TABS.map((item) => <button key={item} type="button" role="tab" aria-selected={tab === item} onClick={() => setTab(item)}>{bt({ quests: "퀘스트", events: "이벤트", activities: "활동", desks: "팀 자리", blueprints: "블루프린트", companion: "모바일" }[item], { quests: "Quests", events: "Events", activities: "Activities", desks: "Desk pods", blueprints: "Blueprints", companion: "Companion" }[item])}</button>)}
    </div>
    ...
    {tab === "activities" ? <div className="studio-vspace-town-cards">
      ...
      {round ? <article className="studio-vspace-mini-game-round" aria-live="polite">
        ...
        <div>{round.choices.map((choice, index) => <button key={choice} type="button" disabled={Boolean(result)} onClick={() => setResult(answerStudioMiniGameRound(round, index))}>{choice}</button>)}</div>
