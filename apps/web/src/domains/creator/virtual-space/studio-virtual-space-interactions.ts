...
function legacyInteraction(
  interaction: Omit<StudioVirtualSpaceInteraction, "x" | "y" | "radius"> & {
    readonly x: number;
    readonly y: number;
    readonly radius: number;
  },
): StudioVirtualSpaceInteraction {
  return Object.freeze({
    ...interaction,
    x: studioVirtualSpaceScaleLegacyX(interaction.x),
    y: studioVirtualSpaceScaleLegacyY(interaction.y),
    radius: studioVirtualSpaceScaleLegacyDistance(interaction.radius),
  });
}
...
export const STUDIO_VIRTUAL_SPACE_INTERACTIONS: readonly StudioVirtualSpaceInteraction[] = Object.freeze([
  legacyInteraction({
    ...
    emoji: "☕", x: 190, y: 105, radius: 96, action: "community",
  }),
  legacyInteraction({
    id: "writers-script-desk", zoneId: "writers",
    labelKo: "대본 데스크", labelEn: "Script desk",
    hintKo: "시놉시스와 38화 대본 작업으로 이동해요.", hintEn: "Open synopsis and episode script work.",
    emoji: "📝", x: 555, y: 105, radius: 96, action: "story",
  }),
  legacyInteraction({
    ...
    emoji: "🖼️", x: 940, y: 100, radius: 100, action: "comic",
  }),
  legacyInteraction({
    id: "live-stage-console", zoneId: "live",
    labelKo: "라이브 콘솔", labelEn: "Live console",
    hintKo: "라이브 드로잉 세션을 시작해요.", hintEn: "Start a live drawing session.",
    emoji: "🎬", x: 590, y: 435, radius: 100, action: "live",
  }),
  legacyInteraction({
    ...
    emoji: "📦", x: 170, y: 310, radius: 106, action: "assets",
  }),
  legacyInteraction({
    ...
    emoji: "🎨", x: 995, y: 305, radius: 112, action: "canvas",
  }),
  legacyInteraction({
    ...
    emoji: "✅", x: 240, y: 550, radius: 112, action: "review",
  }),
  legacyInteraction({
    ...
    emoji: "🤖", x: 925, y: 545, radius: 120, action: "assistant",
  }),
]);
