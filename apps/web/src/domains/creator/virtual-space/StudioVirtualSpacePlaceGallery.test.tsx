...
describe("StudioVirtualSpacePlaceGallery", () => {
  it("moves to an authored room from an ImageGen place card", () => {
    const onMove = vi.fn();
    render(<StudioVirtualSpacePlaceGallery personal currentRoomId="lobby" onMove={onMove} />);
    fireEvent.click(screen.getByRole("button", { name: /개인 아틀리에로 이동/u }));
    expect(onMove).toHaveBeenCalledWith("drawing");
  });

  it("does not advertise project-only control rooms in personal space", () => {
    render(<StudioVirtualSpacePlaceGallery personal currentRoomId="lobby" onMove={vi.fn()} />);
  ...
  it("filters the gallery without losing the current place signal", () => {
    render(<StudioVirtualSpacePlaceGallery personal={false} currentRoomId="review" onMove={vi.fn()} />);
