...
export function StudioWorkspacePage({ surface = "home" }: { readonly surface?: WorkspaceSurface }) {
  ...
  const project = context.selected;
  ...
  return (
    <div className="workspace-shell" data-route-ready="studio-workspace" data-workspace-surface={surface}>
      <header className="workspace-topbar">
        <Link href="/home" className="workspace-brand"><span aria-hidden="true">T</span><strong>ToonSpectrum</strong></Link>
        ...
        <div className="workspace-utilities">
          ...
          <button type="button" className="workspace-work-shortcut" onClick={() => setPanel("work")} aria-label={bt("작업 바로가기 열기", "Open work shortcuts")}><ClipboardList size={20} aria-hidden="true" /></button>
          <Link href="/my" aria-label={bt("내 프로필", "My profile")}><Users size={20} aria-hidden="true" /></Link>
        </div>
      </header>
      <WorkspaceNavigation activeId={activeId} teamHref={project ? `/team?project=${encodeURIComponent(project.id)}` : undefined} />
      <section className="workspace-main" aria-labelledby="workspace-title">
        ...
        {surface === "home" ? <>
          {mode === "virtual-studio" ? <Suspense fallback={<div className="workspace-world-loading" role="status">{bt("공간 보기 불러오는 중…", "Loading space view…")}</div>}>
      ...
      <footer className="workspace-statusbar">
        ...
        {surface === "home" ? <div className="workspace-footer-actions"><button type="button" className="workspace-icon-button" onClick={() => setPanel("tools")} aria-label={bt("도구와 공간 메뉴", "Tools and space menu")}><Settings size={19} aria-hidden="true" /></button>
          {!loading && !library.error && !context.missing ? <Link className="workspace-primary" href={canResume ? links.resume : "/studio/new"}>{canResume ? (resume?.summary ? bt("이어서 작업", "Resume work") : bt("작품 열기", "Open work")) : bt("작품 시작", "Start creating")}<ArrowRight size={18} aria-hidden="true" /></Link> : null}</div> : <Link href={project ? `/home?project=${encodeURIComponent(project.id)}` : "/home"}>{bt("작업실로 돌아가기", "Back to studio")}</Link>}
      </footer>
      <WorkspaceContextPanel open={panel !== null} title={panel === "work" ? bt("작업 바로가기", "Work shortcuts") : panel === "tools" ? bt("도구와 공간", "Tools and space") : bt("도움말", "Help")} onClose={() => setPanel(null)}>
        <div className="workspace-link-list">
          {panel === "work" ? project ? <><Link href={links.resume}>{bt("현재 작품 열기", "Open current work")}</Link><Link href={links.review}>{bt("받은 검수 요청", "Review inbox")}</Link><Link href={links.production}>{bt("진행과 담당 작업", "Production and assignments")}</Link></> : <><p>{bt("작품을 만들거나 가져오면 이곳에서 이어갈 수 있습니다.", "Create or import a work to continue here.")}</p><Link href="/studio/new"><Plus size={20} aria-hidden="true" />{bt("새 작품", "New work")}</Link><Link href="/studio/import">{bt("파일 가져오기", "Import files")}</Link></> : panel === "tools" ? <><Link href={links.assets}>{bt("현재 작품 소재", "Work materials")}</Link><Link href="/market">{bt("새 소재 찾기", "Discover materials")}</Link>{project ? <Link href={links.space}>{bt("공간 입장과 꾸미기", "Enter and customize space")}</Link> : null}<Link href="/settings">{bt("테마·언어·환경 설정", "Theme, language and preferences")}</Link><button type="button" onClick={() => setPanel("help")}><HelpCircle size={20} aria-hidden="true" />{bt("도움말", "Help")}</button></> : <><Link href="/help"><BookOpen size={20} aria-hidden="true" />{bt("사용 도움말", "User help")}</Link><Link href="/product-tour"><Compass size={20} aria-hidden="true" />{bt("서비스 둘러보기", "Product tour")}</Link></>}
        </div>
