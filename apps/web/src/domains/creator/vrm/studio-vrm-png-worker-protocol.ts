...
} from "../bg3d/studio-bg3d-lt-render";

/** Versioned independently from the persisted scene and batch archive protocols. */
export const STUDIO_VRM_PNG_WORKER_PROTOCOL_VERSION = 1 as const;
export const STUDIO_VRM_PNG_WORKER_MAX_LAYERS = 1;
/** Independent single-image admission; the shared BG3D layered worker retains its 8MP budget. */
export const STUDIO_VRM_PNG_MAX_DIMENSION = 4_096;
export const STUDIO_VRM_PNG_MAX_PIXELS = 16_777_216;
/** RGBA worst-case storage plus bounded PNG framing/compression overhead. */
export const STUDIO_VRM_PNG_MAX_OUTPUT_BYTES =
  STUDIO_VRM_PNG_MAX_PIXELS * 4 + 1_048_576;

export interface StudioVrmPngWorkerLayer {
...
export interface StudioVrmPngWorkerRequest {
  readonly version: typeof STUDIO_VRM_PNG_WORKER_PROTOCOL_VERSION;
  readonly kind: "encode";
  readonly requestId: number;
  readonly width: number;
  readonly height: number;
  readonly layers: readonly StudioVrmPngWorkerLayer[];
}

export type StudioVrmPngWorkerResponse =
  | {
...
function dimensions(
  ...
  if (
    ...
    widthValue < 1 || widthValue > STUDIO_VRM_PNG_MAX_DIMENSION ||
    typeof heightValue !== "number" || !Number.isSafeInteger(heightValue) ||
    heightValue < 1 || heightValue > STUDIO_VRM_PNG_MAX_DIMENSION
  ) return null;
  const pixels = widthValue * heightValue;
  return Number.isSafeInteger(pixels) && pixels <= STUDIO_VRM_PNG_MAX_PIXELS
...
function isLayer(
  ...
  width: number,
  height: number,
): value is StudioVrmPngWorkerLayer {
...
export function isStudioVrmPngWorkerRequest(
  value: unknown,
): value is StudioVrmPngWorkerRequest {
  try {
    const shape = isRecord(value) ? dimensions(value.width, value.height) : null;
    if (
      !isRecord(value) || !hasExactKeys(value, REQUEST_KEYS) ||
      value.version !== STUDIO_VRM_PNG_WORKER_PROTOCOL_VERSION ||
      value.kind !== "encode" || !isRequestId(value.requestId) ||
      !shape || !Array.isArray(value.layers) ||
      value.layers.length < 1 || value.layers.length > STUDIO_VRM_PNG_WORKER_MAX_LAYERS
    ) return false;
...
export function isStudioVrmPngWorkerResponse(
  value: unknown,
): value is StudioVrmPngWorkerResponse {
  try {
    if (
      !isRecord(value) ||
      value.version !== STUDIO_VRM_PNG_WORKER_PROTOCOL_VERSION
    ...
    return value.kind === "result" && hasExactKeys(value, RESULT_KEYS) &&
      ...
      value.png.size <= STUDIO_VRM_PNG_MAX_OUTPUT_BYTES;
  } catch {
    return false;
  }
}

export function studioVrmPngWorkerRequestTransfers(
  request: StudioVrmPngWorkerRequest,
): Transferable[] {
  return request.layers.map((layer) => layer.dataBuffer);
}

/** Clone-safe public input shape retained for boundary tests and adapters. */
export type StudioVrmPngInputLayer = Pick<
  StudioBg3dLtRasterLayer,
