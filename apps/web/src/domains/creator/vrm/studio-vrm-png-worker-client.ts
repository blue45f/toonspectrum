...
export const STUDIO_VRM_PNG_WORKER_TIMEOUT_MS = 20_000;
export const STUDIO_VRM_PNG_WORKER_STARTUP_TIMEOUT_MS = 5_000;

interface WorkerMessageLike { readonly data: unknown }
interface WorkerErrorLike { preventDefault?(): void }

export interface StudioVrmPngWorkerLike {
...
export type StudioVrmPngWorkerFactory = () => StudioVrmPngWorkerLike | null;

export interface StudioVrmPngWorkerOptions {
  readonly signal?: AbortSignal;
  readonly timeoutMs?: number;
  readonly startupTimeoutMs?: number;
  /** Omitted creates one lazy Vite module Worker for exactly this encode request. */
  readonly workerFactory?: StudioVrmPngWorkerFactory;
}

export type StudioVrmPngWorkerErrorCode =
  | "aborted"
  | "invalid-request"
  | "worker-unavailable"
...
export class StudioVrmPngWorkerError extends Error {
  constructor(readonly code: StudioVrmPngWorkerErrorCode) {
    super(`studio-vrm-png-worker:${code}`);
...
export function createStudioVrmPngModuleWorker(): StudioVrmPngWorkerLike | null {
...
function boundedTimeout(value: number | undefined): number {
  if (!Number.isFinite(value)) return STUDIO_VRM_PNG_WORKER_TIMEOUT_MS;
  return Math.max(100, Math.min(120_000, Math.floor(value ?? 0)));
}

function boundedStartupTimeout(value: number | undefined): number {
  if (!Number.isFinite(value)) return STUDIO_VRM_PNG_WORKER_STARTUP_TIMEOUT_MS;
...
): StudioVrmPngWorkerRequest | null {
  try {
    if (
      !isRuntimeArray(layersValue) || layersValue.length < 1 ||
      layersValue.length > STUDIO_VRM_PNG_WORKER_MAX_LAYERS
    ) return null;
    ...
    if (
      !Number.isSafeInteger(width) || width < 1 || width > STUDIO_VRM_PNG_MAX_DIMENSION ||
      !Number.isSafeInteger(height) || height < 1 || height > STUDIO_VRM_PNG_MAX_DIMENSION
    ) return null;
    const pixels = width * height;
    if (!Number.isSafeInteger(pixels) || pixels > STUDIO_VRM_PNG_MAX_PIXELS) return null;

    const roles = new Set<StudioBg3dLtRasterLayer["role"]>();
    let previousRoleOrder = -1;
    const snapshots: StudioVrmPngWorkerRequest["layers"] = layersValue.map((layer) => {
    ...
    const request: StudioVrmPngWorkerRequest = Object.freeze({
    ...
    return isStudioVrmPngWorkerRequest(request) ? request : null;
  } catch {
    return null;
  }
}
 ...
 */
export function encodeStudioVrmPngInWorker(
  layers: readonly StudioBg3dLtRasterLayer[],
  options: StudioVrmPngWorkerOptions = {},
): Promise<Blob> {
  if (options.signal?.aborted) {
    return Promise.reject(new StudioVrmPngWorkerError("aborted"));
  }
  const snapshot = snapshotRequest(layers);
  if (!snapshot) return Promise.reject(new StudioVrmPngWorkerError("invalid-request"));
  const request: StudioVrmPngWorkerRequest = snapshot;
  if (options.signal?.aborted) {
    return Promise.reject(new StudioVrmPngWorkerError("aborted"));
  }
  ...
  return new Promise<Blob>((resolve, reject) => {
    ...
    let worker: StudioVrmPngWorkerLike | null = null;
    let settled = false;
    let ready = false;
    let verifying = false;
    ...
    const fail = (code: StudioVrmPngWorkerErrorCode) => {
      finish(() => reject(new StudioVrmPngWorkerError(code)));
    };
    ...
    function handleMessage(event: WorkerMessageLike): void {
      ...
      if (!isStudioVrmPngWorkerResponse(response)) {
        fail("protocol");
        return;
      }
      ...
      if (response.kind === "ready") {
        ...
        try {
          worker.postMessage(request, studioVrmPngWorkerRequestTransfers(request));
        } catch {
          fail("worker-failed");
        }
        return;
    ...
    try {
      worker = (options.workerFactory ?? createStudioVrmPngModuleWorker)();
    } catch {
