...
export function StudioP2pMediaTile({ stream, name, muted, visual }: {
  ...
  return <div className="relative overflow-hidden rounded-xl border border-line bg-panel">
    ...
    <video ref={ref} autoPlay playsInline muted={muted} aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.live.huddle.StudioP2pMediaTile", "ko", "{v0} 영상"), { v0: String(name) })}
      className={visual ? translateCurrentStaticSourceText("domains.creator.live.huddle.StudioP2pMediaTile", "en", "aspect-video w-full object-contain") : "h-0 w-0"} />
    ...
    }}>{translateCurrentStaticSourceText("domains.creator.live.huddle.StudioP2pMediaTile", "ko", "소리·영상 재생")}</button>}
  </div>;
}
