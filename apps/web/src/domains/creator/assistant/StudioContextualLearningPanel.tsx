...
export function StudioContextualLearningPanel({
  ...
  const tx = (source: string) => translateCurrentStaticSourceText("StudioContextualLearningPanel", "ko", source);
    ...
    <div data-testid="studio-contextual-learning-panel" className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-accent">
              ...
              <span className="text-[0.72rem] font-black tracking-wide">{tx("지금 작업에 맞는 학습")}</span>
            </div>
            <h3 className="mt-2 text-sm font-black text-fg">{tx(focus.label)}</h3>
            <p className="mt-1 max-w-2xl text-[0.68rem] leading-relaxed text-fg-2">{tx(focus.reason)}</p>
          </div>
          <div className="flex flex-wrap gap-1.5" aria-label={tx("추천 제작 단계")}>
            {focus.primarySteps.map((step) => (
              <span key={step} className="rounded-full border border-accent/40 bg-card px-2 py-1 text-[0.6rem] font-bold text-accent">
                {tx(PRODUCTION_STEP_LABELS[step])}
              </span>
            ))}
          </div>
        </div>
      ...
      <section aria-labelledby="studio-learning-recommendations-title">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          ...
          >
            {tx("전체 자료 보기")} <ExternalLink className="size-3" aria-hidden />
          </a>
        </div>
          ...
          {resources.map((resource) => (
            <article key={resource.id} className="flex min-w-0 flex-col rounded-xl border border-line bg-card/70 p-3">
              <div className="flex flex-wrap items-center justify-between gap-2 text-[0.58rem] text-fg-3">
                ...
                <span>{tx(RESOURCE_FORMAT_LABELS[resource.format])} · {resource.external ? tx("외부 공식 자료") : tx("자체 강좌")}</span>
              </div>
              <h4 className="mt-2 text-[0.76rem] font-bold leading-snug text-fg">{tx(resource.title)}</h4>
              <p className="mt-1 flex-1 text-[0.64rem] leading-relaxed text-fg-3">{tx(resource.summary)}</p>
              <div className="mt-3 flex flex-wrap gap-1.5">
                ...
                >
                  {resource.external ? tx("원문 보기") : tx("강좌 보기")} <ExternalLink className="size-3" aria-hidden />
                </a>
                {resource.practicePath && resource.practicePath !== "/studio" && (
                  ...
                  >
                    <FlaskConical className="size-3" aria-hidden /> {tx("실습 열기")}
                  </a>
                )}
              </div>
            </article>
      ...
      <p className="rounded-lg border border-line bg-card px-3 py-2 text-[0.61rem] leading-relaxed text-fg-3">
        {tx("학습 링크는 현재 작업을 잃지 않도록 새 탭에서 엽니다. 외부 자료는 검증된 공식·공공 출처의 원문으로 이동합니다.")}
      </p>
    </div>
  );
}
