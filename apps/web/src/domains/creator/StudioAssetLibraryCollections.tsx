...
export function StudioAssetLibraryCollections({ children, items, assets, loading, onDeleteAsset, onUseAsset, onUseItem, onOpen3d, initialView = "explore" }: StudioAssetLibraryCollectionsProps) {
  ...
  return <div className="space-y-3" data-studio-asset-library-collections="true">
    <div className="flex flex-wrap gap-1.5" role="group" aria-label="에셋 라이브러리 보기">{VIEWS.map((option) => <button key={option.id} type="button" className={`${CONTROL} ${view === option.id ? "border-accent bg-accent-soft text-accent" : ""}`} aria-pressed={view === option.id} onClick={() => setView(option.id)}>{option.label}{option.id === "mine" ? ` ${assets.length}` : ""}</button>)}</div>
    {view === "explore" && children}
    {view === "scenes" && <StudioSceneAssetLibrary items={items} onUseItem={onUseItem} />}
    {view === "public" && <section aria-label="고품질 공용 에셋" className="space-y-3"><p className="text-xs leading-relaxed text-fg-3">아래 CC0 라이브러리를 펼쳐 배경·투명 소품·PBR 모델·표면 재질을 찾아보세요. 원본 출처와 해상도, 검수 상태를 함께 확인할 수 있습니다.</p><Suspense fallback={<p role="status" className="text-xs text-fg-3">공용 에셋 라이브러리를 여는 중입니다.</p>}><LazyCc0Library onUseAsset={onUseAsset} /></Suspense><button type="button" className={CONTROL} onClick={onOpen3d}>다운로드한 GLB를 3D 모델로 가져오기</button></section>}
    ...
    {view === "sources" && <section aria-label="외부 소재 출처" className="space-y-2"><p className="text-xs leading-relaxed text-fg-3">외부 사이트에서 소재를 찾는 바로가기입니다. 등록된 내 에셋이나 기본 제공 소재 수에 포함하지 않습니다. 외부 소재의 사용권과 재배포 조건은 각 항목에서 확인해 주세요.</p>{SOURCES.map((source) => <a key={source.name} href={source.url} target="_blank" rel="noopener noreferrer" className="block rounded-xl border border-line bg-card p-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"><span className="block text-xs font-bold text-fg">{source.name} · 새 탭에서 열기</span><span className="mt-1 block text-xs text-fg-3">{source.description}</span><span className="mt-1 block text-[0.65rem] text-fg-3">{source.rights}</span></a>)}</section>}
  </div>;
}
