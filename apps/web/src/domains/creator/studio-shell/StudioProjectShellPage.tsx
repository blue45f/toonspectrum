...
type ProjectAction = Readonly<{
  title: Record<AuthoredLocale, string>;
  description: Record<AuthoredLocale, string>;
  href: (projectId: string) => string;
  icon: LucideIcon;
  badge?: Record<AuthoredLocale, string>;
}>;
    ...
    <Container size="wide" className="py-10">
      <section className="rounded-3xl border border-line bg-card p-6" role="alert">
        <h1 className="text-xl font-bold text-fg">
          {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "프로젝트를 찾을 수 없어요.", "Project not found.")}
        </h1>
        <p className="mt-2 text-sm text-fg-3">
          {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "저장된 작업은 변경하지 않았습니다. 내 작업에서 프로젝트를 다시 선택하세요.", "No saved work was changed. Choose the project again from My work.")}
        </p>
        <Link href="/studio" className={buttonClass({ className: "mt-5" })}>
          {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "내 작업으로", "Go to My work")}
        </Link>
      </section>
    </Container>
  );
...
export function StudioProjectShellPage({ section }: { readonly section: StudioProjectSection }) {
  ...
  useDocumentTitle(`${translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", definition.label)} · ToonStudio`);
  ...
  return (
    <Container size="wide" className="py-6 sm:py-8 lg:py-10">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <Link href="/studio" className="text-xs font-semibold text-accent hover:text-accent-2">
              ← {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "내 작업", "My work")}
            </Link>
            ...
            <h1 className="mt-2 text-pretty text-2xl font-black tracking-tight text-fg sm:text-4xl">
              {translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", definition.title)}
            </h1>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-fg-2">
              {translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", definition.description)}
            </p>
            <p className="mt-3 text-xs text-fg-3">
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "프로젝트", "Project")} · {displayProjectId}
            </p>
          </div>
          <Link href={workHref(displayProjectId)} className={buttonClass({ size: "lg", className: "gap-2" })}>
            <Brush size={17} aria-hidden="true" />
            {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "원고 열기", "Open manuscript")}
          </Link>
        </div>
      </header>

      <nav
        aria-label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "프로젝트 메뉴", "Project navigation")}
        ...
        <div className="flex min-w-max gap-1">
          {PRIMARY_SECTIONS.map((id) => {
            ...
            return (
              ...
              >
                {translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", SECTION_DEFINITIONS[id].label)}
              </Link>
            );
          })}
          <Link
            href={projectSectionHref(displayProjectId, "settings")}
            aria-current={section === "settings" ? translateCurrentStaticSourceText("domains.creator.studio.shell.StudioProjectShellPage", "en", "page") : undefined}
            aria-label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "프로젝트 설정", "Project settings")}
            className={cn(
      ...
      <nav
        aria-label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", `${definition.label.ko} 세부 화면`, `${definition.label.en} views`)}
        ...
        <div className="flex min-w-max gap-2">
          {sectionViews.map((view) => {
            ...
            return (
              ...
              >
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", viewLabel.labelKo, viewLabel.labelEn)}
              </Link>
            );
          })}
        </div>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-1 text-lg font-black text-fg">
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", destination.labelKo, destination.labelEn)}
            </h2>
            <p className="mt-1 text-sm leading-6 text-fg-2">
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", destination.descriptionKo, destination.descriptionEn)}
            </p>
          </div>
          {destination.href ? (
            <Link href={destination.href} className={buttonClass({ className: "shrink-0 gap-2" })}>
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", destination.ctaKo, destination.ctaEn)}
      ...
      <section className="mt-6" aria-labelledby="project-section-actions">
        <div className="flex items-center gap-3">
          ...
          <h2 id="project-section-actions" className="text-xl font-bold text-fg">
            {translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", definition.label)}
          </h2>
        </div>
            ...
            return (
              ...
              >
                ...
                <span className="mt-4 flex items-center gap-2">
                  <strong className="text-sm text-fg">{translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", item.title)}</strong>
                  {item.badge ? (
                    <span className="rounded-full bg-accent-soft px-2 py-0.5 text-[0.62rem] font-bold text-accent">
                      {translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", item.badge)}
                    </span>
                  ) : null}
                </span>
                <span className="mt-1.5 flex-1 text-xs leading-5 text-fg-3">
                  {translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", item.description)}
                </span>
                <span className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-accent">
                  {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "열기", "Open")}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <strong className="text-sm text-fg">
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "기능 이름을 몰라도 괜찮아요.", "You do not need to know feature names.")}
            </strong>
            <p className="mt-1 text-xs leading-5 text-fg-3">
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "⌘K 검색이나 도우미에서 하려는 일을 입력하면 현재 프로젝트에 맞는 기능을 찾습니다.", "Use command search or the assistant to find the right action for this project.")}
            </p>
          </div>
          <button
            type="button"
          ...
          >
            ...
            {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectShellPage", "기능 검색", "Search commands")}
          </button>
        </div>
      </section>
    </Container>
