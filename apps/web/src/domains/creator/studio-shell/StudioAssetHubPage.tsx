...
const bi = <T,>(ko: T, en: T): T =>
  translateBilingualValueForActiveLocale("StudioAssetHubPage", ko, en);;
    ...
    <Container size="wide" className="py-8 sm:py-12">
      <section className="rounded-3xl border border-line bg-card p-6 text-center shadow-sm sm:p-10">
        ...
        <h2 className="mt-4 text-xl font-black text-fg">
          {seriesKit
            ? (bi("프로젝트에서 Series Kit를 열어 주세요", "Open Series Kit from a project"))
            : (bi("확인할 프로젝트를 먼저 선택해 주세요", "Choose a project to check"))}
        </h2>
        <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-fg-2">
          {seriesKit
            ? (bi("Series Kit는 작품별 색상·글꼴·말풍선·출력 규칙을 관리합니다. 내 작업에서 프로젝트를 선택한 뒤 소재의 Series Kit를 열면 됩니다.", "Series Kit manages project colors, typography, balloons and export defaults. Choose a project from My work, then open its Series Kit."))
            : (bi("사용 목적, 구매 내역, 팀 좌석, 글꼴, AI 출처와 확장 기능 권한은 프로젝트마다 달라집니다. 내 작업에서 프로젝트를 선택하면 한 번에 확인할 수 있습니다.", "Usage purpose, purchases, team seats, fonts, AI provenance and extension permissions differ per project. Choose a project from My work to review them together."))}
        </p>
...
export function StudioAssetHubPage() {
  ...
  const currentLabel = useMemo(() => bi((VIEW_LABELS[view]).ko, (VIEW_LABELS[view]).en), [locale, view]);
  ...
  return (
    <div data-route-ready="studio-assets" data-studio-asset-hub={view}>
              ...
              <div>
                ...
                {projectId ? (
                  <p className="mt-0.5 text-xs text-fg-3">
                    {formatI18nTemplate(String(bi("프로젝트 {value0}에 연결", "Connected to project {value0}")), { value0: projectId })}
                  </p>
                ) : null}
              </div>
            </div>

            <nav aria-label={bi("소재 화면", "Material views")} className="overflow-x-auto">
      ...
      {view === "essentials" ? <Suspense fallback={<p role="status" className="p-8 text-sm text-fg-2">{bi("제작 소재 준비 중…", "Loading creator essentials…")}</p>}><CreatorEssentialsPage /></Suspense> : null}
