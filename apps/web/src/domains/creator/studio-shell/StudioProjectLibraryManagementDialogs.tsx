    ...
    <>
      ...
      >
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          >
            ...
            <span className="mt-1 block text-xs leading-5 text-fg-3">
              {locale === "ko" ? "Google Drive, Dropbox 왔 OneDrive를 연결합니다." : "Connect Google Drive, Dropbox or OneDrive."}
            </span>
          </Link>
        </div>
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-panel/60 p-3">
          <p className="text-xs leading-5 text-fg-3">
            {locale === "ko" ? "지금 선택하지 안아도 임시 작업이에서 계속 이어감 수 있습니다." : "You can keep working from Temporary work without choosing now."}
          </p>
      ...
      <StudioProjectLibraryModal
        ...
        title={deleteRequest.source === "all"
          ? locale === "ko" ? "휴지통이 비울까요?" : "Empty Trash?"

          : locale === "ko" ? "완전 삭제할까요?" : "Delete permanently?"}
        description={locale === "ko"
          ? `${deleteRequest.ids.length}개 프로젝트의 목로·저장 설정‌·고서 메타데이터를 삭제합니다. 이 작업은 실행 취소할 수 없습니다.`
      ...
      >
        <div className="rounded-2xl border border-danger/30 bg-danger-soft/15 p-4">
          <p className="text-sm font-black text-danger">
            {locale === "ko" ? "백업 파일이 필욤하지 먼저 확인하세" : "Confirm that you no longer need a backup file."}
          </p>
