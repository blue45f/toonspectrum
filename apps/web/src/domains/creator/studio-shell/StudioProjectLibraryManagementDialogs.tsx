    ...
    <>
      {saveTarget ? (
        <StudioProjectLibraryModal
          title={locale === "ko" ? "어디에 정식 저장할까요?" : "Where should this be formally saved?"}
          description={locale === "ko"
            ? `“${saveTarget.title}”의 임시 자동저장본은 그대로 유지됩니다. 파일 또는 개인 드라이브를 선택하세요.`
            : `The temporary autosave of “${saveTarget.title}” remains intact. Choose a file or personal drive.`}
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            >
              ...
              <b className="mt-3 block text-sm text-fg">{locale === "ko" ? "파일·동기화 폴더" : "File or synced folder"}</b>
              <span className="mt-1 block text-xs leading-5 text-fg-3">
                {locale === "ko" ? ".toonstudio 원본을 컴퓨터나 동기화 폴더에 저장합니다." : "Save an editable .toonstudio original to your computer or synced folder."}
              </span>
            </button>
            <Link
            ...
            >
              ...
              <b className="mt-3 block text-sm text-fg">{locale === "ko" ? "개인 드라이브" : "Personal drive"}</b>
              <span className="mt-1 block text-xs leading-5 text-fg-3">
                {locale === "ko" ? "Google Drive, Dropbox 또는 OneDrive를 연결합니다." : "Connect Google Drive, Dropbox or OneDrive."}
              </span>
            </Link>
          </div>
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-panel/60 p-3">
            <p className="text-xs leading-5 text-fg-3">
              {locale === "ko" ? "지금 선택하지 않아도 임시 작업에서 계속 이어갈 수 있습니다." : "You can keep working from Temporary work without choosing now."}
            </p>
            <button type="button" onClick={() => setSaveTarget(null)} className={buttonClass({ variant: "quiet", size: "sm" })}>
              {locale === "ko" ? "계속 임시 저장" : "Keep temporary"}
            </button>
          </div>
        </StudioProjectLibraryModal>
      ) : null}
      ...
      {deleteRequest ? (
        <StudioProjectLibraryModal
          ...
          title={deleteRequest.source === "all"
            ? locale === "ko" ? "휴지통을 비울까요?" : "Empty Trash?"
            : locale === "ko" ? "완전히 삭제할까요?" : "Delete permanently?"}
          description={locale === "ko"
            ? `${deleteRequest.ids.length}개 프로젝트의 목록·저장 설정·문서 메타데이터를 삭제합니다. 이 작업은 실행 취소할 수 없습니다.`
            : `This removes the library entries, save settings and document metadata for ${deleteRequest.ids.length} project(s). This cannot be undone.`}
          closeLabel={locale === "ko" ? "닫기" : "Close"}
          onClose={() => setDeleteRequest(null)}
        >
          <div className="rounded-2xl border border-danger/30 bg-danger-soft/15 p-4">
            <p className="text-sm font-black text-danger">
              {locale === "ko" ? "백업 파일이 필요한지 먼저 확인하세요." : "Confirm that you no longer need a backup file."}
            </p>
            <ul className="mt-3 space-y-1 text-xs text-fg-2">
              ...
              {deleteRequest.ids.length > 5 ? (
                <li>• {locale === "ko" ? `외 ${deleteRequest.ids.length - 5}개` : `and ${deleteRequest.ids.length - 5} more`}</li>
              ) : null}
            </ul>
          </div>
          <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            ...
            >
              ...
              {deleteRequest.source === "all"
                ...
                : locale === "ko" ? "완전 삭제" : "Delete permanently"}
            </button>
          </div>
        </StudioProjectLibraryModal>
      ) : null}
