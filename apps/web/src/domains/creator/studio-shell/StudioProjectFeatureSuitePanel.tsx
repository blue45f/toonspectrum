...
function OverviewSuite({
  ...
  if (!state) {
    return <p className="text-sm text-fg-2">{suite.error ?? (bt("성과 데이터를 준비하고 있습니다.", "Preparing project analytics."))}</p>;
  }
  ...
  return (
    <FeatureCard
      ...
      title={bt("제작과 공개 결과를 한눈에", "Production and audience signals together")}
      description={bt("플랫폼에서 가져온 데이터와 제작 비용을 한 프로젝트 기준으로 합칩니다. 연결 전에는 예제 데이터로 흐름을 확인할 수 있습니다.", "Combine platform metrics and production costs per project. Sample data demonstrates the workflow before connectors are enabled.")}
    >
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <Metric label={bt("순 방문자", "Unique readers")} value={episode?.uniqueReaders ?? 0} />
    ...
    <div className="space-y-5">
      ...
      {showPlanner ? (
        <FeatureCard
          icon={FileText}
          eyebrow="STORYBOARD"
          title={bt("대본에서 컷 계획 만들기", "Plan panels from story beats")}
          description={bt("장면의 목적과 대사를 입력하면 샷 크기·카메라·말풍선 여백·스크롤 간격을 제안합니다. 원고는 자동으로 변경하지 않습니다.", "Story beats produce shot, camera, dialogue-space and scroll-gap suggestions without changing the manuscript.")}
        >
          <div className="grid gap-2 sm:grid-cols-[9rem_1fr]">
            <select value={kind} onChange={(event) => setKind(event.target.value as StudioStoryBeatKind)} className={FIELD_CLASS} aria-label={bt("장면 종류", "Beat kind")}>
              <option value="setup">{bt("도입", "Setup")}</option>
              <option value="dialogue">{bt("대화", "Dialogue")}</option>
              <option value="action">{bt("행동", "Action")}</option>
              <option value="reaction">{bt("반응", "Reaction")}</option>
              <option value="reveal">{bt("반전·공개", "Reveal")}</option>
              <option value="transition">{bt("전환", "Transition")}</option>
            </select>
            <input value={summary} onChange={(event) => setSummary(event.target.value)} placeholder={bt("장면에서 일어나는 핵심 사건", "What happens in this beat")} className={FIELD_CLASS} />
          </div>
          <div className="mt-2 flex gap-2">
            <input value={dialogue} onChange={(event) => setDialogue(event.target.value)} placeholder={bt("선택 사항: 주요 대사", "Optional dialogue")} className={FIELD_CLASS} />
          ...
          <div className="mt-4 space-y-2">
            {state.storyBeats.map((beat) => (
              <div key={beat.id} className="flex items-start gap-3 rounded-xl border border-line bg-panel p-3">
                ...
                <button type="button" onClick={() => removeBeat(beat.id)} disabled={state.storyBeats.length <= 1} aria-label={bt("장면 삭제", "Remove beat")} className={buttonClass({ variant: "quiet", size: "icon" })}>
      ...
      {showContinuity ? (
        <FeatureCard
          icon={Workflow}
          eyebrow="CONTINUITY"
          title={bt("설정과 장면 연결 확인", "Check story continuity")}
          description={bt("캐릭터 의상·외형·부상·소품·알고 있는 정보·장소가 장면 사이에서 설명 없이 바뀌는지 검사합니다.", "Detect unexplained changes in costume, appearance, injuries, props, knowledge and location.")}
        >
          <div className="grid gap-3 sm:grid-cols-3">
            <Metric label={bt("캐릭터", "Characters")} value={workspaceState?.story.bible.characters.length ?? 0} />
            <Metric label={bt("장소", "Locations")} value={workspaceState?.story.bible.locations.length ?? 0} />
            <Metric label={bt("설정 사실", "Facts")} value={workspaceState?.story.bible.facts.length ?? 0} />
          </div>
          <div className="mt-4 flex items-center justify-between rounded-xl border border-line bg-panel p-3">
            <div>
              <b className="text-sm text-fg">{bt("연속성 검사 결과", "Continuity result")}</b>
              <p className="mt-1 text-xs text-fg-3">
                {continuity
                  ? (bt(`오류 ${continuity.blockingCount}개 · 확인 ${continuity.warningCount}개`, `${continuity.blockingCount} blocking · ${continuity.warningCount} warnings`))
                  : (bt("스토리 데이터를 준비하고 있습니다.", "Preparing story data."))}
              </p>
            </div>
...
function ProductionPipeline({ projectId, locale }: { readonly projectId: string; readonly locale: string }) {
  ...
  return (
    <FeatureCard
      icon={Workflow}
      eyebrow="PRODUCTION"
      title={bt("제작 단계와 병목 관리", "Production stages and bottlenecks")}
      description={bt("대본부터 출력까지 의존 관계와 담당 작업량을 계산하고, 다음으로 진행 가능한 작업을 바로 표시합니다.", "Calculate dependencies and workload from story through export, then surface tasks that can move next.")}
    >
      <div className="grid gap-3 sm:grid-cols-3">
        <Metric label={bt("전체 진행", "Overall progress")} value={`${Math.round(report.progress * 100)}%`} />
        <Metric label={bt("바로 시작 가능", "Ready now")} value={report.readyTaskIds.length} />
        <Metric label={bt("막힌 작업", "Blocked")} value={report.dependencyBlockedTaskIds.length} />
      </div>
      <div className="mt-4 space-y-2">
...
function QualityPanel({ projectId, locale }: { readonly projectId: string; readonly locale: string }) {
  ...
  return (
    <FeatureCard
      ...
      title={bt("모바일 가독성과 스크롤 리듬", "Mobile readability and scroll rhythm")}
      description={bt("작품을 자동 수정하지 않고 글자 크기·읽기 순서·말풍선 겹침·장면 전환 간격을 검사합니다.", "Inspect text size, reading order, balloon overlap and scene spacing without changing artwork.")}
    >
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-line bg-panel p-3">
        <div className="grid flex-1 gap-3 sm:grid-cols-3">
          <Metric label={bt("품질 점수", "Quality score")} value={report.score} />
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-2">
      <FeatureCard
        icon={Boxes}
        eyebrow="3D RENDER"
        title={bt("웹툰용 3D 분리 출력", "Webtoon-ready 3D render passes")}
        description={bt("카메라·조명·권리·장면 부담을 검사하고 색·선화·그림자·깊이·마스크를 편집 가능한 레이어로 계획합니다.", "Check camera, lighting, rights and scene load, then plan editable color, line, shadow, depth and mask layers.")}
      >
      ...
      <FeatureCard
        ...
        title={bt("대사와 장면 타이밍 연결", "Connect dialogue, voice and motion timing")}
        description={bt("대사가 바뀐 구간만 다시 생성하고, 음성 권리와 출처 문구를 확인한 뒤 장면 타이밍을 계산합니다.", "Regenerate only changed dialogue, verify voice rights and attribution, and calculate scene timing.")}
      >
        <label className="text-xs font-bold text-fg-2">
          {bt("대표 대사", "Sample dialogue")}
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <Metric label={bt("다시 생성", "Regenerate")} value={voicePlan.regenerateLineIds.length} />
          <Metric label={bt("재사용", "Reuse")} value={voicePlan.reuseLineIds.length} />
          <Metric label={bt("전체 길이", "Duration")} value={`${Math.round((schedule.at(-1)?.endMs ?? 0) / 100) / 10}s`} />
        </div>
...
function DesignTemplatePanel({ projectId, locale }: { readonly projectId: string; readonly locale: string }) {
  ...
  return (
    <FeatureCard
      ...
      title={bt("템플릿을 구조적으로 적용", "Apply structured templates")}
      description={bt("텍스트·이미지·색상 슬롯과 사용 권리를 확인하고, 발표 자료의 글자 크기·겹침·레이아웃을 함께 검사합니다.", "Validate text, image, color slots and rights, then audit presentation type size, overlap and layout.")}
    >
      <label className="text-xs font-bold text-fg-2">
        {bt("작품 제목", "Project title")}
...
function AutomationPanel({ projectId, locale }: { readonly projectId: string; readonly locale: string }) {
  ...
  return (
    <FeatureCard
      icon={WandSparkles}
      eyebrow="AUTOMATION"
      title={bt("안전한 작업은 자동으로, 외부 작업은 확인 후", "Automate safe work and confirm external actions")}
      description={bt("품질·사전검사는 바로 실행하고 게시·유료·파괴 작업만 명시적으로 확인하는 자동화 계획입니다.", "Quality and preflight can run directly; publishing, paid and destructive steps require explicit confirmation.")}
    >
  ...
  if (section === "settings" && view === "archive") {
    return (
      <FeatureCard
        ...
        title={bt("보관 전 안전 확인", "Safety checks before archiving")}
        description={bt("원고·에셋·Series Kit·현지화·검토·출력 기록을 포함한 완전한 프로젝트 사본을 먼저 만드는 흐름을 사용합니다.", "Create a complete project copy with documents, assets, Series Kit, localization, review and export history before archiving.")}
      >
        <div className="flex items-center gap-3 rounded-xl border border-success/30 bg-success-soft/15 p-3 text-sm text-fg-2">
          ...
          {bt("보관은 원본 삭제가 아니며, 복원 가능한 상태로 유지됩니다.", "Archiving does not delete the original and remains reversible.")}
        </div>
      </FeatureCard>
    );
  }
