...
function projectStartActions(_locale): readonly StudioIntentAction[] {
  return [
    {
    ...
    },
    {
      ...
      description: bi("기존 대본·캐릭터·장면을 정리하고 회차 제작으로 연결합니다.", "Organize existing scripts, characters and scenes, then connect them to production."),
    },
    {
      href: "/studio/import",
      icon: FileUp,
    ...
    },
    {
      ...
      description: bi("기획부터 검토·연재 준비까지 연결된 샘플을 안전하게 체험합니다.", "Explore a safe sample connected from planning through review and publishing."),
...
function projectFlow(_locale): readonly StudioTaskFlowStep[] {
  const labels: readonly (readonly [string, string, string])[] = bi([
    ...
    <section className="mt-7 min-w-0 rounded-3xl border border-line bg-panel/55 p-4 shadow-sm sm:p-6" aria-labelledby="studio-starting-point-title">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-3xl break-words text-sm leading-6 text-fg-3">
            {bi("실력 수준이 아니라 현재 재료를 기준으로 가장 짧은 시작 경로를 안내합니다. 기존 프로젝트는 아래에서 바로 이어서 작업할 수 있습니다.", "The shortest path is based on your current material, not an expertise label. Continue existing projects below.")}
          </p>
        </div>
      ...
      <div className="mt-5 min-w-0">
        <p className="mb-2 break-words text-xs font-black text-fg-2">
          {bi("한 프로젝트에서 이어지는 전체 제작 흐름", "The complete flow inside one project")}
        </p>
        <StudioTaskFlow
