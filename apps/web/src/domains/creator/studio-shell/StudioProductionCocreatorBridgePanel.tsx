      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-3xl">
          <p className="text-[0.6875rem] font-bold uppercase tracking-[0.14em] text-accent">
            {bi("공동 창작 프로덕션", "Co-creation production")}
          </p>
          <h2 className="mt-2 text-lg font-black tracking-tight text-fg">
            {bi("스토리·작화·검수·권리 정본을 한 흐름으로 관리", "Manage story, art, review and rights in one lineage")}
          </h2>
          <p className="mt-2 text-xs leading-6 text-fg-2">
            {bi("StoryLock과 작화 인수인계, 역할별 승인, 발주 범위, 크레딧·보상까지 현재 프로젝트 ID를 유지한 채 이어집니다.", "Continue with StoryLock, art handoff, role-based approvals, procurement scope, credits and compensation while preserving the current project identity.")}
          </p>
        </div>
        <Link className={buttonClass()} to={href}>
          {bi("프로덕션 허브 열기", "Open production hub")}
          <ArrowRight className="size-4" aria-hidden="true" />
        </Link>
      </div>
      <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        {[
          [Handshake, bi("Story → Art 인수인계", "Story → Art handoff")],
          [ClipboardCheck, bi("lane별 공동 승인", "Lane-based approval")],
          [Users, bi("역할·결정권 분리", "Role and authority separation")],
          [Scale, bi("크레딧·권리·보상", "Credits, rights and pay")],
        ].map(([Icon, label]) => {
