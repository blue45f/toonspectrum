...
function statusLabel(
  ...
  if (status === "ready") return translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", "준비됨", "Ready");
  if (status === "warning") return translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", "확인 필요", "Review");
  return translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", "해결 필요", "Blocked");
}
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-fg">
                {failure
                  ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", "프로젝트 준비도 · 연결 데이터 없음", "Project readiness · Source unavailable"))
                  : (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", "프로젝트 준비도 · 검사 전", "Project readiness · Not checked"))}
              </h2>
            </div>
            <p className="mt-2 max-w-3xl text-xs leading-5 text-fg-3">
              {failure
                ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", "현재 프로젝트의 스토리·제작·에셋·검토·현지화·출력 데이터가 아직 진단 경계에 연결되지 않았습니다. 준비 완료로 추정하지 않았으며 저장된 작업도 변경하지 않았습니다.", "This project has not yet supplied connected story, production, asset, review, localization and export data. It was not guessed ready and no saved work was changed."))
                : (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", "스토리·제작·권리·검토·현지화·출력 상태가 연결되면 실제 결과를 계산합니다. 데이터가 없을 때 준비 완료로 표시하지 않습니다.", "Readiness is calculated from connected story, production, rights, review, localization and export data. Missing data is never shown as ready."))}
            </p>
            {failure ? (
          ...
          >
            ...
            {failure
              ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", "다시 연결 확인", "Check connection again"))
              : (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", "상태 검사", "Check status"))}
          </button>
        </div>
      </section>
    );
    ...
    <section className="mt-5 rounded-2xl border border-line bg-card p-4 sm:p-5" aria-live="polite">
      {failure ? (
        <div className="mb-4 rounded-xl border border-warning/40 bg-warning-soft/15 px-3 py-2 text-xs text-warning" role="alert">
          {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", `최신 재검사에 실패해 ${new Date(snapshot.updatedAt).toLocaleString("ko-KR")}의 마지막 정상 결과를 유지합니다.`, `The latest refresh failed, so the last valid result from ${new Date(snapshot.updatedAt).toLocaleString("en-US")} remains visible.`)}
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-fg">
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", `프로젝트 준비도 ${percentage}%`, `Project readiness ${percentage}%`)}
            </h2>
          </div>
          <p className="mt-2 text-xs leading-5 text-fg-3">
            {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", `해결 필요 ${report.blockingCount}건 · 확인 필요 ${report.warningCount}건 · ${new Date(snapshot.updatedAt).toLocaleString("ko-KR")} 검사`, `${report.blockingCount} blocker(s) · ${report.warningCount} warning(s) · checked ${new Date(snapshot.updatedAt).toLocaleString("en-US")}`)}
          </p>
        </div>
        <button
          type="button"
        ...
        >
          ...
          {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", "다시 검사", "Check again")}
        </button>
      </div>
            ...
            return (
              ...
              >
                <span className="flex items-center justify-between gap-2">
                  <strong className="text-xs text-fg">{translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", SECTION_LABELS[section.id])}</strong>
                  <ReadinessStatusIcon
        ...
        <div className="mt-4 grid gap-2 lg:grid-cols-3">
          {primaryActions.map((action) => (
            ...
            >
              ...
              <span>{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioProjectReadinessPanel", action.messageKo, action.messageEn)}</span>
            </Link>
          ))}
        </div>
      ) : null}
