...
export function StudioSaveFirstProjectCreatePage() {
  ...
  const selectedStorage = STUDIO_PROJECT_CREATE_STORAGE.find((option) => option.id === storageProvider)
    ?? STUDIO_PROJECT_CREATE_STORAGE[0]!;
    ...
    <main className="min-h-[calc(100vh-4rem)] bg-bg">
      <Container size="wide" className="py-7 sm:py-12">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black tracking-tight text-fg sm:text-4xl">
                {locale === "ko" ? "무엇을 만들까요?" : "What would you like to make?"}
              </h1>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-fg-2 sm:text-base">
                {locale === "ko"
                  ? "만들 종류와 시작 형식만 고르면 추천 설정으로 작업을 준비합니다. 이름과 저장 위치는 나중에도 바꿀 수 있어요."
                  : "Choose an outcome and starting format, and recommended settings are prepared. You can change the name and storage later."}
              </p>
            </div>
          ...
          <details className="mt-7 rounded-2xl border border-line bg-card" data-studio-storage-settings>
            <summary className="flex min-h-20 cursor-pointer list-none items-center gap-3 px-5 py-4 [&::-webkit-details-marker]:hidden">
              ...
              <span className="min-w-0 flex-1">
                <h2 id="storage-title" className="text-base font-black text-fg">
                  {locale === "ko" ? "3. 저장 위치" : "3. Save location"}
                </h2>
                <span className="mt-1 block text-xs leading-5 text-fg-3">
                  {locale === "ko"
                    ? `기본값: ${selectedStorage.titleKo}. 필요할 때 파일이나 개인 드라이브 백업을 선택할 수 있어요.`
                    : `Default: ${selectedStorage.titleEn}. Choose a file or personal-drive backup when needed.`}
                </span>
              </span>
              <span className="shrink-0 rounded-full bg-panel px-3 py-1.5 text-[0.68rem] font-black text-fg-2">
                {locale === "ko" ? selectedStorage.badgeKo : selectedStorage.badgeEn}
              </span>
            </summary>
            <div className="border-t border-line px-5 pb-5 pt-4">
              <div role="radiogroup" aria-labelledby="storage-title" className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        active ? "border-accent bg-accent-soft/35" : "border-line bg-panel/40 hover:border-accent/40",
                      )}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <span className={cn("grid size-10 place-items-center rounded-xl", active ? "bg-accent text-on-accent" : "bg-card text-fg-2")}>
                          <Icon size={18} aria-hidden="true" />
                        </span>
                        <span className="rounded-full bg-card px-2 py-1 text-[0.62rem] font-black text-fg-3">
          ...
          <details className="mt-3 rounded-2xl border border-line bg-card" data-studio-advanced-save-settings>
            <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-3 px-5 py-3 text-sm font-black text-fg [&::-webkit-details-marker]:hidden">
              <span>{locale === "ko" ? "고급 저장 설정" : "Advanced save settings"}</span>
              <span className="text-xs font-semibold text-fg-3">
                {locale === "ko" ? "자동 저장 켜짐" : "Autosave on"}
              </span>
            </summary>
            <div className="grid gap-3 border-t border-line p-5 sm:grid-cols-2">
              <label htmlFor="studio-save-first-autosave" aria-label={locale === "ko" ? "작업 중 자동 저장" : "Autosave while working"} className="flex cursor-pointer items-start gap-3 rounded-xl border border-line bg-panel/45 p-3">
                ...
                <span><b className="block text-sm text-fg">{locale === "ko" ? "작업 중 자동 저장" : "Autosave while working"}</b><small className="mt-1 block leading-5 text-fg-3">{locale === "ko" ? "편집 내용을 이 브라우저에 계속 보존합니다." : "Continuously preserve edits in this browser."}</small></span>
              </label>
              <label htmlFor="studio-save-first-versioning" aria-label={locale === "ko" ? "중요한 변경 시 버전 생성" : "Create versions for important changes"} className="flex cursor-pointer items-start gap-3 rounded-xl border border-line bg-panel/45 p-3">
                ...
                <span><b className="block text-sm text-fg">{locale === "ko" ? "중요한 변경 시 버전 생성" : "Create versions for important changes"}</b><small className="mt-1 block leading-5 text-fg-3">{locale === "ko" ? "내보내기와 게시 전에 복구 지점을 남깁니다." : "Keep recovery points before export and publishing."}</small></span>
              </label>
            </div>
          </details>
          ...
          <div className="mt-7 flex flex-col gap-3 rounded-3xl border border-accent/30 bg-accent-soft/20 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="flex items-center gap-2 text-sm font-black text-fg"><LockKeyhole size={16} aria-hidden="true" />{locale === "ko" ? "자동 저장으로 안전하게 시작" : "Start safely with autosave"}</p>
              <p className="mt-1 text-xs leading-5 text-fg-3">
                {storageProvider === "browser"
                  ? locale === "ko" ? "기본 설정으로 바로 시작합니다. 파일이나 개인 드라이브 백업은 내 작업에서 추가할 수 있어요." : "Start with the recommended defaults. Add a file or personal-drive backup later from My work."
