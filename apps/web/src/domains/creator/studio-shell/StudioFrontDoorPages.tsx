  ...
  return (
    ...
    >
      ...
      <span className="mt-4 flex min-w-0 flex-wrap items-start gap-2">
        <strong className="min-w-0 break-words text-sm text-fg">{card.title[locale]}</strong>
        {card.badge ? (
      ...
      <span className="mt-1.5 min-w-0 flex-1 break-words text-xs leading-5 text-fg-3">{card.description[locale]}</span>
      <span className="mt-3 inline-flex min-w-0 items-center gap-1 text-xs font-bold text-accent">
        <span className="break-words">{locale === "ko" ? "열기" : "Open"}</span>
        ...
        <div className="flex w-full min-w-0 flex-wrap gap-2 lg:w-auto lg:shrink-0 lg:justify-end">
          {action ?? (
            <Link href="/studio/new" className={buttonClass({ size: "lg", className: "w-full min-w-0 gap-2 sm:w-auto" })}>
              ...
              <span className="break-words">{locale === "ko" ? "새로 만들기" : "Create new"}</span>
            </Link>
          )}
        </div>
      </div>
...
export function StudioHomePage() {
  ...
  return (
    <Container size="wide" className="min-w-0 py-7 sm:py-10 lg:py-12">
      <PageHeader
        eyebrow="TOONSTUDIO"
        title={locale === "ko" ? "기획부터 연재까지, 필요한 다음 행동이 바로 보입니다." : "From planning to publishing, the next useful action stays clear."}
        description={locale === "ko"
          ? "현재 가진 작업물을 고르면 ToonStudio가 적절한 시작점과 작업공간을 연결합니다. 전문 기능은 필요할 때 펼치고, 저장·복구 상태는 항상 분명하게 보여 줍니다."
          : "Choose what you already have and ToonStudio connects the right starting point and workspace. Advanced tools appear when needed, while save and recovery status stays explicit."}
        locale={locale}
      />
        ...
        <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-3xl break-words text-sm leading-6 text-fg-3">
              {locale === "ko"
                ? "실력 수준이 아니라 현재 재료를 기준으로 가장 짧은 시작 경로를 안내합니다."
                : "The shortest path is based on your current material, not a beginner or expert label."}
            </p>
          </div>
          <button
            type="button"
          ...
          >
            ...
            <span className="break-words">{locale === "ko" ? "기능·작업 검색" : "Search tools and work"}</span>
          </button>
        </div>
        <StudioIntentLauncher
        ...
        <div className="flex min-w-0 flex-wrap items-end justify-between gap-3">
          <div className="min-w-0">
            ...
            <h2 id="studio-production-flow-title" className="mt-1 break-words text-2xl font-bold tracking-tight text-fg">
              {locale === "ko" ? "한 작품 안에서 끝까지 이어집니다" : "Stay in one project from start to finish"}
            </h2>
          </div>
      ...
      <section className="mt-10 min-w-0" aria-labelledby="studio-home-actions">
        <div className="min-w-0">
          ...
          <h2 id="studio-home-actions" className="mt-1 break-words text-2xl font-bold tracking-tight text-fg">
            {locale === "ko" ? "내 작업 관리" : "Manage my work"}
          </h2>
        </div>
      ...
      <section className="mt-12 min-w-0" aria-labelledby="studio-workflow-links">
        <h2 id="studio-workflow-links" className="break-words text-2xl font-bold tracking-tight text-fg">
          {locale === "ko" ? "기획·검토·연재 연결" : "Connect planning, review and publishing"}
        </h2>
...
export function StudioNewPage() {
  ...
  const steps: readonly StudioTaskFlowStep[] = locale === "ko"
    ? [
  ...
  return (
    <Container size="wide" className="min-w-0 py-7 sm:py-10 lg:py-12">
      <PageHeader
        ...
        action={
          <Link href="/studio/projects" className={buttonClass({ variant: "outline", size: "lg", className: "w-full min-w-0 gap-2 sm:w-auto" })}>
            ...
            <span className="break-words">{locale === "ko" ? "프로젝트 보기" : "View projects"}</span>
          </Link>
        }
      />
      <StudioTaskFlow steps={steps} ariaLabel={locale === "ko" ? "새 작업 시작 단계" : "New work start steps"} className="mt-5" />
      ...
      <RecoverableActionNotice
        tone="info"
        title={locale === "ko" ? "기존 파일도 원본을 보존한 채 시작할 수 있습니다" : "You can start from an existing file while preserving the original"}
        description={locale === "ko"
          ? "프로젝트, 이미지, 브러시와 3D 파일을 먼저 분석하고 편집 가능한 범위와 변환 내용을 적용 전에 보여 줍니다."
          : "Projects, images, brushes and 3D files are analyzed first so editable coverage and conversions are visible before changes are applied."}
        action={
          <Link href="/studio/import" className={buttonClass({ className: "w-full min-w-0 gap-2 sm:w-auto" })}>
            ...
            <span className="break-words">{locale === "ko" ? "파일 가져오기" : "Import files"}</span>
          </Link>
        }
        className="mt-8"
      />
  ...
  return (
    <Container size="wide" className="min-w-0 py-7 sm:py-10 lg:py-12">
      <PageHeader
        ...
        locale={locale}
      />
      <StudioTaskFlow steps={steps} ariaLabel={locale === "ko" ? "파일 가져오기 단계" : "File import steps"} className="mt-5" />
      ...
      <div className="mt-8 grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          <Link key={href} href={href} className="group min-w-0 rounded-2xl border border-line bg-card p-5 transition-all hover:-translate-y-0.5 hover:border-accent/45 hover:bg-raised motion-reduce:transform-none">
            ...
            <span className="mt-4 inline-flex min-w-0 items-center gap-1 text-xs font-bold text-accent"><span className="break-words">{locale === "ko" ? "계속" : "Continue"}</span><ArrowRight size={13} className="shrink-0" aria-hidden="true" /></span>
          </Link>
        ))}
      </div>
      ...
      <RecoverableActionNotice
        tone="warning"
        title={locale === "ko" ? "지원하지 않는 객체는 몰래 평탄화하지 않습니다" : "Unsupported objects are never flattened silently"}
        description={locale === "ko"
          ? "가져오기 전에 완전 보존, 편집 가능한 변환, 래스터 변환과 제외 항목을 나눠 보여 주고 원본 파일을 별도로 보관합니다."
          : "Before import, ToonStudio separates fully preserved, editable conversion, rasterization and excluded items while keeping the original file separately."}
        className="mt-6"
      />
    </Container>
  );
      ...
      <PageHeader
        ...
        action={
          <Link href="/studio/assets?view=market" className={buttonClass({ size: "lg", className: "w-full min-w-0 gap-2 sm:w-auto" })}>
            ...
            <span className="break-words">{locale === "ko" ? "마켓에서 찾기" : "Browse market"}</span>
          </Link>
        }
      />
      <StudioTaskFlow steps={steps} ariaLabel={locale === "ko" ? "소재 사용 흐름" : "Material usage flow"} className="mt-5" />
