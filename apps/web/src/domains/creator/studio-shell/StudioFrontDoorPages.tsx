...
function intentActions(locale: StudioFrontDoorLocale): readonly StudioIntentAction[] {
  return [
    {
      href: "/studio/new?kind=webtoon&template=webtoon-vertical",
      icon: Lightbulb,
      title: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "아이디어만 있어요", "I only have an idea"),
      description: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "작품 이름과 첫 회차부터 시작하고, 기획·콘티·작화 순서로 이어갑니다.", "Start with a title and first episode, then move through planning, storyboard and art."),
      badge: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "처음 시작 추천", "Recommended first start"),
      visual: "/brand/theme-scenes/ink-studio.svg",
    },
    {
      href: "/story-lab",
      icon: FileText,
      title: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "대본이나 콘티가 있어요", "I have a script or storyboard"),
      visual: "/brand/theme-scenes/graphite-studio.svg",
      description: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "기존 대본·캐릭터·장면을 정리하고 바로 회차 제작으로 연결합니다.", "Organize existing scripts, characters and scenes, then connect them to production."),
    },
    {
      href: "/studio/import",
      icon: FileUp,
      title: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "그리던 파일이 있어요", "I have work-in-progress files"),
      visual: "/brand/atelier-process-640.webp",
      description: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "PSD·ORA·이미지·브러시·3D 파일을 분석하고 원본을 보존한 채 가져옵니다.", "Analyze PSD, ORA, image, brush and 3D files while preserving the originals."),
    },
    {
      href: "/production",
      icon: UsersRound,
      title: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "팀 프로젝트를 시작해요", "I am starting a team project"),
      visual: "/brand/production-os-journey.svg",
      description: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "역할·마감·작업 넘기기·검수 기준을 먼저 정하고 함께 제작합니다.", "Set roles, deadlines, handoffs and review rules before producing together."),
    },
    {
      href: "/production/projects/sample-project/overview",
      icon: PlayCircle,
      title: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "샘플로 먼저 둘러볼게요", "Show me a sample first"),
      description: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "기획부터 검토·연재 준비까지 연결된 샘플 프로젝트를 안전하게 체험합니다.", "Explore a safe sample project connected from planning through review and publishing."),
      badge: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "원본 유지", "Original stays intact"),
      visual: "/brand/production-os-hero.svg",
    },
  ];
}

function productionFlow(locale: StudioFrontDoorLocale): readonly StudioTaskFlowStep[] {
  const labels = translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", [
  ...
  return (
    ...
    >
      ...
      <span className="mt-4 flex min-w-0 flex-wrap items-start gap-2">
        <strong className="min-w-0 break-words text-sm text-fg">{translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", card.title)}</strong>
        {card.badge ? (
          <span className="max-w-full break-words rounded-full border border-accent/30 bg-accent-soft px-2 py-0.5 text-[0.62rem] font-bold leading-4 text-accent">
            {translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", card.badge)}
          </span>
        ) : null}
      </span>
      <span className="mt-1.5 min-w-0 flex-1 break-words text-xs leading-5 text-fg-3">{translateLocaleBranchForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", card.description)}</span>
      <span className="mt-3 inline-flex min-w-0 items-center gap-1 text-xs font-bold text-accent">
        <span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "열기", "Open")}</span>
        ...
        <div className="flex w-full min-w-0 flex-wrap gap-2 lg:w-auto lg:shrink-0 lg:justify-end">
          {action ?? (
            <Link href="/studio/new" className={buttonClass({ size: "lg", className: "w-full min-w-0 gap-2 sm:w-auto" })}>
              ...
              <span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "새로 만들기", "Create new")}</span>
            </Link>
          )}
        </div>
      </div>
...
export function StudioHomePage() {
  ...
  useDocumentTitle(translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "내 작업", "My work"));

  return (
    <Container size="wide" className="min-w-0 py-7 sm:py-10 lg:py-12">
      <PageHeader
        eyebrow="TOONSTUDIO"
        title={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "기획부터 연재까지, 필요한 다음 행동이 바로 보입니다.", "From planning to publishing, the next useful action stays clear.")}
        description={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "현재 가진 작업물을 고르면 ToonStudio가 적절한 시작점과 작업공간을 연결합니다. 전문 기능은 필요할 때 펼치고, 저장·복구 상태는 항상 분명하게 보여 줍니다.", "Choose what you already have and ToonStudio connects the right starting point and workspace. Advanced tools appear when needed, while save and recovery status stays explicit.")}
        locale={locale}
      />
        ...
        <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="min-w-0">
            ...
            <h2 id="studio-starting-point-title" className="mt-1 break-words text-2xl font-bold tracking-tight text-fg">
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "지금 무엇을 가지고 있나요?", "What do you have right now?")}
            </h2>
            <p className="mt-1 max-w-3xl break-words text-sm leading-6 text-fg-3">
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "실력 수준이 아니라 현재 재료를 기준으로 가장 짧은 시작 경로를 안내합니다.", "The shortest path is based on your current material, not a beginner or expert label.")}
            </p>
          </div>
          <button
            type="button"
          ...
          >
            ...
            <span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "기능·작업 검색", "Search tools and work")}</span>
          </button>
        </div>
        <StudioIntentLauncher
          actions={intentActions(locale)}
          ariaLabel={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "현재 재료별 시작 경로", "Starting paths by current material")}
          actionLabel={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "시작", "Start")}
          className="mt-5"
        />
      </section>
      ...
      <section className="mt-8 min-w-0" aria-labelledby="studio-production-flow-title">
        <div className="flex min-w-0 flex-wrap items-end justify-between gap-3">
          <div className="min-w-0">
            ...
            <h2 id="studio-production-flow-title" className="mt-1 break-words text-2xl font-bold tracking-tight text-fg">
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "한 작품 안에서 끝까지 이어집니다", "Stay in one project from start to finish")}
            </h2>
          </div>
        ...
        <StudioTaskFlow
          ...
          ariaLabel={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "웹툰 제작 전체 흐름", "Complete webtoon production flow")}
          className="mt-4"
        />
      </section>
      ...
      <section className="mt-10 min-w-0" aria-labelledby="studio-home-actions">
        <div className="min-w-0">
          ...
          <h2 id="studio-home-actions" className="mt-1 break-words text-2xl font-bold tracking-tight text-fg">
            {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "내 작업 관리", "Manage my work")}
          </h2>
        </div>
      ...
      <section className="mt-12 min-w-0" aria-labelledby="studio-quick-starts">
        <h2 id="studio-quick-starts" className="break-words text-2xl font-bold tracking-tight text-fg">
          {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "바로 만들기", "Quick start")}
        </h2>
        <p className="mt-1 break-words text-sm text-fg-3">
          {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "결과물을 고르면 필요한 편집 환경을 바로 엽니다.", "Choose an outcome and open the right editing environment.")}
        </p>
      ...
      <section className="mt-12 min-w-0" aria-labelledby="studio-workflow-links">
        <h2 id="studio-workflow-links" className="break-words text-2xl font-bold tracking-tight text-fg">
          {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "기획·검토·연재 연결", "Connect planning, review and publishing")}
        </h2>
...
export function StudioNewPage() {
  const language = useI18n((state) => state.lang);
  const locale = studioFrontDoorLocale(language);
  useDocumentTitle(translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "새로 만들기", "Create new"));
  const steps: readonly StudioTaskFlowStep[] = translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", [
  ...
  return (
    <Container size="wide" className="min-w-0 py-7 sm:py-10 lg:py-12">
      <PageHeader
        eyebrow="CREATE"
        title={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "만들 결과만 고르세요.", "Choose the result you want.")}
        description={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "크기·색 공간·레이어 구조는 추천값으로 준비하고, 전문 설정은 필요할 때만 펼칩니다.", "Recommended size, color and layer structure are prepared automatically. Advanced settings stay available when needed.")}
        locale={locale}
        action={
          <Link href="/studio/projects" className={buttonClass({ variant: "outline", size: "lg", className: "w-full min-w-0 gap-2 sm:w-auto" })}>
            ...
            <span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "프로젝트 보기", "View projects")}</span>
          </Link>
        }
      />
      <StudioTaskFlow steps={steps} ariaLabel={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "새 작업 시작 단계", "New work start steps")} className="mt-5" />
      ...
      <section className="mt-8 min-w-0" aria-labelledby="studio-new-document-types">
        <h2 id="studio-new-document-types" className="sr-only">
          {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "새 문서 종류", "New document types")}
        </h2>
      ...
      <RecoverableActionNotice
        tone="info"
        title={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "기존 파일도 원본을 보존한 채 시작할 수 있습니다", "You can start from an existing file while preserving the original")}
        description={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "프로젝트, 이미지, 브러시와 3D 파일을 먼저 분석하고 편집 가능한 범위와 변환 내용을 적용 전에 보여 줍니다.", "Projects, images, brushes and 3D files are analyzed first so editable coverage and conversions are visible before changes are applied.")}
        action={
          <Link href="/studio/import" className={buttonClass({ className: "w-full min-w-0 gap-2 sm:w-auto" })}>
            ...
            <span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "파일 가져오기", "Import files")}</span>
          </Link>
        }
        className="mt-8"
      />
...
export function StudioImportPage() {
  const language = useI18n((state) => state.lang);
  const locale = studioFrontDoorLocale(language);
  useDocumentTitle(translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "파일 가져오기", "Import files"));
  const steps: readonly StudioTaskFlowStep[] = translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", [
  ...
  return (
    <Container size="wide" className="min-w-0 py-7 sm:py-10 lg:py-12">
      <PageHeader
        eyebrow="IMPORT"
        title={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "파일 종류를 몰라도 괜찮아요.", "You do not need to know the file type.")}
        description={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "가져올 대상을 고르면 기존 편집기와 에셋 도구로 연결하고, 손실 가능성이 있는 항목은 적용 전에 알려 줍니다.", "Choose what you are importing. ToonStudio opens the right editor and reports possible losses before applying changes.")}
        locale={locale}
      />
      <StudioTaskFlow steps={steps} ariaLabel={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "파일 가져오기 단계", "File import steps")} className="mt-5" />
        ...
        {[
          {
            href: "/studio/canvas",
            icon: FileImage,
            title: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "프로젝트·이미지", "Projects & images"),
            body: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "Studio 프로젝트, PSD, ORA, CBZ와 일반 이미지를 편집기에서 가져옵니다.", "Import Studio projects, PSD, ORA, CBZ and standard images in the editor."),
          },
          {
            href: "/studio/brushes",
            icon: Brush,
            title: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "브러시", "Brushes"),
            body: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "ToonStudio 브러시와 ABR·MYB·KPP 파일을 내 브러시에 추가합니다.", "Add ToonStudio, ABR, MYB and KPP files to your brushes."),
          },
          {
            href: "/studio/bg3d",
            icon: Boxes,
            title: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "3D 모델", "3D models"),
            body: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "GLB·glTF·VRM과 지원되는 3D 파일을 배경·포즈 작업에 연결합니다.", "Connect GLB, glTF, VRM and supported 3D files to backgrounds and poses."),
          },
          {
            href: "/studio/assets/audio",
            icon: Music2,
            title: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "오디오", "Audio"),
            body: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "음악·효과음·음성을 애니매틱과 모션 작업에 추가합니다.", "Add music, sound effects and voice to animatics and motion work."),
          },
          ...
          <Link key={href} href={href} className="group min-w-0 rounded-2xl border border-line bg-card p-5 transition-all hover:-translate-y-0.5 hover:border-accent/45 hover:bg-raised motion-reduce:transform-none">
            ...
            <span className="mt-4 inline-flex min-w-0 items-center gap-1 text-xs font-bold text-accent"><span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "계속", "Continue")}</span><ArrowRight size={13} className="shrink-0" aria-hidden="true" /></span>
          </Link>
        ))}
      </div>
      ...
      <RecoverableActionNotice
        tone="warning"
        title={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "지원하지 않는 객체는 몰래 평탄화하지 않습니다", "Unsupported objects are never flattened silently")}
        description={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "가져오기 전에 완전 보존, 편집 가능한 변환, 래스터 변환과 제외 항목을 나눠 보여 주고 원본 파일을 별도로 보관합니다.", "Before import, ToonStudio separates fully preserved, editable conversion, rasterization and excluded items while keeping the original file separately.")}
        className="mt-6"
      />
    </Container>
  );
...
export function StudioAssetsPage() {
  const language = useI18n((state) => state.lang);
  const locale = studioFrontDoorLocale(language);
  useDocumentTitle(translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "소재", "Materials"));
  const steps: readonly StudioTaskFlowStep[] = translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", [
  ...
  return (
    <Container size="wide" className="min-w-0 py-7 sm:py-10 lg:py-12">
      <PageHeader
        eyebrow="MATERIALS"
        title={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "찾기부터 제작·설치·사용까지 한곳에서.", "Discover, create, install and use materials in one place.")}
        description={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "브러시·캐릭터·3D·오디오를 각각 다른 제품처럼 찾지 않고, 현재 프로젝트에 필요한 소재로 연결합니다.", "Use brushes, characters, 3D and audio as project materials instead of separate products.")}
        locale={locale}
        action={
          <Link href="/studio/assets?view=market" className={buttonClass({ size: "lg", className: "w-full min-w-0 gap-2 sm:w-auto" })}>
            ...
            <span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "마켓에서 찾기", "Browse market")}</span>
          </Link>
        }
      />
      <StudioTaskFlow steps={steps} ariaLabel={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "소재 사용 흐름", "Material usage flow")} className="mt-5" />

      <section className="mt-8 min-w-0" aria-labelledby="studio-asset-categories">
        <h2 id="studio-asset-categories" className="sr-only">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioFrontDoorPages", "소재 종류", "Material categories")}</h2>
        <div className="grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3">
