...
export function StudioProjectLibraryManagementHeader({
  ...
  return (
    <>
      <header className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black tracking-tight text-fg sm:text-4xl">
            {STUDIO_PROJECT_LIBRARY_MANAGEMENT_LABELS[view][locale]}
          </h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-fg-2 sm:text-base">
            {STUDIO_PROJECT_LIBRARY_MANAGEMENT_DESCRIPTIONS[view][locale]}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link href="/studio/projects" className={buttonClass({ variant: "quiet" })}>
            {locale === "ko" ? "제작 관리" : "Production management"}
          </Link>
        ...
        <nav className="overflow-x-auto" aria-label={locale === "ko" ? "내 작업 보기" : "My work views"}>
          <div className="flex min-w-max gap-1 rounded-2xl border border-line bg-card p-1">
            {(["active", "archived", "trash"] as const).map((candidate) => (
              ...
              >
                {STUDIO_PROJECT_LIBRARY_MANAGEMENT_LABELS[candidate][locale]}
      ...
      <section className="mt-7 rounded-2xl border border-line bg-card p-3 sm:p-4" aria-label={locale === "ko" ? "프로젝트 찾기와 선택" : "Find and select projects"}>
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <label className="relative block min-w-0 flex-1 lg:max-w-xl">
            ...
            <input
              ...
              placeholder={locale === "ko" ? "제목·종류·설명 검색" : "Search title, type or description"}
          ...
          <div className="flex flex-wrap items-center gap-2">
            <label className="text-xs font-semibold text-fg-3">
              <span className="sr-only">{locale === "ko" ? "정렬" : "Sort"}</span>
              <select
                value={sort}
              ...
              >
                ...
                <option value="created">{locale === "ko" ? "최근 생성순" : "Recently created"}</option>
              </select>
            </label>
            <button
              type="button"
            ...
            >
              {allVisibleSelected
                ...
                : locale === "ko" ? `전체 선택 (${visibleProjects.length})` : `Select all (${visibleProjects.length})`}
            </button>
          </div>
        </div>
      </section>
      ...
      {selectedProjects.length > 0 ? (
        <section className="sticky top-3 z-30 mt-3 rounded-2xl border border-accent/35 bg-card/95 p-3 shadow-lg backdrop-blur" aria-label={locale === "ko" ? "선택 작업" : "Selection actions"}>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-black text-fg">
                {locale === "ko" ? `${selectedProjects.length}개 선택` : `${selectedProjects.length} selected`}
              </p>
              <p className="mt-0.5 text-[0.68rem] text-fg-3">
                {locale === "ko" ? "검색 결과 전체 선택과 개별 선택을 함께 사용할 수 있습니다." : "Combine select-all results with individual selection."}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              {view === "active" ? (
                <>
                  <button type="button" onClick={() => archiveProjects(selectedProjects.map((project) => project.id))} className={buttonClass({ variant: "outline", size: "sm", className: "gap-1.5" })}>
                    <Archive size={14} aria-hidden="true" />{locale === "ko" ? "선택 보관" : "Archive selected"}
                  </button>
                  <button type="button" onClick={() => trashProjects(selectedProjects.map((project) => project.id))} className={buttonClass({ variant: "quiet", size: "sm", className: "gap-1.5 text-danger" })}>
                    <Trash2 size={14} aria-hidden="true" />{locale === "ko" ? "선택 삭제" : "Trash selected"}
                  </button>
                </>
              ) : view === "archived" ? (
                <>
                  <button type="button" onClick={() => restoreProjects(selectedProjects.map((project) => project.id))} className={buttonClass({ variant: "outline", size: "sm", className: "gap-1.5" })}>
                    <RotateCcw size={14} aria-hidden="true" />{locale === "ko" ? "선택 복원" : "Restore selected"}
                  </button>
                  <button type="button" onClick={() => trashProjects(selectedProjects.map((project) => project.id))} className={buttonClass({ variant: "quiet", size: "sm", className: "gap-1.5 text-danger" })}>
                    <Trash2 size={14} aria-hidden="true" />{locale === "ko" ? "선택 삭제" : "Trash selected"}
                  </button>
                ...
                <>
                  <button type="button" onClick={() => restoreProjects(selectedProjects.map((project) => project.id))} className={buttonClass({ variant: "outline", size: "sm", className: "gap-1.5" })}>
                    <RotateCcw size={14} aria-hidden="true" />{locale === "ko" ? "선택 복구" : "Restore selected"}
                  </button>
                  <button type="button" onClick={() => setDeleteRequest({ ids: selectedProjects.map((project) => project.id), source: "selected" })} className={buttonClass({ variant: "quiet", size: "sm", className: "gap-1.5 text-danger" })}>
                    <Trash2 size={14} aria-hidden="true" />{locale === "ko" ? "선택 완전 삭제" : "Delete selected"}
                  </button>
                </>
              )}
      ...
      {view !== "active" && listedProjects.length > 0 ? (
        <section className="mt-3 flex flex-wrap justify-end gap-2" aria-label={locale === "ko" ? "전체 작업" : "All-project actions"}>
          <button
            type="button"
          ...
          >
            ...
            {view === "trash"
              ? locale === "ko" ? "전체 복구" : "Restore all"
              : locale === "ko" ? "전체 복원" : "Restore all"}
          </button>
          {view === "trash" ? (
            <button
              type="button"
