...
const bi = <TKo, TEn>(locale: string, ko: TKo, en: TEn): TKo =>
  translateBilingualValueForLocale(locale, "StudioQuickStart", ko, en);
...
export function StudioQuickStart({ locale }: { readonly locale: "ko" | "en" }) {
  ...
  function start(item: QuickStart) {
    ...
    try {
      ...
      const result = createStudioProjectWithInitialDocument(window.localStorage, {
        title: bi(locale, item.titleKo, item.titleEn),
        kind: item.kind,
      ...
      setBusyId(null);
      setError(bi(locale, "이 브라우저에서 새 작업을 저장하지 못했어요. 저장 공간과 브라우저 설정을 확인한 뒤 다시 시도해 주세요. 기존 작업은 변경하지 않았습니다.", "This browser could not save a new project. Check available storage and browser settings, then retry. Existing work was not changed."));
    }
  }
  ...
  return (
    <section
      ...
      aria-label={bi(locale, "빠른 시작", "Quick start")}
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <p className="flex items-center gap-2 text-xs font-black text-accent">
            ...
            {bi(locale, "설정 없이 시작", "Start without setup")}
          </p>
          <h2 className="mt-2 text-xl font-black text-fg">
            {bi(locale, "바로 시작하기", "Start right away")}
          </h2>
        ...
        <div className="flex flex-wrap gap-2">
          {QUICK_STARTS.map((item) => {
            ...
            return (
              ...
              >
                ...
                {busy ? (bi(locale, "여는 중…", "Opening…")) : bi(locale, item.labelKo, item.labelEn)}
          ...
          <Link href="/studio/new" className={buttonClass({ variant: "quiet", size: "lg", className: "min-h-12 gap-2" })}>
            ...
            {bi(locale, "다른 작업 만들기", "More project types")}
          </Link>
        </div>
      </div>
