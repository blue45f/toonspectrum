...
const bi = <T,>(ko: T, en: T): T =>
  translateBilingualValueForActiveLocale("StudioSaveFirstProjectLibraryPage", ko, en);;
...
function dateLabel(value: string | null, _locale): string {
  if (!value || !Number.isFinite(Date.parse(value))) return bi("아직 없음", "Not yet");
...
function distributionLabel(profile: StudioSaveProfile, _locale): string {
  const labels = {
  ...
  return bi((labels[profile.distributionState]).ko, (labels[profile.distributionState]).en);
}
...
function uploadFailureMessage(error: unknown, _locale): string {
  if (error instanceof Error && error.message.trim()) {
    return error.message.trim().slice(0, 500);
  }
  return bi("개인 저장소 업로드를 완료하지 못했습니다.", "The personal storage upload could not be completed.");
}
...
export function StudioSaveFirstProjectLibraryPage({
  ...
  useEffect(() => {
    ...
    if (result === "connected") {
      setMessage(formatI18nTemplate(String(bi("{value0} 개인 계정을 연결했습니다. 이제 프로젝트를 직접 저장할 수 있습니다.", "{value0} is connected. Projects can now be saved directly.")), { value0: label }));
      void reloadCloudConnections();
    } else {
      const reason = searchParams.get("cloudError");
      setMessage(formatI18nTemplate(String(bi("{value0} 연결을 완료하지 못했습니다.{value1}", "{value0} connection could not be completed.{value1}")), { value0: label, value1: reason ? ` (${reason})` : "" }));
    }
  ...
  const savePackage = async (project: StudioProjectLibraryEntry) => {
    ...
    try {
      ...
      if (!(cause instanceof DOMException && cause.name === "AbortError")) {
        setMessage(bi("프로젝트 파일을 저장하지 못했습니다.", "The project file could not be saved."));
      }
    } finally {
  ...
  const connectOrUpload = async (
    ...
    if (!connection?.configured) {
      setMessage(formatI18nTemplate(String(bi("{value0} OAuth 서버 설정이 필요합니다.", "{value0} OAuth server configuration is required.")), { value0: personalCloudProviderLabel(provider) }));
      return;
    }
    ...
    try {
      ...
      setMessage(formatI18nTemplate(String(bi("“{value0}”을 {value1}에 저장했습니다.", "Saved “{value0}” to {value1}.")), { value0: project.title, value1: personalCloudProviderLabel(provider) }));
    } catch (cause) {
  ...
  const duplicateProject = (project: StudioProjectLibraryEntry) => {
    ...
    setMessage(formatI18nTemplate(String(bi("“{value0}”의 새 비공개 복사본을 만들었습니다.", "Created a new private copy of “{value0}”.")), { value0: project.title }));
  };
      ...
      <Container size="wide" className="py-7 sm:py-11">
        <header className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black tracking-tight text-fg sm:text-4xl">{bi((VIEW_LABELS[view]).ko, (VIEW_LABELS[view]).en)}</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-fg-2 sm:text-base">
              {bi((VIEW_DESCRIPTIONS[view]).ko, (VIEW_DESCRIPTIONS[view]).en)}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link href="/studio/import" className={buttonClass({ variant: "outline" })}>{bi("파일 가져오기", "Import files")}</Link>
            <Link href="/studio/new" className={buttonClass({ className: "gap-2" })}><Plus size={16} aria-hidden="true" />{bi("새 작품 만들기", "Create new work")}</Link>
          </div>
        </header>
          ...
          <nav className="overflow-x-auto" aria-label={bi("내 작업 보기", "My work views")}>
            <div className="flex min-w-max gap-1 rounded-2xl border border-line bg-card p-1">
              {primaryNavigation.map((candidate) => (
                ...
                >
                  {bi((VIEW_LABELS[candidate]).ko, (VIEW_LABELS[candidate]).en)}
                </Link>
              ))}
            </div>
          </nav>
        ...
        {message ? (
          <div role="status" className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-accent/30 bg-accent-soft/20 px-3 py-2 text-sm font-semibold text-fg">
            <span>{message}</span><button type="button" onClick={() => setMessage(null)} className="text-xs underline">{bi("닫기", "Dismiss")}</button>
          </div>
        ) : null}
        ...
        {(view === "active" || view === "archived" || view === "trash") ? (
          <section className="mt-7">
            <label className="relative block max-w-md">
              ...
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={bi("내 작업 검색", "Search my work")} className="min-h-11 w-full rounded-xl border border-line bg-card pl-10 pr-3 text-sm text-fg outline-none focus:border-accent" />
            </label>
            {filteredProjects.length === 0 ? (
              <div className="mt-5 rounded-3xl border border-dashed border-line bg-card/60 px-5 py-14 text-center">
                ...
                <h2 className="mt-3 text-xl font-black text-fg">{bi("표시할 작업이 없습니다", "No work to show")}</h2>
                {view === "active" ? (
                  <Link href="/studio/new" className={buttonClass({ className: "mt-5 gap-2" })}>
                    <Plus size={16} aria-hidden="true" />
                    {bi("새 작품 만들기", "Create new work")}
                  </Link>
                ) : null}
              </div>
            ) : view === "active" ? (
                  ...
                  return (
                    <article key={project.id} className="overflow-hidden rounded-2xl border border-line bg-card p-4 shadow-sm">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="inline-flex min-h-7 items-center gap-1 rounded-full bg-accent-soft px-2.5 text-[0.68rem] font-black text-accent"><ShieldCheck size={13} aria-hidden="true" />{bi("나만 보기", "Owner only")}</span>
                        <SaveBadge profile={profile} locale={locale} />
                      </div>
                      <h2 className="mt-3 truncate text-lg font-black text-fg">{project.title}</h2>
                      <p className="mt-1 text-xs text-fg-3">{bi("마지막 작업", "Last opened")} {dateLabel(project.lastOpenedAt, locale)}</p>
                      <div className="mt-4 grid grid-cols-2 gap-2 rounded-xl bg-panel/60 p-3 text-[0.68rem]">
                        <div><p className="font-semibold text-fg-3">{bi("마지막 저장", "Last save")}</p><p className="mt-1 font-black text-fg">{dateLabel(profile.lastManualSaveAt, locale)}</p></div>
                        <div><p className="font-semibold text-fg-3">{bi("배포", "Distribution")}</p><p className="mt-1 font-black text-fg">{distributionLabel(profile, locale)}</p></div>
                      </div>
                      <details className="mt-3 rounded-xl border border-line bg-panel/55">
                        ...
                        <div className="border-t border-line p-3">
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    <p className="mt-1 text-[0.64rem] text-fg-3">
                                      {binding.lastSyncedAt
                                        ? `${bi("마지막 저장", "Last saved")} ${dateLabel(binding.lastSyncedAt, locale)}`
                                        : binding.connectionRequired
                                          ? bi("계정 연결 필요", "Account connection required")
                                          : bi("작업 사본", "Working copy")}
                                    </p>
                                  </div>
                                  <span className={cn(
                          ...
                          <p className="mt-3 text-[0.64rem] leading-4 text-fg-3">
                            {bi("WebDAV·S3 직접 연결은 자격 증명 보관 방식을 선택한 뒤 별도 설정에서 제공합니다. 연결되지 않은 저장소를 백업 완료로 표시하지 않습니다.", "WebDAV and S3 direct connections remain in advanced settings. Unconnected storage is never shown as a completed backup.")}
                          </p>
                        </div>
                      </details>
                      <div className="mt-4 flex gap-2 border-t border-line pt-3">
                        <Link href={`/studio/p/${encodeURIComponent(project.id)}/overview`} onClick={() => { library.touch(project.id, project.lastOpenedDocumentId); }} className={buttonClass({ size: "sm", className: "flex-1 gap-1.5" })}><FolderOpen size={15} aria-hidden="true" />{bi("이어서 작업", "Continue")}</Link>
                        <button type="button" onClick={() => { void savePackage(project); }} disabled={busyProjectId === project.id} className={buttonClass({ variant: "outline", size: "sm", className: "gap-1.5" })}><Download size={15} aria-hidden="true" />{bi("저장", "Save")}</button>
                        <button type="button" onClick={() => duplicateProject(project)} aria-label={bi("프로젝트 복제", "Duplicate project")} className={buttonClass({ variant: "quiet", size: "icon" })}><Copy size={15} aria-hidden="true" /></button>
                      </div>
                      <div className="mt-2 flex flex-wrap justify-end gap-2">
                        <button type="button" onClick={() => { library.archive(project.id); }} className="text-[0.68rem] font-semibold text-fg-3 hover:text-fg">{bi("보관", "Archive")}</button>
                        <button type="button" onClick={() => { library.trash(project.id); }} className="text-[0.68rem] font-semibold text-danger">{bi("휴지통", "Trash")}</button>
                      </div>
                    </article>
                  );
                })}
                ...
                {filteredProjects.map((project) => (
                  <article key={project.id} className="flex flex-col gap-3 rounded-2xl border border-line bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex gap-2">
                      <button type="button" onClick={() => { if (view === "archived") library.activate(project.id); else library.restore(project.id); }} className={buttonClass({ variant: "outline", size: "sm", className: "gap-1.5" })}><RotateCcw size={14} aria-hidden="true" />{bi("복원", "Restore")}</button>
                      {view === "trash" ? <button type="button" onClick={() => { if (library.removePermanently(project.id)) profiles.remove(project.id); }} className={buttonClass({ variant: "quiet", size: "sm", className: "text-danger" })}><Trash2 size={14} aria-hidden="true" />{bi("완전 삭제", "Delete")}</button> : null}
                    </div>
                  </article>
                ))}
              </div>
            ...
            <section className="rounded-2xl border border-line bg-card p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="text-lg font-black text-fg">
                    {bi("프로젝트 저장 위치", "Project storage locations")}
                  </h2>
                  <p className="mt-1 text-xs leading-5 text-fg-3">
                    {bi("로컬 파일 백업과 연결한 개인 드라이브 업로드를 프로젝트별로 관리합니다.", "Manage local file backups and connected personal-drive uploads per project.")}
                  </p>
                </div>
              </div>
              ...
              {activeProjects.length === 0 ? (
                <p className="mt-5 rounded-xl bg-panel/60 px-4 py-6 text-center text-sm text-fg-3">
                  {bi("저장할 비공개 작업이 없습니다.", "There are no private projects to save.")}
                </p>
              ) : (
                <div className="mt-5 space-y-3">
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            ...
                            <p className="mt-1 text-[0.66rem] text-fg-3">
                              {bi("마지막 수동 저장", "Last manual save")} {dateLabel(profile.lastManualSaveAt, locale)}
                            </p>
                          </div>
                          <button
                            type="button"
                          ...
                          >
                            ...
                            {bi(".toonstudio 파일 저장", "Save .toonstudio file")}
                          </button>
                        </div>
