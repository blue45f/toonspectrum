...
function projectTitle(option: StudioProjectCreateKindOption, locale: Locale): string {
  return translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", option.titleKo, option.titleEn);
}

function defaultTitle(option: StudioProjectCreateKindOption, locale: Locale): string {
  return translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", option.defaultTitleKo, option.defaultTitleEn);
}
      ...
      >
        {options.map((option) => (
          <option key={option.id} value={option.id}>
            {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", option.labelKo, option.labelEn)}
          </option>
        ))}
      </select>
    </label>
...
export function StudioDeferredSaveProjectCreatePage() {
  ...
  const selectedTemplateLabel = selectedTemplate
    ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", selectedTemplate.labelKo, selectedTemplate.labelEn))
    : templateId;
  ...
  const steps: readonly StudioTaskFlowStep[] = [
    {
      id: "kind",
      label: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "만들 작업 선택", "Choose work type"),
      description: projectTitle(selected, locale),
      state: "complete",
    },
    {
      id: "details",
      label: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "이름과 시작 형식", "Name and format"),
      description: selectedTemplateLabel,
      state: titleReady ? "complete" : "current",
    },
    ...(structuredWebtoonFlow ? [{
      id: "production-track",
      label: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "제작 트랙 확인", "Confirm production track"),
      description: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", onboardingPlan.titleKo, onboardingPlan.titleEn),
      state: "complete" as const,
    }] : []),
    {
      id: "start",
      label: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "자동 저장하며 시작", "Start with autosave"),
      description: structuredWebtoonFlow
        ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "프로젝트와 첫 제작 계획 생성", "Create the project and first production plan"))
        : (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "기기 복구 저장 후 클라우드 연결", "Device recovery first, cloud next")),
  ...
  const create = () => {
    ...
    try {
      ...
      setError(cause instanceof Error
        ? cause.message
        : translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "임시 작업을 시작하지 못했습니다.", "The temporary work could not be started."));
      setCreating(false);
    }
  };

  const startLabel = structuredWebtoonFlow
    ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "프로젝트와 제작 계획 만들기", "Create project and production plan"))
    : translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", `${selected.titleKo} 시작`, `Start ${selected.titleEn}`);
        ...
        <div className="mx-auto min-w-0 max-w-6xl">
          <header className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="min-w-0">
              ...
              <h1 className="mt-2 break-words text-3xl font-black tracking-tight text-fg sm:text-4xl">
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "바로 만들기 시작하세요", "Start creating right away")}
              </h1>
              <p className="mt-2 max-w-3xl break-words text-sm leading-6 text-fg-2 sm:text-base">
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "작업 종류와 시작 형식만 고르면 됩니다. 시작하는 즉시 이 기기에 복구 저장되며, 프로젝트 저장 후에는 ToonStudio 클라우드에서 다른 기기와 팀 작업으로 이어갈 수 있습니다. 기존 파일과 외부 드라이브는 가져오기·백업 옵션입니다.", "Choose only the work type and starting format. Recovery storage begins on this device immediately, and a saved project can continue through ToonStudio Cloud across devices and teams. Existing files and external drives remain optional import and backup paths.")}
              </p>
            </div>
            <Link href="/studio/import" className={buttonClass({ variant: "outline", className: "w-full min-w-0 sm:w-auto" })}>
              <span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "기존 파일 가져오기", "Import existing files")}</span>
            </Link>
          </header>

          <StudioTaskFlow
            steps={steps}
            ariaLabel={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "새 프로젝트 시작 단계", "New project start steps")}
            className="mt-5"
          />
          ...
          <section className="mt-7 min-w-0" aria-labelledby="project-kind-title">
            <h2 id="project-kind-title" className="break-words text-base font-black text-fg">
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "1. 만들 작업", "1. Project type")}
            </h2>
                ...
                return (
                  ...
                  >
                    ...
                    <span id={descriptionId} className="mt-1 block break-words text-xs leading-5 text-fg-3">
                      {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", option.descriptionKo, option.descriptionEn)}
                    </span>
                  </button>
                );
              })}
            ...
            {!showMoreKinds ? (
              ...
              >
                <span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "다른 작업 종류 보기", "Show more project types")}</span>
              </button>
            ) : null}
          </section>
            ...
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <h2 id="project-settings-title" className="break-words text-xl font-black text-fg">
                  {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "2. 이름과 시작 형식", "2. Name and starting format")}
                </h2>
                <p className="mt-1 break-words text-xs leading-5 text-fg-3">
                  {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "공개 제목·소개·장르·연재 위치는 지금 정하지 않아도 됩니다.", "A public title, synopsis, genre and publishing destination are not required now.")}
                </p>
              </div>
            </div>
            <div className="mt-5 grid min-w-0 gap-4 md:grid-cols-2">
              <label className="min-w-0 text-xs font-bold text-fg-2" htmlFor="studio-project-title">
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "프로젝트 이름", "Project name")}
                <input
                ...
                <span id="studio-project-title-help" className={cn("mt-2 block break-words text-[0.6875rem] leading-5", titleReady ? "text-fg-3" : "text-danger")}>
                  {titleReady
                    ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "작업용 이름이며 공개 전 언제든 바꿀 수 있습니다.", "This is a working name and can change before publishing."))
                    : (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "프로젝트 이름을 입력하면 시작할 수 있습니다.", "Enter a project name to continue."))}
                </span>
              </label>
              <label className="min-w-0 text-xs font-bold text-fg-2" htmlFor="studio-project-template">
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "시작 템플릿", "Starting template")}
                <select
              ...
              <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <h2 id="webtoon-production-onboarding-title" className="mt-1 break-words text-xl font-black text-fg">
                      {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "실제 제작 단계에 맞춰 시작", "Start from your real production stage")}
                    </h2>
                    <p className="mt-1 max-w-3xl break-words text-xs leading-5 text-fg-3">
                      {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "현재 가진 자료와 목표를 기준으로 첫 승인 마일스톤과 작업 체크리스트를 만듭니다. 기능 설명만 보고 끝나는 온보딩이 아닙니다.", "Create the first approval milestone and task checklist from the material and goal you already have.")}
                    </p>
                  </div>
                </div>
                <button
                ...
                >
                  {webtoonOnboardingEnabled
                    ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "제작 트랙 사용 중", "Production track enabled"))
                    : (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "제작 트랙 설정", "Set production track"))}
                </button>
              </div>
                ...
                <div className="mt-5">
                  <div className="grid min-w-0 gap-4 md:grid-cols-2">
                    <WebtoonOnboardingSelect<WebtoonStartingPointId>
                      ...
                      label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "현재 가지고 있는 자료", "What you already have")}
                    ...
                    <WebtoonOnboardingSelect<WebtoonOnboardingGoalId>
                      ...
                      label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "프로젝트 목표", "Project goal")}
                    ...
                    <WebtoonOnboardingSelect<WebtoonTeamModelId>
                      ...
                      label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "제작 인원", "Team model")}
                    ...
                    <WebtoonOnboardingSelect<WebtoonCadenceId>
                      ...
                      label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "예상 연재 주기", "Publishing cadence")}
                  ...
                  <div className="mt-5 grid gap-4 rounded-2xl border border-accent/25 bg-accent-soft/20 p-4 lg:grid-cols-[minmax(0,1fr)_minmax(16rem,.8fr)]">
                    <div>
                      <p className="text-xs font-black text-accent">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "추천 제작 트랙", "Recommended production track")}</p>
                      <h3 className="mt-1 text-lg font-black text-fg">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", onboardingPlan.titleKo, onboardingPlan.titleEn)}</h3>
                      <p className="mt-2 text-xs leading-5 text-fg-2">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", onboardingPlan.summaryKo, onboardingPlan.summaryEn)}</p>
                    </div>
                    <div className="rounded-xl bg-panel p-3">
                      <p className="text-[0.65rem] font-bold text-fg-3">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "첫 승인 마일스톤", "First approval milestone")}</p>
                      <p className="mt-1 text-sm font-bold text-fg">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", onboardingPlan.milestoneKo, onboardingPlan.milestoneEn)}</p>
                    </div>
                  </div>
                  <Link href="/learn/process#production-onboarding" className={buttonClass({ variant: "quiet", size: "sm", className: "mt-3 w-full min-w-0 sm:w-auto" })}>
                    <span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "업계 제작 과정과 트랙 설명 보기", "Review the industry workflow and tracks")}</span>
                  </Link>
                </div>
              ) : (
                <div className="mt-5 rounded-2xl bg-panel p-4 text-xs leading-5 text-fg-2">
                  {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "바로 드로잉으로 시작하려면 지금 상태를 유지하세요. 기획·대본·콘티·완성 원고·연재 중 상태에서 이어가려면 제작 트랙을 설정하세요.", "Keep this off to open the drawing workspace immediately, or enable it to continue from planning, script, storyboard, finished art or a live series.")}
                </div>
              )}
            </section>
          ) : null}

          <section className="mt-5 flex min-w-0 items-start gap-3 rounded-2xl border border-success/30 bg-success-soft/15 p-4" aria-label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "자동 저장 안내", "Autosave notice")}>
          ...
          {error ? (
            <RecoverableActionNotice
              tone="danger"
              title={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "프로젝트를 시작하지 못했습니다", "The project could not be started")}
              description={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", `${error} 입력한 이름과 시작 형식은 그대로 유지되어 있습니다.`, `${error} Your project name and starting format are still preserved.`)}
              action={
                <button
                  type="button"
                  onClick={create}
                ...
                >
                  ...
                  <span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "다시 시도", "Try again")}</span>
                </button>
              }
              className="mt-5"
            />
          ...
          <div className="mt-7 grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <StudioTaskSummary
              eyebrow={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "시작할 작업", "Ready to start")}
              title={`${title.trim() || (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "이름 없는 프로젝트", "Untitled project"))} · ${projectTitle(selected, locale)}`}
              description={structuredWebtoonFlow
                ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", `${onboardingPlan.titleKo} · ${onboardingPlan.milestoneKo}`, `${onboardingPlan.titleEn} · ${onboardingPlan.milestoneEn}`))
                : translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", `${selectedTemplateLabel} 템플릿을 적용하고 드로잉 도구로 시작합니다.`, `Apply the ${selectedTemplateLabel} template and open the drawing tool.`)}
            ...
            <div className="flex min-w-0 flex-col gap-3 lg:min-w-72">
              <DisabledReason id="studio-create-disabled-reason" visible={!titleReady}>
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "프로젝트 이름을 입력하면 자동 저장되는 작업공간을 시작할 수 있습니다.", "Enter a project name to start an autosaved workspace.")}
              </DisabledReason>
              <div className="flex min-w-0 flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                <Link href="/studio" className={buttonClass({ variant: "quiet", className: "w-full min-w-0 sm:w-auto" })}>
                  <span className="break-words">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "내 작업으로", "Back to My work")}</span>
                </Link>
                <button
                  type="button"
                  disabled={creating || !titleReady}
                ...
                >
                  <span className="break-words">{creating ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioDeferredSaveProjectCreatePage", "준비 중…", "Preparing…")) : startLabel}</span>
                </button>
              </div>
            </div>
          </div>
