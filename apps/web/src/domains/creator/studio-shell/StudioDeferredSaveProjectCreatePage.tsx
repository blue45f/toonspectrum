...
function projectTitle(option: StudioProjectCreateKindOption, localize: Localizer): string {
  return localize(option.titleKo, option.titleEn);
}

function defaultTitle(option: StudioProjectCreateKindOption, localize: Localizer): string {
  return localize(option.defaultTitleKo, option.defaultTitleEn);
}
...
function WebtoonOnboardingSelect<T extends string>({
  ...
  const l = useBilingualLocalizer("studioDeferredCreate.onboardingSelect");
  return (
      ...
      >
        {options.map((option) => (
          <option key={option.id} value={option.id}>
            {l(option.labelKo, option.labelEn)}
          </option>
        ))}
      </select>
    </label>
...
export function StudioDeferredSaveProjectCreatePage() {
  ...
  const [title, setTitle] = useState(() => defaultTitle(initialKind, l));
  ...
  const selectedTemplateLabel = selectedTemplate
    ? (l(selectedTemplate.labelKo, selectedTemplate.labelEn))
    : templateId;
  ...
  const steps: readonly StudioTaskFlowStep[] = [
    {
      id: "kind",
      label: l("만들 작업 선택", "Choose work type"),
      description: projectTitle(selected, l),
      state: "complete",
    },
    {
      id: "details",
      label: l("이름과 시작 형식", "Name and format"),
      description: selectedTemplateLabel,
      state: titleReady ? "complete" : "current",
    },
    ...(structuredWebtoonFlow ? [{
      id: "production-track",
      label: l("제작 트랙 확인", "Confirm production track"),
      description: l(onboardingPlan.titleKo, onboardingPlan.titleEn),
      state: "complete" as const,
    }] : []),
    {
      id: "start",
      label: l("자동 저장하며 시작", "Start with autosave"),
      description: structuredWebtoonFlow
        ? (l("프로젝트와 첫 제작 계획 생성", "Create the project and first production plan"))
        : (l("기기 복구 저장 후 클라우드 연결", "Device recovery first, cloud next")),
  ...
  const selectKind = (option: StudioProjectCreateKindOption) => {
    ...
    if (!titleEdited) setTitle(defaultTitle(option, l));
    setError(null);
  };
  ...
  const create = () => {
    ...
    try {
      ...
      setError(cause instanceof Error
        ? cause.message
        : l("임시 작업을 시작하지 못했습니다.", "The temporary work could not be started."));
      setCreating(false);
    }
  };

  const startLabel = structuredWebtoonFlow
    ? (l("프로젝트와 제작 계획 만들기", "Create project and production plan"))
    : l(`${selected.titleKo} 시작`, `Start ${selected.titleEn}`);
        ...
        <div className="mx-auto min-w-0 max-w-6xl">
          ...
          <section className="mt-7 min-w-0" aria-labelledby="project-kind-title">
            ...
            {!showMoreKinds ? (
              ...
              >
                <span className="break-words">{l("다른 작업 종류 보기", "Show more project types")}</span>
              </button>
            ) : null}
          </section>
            ...
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <h2 id="project-settings-title" className="break-words text-xl font-black text-fg">
                  {l("2. 이름과 시작 형식", "2. Name and starting format")}
                </h2>
                <p className="mt-1 break-words text-xs leading-5 text-fg-3">
                  {l("공개 제목·소개·장르·연재 위치는 지금 정하지 않아도 됩니다.", "A public title, synopsis, genre and publishing destination are not required now.")}
                </p>
              </div>
            </div>
            <div className="mt-5 grid min-w-0 gap-4 md:grid-cols-2">
              <label className="min-w-0 text-xs font-bold text-fg-2" htmlFor="studio-project-title">
                {l("프로젝트 이름", "Project name")}
                <input
                ...
                <span id="studio-project-title-help" className={cn("mt-2 block break-words text-[0.6875rem] leading-5", titleReady ? "text-fg-3" : "text-danger")}>
                  {titleReady
                    ? (l("작업용 이름이며 공개 전 언제든 바꿀 수 있습니다.", "This is a working name and can change before publishing."))
                    : (l("프로젝트 이름을 입력하면 시작할 수 있습니다.", "Enter a project name to continue."))}
                </span>
              </label>
              <label className="min-w-0 text-xs font-bold text-fg-2" htmlFor="studio-project-template">
                {l("시작 템플릿", "Starting template")}
                <select
              ...
              <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <h2 id="webtoon-production-onboarding-title" className="mt-1 break-words text-xl font-black text-fg">
                      {l("실제 제작 단계에 맞춰 시작", "Start from your real production stage")}
                    </h2>
                    <p className="mt-1 max-w-3xl break-words text-xs leading-5 text-fg-3">
                      {l("현재 가진 자료와 목표를 기준으로 첫 승인 마일스톤과 작업 체크리스트를 만듭니다. 기능 설명만 보고 끝나는 온보딩이 아닙니다.", "Create the first approval milestone and task checklist from the material and goal you already have.")}
                    </p>
                  </div>
                </div>
                <button
                ...
                >
                  {webtoonOnboardingEnabled
                    ? (l("제작 트랙 사용 중", "Production track enabled"))
                    : (l("제작 트랙 설정", "Set production track"))}
                </button>
              </div>
                ...
                <div className="mt-5">
                  <div className="grid min-w-0 gap-4 md:grid-cols-2">
                    <WebtoonOnboardingSelect<WebtoonStartingPointId>
                      ...
                      label={l("현재 가지고 있는 자료", "What you already have")}
                    ...
                    <WebtoonOnboardingSelect<WebtoonCadenceId>
                      ...
                      label={l("예상 연재 주기", "Publishing cadence")}
                  ...
                  <div className="mt-5 grid gap-4 rounded-2xl border border-accent/25 bg-accent-soft/20 p-4 lg:grid-cols-[minmax(0,1fr)_minmax(16rem,.8fr)]">
                    <div>
                      <p className="text-xs font-black text-accent">{l("추천 제작 트랙", "Recommended production track")}</p>
                      <h3 className="mt-1 text-lg font-black text-fg">{l(onboardingPlan.titleKo, onboardingPlan.titleEn)}</h3>
                      <p className="mt-2 text-xs leading-5 text-fg-2">{l(onboardingPlan.summaryKo, onboardingPlan.summaryEn)}</p>
                    </div>
                    <div className="rounded-xl bg-panel p-3">
                      <p className="text-[0.65rem] font-bold text-fg-3">{l("첫 승인 마일스톤", "First approval milestone")}</p>
                      <p className="mt-1 text-sm font-bold text-fg">{l(onboardingPlan.milestoneKo, onboardingPlan.milestoneEn)}</p>
                    </div>
                  </div>
                  <Link href="/learn/process#production-onboarding" className={buttonClass({ variant: "quiet", size: "sm", className: "mt-3 w-full min-w-0 sm:w-auto" })}>
                    <span className="break-words">{l("업계 제작 과정과 트랙 설명 보기", "Review the industry workflow and tracks")}</span>
                  </Link>
                </div>
              ) : (
                <div className="mt-5 rounded-2xl bg-panel p-4 text-xs leading-5 text-fg-2">
                  {l("바로 드로잉으로 시작하려면 지금 상태를 유지하세요. 기획·대본·콘티·완성 원고·연재 중 상태에서 이어가려면 제작 트랙을 설정하세요.", "Keep this off to open the drawing workspace immediately, or enable it to continue from planning, script, storyboard, finished art or a live series.")}
                </div>
              )}
            </section>
          ) : null}

          <section className="mt-5 flex min-w-0 items-start gap-3 rounded-2xl border border-success/30 bg-success-soft/15 p-4" aria-label={l("자동 저장 안내", "Autosave notice")}>
          ...
          {error ? (
            <RecoverableActionNotice
              tone="danger"
              title={l("프로젝트를 시작하지 못했습니다", "The project could not be started")}
              description={l(`${error} 입력한 이름과 시작 형식은 그대로 유지되어 있습니다.`, `${error} Your project name and starting format are still preserved.`)}
              action={
                <button
                  type="button"
                  onClick={create}
                ...
                >
                  ...
                  <span className="break-words">{l("다시 시도", "Try again")}</span>
                </button>
              }
              className="mt-5"
            />
          ...
          <div className="mt-7 grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <StudioTaskSummary
              eyebrow={l("시작할 작업", "Ready to start")}
              title={`${title.trim() || (l("이름 없는 프로젝트", "Untitled project"))} · ${projectTitle(selected, l)}`}
              description={structuredWebtoonFlow
                ? (l(`${onboardingPlan.titleKo} · ${onboardingPlan.milestoneKo}`, `${onboardingPlan.titleEn} · ${onboardingPlan.milestoneEn}`))
                : l(`${selectedTemplateLabel} 템플릿을 적용하고 드로잉 도구로 시작합니다.`, `Apply the ${selectedTemplateLabel} template and open the drawing tool.`)}
              meta={<WorkflowTrustBadge state="device-saved" locale={legacyLocale} compact={false} />}
            />
            <div className="flex min-w-0 flex-col gap-3 lg:min-w-72">
              <DisabledReason id="studio-create-disabled-reason" visible={!titleReady}>
                {l("프로젝트 이름을 입력하면 자동 저장되는 작업공간을 시작할 수 있습니다.", "Enter a project name to start an autosaved workspace.")}
              </DisabledReason>
              <div className="flex min-w-0 flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-end">
                <Link href="/studio" className={buttonClass({ variant: "quiet", className: "w-full min-w-0 sm:w-auto" })}>
                  <span className="break-words">{l("내 작업으로", "Back to My work")}</span>
                </Link>
                <button
                  type="button"
                  disabled={creating || !titleReady}
                ...
                >
                  <span className="break-words">{creating ? (l("준비 중…", "Preparing…")) : startLabel}</span>
                </button>
              </div>
            </div>
          </div>
