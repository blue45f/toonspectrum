...
function localized(option: StudioProjectCreateKindOption, locale: Locale): string {
  return translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", option.titleKo, option.titleEn);
}

function defaultTitle(option: StudioProjectCreateKindOption, locale: Locale): string {
  return translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", option.defaultTitleKo, option.defaultTitleEn);
}
      ...
      >
        {choices.map((choice) => (
          <option key={choice.id} value={choice.id}>
            {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", choice.labelKo, choice.labelEn)}
          </option>
        ))}
      </select>
    </label>
...
export function StudioModeProjectCreatePage() {
  ...
  const selectedTemplateLabel = translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", selectedTemplate.labelKo, selectedTemplate.labelEn);
  ...
  const steps: readonly StudioTaskFlowStep[] = [
    {
      id: "kind",
      label: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "만들 작업 선택", "Choose work type"),
      description: modeName,
      state: "complete",
    },
    {
      id: "details",
      label: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "이름과 시작 형식", "Name and format"),
      description: selectedTemplateLabel,
      state: titleReady ? "complete" : "current",
    },
    ...(structuredWebtoon ? [{
      id: "production-track",
      label: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "제작 트랙 확인", "Confirm production track"),
      description: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", onboardingPlan.titleKo, onboardingPlan.titleEn),
      state: "complete" as const,
    }] : []),
    {
      id: "start",
      label: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "자동 저장하며 시작", "Start with autosave"),
      description: translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "이 기기에 복구 저장 후 작업 시작", "Start after creating device recovery storage"),
  ...
  function create() {
    ...
    try {
      ...
      setError(cause instanceof Error
        ? cause.message
        : translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "새 작업을 만들지 못했습니다.", "The new work could not be created."));
    }
  }

  const startLabel = structuredWebtoon
    ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "프로젝트와 제작 계획 만들기", "Create project and production plan"))
    : translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", `${modeName} 시작`, `Start ${modeName}`);
        ...
        <div className="mx-auto max-w-6xl">
          <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black tracking-tight text-fg sm:text-4xl">
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "무엇을 만들지 고르면 작업공간도 바뀝니다", "Choose what to make — the workspace changes with it")}
              </h1>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-fg-2 sm:text-base">
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "작업 종류와 시작 형식만 고르면 됩니다. 시작하는 즉시 이 기기에 복구 저장되며, 종류마다 패널·도구·AI 추천·제작 흐름과 내보내기 목표가 달라집니다.", "Choose the work type and starting format. Recovery storage begins on this device immediately, and each mode adapts panels, tools, AI, workflow and delivery.")}
              </p>
            </div>
            <Link href="/studio/import" className={buttonClass({ variant: "outline" })}>
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "기존 파일 가져오기", "Import existing files")}
            </Link>
          </header>

          <StudioTaskFlow
            steps={steps}
            ariaLabel={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "새 프로젝트 시작 단계", "New project start steps")}
            className="mt-5"
          />
          ...
          <section className="mt-7" aria-labelledby="studio-mode-kind-title">
            ...
            <h2 id="studio-mode-kind-title" className="mt-1 text-lg font-black text-fg">
              {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "만들 작업", "What are you making?")}
            </h2>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="mt-1 block text-xs leading-5 text-fg-3">
                      {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", option.descriptionKo, option.descriptionEn)}
                    </span>
                  </button>
                );
              })}
            ...
            {!showMoreKinds ? (
              <button type="button" onClick={() => setShowMoreKinds(true)} className={buttonClass({ variant: "quiet", size: "sm", className: "mt-2" })}>
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "다른 작업 종류 보기", "Show more project types")}
              </button>
            ) : null}
          </section>
          ...
          <section className="mt-7 grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(20rem,.65fr)]">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="text-xs font-black text-accent">
                        {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "실제 제작 단계에 맞춰 시작", "Start from your real production stage")}
                      </p>
                      <p className="mt-1 text-xs leading-5 text-fg-3">
                        {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "대본·콘티·팀 구성과 연재 주기를 알려주면 첫 제작 계획까지 준비합니다.", "Use your current material, goal, team and cadence to prepare the first production plan.")}
                      </p>
                    </div>
                    <button
                      type="button"
                    ...
                    >
                      {onboardingEnabled
                        ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "제작 계획 사용 중", "Production plan on"))
                        : (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "제작 단계 설정하기", "Configure production"))}
                    </button>
                  </div>
                  ...
                  {onboardingEnabled ? (
                    <div className="mt-4">
                      <h3 className="text-base font-black text-fg">
                        {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", onboardingPlan.titleKo, onboardingPlan.titleEn)}
                      </h3>
                      <p className="mt-1 text-xs leading-5 text-fg-3">
                        {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", onboardingPlan.summaryKo, onboardingPlan.summaryEn)}
                      </p>
                      <div className="mt-4 grid gap-3 sm:grid-cols-2">
                        <OnboardingSelect<WebtoonStartingPointId>
                          id="studio-webtoon-starting-point"
                          label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "현재 가지고 있는 자료", "What you already have")}
                        ...
                        <OnboardingSelect<WebtoonOnboardingGoalId>
                          ...
                          label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "이번 프로젝트 목표", "Project goal")}
                        ...
                        <OnboardingSelect<WebtoonTeamModelId>
                          ...
                          label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "제작 인원", "Team model")}
                        ...
                        <OnboardingSelect<WebtoonCadenceId>
                          ...
                          label={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "제작 주기", "Cadence")}
            ...
            <div className="rounded-2xl border border-line bg-card p-5 shadow-sm">
              ...
              <h2 className="mt-1 text-lg font-black text-fg">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "시작 형식", "Starting format")}</h2>
              <label className="mt-4 block text-xs font-bold text-fg-2" htmlFor="studio-mode-project-title">
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "프로젝트 이름", "Project name")}
                <input
              ...
              <label className="mt-4 block text-xs font-bold text-fg-2" htmlFor="studio-mode-template">
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "시작 템플릿", "Starting template")}
                <select
                ...
                >
                  {templates.map((template) => (
                    <option key={template.id} value={template.id}>
                      {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", template.labelKo, template.labelEn)}
                    </option>
                  ))}
                </select>
              </label>
              ...
              <StudioTaskSummary
                eyebrow={translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "선택한 시작 설정", "Selected setup")}
                title={title.trim() || (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "프로젝트 이름 없음", "Untitled project"))}
                ...
                meta={(
                  <>
                    <span className="rounded-full border border-good/30 bg-good/10 px-2 py-1 text-[0.65rem] font-bold text-fg-2">
                      {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "이 기기에 저장됨", "Saved on this device")}
                    </span>
              ...
              <DisabledReason id={CREATE_DISABLED_REASON_ID} visible={!titleReady} className="mt-3">
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "프로젝트 이름을 입력하면 자동 저장되는 작업공간을 시작할 수 있습니다.", "Enter a project name to start an autosaved workspace.")}
              </DisabledReason>
              ...
              >
                {creating ? (translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "작업공간 만드는 중…", "Creating workspace…")) : startLabel}
                {!creating ? <ArrowRight size={16} aria-hidden="true" /> : null}
              </button>
              <p className="mt-2 text-center text-[0.68rem] leading-5 text-fg-3">
                {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioModeProjectCreatePage", "자동 복구 저장과 버전 기록이 즉시 시작됩니다. 저장 위치는 작업 중 언제든 연결할 수 있습니다.", "Recovery autosave and version history start immediately. You can connect another save destination later.")}
              </p>
