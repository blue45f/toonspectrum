...
function statusLabel(status: StudioImportPlanItem["status"], locale: StudioImportIntakeLocale): string {
  if (status === "accepted") return translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "바로 가져올 수 있음", "Ready to import");
  if (status === "review") return translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "변환 내용 확인 필요", "Review conversion");
  return translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "가져오기 차단", "Import blocked");
}
    ...
    if (selection.item.format === "toonstudio") {
      ...
      try {
        ...
        setMessage(error instanceof Error
          ? error.message
          : translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "ToonStudio 프로젝트 파일을 복원하지 못했습니다.", "The ToonStudio project file could not be restored."));
        setImporting(false);
      }
      return;
    }
    if (!studioImportHandoffTargetForFormat(selection.item.format)) {
      setMessage(translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "이 형식은 아래 전용 작업공간에서 가져와야 합니다. 파일은 전송하거나 저장하지 않았습니다.", "This format must be imported in its dedicated workspace. The file was not uploaded or stored."));
      return;
    }
    try {
      ...
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="max-w-3xl">
          ...
          <h2 id="studio-import-intake-title" className="mt-1 text-xl font-black text-fg">
            {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "먼저 분석하고, 기존 편집기로 안전하게 전달", "Analyze first, then hand off safely to the editor")}
          </h2>
          <p className="mt-2 text-sm leading-6 text-fg-3">
            {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "ToonStudio 프로젝트·JSON·PSD·ORA·CBZ·브러시 팩과 이미지 파일은 보존 항목과 변환 손실을 확인한 뒤 검증된 가져오기 경로로 전달합니다. 프로젝트 파일은 원고 스냅샷까지 새 로컬 프로젝트로 복원합니다.", "ToonStudio projects, JSON, PSD, ORA, CBZ, brush packs and images are previewed for preservation and loss, then sent through verified import paths. Project packages restore canvas snapshots into a new local project.")}
          </p>
        </div>
        <label className={buttonClass({ size: "lg", className: "shrink-0 cursor-pointer gap-2" })}>
          <FileUp size={17} aria-hidden="true" />
          {translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "파일 선택", "Choose file")}
          <input
            type="file"
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              ...
              <dl className="mt-4 grid gap-3 text-xs sm:grid-cols-2">
                <div>
                  <dt className="font-bold text-fg-2">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "보존", "Preserved")}</dt>
                  <dd className="mt-1 leading-5 text-fg-3">
                    {listLabel(selection.item.preservedFeatures, translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "보존 항목 없음", "No preserved features"))}
                  </dd>
                </div>
                <div>
                  <dt className="font-bold text-fg-2">{translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "변환·손실", "Conversion and loss")}</dt>
                  <dd className="mt-1 leading-5 text-fg-3">
                    {listLabel(selection.item.losses, translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "알려진 손실 없음", "No known loss"))}
                  </dd>
                </div>
              </dl>
            ...
            >
              {importing
                ? translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "프로젝트 복원 중…", "Restoring project…")
                : selection.item.format === "toonstudio"
                  ? translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "프로젝트 복원", "Restore project")
                  : translateBilingualValueForLocale(locale, "domains.creator.studio.shell.StudioImportIntake", "편집기에서 가져오기", "Import in editor")}
