...
const bi = <T,>(ko: T, en: T): T =>
  translateBilingualValueForActiveLocale("StudioLocalizationPanel", ko, en);;
...
function nextActionLabel(status: StudioLocalizationStatus, _locale): string {
  ...
  return bi((labels).ko, (labels).en)[status];
}
...
export function StudioLocalizationPanel({
  ...
  useEffect(() => {
    const load = () => {
      try {
        ...
        setError(bi("이 기기에서 현지화 작업을 불러오지 못했습니다.", "Localization work could not be loaded on this device."));
      }
    };
    load();
  ...
  const persist = (units: readonly StudioLocalizationUnit[], successMessage?: string): boolean => {
    ...
    try {
      ...
      setError(cause instanceof Error
        ? cause.message
        : (bi("현지화 작업을 저장하지 못했습니다.", "Localization work could not be saved.")));
      return false;
    }
  };
  ...
  const addUnit = () => {
    if (!sourceText.trim()) {
      setError(bi("원문을 입력해 주세요.", "Enter the source text."));
      return;
    }
    try {
      ...
      if (persist(
        [...document.units, unit],
        bi("번역할 대사를 추가했습니다.", "Added a localization unit."),
      )) {
        setSelectedId(unit.id);
        setSourceText("");
      }
    } catch {
      setError(bi("언어 코드와 원문을 확인해 주세요. 예: ko-KR → en-US", "Check the locales and source text. Example: ko-KR → en-US"));
    }
  };

  const translatedUnit = (unit: StudioLocalizationUnit): StudioLocalizationUnit | null => {
    if (!translationDraft.trim()) {
      setError(bi("번역문을 입력해 주세요.", "Enter the translated text."));
      return null;
    }
    try {
      let next = unit;
      ...
      setError(bi("현재 단계에서는 번역문을 수정할 수 없습니다.", "Translation cannot be edited at this stage."));
      return null;
    }
  };
  ...
  const runNextAction = () => {
    ...
    try {
      ...
      switch (selected.status) {
        ...
        case "layout-check": {
          ...
          if (blocking.length > 0) {
            replaceUnit(
              prepared,
              bi("확인한 원고 상태를 저장했습니다.", "Saved the verified layout state."),
            );
            setError(formatI18nTemplate(String(bi("완료하려면 오류 {value0}개를 해결해 주세요.", "Resolve {value0} blocking issues before completion.")), { value0: blocking.length }));
            return;
          }
      ...
      replaceUnit(
        next,
        bi("현지화 단계를 업데이트했습니다.", "Localization status updated."),
      );
    } catch (cause) {
      setError(cause instanceof Error
        ? cause.message
        : (bi("다음 단계로 이동할 수 없습니다.", "Cannot move to the next step.")));
    }
  };
  ...
  const removeSelected = () => {
    ...
    if (persist(
      remaining,
      bi("현지화 대사를 삭제했습니다.", "Localization unit removed."),
    )) {
    ...
    >
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h2 id="localization-title" className="mt-2 text-2xl font-black tracking-tight text-fg">
            {bi("번역부터 레터링 QA까지 한 흐름으로", "From translation to lettering QA in one flow")}
          </h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-fg-2">
            {bi("번역 승인만으로 완료되지 않습니다. 실제 원문 제거·배경 복원·레터링·글꼴·읽기 순서와 말풍선 결과를 확인해야 합니다.", "Translation approval is not completion. Verify actual cleanup, restoration, lettering, fonts, reading order and balloon fit.")}
          </p>
        </div>
        <div className="grid grid-cols-3 gap-2 text-center text-xs">
          <span className="rounded-xl border border-line bg-panel px-3 py-2"><b className="block text-base text-fg">{summary.total}</b>{bi("전체", "Total")}</span>
          <span className="rounded-xl border border-success/30 bg-success-soft/15 px-3 py-2 text-success"><b className="block text-base">{summary.completed}</b>{bi("완료", "Done")}</span>
          <span className="rounded-xl border border-danger/30 bg-danger-soft/15 px-3 py-2 text-danger"><b className="block text-base">{summary.blocking}</b>{bi("확인", "Issues")}</span>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-[18rem_1fr]">
        <aside className="rounded-2xl border border-line bg-panel/55 p-3">
          <h3 className="text-sm font-black text-fg">{bi("대사·문구", "Lines")}</h3>
          <div className="mt-3 max-h-[30rem] space-y-2 overflow-y-auto">
            {document.units.length === 0 ? (
              <p className="rounded-xl border border-dashed border-line p-4 text-xs leading-5 text-fg-3">
                {bi("아래에서 첫 원문을 추가하세요.", "Add the first source line below.")}
              </p>
            ) : null}
              ...
              >
                <span className="flex items-center justify-between gap-2">
                  ...
                  <span className={cn(
                    ...
                    {bi((STATUS_LABELS[unit.status]).ko, (STATUS_LABELS[unit.status]).en)}
                  </span>
                </span>
          ...
          <div className="mt-4 border-t border-line pt-4">
            <div className="grid grid-cols-2 gap-2">
              <input aria-label={bi("원문 언어", "Source locale")} value={sourceLocale} onChange={(event) => setSourceLocale(event.target.value)} className="min-h-10 rounded-lg border border-line bg-card px-2 text-xs text-fg" />
              <input aria-label={bi("번역 언어", "Target locale")} value={targetLocale} onChange={(event) => setTargetLocale(event.target.value)} className="min-h-10 rounded-lg border border-line bg-card px-2 text-xs text-fg" />
            </div>
            <select aria-label={bi("문구 종류", "Unit kind")} value={kind} onChange={(event) => setKind(event.target.value as StudioLocalizationUnitKind)} className="mt-2 min-h-10 w-full rounded-lg border border-line bg-card px-2 text-xs text-fg">
              {Object.entries(KIND_LABELS).map(([value, label]) => <option key={value} value={value}>{bi((label).ko, (label).en)}</option>)}
            </select>
            <textarea aria-label={bi("새 원문", "New source text")} value={sourceText} onChange={(event) => setSourceText(event.target.value)} rows={3} className="mt-2 w-full rounded-xl border border-line bg-card px-3 py-2 text-xs leading-5 text-fg" placeholder={bi("번역할 대사나 문구", "Source dialogue or text")} />
            <button type="button" onClick={addUnit} className={buttonClass({ size: "sm", className: "mt-2 w-full gap-1.5" })}>
              <Plus size={14} aria-hidden="true" />{bi("추가", "Add")}
            </button>
          </div>
        </aside>
        ...
        <div className="rounded-2xl border border-line bg-panel/55 p-4">
          {!selected ? (
            <div className="grid min-h-64 place-items-center text-sm text-fg-3">
              {bi("대사를 선택하거나 새로 추가하세요.", "Select or add a line.")}
            </div>
          ) : (
            <>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-bold text-accent">
                    {bi((KIND_LABELS[selected.kind]).ko, (KIND_LABELS[selected.kind]).en)} · {selected.sourceLocale} → {selected.targetLocale}
                  </p>
                ...
                <span className={cn("rounded-full border px-3 py-1 text-xs font-bold", statusTone(selected.status))}>
                  {bi((STATUS_LABELS[selected.status]).ko, (STATUS_LABELS[selected.status]).en)}
                </span>
              </div>
              ...
              {showLayout ? (
                <fieldset className="mt-5 rounded-2xl border border-line bg-card p-4">
                  <legend className="px-1 text-sm font-black text-fg">
                    {bi("실제 원고 처리 결과", "Verified artwork results")}
                  </legend>
                  <p className="mt-1 text-xs leading-5 text-fg-3">
                    {bi("실제 편집기나 현지화 처리기가 완료한 항목만 선택하세요. 체크 자체는 이미지 처리를 수행하지 않습니다.", "Select only work completed by the editor or localization processor. These checks do not alter artwork.")}
                  </p>
                  <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {([
                      ["sourceRemoved", bi("원문 제거 완료", "Source removed")],
                      ["backgroundRestored", bi("배경 복원 확인", "Background restored")],
                      ["letteringApplied", bi("번역문 레터링 완료", "Lettering applied")],
                      ["readingOrderAssigned", bi("읽기 순서 지정", "Reading order assigned")],
                      ["fontAvailable", bi("대상 언어 글꼴 확인", "Target font available")],
                  ...
                  {selected.kind === "dialogue" ? (
                    <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                      {([
                        ["availableWidth", bi("말풍선 폭", "Balloon width")],
                        ["availableHeight", bi("말풍선 높이", "Balloon height")],
                        ["renderedWidth", bi("글자 사용 폭", "Text width")],
                        ["renderedHeight", bi("글자 사용 높이", "Text height")],
                        ["minimumFontSize", bi("최소 글자", "Minimum font")],
                        ["actualFontSize", bi("실제 글자", "Actual font")],
                        ["lineCount", bi("줄 수", "Line count")],
                        ["maxLineCount", bi("허용 줄 수", "Maximum lines")],
                      ] as const).map(([key, label]) => (
                ...
                {["ai-draft", "translating", "review-required"].includes(selected.status) ? (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (next) replaceUnit(next, bi("번역문을 저장했습니다.", "Translation saved."));
                    }}
                    className={buttonClass({ variant: "outline", size: "sm", className: "gap-1.5" })}
                  >
                    <Save size={14} aria-hidden="true" />
                    {bi("번역문 저장", "Save translation")}
                  </button>
                ) : null}
                    ...
                    onClick={() => {
                      try {
                        replaceUnit(
                          ...
                          bi("번역 수정 단계로 되돌렸습니다.", "Returned to translation edits."),
                        );
                      } catch {
                        setError(bi("현재 단계에서는 수정 요청을 만들 수 없습니다.", "Changes cannot be requested at this stage."));
                      }
                    }}
                    className={buttonClass({ variant: "outline", size: "sm", className: "gap-1.5" })}
                  >
                    <RotateCcw size={14} aria-hidden="true" />
                    {bi("수정 요청", "Request changes")}
                  </button>
                ) : null}
              ...
              <div className={cn(
                ...
                <h4 className="text-sm font-black text-fg">{bi("현재 QA", "Current QA")}</h4>
                {qa.length === 0 ? (
                  <p className="mt-2 text-xs font-semibold text-success">
                    {bi("차단 문제가 없습니다.", "No blocking issues.")}
                  </p>
                ) : (
                  <ul className="mt-2 space-y-1.5 text-xs leading-5 text-fg-2">
                    {qa.map((finding) => (
                      <li key={finding.code}>• {bi(finding.messageKo, finding.messageEn)}</li>
                    ))}
                  </ul>
                )}
              </div>
