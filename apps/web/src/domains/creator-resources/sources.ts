...
export const RESOURCE_SOURCES: ResourceSource[] = [
  ...
  { name: "Google Books", freeAccess: true, category: "글로벌 판본", status: "무료 제한키 발급·검색 어댑터 구현", commercial: "조건부 상업 이용", url: "https://developers.google.com/books/docs/v1/using", note: "결제 계정이 연결되지 않은 전용 프로젝트에서 Books API로만 제한한 서버 키를 발급했습니다. 해외 판본·언어·ISBN 메타데이터를 통합 검색하며 표지·미리보기·본문은 가져오지 않습니다. 사용자 개인 서재는 별도 OAuth와 동의가 있을 때만 연결합니다." },
