...
export function ReferenceAssetsPage() {
  ...
  return (
    <ResourceLayout
      width="wide"
      title={translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "창작 레퍼런스 아틀라스")}
      intro={translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "The Met의 공개 미술 자료를 장면 목적에 맞게 탐색하고, 형태·시대·재료를 비교한 뒤 출처와 함께 연구 보드에 보관하세요. 공개 도메인과 CC0 표시, 안전한 이미지 주소가 확인된 자료만 미리보기를 제공합니다.")}
    >
        ...
        <div className="relative z-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_24rem] lg:items-end">
          <div>
            <div className="flex flex-wrap gap-2">
              <CountBadge><ShieldCheck size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "공개 이용 검증")}</CountBadge>
              <CountBadge><Filter size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "장면 목적 필터")}</CountBadge>
              <CountBadge><Scale size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "최대 4개 비교")}</CountBadge>
            </div>
            <h2 className="mt-6 max-w-3xl font-display text-2xl font-bold leading-tight text-fg sm:text-3xl">
              {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "검색 결과를 모으는 페이지에서")}<br className="hidden sm:block" /> {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "장면의 시각 근거를 설계하는 작업대로")}</h2>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-fg-2">
              {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "복식, 소품, 공간, 자세, 패턴을 검색하고 현재 결과 안에서 문화권과 분류를 좁혀 보세요. 상세 화면에서는 고해상도 원본, 치수, 재료, 태그를 확인하고 출처 문구를 바로 복사할 수 있습니다.")}</p>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-2xl border border-line bg-canvas/80 p-4 backdrop-blur">
              ...
              <p className="mt-1 text-xs text-fg-3">{translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "저장 자료")}</p>
            </div>
            <div className="rounded-2xl border border-line bg-canvas/80 p-4 backdrop-blur">
              <p className="text-2xl font-bold text-fg">{result?.items.length ?? 0}</p>
              <p className="mt-1 text-xs text-fg-3">{translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "현재 검증")}</p>
            </div>
            <div className="rounded-2xl border border-line bg-canvas/80 p-4 backdrop-blur">
              <p className="text-2xl font-bold text-fg">{comparison.length}</p>
              <p className="mt-1 text-xs text-fg-3">{translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "비교 선택")}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div aria-live="polite" aria-atomic="true" className="space-y-2 text-sm leading-6 text-fg-2">
        {loading ? <p role="status" className="inline-flex items-center gap-2"><Loader2 className="size-4 animate-spin" aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "공개 이용 조건과 작품 메타데이터를 확인하고 있습니다…")}</p> : null}
        ...
        {result?.status === "partial" ? <p>{translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "일부 상세 응답은 제외했지만 확인된 자료는 계속 사용할 수 있습니다.")}</p> : null}
      </div>

      {requestError ? (
        <button type="button" className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "en", "{v0} gap-2"), { v0: String(RESOURCE_BUTTON) })} onClick={() => setRetry((value) => value + 1)}>
          <RotateCcw size={15} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "다시 시도")}</button>
      ) : null}

      {loading && view.mode === "results" ? <ResultSkeleton density={view.density} /> : null}

      {!loading && visibleItems.length > 0 ? (
        <div className={gridClass} aria-label={translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "창작 레퍼런스 결과")}>
      ...
      {!loading && view.mode === "saved" && savedItems.length === 0 ? (
        <EmptyState title={translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "아직 저장한 레퍼런스가 없습니다")} description={translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "검색 결과에서 북마크 버튼을 누르면 공개 이용 정보와 출처를 함께 이 브라우저의 연구 보드에 보관합니다.")} />
      ) : null}

      {!loading && sourceItems.length > 0 && visibleItems.length === 0 ? (
        <EmptyState
          title={translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "현재 결과 필터와 일치하는 자료가 없습니다")}
          description={translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "결과 내 검색어나 문화권·분류 필터를 줄이면 다시 표시됩니다.")}
          action={<button type="button" className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "en", "{v0} gap-2"), { v0: String(RESOURCE_BUTTON) })} onClick={() => updateView({ within: "", department: "", culture: "", classification: "" })}><RotateCcw size={15} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "결과 필터 초기화")}</button>}
        />
      ) : null}

      {!loading && !requestError && view.mode === "results" && search.query && result && result.items.length === 0 ? (
        <EmptyState
          title={translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "검증해 표시할 자료를 찾지 못했습니다")}
          description={translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "검색 후보가 있어도 공개 도메인 표시나 안전한 이미지가 확인되지 않으면 제외됩니다. 더 넓은 키워드나 다른 부서로 다시 검색해 보세요.")}
          action={<button type="button" className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "en", "{v0} gap-2"), { v0: String(RESOURCE_BUTTON) })} onClick={() => setAdvancedOpen(true)}><SlidersHorizontal size={15} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "검색 조건 조정")}</button>}
        />
      ) : null}
      ...
      {!search.query && view.mode === "results" ? (
        <div className="grid gap-4 rounded-3xl border border-line bg-panel p-6 md:grid-cols-3">
          <div>
            ...
            <h3 className="mt-3 font-bold text-fg">{translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "권리 상태 우선")}</h3>
            <p className="mt-2 text-sm leading-6 text-fg-2">{translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "공개 도메인·CC0와 안전한 Met 이미지 호스트가 확인된 결과만 화면에 남깁니다.")}</p>
          </div>
          <div>
            <Tags className="size-5 text-accent" aria-hidden="true" />
            <h3 className="mt-3 font-bold text-fg">{translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "메타데이터 기반 탐색")}</h3>
            <p className="mt-2 text-sm leading-6 text-fg-2">{translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "부서, 문화권, 시대, 오브젝트 유형, 재료와 태그를 장면 설계의 단서로 사용합니다.")}</p>
          </div>
          <div>
            <Scale className="size-5 text-accent" aria-hidden="true" />
            <h3 className="mt-3 font-bold text-fg">{translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "비교 후 저장")}</h3>
            <p className="mt-2 text-sm leading-6 text-fg-2">{translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "최대 네 작품을 나란히 비교하고 선택한 자료만 출처와 함께 연구 보드로 넘깁니다.")}</p>
          </div>
        </div>
      ) : null}

      {!loading && view.mode === "results" && result && (result.status === "ready" || result.status === "partial") ? (
        <nav className="flex items-center justify-center gap-4" aria-label={translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "레퍼런스 검색 결과 페이지")}>
          <button
            type="button"
            className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "en", "{v0} gap-2"), { v0: String(RESOURCE_BUTTON) })}
            disabled={search.page <= 1}
            onClick={() => replaceUrl({ ...search, page: search.page - 1 }, { ...view, within: "", department: "", culture: "", classification: "" })}
          >
            <ChevronLeft size={16} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "이전")}</button>
          <span className="min-w-24 text-center text-sm font-semibold text-fg-2">{search.page} {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "/ 20 페이지")}</span>
          <button
            type="button"
            className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "en", "{v0} gap-2"), { v0: String(RESOURCE_BUTTON) })}
            ...
            onClick={() => replaceUrl({ ...search, page: search.page + 1 }, { ...view, within: "", department: "", culture: "", classification: "" })}
          >
            {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "다음 ")}<ChevronRight size={16} aria-hidden="true" />
          </button>
        </nav>
      ) : null}
      ...
      {comparison.length > 0 ? (
        <aside className="sticky bottom-4 z-40 mx-auto flex max-w-4xl flex-wrap items-center gap-3 rounded-2xl border border-accent/40 bg-canvas/95 p-3 shadow-2xl backdrop-blur" aria-label={translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "레퍼런스 비교 선택")}>
          ...
          <button type="button" className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "en", "{v0} gap-2 border-accent bg-accent text-white hover:bg-accent/90"), { v0: String(RESOURCE_BUTTON) })} disabled={comparison.length < 2} onClick={(event) => {
            setCompareReturnFocus(event.currentTarget);
            setCompareOpen(true);
          }}>
            <Scale size={15} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "비교하기")}</button>
          <button type="button" className={RESOURCE_BUTTON} onClick={() => setComparison([])}>{translateCurrentStaticSourceText("domains.creator.resources.ReferenceAssetsPage", "ko", "비우기")}</button>
        </aside>
      ) : null}
