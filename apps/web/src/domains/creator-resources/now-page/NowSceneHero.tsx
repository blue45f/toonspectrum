...
function ScenePoster({
  ...
  return (
    <aside
      ...
      aria-label={translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "오늘의 장면 DNA 포스터")}
    >
      ...
      <div className="relative flex min-h-[22rem] flex-col justify-between">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="font-display text-xs font-bold tracking-[0.16em] text-accent">{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "en", "SCENE DNA / DAILY ISSUE")}</p>
        ...
        <div className="grid gap-3 border-t border-line pt-4 sm:grid-cols-2">
          <div>
            <span className="text-[0.68rem] font-bold tracking-[0.14em] text-fg-3">{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "en", "KEY OBJECT")}</span>
            <p className="mt-1 text-sm font-semibold leading-6 text-fg">{theme.object}</p>
          </div>
          <div>
            <span className="text-[0.68rem] font-bold tracking-[0.14em] text-fg-3">{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "en", "DIRECTING MODE")}</span>
  ...
  return (
    ...
    >
      ...
      <div className="relative grid gap-8 xl:grid-cols-[minmax(0,1.08fr)_minmax(25rem,0.92fr)] xl:items-stretch">
        <div className="flex flex-col justify-center">
          <div className="flex flex-wrap items-center gap-2 text-xs font-semibold text-accent">
            ...
            {selectedOffset === 0 ? translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "en", "TODAY’S SPARK") : translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "en", "SPARK ARCHIVE")} · {day.label}
            <span className="rounded-full border border-good/30 bg-good/10 px-2 py-1 text-[0.68rem] font-bold text-good">
              {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "en", "HUMAN-CURATED · ORIGINAL")}</span>
          </div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {theme.moods.map((mood) => (
              <Link
                ...
                to={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "en", "/research/assets?q={v0}&page=1"), { v0: String(encodeURIComponent(mood)) })}
              >
                #{mood}
              </Link>
            ))}
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link className={PRIMARY_BUTTON} to="/studio" reloadDocument>
              {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "Studio에서 시작 ")}<ArrowRight size={15} aria-hidden="true" />
            </Link>
            <a className={ACTION_BUTTON} href="#creation-loop">
              <Target size={16} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "다음 단계 · ")}{nextStepLabel}
            </a>
            <button type="button" className={ACTION_BUTTON} onClick={onCopyBrief}>
              ...
              {briefCopyStatus === "copied" ? translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "브리프 복사됨") : translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "브리프 복사")}
            </button>
            <button
              type="button"
            ...
            >
              <Bookmark size={16} fill={isSaved ? translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "en", "currentColor") : translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "en", "none")} aria-hidden="true" />
              {isSaved ? translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "영감 저장 해제") : translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "영감 저장")}
            </button>
            <button type="button" className={ACTION_BUTTON} onClick={onCopyShareLink}>
              ...
              {linkCopyStatus === "copied" ? translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "링크 복사됨") : translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "이 영감 링크 복사")}
            </button>
          </div>
          {briefCopyStatus === "copied" && (
            <span className="sr-only" role="status">
              {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "브리프가 클립보드에 복사되었습니다.")}</span>
          )}
          {linkCopyStatus === "copied" && (
            <span className="sr-only" role="status">
              {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "날짜가 포함된 영감 링크가 클립보드에 복사되었습니다.")}</span>
          )}
          {(briefCopyStatus === "error" || linkCopyStatus === "error") && (
            <p className="mt-3 text-sm font-semibold text-bad" role="alert">
              {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "클립보드에 복사하지 못했습니다. 브라우저 권한을 확인하세요.")}</p>
          )}
        </div>
      ...
      <div
        ...
        aria-label={translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "이번 주 창작 페이스")}
      >
        <div className="border-b border-line p-3 sm:border-b-0 sm:border-r">
          <Flame size={18} className="text-warn" aria-hidden="true" />
          <strong className="mt-2 block font-display text-2xl tabular-nums text-fg">{streak}{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "일")}</strong>
          <span className="text-xs text-fg-3">{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "연속 완주")}</span>
        </div>
        <div className="border-b border-line p-3 sm:border-b-0 sm:border-r">
          ...
          <span className="text-xs text-fg-3">{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "최근 7일 완주")}</span>
        </div>
        <div className="border-r border-line p-3">
          ...
          <span className="text-xs text-fg-3">{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "저장한 영감")}</span>
        </div>
        <div className="p-3">
          ...
          <span className="text-xs text-fg-3">{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowSceneHero", "ko", "선택한 날 진행률")}</span>
        </div>
      </div>
    </section>
  );
