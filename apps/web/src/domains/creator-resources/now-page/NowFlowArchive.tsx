...
function CreationFlowNav({ progressPercent, nextStep }: { progressPercent: number; nextStep: string }) {
  return (
    <nav
      aria-label={translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "오늘의 제작 흐름")}
    ...
    >
      <a className={FLOW_LINK} href="#daily-spark">
        <Sparkles size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "브리프")}</a>
      <a className={FLOW_LINK} href="#session-plan">
        <Timer size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "세션")}</a>
      <a className={FLOW_LINK} href="#directing-mode">
        <Compass size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "연출")}</a>
      <a className={FLOW_LINK} href="#variation-lab">
        <Shuffle size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "변주")}</a>
      <a className={FLOW_LINK} href="#scene-ingredients">
        <PackageSearch size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "재료")}</a>
      <a className={FLOW_LINK} href="#storyboard">
        <PanelsTopLeft size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "5컷")}</a>
      <a className={FLOW_LINK} href="#creation-loop">
        <Gauge size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "제작")}</a>
      <span className="ml-auto hidden shrink-0 rounded-lg bg-raised px-3 py-2 text-xs font-semibold text-fg-2 lg:block">
        {progressPercent}{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "% · 다음 ")}{nextStep}
      </span>
    </nav>
  );
}
      ...
      <section className="space-y-4" aria-labelledby="spark-archive-title">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-bold tracking-[0.14em] text-accent">
              {archive.length}{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "en", "-DAY EDITORIAL ARCHIVE")}</p>
            <h2 id="spark-archive-title" className="mt-1 text-2xl font-bold text-fg">
              {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "놓친 영감 다시 열기")}</h2>
            <p className="mt-2 max-w-2xl text-sm leading-7 text-fg-2">
              {archive.length}{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "개의 오리지널 테마를 한 주기 전체로 둘러보고, 저장·완주 상태로 좁혀서 작업을 이어갈 수 있습니다.")}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            >
              <Shuffle size={16} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "다른 영감")}</button>
            {selectedOffset > 0 && (
              <button type="button" className={ACTION_BUTTON} onClick={() => onSelectOffset(0)}>
                {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "오늘로 돌아가기")}</button>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-line bg-panel p-3">
          <div className="flex items-center gap-2 text-xs font-bold text-fg-3">
            <ListFilter size={15} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "아카이브 보기")}</div>
          <div className="flex flex-wrap gap-2" role="group" aria-label={translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "영감 아카이브 필터")}>
            {[
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          >
            <ChevronLeft size={16} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "하루 전")}</button>
          <p className="text-center text-xs font-semibold text-fg-3">
            {selectedOffset === 0 ? translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "오늘의 발행본") : formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "{v0}일 전 발행본"), { v0: String(selectedOffset) })}
          </p>
          <button
            type="button"
            className={ACTION_BUTTON}
          ...
          >
            {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "하루 후 ")}<ChevronRight size={16} aria-hidden="true" />
          </button>
        </div>

        {visibleArchive.length > 0 ? (
          <div className="-mx-1 flex gap-3 overflow-x-auto px-1 pb-2" aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "최근 {v0}일 영감"), { v0: String(archive.length) })}>
              ...
              return (
                ...
                >
                  ...
                  <span className="relative flex items-center justify-between gap-2 text-xs font-bold text-fg-3">
                    {offset === 0 ? translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "오늘") : day.shortLabel}
          ...
          <div className="rounded-2xl border border-dashed border-line bg-panel p-8 text-center">
            <p className="font-semibold text-fg">{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "이 조건에 맞는 영감이 아직 없습니다.")}</p>
            <p className="mt-2 text-sm text-fg-3">{translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "오늘의 브리프를 저장하거나 5단계를 완주하면 여기에 모입니다.")}</p>
            <button type="button" className={formatI18nTemplate(translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "en", "{v0} mt-4"), { v0: String(ACTION_BUTTON) })} onClick={() => onArchiveFilterChange("all")}>
              {translateCurrentStaticSourceText("domains.creator.resources.now.page.NowFlowArchive", "ko", "전체 아카이브 보기")}</button>
          </div>
        )}
      </section>
    </>
