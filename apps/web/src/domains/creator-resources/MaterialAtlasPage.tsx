  ...
  return <ResourceLayout title="무료 배경·소품 소재 도감" intro="Poly Haven과 ambientCG의 공개 API 자료로 만든 작은 CC0 소재 카탈로그입니다. 재질, 3D 소품, 빛과 환경을 한글로 찾아 장면별 소재 목록과 출처 명세서로 정리하세요.">
    ...
    <section aria-labelledby="material-search-title" className="space-y-4 rounded-2xl border border-line bg-panel p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-[minmax(0,2fr)_1fr_1fr]">
        <label htmlFor="material-query" className="min-w-0 text-sm font-semibold">검색어<input id="material-query" type="search" className={`${RESOURCE_INPUT} mt-2`} value={query} maxLength={80} placeholder="나무, 벽돌, 의자, 야경…" onChange={(event) => changeParams({ q: event.target.value })} /></label>
        <label htmlFor="material-provider" className="min-w-0 text-sm font-semibold">제공처<select id="material-provider" className={`${RESOURCE_INPUT} mt-2`} value={provider} onChange={(event) => changeParams({ provider: event.target.value })}><option value="all">모든 제공처</option>{Object.entries(MATERIAL_PROVIDERS).map(([id, value]) => <option key={id} value={id}>{value.name}</option>)}</select></label>
        <label htmlFor="material-kind" className="min-w-0 text-sm font-semibold">소재 종류<select id="material-kind" className={`${RESOURCE_INPUT} mt-2`} value={kind} onChange={(event) => changeParams({ kind: event.target.value })}><option value="all">모든 종류</option>{Object.entries(MATERIAL_KINDS).map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label>
      </div>
      <div className="flex flex-wrap gap-2" aria-label="한글 추천 검색어">{["나무", "벽돌", "콘크리트", "의자", "숲", "야경"].map((word) => <button key={word} type="button" className={RESOURCE_BUTTON} onClick={() => changeParams({ q: word, study: "" })}>{word}</button>)}<button type="button" className={RESOURCE_BUTTON} onClick={() => changeParams({ q: "", provider: "", kind: "", study: "" })}>검색 조건 초기화</button></div>
    ...
    {shown.length ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" aria-label="소재 검색 결과">{shown.map((asset) => <article key={asset.id} className="flex min-w-0 flex-col gap-3 rounded-2xl border border-line bg-panel p-4">
      <MaterialPreview asset={asset} enabled={previews} /><div><p className="text-xs text-accent">{MATERIAL_PROVIDERS[asset.provider].name} · {MATERIAL_KINDS[asset.kind]} · CC0</p><h3 className="mt-2 break-words font-bold">{asset.title}</h3></div>
      ...
      <button type="button" className={`${RESOURCE_BUTTON} ${selectedIds.includes(asset.id) ? "border-accent bg-accent-soft" : ""}`} aria-pressed={selectedIds.includes(asset.id)} aria-label={`${asset.title} 소재 담기`} onClick={() => toggleSelected(asset.id)}>{selectedIds.includes(asset.id) ? "담은 소재 해제" : "소재 담기"}</button>
      <a className={RESOURCE_BUTTON} href={asset.sourceUrl} target="_blank" rel="noopener noreferrer">원문·다운로드 확인 ↗</a>
    </article>)}</div> : <div className="rounded-2xl border border-line bg-panel p-6"><h3 className="font-bold">조건에 맞는 소재가 없습니다</h3><p className="mt-2 text-sm leading-7 text-fg-2">이 도감은 선별된 작은 목록입니다. 검색 조건을 줄이거나 제공처 원문에서 더 많은 소재를 확인하세요.</p></div>}
