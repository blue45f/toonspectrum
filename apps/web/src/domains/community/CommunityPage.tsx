    ...
    <Container size="wide" className="relative py-6 sm:py-8 lg:py-10">
      ...
      >
        <div className="flex flex-wrap gap-3">
          <Link href="/community/promote" className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-accent px-5 text-sm font-bold text-on-accent transition-colors hover:bg-accent-2">신작·작가 홍보<ArrowRight size={16} aria-hidden="true" /></Link>
...
export function CommunityScopePage() {
  ...
  if (!scope) return <Container size="wide" className="py-16"><p className="eyebrow text-accent">COMMUNITY</p><h1 className="mt-2 text-2xl font-bold">커뮤니티 범주를 찾을 수 없어요</h1><Link href="/community" className="mt-5 inline-flex text-sm font-medium text-accent">통합 커뮤니티로 이동</Link></Container>;
  return <Container size="wide" className="relative py-6 sm:py-8 lg:py-10"><header className="mb-6 sm:mb-8"><p className="eyebrow flex items-center gap-1.5 text-accent"><UsersRound size={14} />COMMUNITY DIRECTORY</p><h1 className="mt-2 text-[clamp(1.6rem,7vw,1.875rem)] font-bold tracking-tight sm:text-4xl">{COMMUNITY_SCOPE_LABEL[scope]} 커뮤니티</h1><p className="lede mt-2 max-w-xl text-pretty text-sm leading-relaxed text-fg-2">{COMMUNITY_SCOPE_DESCRIPTION[scope]}</p></header><CommunityScopeDirectory key={scope} scope={scope} /></Container>;
}
