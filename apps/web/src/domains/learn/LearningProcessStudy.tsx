...
export function LearningProcessStudy() {
  ...
  return <section className="learn-process-study" aria-labelledby="learn-process-title">
    <div className="learn-process-heading"><div><p className="learn-eyebrow">{translateCurrentStaticSourceText("domains.learn.LearningProcessStudy", "en", "THE MAKING OF A SCENE")}</p><h2 id="learn-process-title">{translateCurrentStaticSourceText("domains.learn.LearningProcessStudy", "ko", "한 장면이 완성되는 과정.")}</h2></div><p>{translateCurrentStaticSourceText("domains.learn.LearningProcessStudy", "ko", "과정을 선택하고 필요한 실습으로 이어가세요.")}</p></div>
    <figure className="learn-process-art">
      <img src="/brand/atelier-process.webp" alt={translateCurrentStaticSourceText("domains.learn.LearningProcessStudy", "ko", "같은 장면을 구상 스케치, 선화, 완성 채색으로 펼친 제작 과정 콘셉트 이미지")} loading="lazy" width={1536} height={1024} />
      <figcaption>{translateCurrentStaticSourceText("domains.learn.LearningProcessStudy", "ko", "제작 과정 콘셉트 아트 · 단계별 표현을 살펴보세요")}</figcaption>
    </figure>
    <div className="learn-process-steps" role="group" aria-label={translateCurrentStaticSourceText("domains.learn.LearningProcessStudy", "ko", "제작 과정 살펴보기")}>
