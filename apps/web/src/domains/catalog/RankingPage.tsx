  ...
  return (
    <Container size="wide" className="py-6 sm:py-10">
      <header className="relative mb-6 overflow-hidden rounded-2xl border border-line bg-panel/55 p-4 surface-hl sm:mb-8 sm:p-6">
        ...
        />
        <p className="eyebrow text-accent">{translateCurrentStaticSourceText("domains.catalog.RankingPage", "en", "UNIFIED RANKING")}</p>
        <h1 className="mt-2 text-[clamp(1.6rem,7vw,1.875rem)] font-bold tracking-tight sm:text-4xl">{translateCurrentStaticSourceText("domains.catalog.RankingPage", "ko", "통합 랭킹")}</h1>
        <p className="lede mt-2 max-w-2xl text-pretty text-sm leading-relaxed text-fg-2">
          {translateCurrentStaticSourceText("domains.catalog.RankingPage", "ko", "검증된 카탈로그 스냅샷에 투명 산식을 적용해 지금 볼 작품을 고릅니다. 기간(일간·주간·월간·전체)은 무작위 변주가 아니라 실 신호(순위 변동·트렌드·누적 조회/관심/평점)의 가중 블렌딩으로 달라집니다.")}</p>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 sm:mt-6">
          ...
          >
            ...
            {translateCurrentStaticSourceText("domains.catalog.RankingPage", "ko", "랭킹 시작점으로 이동")}</a>
          <span className="inline-flex min-h-9 items-center gap-1.5 rounded-full border border-line bg-card px-3 py-1.5 text-xs text-fg-2">
            ...
            {translateCurrentStaticSourceText("domains.catalog.RankingPage", "ko", "현재 축: ")}<span className="font-medium text-fg">{axis}</span>
          </span>
          {/* 랭킹 공유 — OS 공유 시트 → 클립보드 폴백 */}
          <SharePageButton path="/ranking" text={translateCurrentStaticSourceText("domains.catalog.RankingPage", "ko", "툰스펙트럼 통합 랭킹")} label={translateCurrentStaticSourceText("domains.catalog.RankingPage", "ko", "랭킹 공유")} />
        </div>
      </header>
