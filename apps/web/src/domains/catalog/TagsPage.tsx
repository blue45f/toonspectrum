    ...
    <Container size="wide" className="py-8 sm:py-11 lg:py-14">
      <header className="max-w-3xl">
        <p className="eyebrow flex items-center gap-1.5 text-accent">
          <Hash size={14} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.catalog.TagsPage", "en", "TAG SPECTRUM")}</p>
        <h1 className="mt-2 text-pretty font-display text-[clamp(2.25rem,7vw,4.75rem)] font-bold leading-[1] tracking-[-0.055em] text-fg">
          {translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "태그로 작품 찾기")}</h1>
        <p className="mt-4 max-w-2xl text-sm leading-7 text-fg-2 sm:text-base">
          {translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "작품의 분위기와 소재를 검색하고, 인기순 또는 이름순으로 좁혀 보세요. 태그는 한 번에 120개씩 표시해 모바일에서도 빠르게 탐색할 수 있습니다.")}</p>
      </header>
      ...
      <section className="mt-7 rounded-3xl border border-line bg-panel/60 p-4 sm:p-5" aria-labelledby="tag-filter-title">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 id="tag-filter-title" className="font-display text-lg font-bold text-fg">{translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "태그 검색과 정렬")}</h2>
            <p className="mt-1 text-xs leading-6 text-fg-3">{translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "검색과 정렬 조건은 주소에 남아 같은 결과를 다시 열거나 공유할 수 있습니다.")}</p>
          </div>
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
          <label className="block">
            <span className="sr-only">{translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "태그 검색")}</span>
            <span className="flex min-h-12 items-center gap-3 rounded-xl border border-line-strong bg-card px-3 text-fg-3 focus-within:outline focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-accent">
              ...
              <input
                ...
                placeholder={translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "예: 힐링, 회귀, 학원물")}
              ...
              {query ? (
                <button
                  ...
                  aria-label={translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "태그 검색 지우기")}
                >
          ...
          <fieldset className="flex min-h-12 gap-2" aria-label={translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "태그 정렬")}>
            {(["popular", "name"] as const).map((value) => (
              ...
              >
                {value === "popular" ? translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "인기순") : translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "이름순")}
              </button>
            ))}
          </fieldset>
        </div>
      ...
      <p className="mt-5 text-xs text-fg-3" role="status" aria-live="polite">
        {loading
          ? translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "태그를 불러오는 중입니다.")
          : formatI18nTemplate(translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "{v0}개 태그 중 {v1}개 표시"), { v0: String(filtered.length.toLocaleString("ko-KR")), v1: String(visible.length.toLocaleString("ko-KR")) })}
      </p>

      {error ? (
        <div className="mt-5">
          <ErrorState title={translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "태그를 불러오지 못했습니다.")} message={error} onRetry={reload} />
        </div>
      ) : loading ? (
        <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" aria-label={translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "태그 목록 불러오는 중")}>
          {Array.from({ length: 24 }).map((_, index) => (
            <span key={index} className="skeleton h-12 rounded-xl" />
          ))}
        </div>
      ) : visible.length ? (
        <ul className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" aria-label={translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "태그 목록")}>
          {visible.map(({ tag, count }) => (
            <li key={tag} className="min-w-0">
              <Link
                href={formatI18nTemplate(translateCurrentStaticSourceText("domains.catalog.TagsPage", "en", "/explore?tags={v0}"), { v0: String(encodeURIComponent(tag)) })}
                aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "{v0}, 작품 {v1}편"), { v0: String(tag), v1: String(count.toLocaleString("ko-KR")) })}
        ...
        <div className="mt-5 rounded-2xl border border-dashed border-line-strong bg-panel/50 p-8 text-center">
          ...
          <h2 className="mt-3 font-display text-lg font-bold text-fg">{translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "일치하는 태그가 없습니다.")}</h2>
          <p className="mt-2 text-sm leading-6 text-fg-3">{translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "검색어를 줄이거나 다른 표현으로 찾아보세요.")}</p>
          <button
            type="button"
            onClick={() => updateParam("q", "", "")}
          ...
          >
            {translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "검색 초기화")}</button>
        </div>
      )}
      ...
      {visible.length < filtered.length ? (
        <div className="mt-7 flex justify-center">
          ...
          >
            {translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "다음 ")}{Math.min(VISIBLE_STEP, filtered.length - visible.length).toLocaleString("ko-KR")}{translateCurrentStaticSourceText("domains.catalog.TagsPage", "ko", "개 태그 보기")}</button>
        </div>
      ) : null}
    </Container>
  );
