...
export function LibraryBackupImport({ onRestore, locale: _locale, ownerId }: {
  ...
  async function selectFile(file: File) {
    ...
    try {
      ...
      if (generation.current !== ticket) return;
      setFailed(true);
      setMessage(bi("올바른 ToonStudio 서재 백업 JSON 파일(최대 5MB)을 선택해 주세요. 기존 기록은 변경하지 않았습니다.", "Choose a valid ToonStudio library backup JSON file (up to 5MB). Existing records were not changed."));
    } finally {
  ...
  return (
    <div className="w-full min-w-0 max-w-md text-sm" data-library-import="">
      <button ref={trigger} type="button" disabled={reading} onClick={() => input.current?.click()} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-line px-3 py-2 font-medium text-fg-2 hover:bg-raised disabled:opacity-60">
        <Upload size={14} aria-hidden="true" />{reading ? (bi("백업 확인 중…", "Checking backup…")) : (bi("백업 가져오기", "Import backup"))}
      </button>
      <input ref={input} type="file" accept="application/json,.json" className="hidden" aria-label={bi("서재 백업 파일", "Library backup file")} onChange={(event) => {
      ...
      {pending && <div ref={preview} tabIndex={-1} role="region" aria-labelledby={id} className="mt-3 rounded-xl border border-accent/35 bg-accent-soft/35 p-4 outline-none focus-visible:ring-2 focus-visible:ring-accent">
        ...
        <p className="mt-2 text-xs leading-6 text-fg-2">{formatI18nTemplate(String(bi("별점 {value0} · 읽기 기록 {value1} · 리뷰 {value2} · 컬렉션 {value3}", "{value0} ratings · {value1} reading records · {value2} reviews · {value3} collections")), { value0: Object.keys(pending.data.ratings).length, value1: Object.keys(pending.data.reads).length, value2: Object.keys(pending.data.reviews).length, value3: pending.data.collections.length })}</p>
        <p className="mt-2 text-xs leading-6 text-fg">{bi("확인하면 현재 브라우저의 서재 기록을 이 백업으로 교체합니다. 필요한 현재 기록은 먼저 내보내세요.", "Confirming replaces this browser’s library with this backup. Export any current records you need first.")}</p>
        <div className="mt-3 flex flex-wrap gap-2">
          <button type="button" className="min-h-11 rounded-lg border border-line bg-panel px-3 py-2 text-fg" onClick={() => { generation.current += 1; returnFocus.current = true; setReading(false); setPending(null); }}>{bi("취소", "Cancel")}</button>
          <button type="button" className="min-h-11 rounded-lg bg-accent px-3 py-2 font-semibold text-on-accent" onClick={() => {
            ...
            try {
              ...
              setMessage(bi("서재 백업을 복원했습니다.", "Library backup restored."));
            } catch {
              setFailed(true);
              setMessage(bi("브라우저 저장소에 기록하지 못했습니다. 저장 공간과 접근 권한을 확인해 주세요.", "Could not persist the library. Check browser storage space and permissions."));
            }
          }}>{bi("기존 기록 교체 확인", "Confirm replacing existing records")}</button>
        </div>
      </div>}
    </div>
  );
