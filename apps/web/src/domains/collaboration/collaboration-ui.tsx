...
export function CollabNotice({ children, error = false }: { children: ReactNode; error?: boolean }) {
  return <div role={error ? "alert" : "status"} className={`rounded-2xl border p-4 text-sm leading-relaxed ${error ? "border-bad/40 bg-bad/10 text-bad" : "border-accent/30 bg-accent/10 text-fg"}`}>{children}</div>;
}
export function CollabLogin() {
  return <CollabNotice>공고 열람은 누구나 가능해요. 등록·지원·저장은 로그인 후 이용할 수 있어요. <Link href="/me" className="font-bold underline underline-offset-4">로그인 / 회원가입</Link></CollabNotice>;
}
...
export function CollaborationCard({ post, onSave, busy }: { post: CollaborationPost; onSave: () => void; busy: boolean }) {
  ...
  return <article className="flex h-full flex-col rounded-2xl border border-line bg-panel p-5 transition-colors hover:border-accent/50">
    <div className="flex items-start justify-between gap-3">
      ...
      <button type="button" onClick={onSave} disabled={busy} aria-label={post.saved ? `${post.title} 저장 취소` : `${post.title} 저장`} aria-pressed={post.saved} className="inline-flex size-11 shrink-0 items-center justify-center rounded-xl border border-line text-accent hover:bg-raised disabled:opacity-50"><Bookmark size={18} fill={post.saved ? "currentColor" : "none"} aria-hidden="true" /></button>
    </div>
    <Link href={`/collaborate/${post.id}`} className="mt-4 block text-lg font-bold leading-snug text-fg hover:text-accent"><h2>{post.title}</h2></Link>
    ...
    <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2 border-t border-line pt-4 text-xs text-fg-3"><span className="inline-flex items-center gap-1"><MapPin size={13} aria-hidden="true" />{COLLABORATION_MODES[post.workMode]}{post.details.location ? ` · ${post.details.location}` : ""}</span><span className="inline-flex items-center gap-1"><CalendarDays size={13} aria-hidden="true" />{post.details.deadline || "상시 접수"}</span></div>
    <div className="mt-auto flex items-center justify-between gap-3 pt-4 text-xs"><span className="text-fg-3">{post.author.name}</span><span className={status === "open" ? "font-semibold text-accent" : "text-fg-3"}>{post.hidden ? "운영자 비공개" : COLLABORATION_STATUS[status]}</span></div>
  </article>;
}
export function CollaborationSafety() {
  return <section className="rounded-2xl border border-line bg-panel p-5"><h2 className="font-bold text-fg">안전한 협업을 위한 약속</h2><p className="mt-3 text-sm leading-7 text-fg-2">보수와 지급일, 회차·컷 수, 수정 횟수, 납품 형식, 저작권과 크레딧을 작업 전에 서면으로 합의하세요. 과도한 무상 테스트나 선입금을 요구하는 공고는 주의해 주세요.</p><p className="mt-3 text-xs leading-6 text-fg-3">게시판 이용은 무료입니다. ToonStudio는 결제·에스크로·계약 체결을 중개하거나 거래 이행을 보증하지 않아요. 개인정보와 미공개 원고는 공개 본문에 넣지 마세요.</p></section>;
}
