    ...
    {data && post && <div className="mt-5 grid items-start gap-7 lg:grid-cols-[minmax(0,1fr)_340px]">
      <div className="min-w-0 space-y-6">
        <header className="rounded-3xl border border-line bg-panel p-6 sm:p-8">
          ...
          <p className="mt-4 text-sm text-fg-3"><Link href={`/u/${encodeURIComponent(post.author.id)}`} className="underline underline-offset-4">{post.author.name}</Link> · {new Date(post.createdAt).toLocaleDateString("ko-KR")} 등록</p>
