...
export function MotionPanelFrame({ svgRef, preset, progress, caption, cast, image, bubble, sfx }: { svgRef: Ref<SVGSVGElement>; preset: MotionPresetId; progress: number; caption: string; cast: ComicCastId; image: string | null; bubble: boolean; sfx: boolean }) {
  ...
  return <svg ref={svgRef} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 960 600" className="motion-panel-frame" role="img" aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.play.lab.MotionPanelFrame", "ko", "{v0}의 모션 컷 미리보기. {v1}"), { v0: String(comicCast(cast).name), v1: String(caption) })}>
    <defs><clipPath id={formatI18nTemplate(translateCurrentStaticSourceText("domains.play.lab.MotionPanelFrame", "en", "{v0}-frame"), { v0: String(id) })}><rect x="14" y="14" width="932" height="572" rx="12" /></clipPath></defs>
    <rect width="960" height="600" fill={paper} />
    <g clipPath={formatI18nTemplate(translateCurrentStaticSourceText("domains.play.lab.MotionPanelFrame", "en", "url(#{v0}-frame)"), { v0: String(id) })}>
