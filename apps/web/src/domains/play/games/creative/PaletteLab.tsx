...
export default function PaletteLab() {
  ...
  return <div className="play-lab-content">
    <div className="play-exercise-heading"><div><span className="play-eyebrow">PALETTE LAB · SET THE MOOD</span><h2>한 장면의 분위기, 다섯 가지 색.</h2><p>배색을 바꾸고 마음에 드는 색은 잠가 두세요. 완성한 팔레트는 작업 자료로 가져갈 수 있습니다.</p></div>
      <button type="button" className="play-button primary" disabled={value.locks.every(Boolean)} onClick={() => { const generated = paletteFor(freshSeed(), value.harmony); setValue({ ...value, colors: value.colors.map((color, i) => value.locks[i] ? color : generated[i]), name: `${HARMONIES[value.harmony]} 창작 팔레트` }); setMessage("잠긴 색을 유지하며 새 배색을 만들었습니다."); }}><Shuffle size={16} />새 배색 만들기</button>
    </div>
    <div className="play-actions" aria-label="배색 규칙">{(Object.entries(HARMONIES) as [Harmony, string][]).map(([id, label]) => <button type="button" className="play-chip" key={id} aria-pressed={value.harmony === id} onClick={() => setValue({ ...value, harmony: id })}>{label}</button>)}</div>
    <div className="play-palette">{value.colors.map((color, index) => <div className="play-palette-color" key={index} style={{ background: color }}><div className="play-palette-controls">
      <button type="button" className="play-icon-button" aria-label={`${index + 1}번 색 ${value.locks[index] ? "잠금 해제" : "잠그기"}`} aria-pressed={value.locks[index]} onClick={() => setValue({ ...value, locks: value.locks.map((lock, i) => i === index ? !lock : lock) })}>{value.locks[index] ? <LockKeyhole size={16} /> : <UnlockKeyhole size={16} />}</button>
      <label><span className="sr-only">{index + 1}번 색 직접 선택</span><input type="color" value={color} onChange={(event) => setValue({ ...value, colors: value.colors.map((c, i) => i === index ? event.target.value : c) })} /><code>{color.toUpperCase()}</code></label>
    </div></div>)}</div>
