...
export function HelpCenterPage() {
  ...
  const filtered = TOPICS.filter((topic) => {
      ...
      const haystack = bi((topic).ko, (topic).en).join(" ").toLocaleLowerCase();
      return normalizedQuery.split(/\s+/u).every((token) => haystack.includes(token));
    });

  useDocumentTitle(bi("도움말", "Help Center"));
  ...
  return (
    <Container size="wide" className="py-7 sm:py-10 lg:py-12">
      <PublicStoryHero
        ...
        image="materials"
        imageAlt={bi("브러시와 드로잉 재료의 질감을 보여주는 웹툰 작업실 콘셉트 아트", "Webtoon atelier concept art featuring drawing tools and material textures")}
        caption={bi("YOUR CREATIVE COMPANION · 드로잉 재료 콘셉트 아트", "YOUR CREATIVE COMPANION · Drawing materials concept art")}
      >
        ...
        <div className="mt-4 flex flex-wrap gap-2" aria-label={bi("자주 찾는 도움말", "Popular help topics")}>
          {(bi(["브러시", "저장", "말풍선", "라이선스"], ["brushes", "save", "balloons", "license"])).map((term) => <button key={term} type="button" onClick={() => setQuery(term)} aria-pressed={query === term} className="min-h-11 rounded-lg border border-line px-3 text-xs text-fg-2 transition-colors hover:border-accent hover:text-accent aria-pressed:border-accent aria-pressed:text-accent">{term}</button>)}
        </div>
      </PublicStoryHero>
        ...
        {filtered.length ? (
          <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {filtered.map((topic) => {
              ...
              const [title, description] = bi((topic).ko, (topic).en);
              return (
      ...
      <section className="mt-12 border-t border-line pt-8" aria-labelledby="help-questions-title">
        <h2 id="help-questions-title" className="text-2xl font-bold tracking-tight text-fg">{bi("작업을 시작하기 전에.", "Before you start drawing.")}</h2>
        <div className="mt-5 divide-y divide-line border-y border-line">
          {[
            { ko: ["웹툰 작업은 어디서 시작하나요?", "새로 만들기에서 만들고 싶은 작업을 고르세요. 도구가 낯설다면 Studio 사용법에서 캔버스·레이어·브러시·말풍선 순서로 살펴볼 수 있습니다."], en: ["Where do I start a webtoon?", "Choose what to make from Create. If the tools are new to you, the Studio manual walks through the canvas, layers, brushes and speech balloons."], href: "/make", link: bi("첫 작업 시작", "Start your first work") },
            { ko: ["로컬 저장과 백업은 어떻게 다른가요?", "로컬 작업은 현재 기기와 브라우저에 저장됩니다. 브라우저 데이터를 지우거나 기기를 바꾸기 전에 중요한 작업을 파일로 내보내 보관하세요. 로컬 복구가 클라우드 백업을 대신하지는 않습니다."], en: ["How does local saving differ from a backup?", "Local work is stored in your current browser and device. Export important work before clearing browser data or moving to another device. Local recovery does not replace a cloud backup."], href: "/studio/manual", link: bi("저장·내보내기 안내", "Saving and export guide") },
            { ko: ["마켓 리소스를 작품에 사용해도 되나요?", "각 리소스의 라이선스와 출처, 상업 이용·수정·재배포 조건을 먼저 확인하세요. 같은 마켓 안에서도 사용 조건은 리소스마다 다를 수 있습니다."], en: ["Can I use marketplace resources in my work?", "Check each resource's license, source and terms for commercial use, modification and redistribution. Conditions can differ between resources."], href: "/market", link: bi("리소스와 사용 조건 확인", "Explore resources and terms") },
          ].map((entry) => <details key={entry.href} className="group py-5"><summary className="cursor-pointer text-sm font-semibold text-fg marker:text-accent">{bi((entry).ko, (entry).en)[0]}</summary><p className="mt-4 max-w-3xl text-sm leading-7 text-fg-2">{bi((entry).ko, (entry).en)[1]}</p><Link href={entry.href} className="mt-2 inline-flex min-h-11 items-center gap-2 text-xs font-bold text-accent">{entry.link}<ArrowRight size={14} aria-hidden="true" /></Link></details>)}
        </div>
      </section>
