...
export function EngineeringLicensesPage() {
  ...
  useDocumentTitle(
    ko
      ? "ToonStudio 오픈소스와 라이선스 · 코드부터 생성 결과까지"
      : "ToonStudio open source and licensing · From code to generated output",
  );
  ...
  return (
    <Container size="wide" className="py-7 sm:py-10 lg:py-12">
      ...
      <EngineeringPageIntro
        ...
        title={
          ko
            ? "라이선스는 패키지 이름이 아니라 배포 방식과 자산의 권리까지 함께 봅니다."
            : "Licensing is reviewed with distribution and asset rights, not package names alone."
        }
        description={
          ko
            ? "ToonStudio는 실행 코드, 개발 도구, 폰트·이미지·브러시·3D, 외부 API, AI 모델과 생성 결과를 별도의 권리 계층으로 관리합니다. 이 페이지는 법률 자문을 대신하지 않으며, 실제 배포 시점의 버전과 약관을 다시 확인합니다."
            : "ToonStudio tracks runtime code, development tools, fonts, images, brushes, 3D assets, external APIs, AI models and generated output as separate rights layers. This page is not legal advice; versions and terms are rechecked at distribution time."
        }
        aside={
          <div className="rounded-3xl border border-accent/25 bg-accent-soft/30 p-5">
            ...
            <p className="mt-4 text-sm font-black text-fg">
              {ko ? "고지 생성과 라이선스 audit를 CI에 포함" : "Notice generation and license audit run in CI"}
            </p>
            <p className="mt-2 text-xs leading-6 text-fg-3">
              {ko
                ? "자동 수집 결과는 출발점이며, 자산과 외부 약관은 사람이 검토한 기록을 함께 유지합니다."
                : "Automated inventory is the starting point; human review records are retained for assets and external terms."
              }
            </p>
          </div>
        }
      ...
      <section aria-labelledby="open-source-map-title">
        ...
        <h2 id="open-source-map-title" className="mt-3 text-2xl font-black tracking-tight text-fg sm:text-3xl">
          {ko ? "사용한 기술과 소유하지 않는 역할을 함께 공개합니다." : "Document what each technology does—and what it does not own."}
        </h2>
        <div className="mt-7 grid gap-4 lg:grid-cols-3">
          {OPEN_SOURCE_ROLES.map((item) => (
            <article key={item.role.en} className="rounded-[1.75rem] border border-line/70 bg-panel/60 p-6 shadow-sm">
              ...
              <h3 className="mt-4 text-lg font-black text-fg">{item.role[locale]}</h3>
      ...
      <section className="mt-16" aria-labelledby="rights-layers-title">
        ...
        <h2 id="rights-layers-title" className="mt-3 text-2xl font-black tracking-tight text-fg sm:text-3xl">
          {ko ? "코드 라이선스 하나로 모든 권리를 판단하지 않습니다." : "One code license cannot answer every rights question."}
        </h2>
          ...
          {RIGHTS_LAYERS.map((layer, index) => (
            <article key={layer.title.en} className="rounded-[1.75rem] border border-line/70 bg-card/70 p-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-lg font-black text-fg">{layer.title[locale]}</h3>
                  <p className="mt-3 text-sm leading-7 text-fg-2">{layer.rule[locale]}</p>
                </div>
              </div>
      ...
      <section className="mt-16" aria-labelledby="license-groups-title">
        ...
        <h2 id="license-groups-title" className="mt-3 text-2xl font-black tracking-tight text-fg sm:text-3xl">
          {ko ? "종류별 기본 의무와 추가 확인 지점" : "Baseline obligations and review points by license family"}
        </h2>
        <p className="mt-4 max-w-3xl text-sm leading-7 text-fg-2">
          {ko
            ? "아래 내용은 빠른 분류 기준입니다. 정확한 의무는 설치한 버전의 원문, 수정·결합·배포 방식과 관할 법률에 따라 검토합니다."
            : "These are triage rules. Exact obligations depend on the installed version's text, modification, combination and distribution model, and applicable law."
          }
        </p>
        <div className="mt-7 overflow-hidden rounded-[1.75rem] border border-line/70 bg-panel/55">
          <div className="hidden grid-cols-[0.8fr_1.1fr_1.45fr_1.45fr] gap-5 border-b border-line bg-raised/70 px-6 py-3 font-display text-[0.64rem] font-black uppercase tracking-[0.12em] text-fg-3 lg:grid">
            <span>{ko ? "종류" : "Family"}</span>
            <span>{ko ? "예시" : "Examples"}</span>
            <span>{ko ? "기본 의무" : "Baseline obligation"}</span>
            <span>{ko ? "주의" : "Caution"}</span>
          </div>
          {ENGINEERING_LICENSE_GROUPS.map((group) => (
            <article key={group.id} className="grid gap-4 border-b border-line/70 px-5 py-6 last:border-b-0 sm:px-6 lg:grid-cols-[0.8fr_1.1fr_1.45fr_1.45fr] lg:gap-5">
              <div>
                <span className="lg:hidden text-[0.62rem] font-black uppercase tracking-[0.12em] text-fg-3">{ko ? "종류" : "Family"}</span>
                <h3 className="mt-1 font-black text-fg lg:mt-0">{group.title[locale]}</h3>
              </div>
              <div>
                <span className="lg:hidden text-[0.62rem] font-black uppercase tracking-[0.12em] text-fg-3">{ko ? "예시" : "Examples"}</span>
                <p className="mt-1 text-xs leading-6 text-fg-2 lg:mt-0">{group.examples.join(" · ")}</p>
              </div>
              <div>
                <span className="lg:hidden text-[0.62rem] font-black uppercase tracking-[0.12em] text-fg-3">{ko ? "기본 의무" : "Baseline obligation"}</span>
                <p className="mt-1 flex items-start gap-2 text-xs leading-6 text-fg-2 lg:mt-0">
                  ...
                  <span>{group.obligation[locale]}</span>
                </p>
              </div>
              <div>
                <span className="lg:hidden text-[0.62rem] font-black uppercase tracking-[0.12em] text-fg-3">{ko ? "주의" : "Caution"}</span>
                <p className="mt-1 flex items-start gap-2 text-xs leading-6 text-fg-2 lg:mt-0">
                  <AlertTriangle size={14} className="mt-1 shrink-0 text-accent" aria-hidden="true" />
                  <span>{group.caution[locale]}</span>
                </p>
              </div>
            </article>
          ))}
      ...
      <section className="mt-16 grid gap-6 lg:grid-cols-[0.95fr_1.05fr]" aria-labelledby="license-pipeline-title">
        <div className="rounded-[2rem] border border-line/70 bg-panel/65 p-6 shadow-sm sm:p-8">
          ...
          <h2 id="license-pipeline-title" className="mt-3 text-2xl font-black tracking-tight text-fg">
            {ko ? "고지는 자동화하고, 판단은 검토 기록으로 남깁니다." : "Automate notices and record human judgment."}
          </h2>
        ...
        <div className="space-y-4">
          <article className="rounded-[1.75rem] border border-line/70 bg-card/70 p-6">
            ...
            <p className="mt-3 text-sm leading-7 text-fg-2">
              {ko
                ? "빌드 후 배포물에 포함되는 제3자 코드 고지를 생성합니다. 생성 결과가 현재 의존성과 달라지면 audit가 실패해 고지 누락을 조기에 드러냅니다."
                : "The build generates third-party code notices for distribution. License audit exposes missing updates when the generated result diverges from current dependencies."
              }
            </p>
          ...
          <article className="rounded-[1.75rem] border border-line/70 bg-card/70 p-6">
            ...
            <h3 className="mt-4 text-lg font-black text-fg">
              {ko ? "Remotion은 일반적인 permissive 패키지로 단정하지 않습니다." : "Remotion is not assumed to be an ordinary permissive package."}
            </h3>
            <p className="mt-3 text-sm leading-7 text-fg-2">
              {ko
                ? "조직 규모, 제작 방식과 자동 렌더 사용량에 따라 적용 조건이 달라질 수 있으므로 영상 제작 또는 렌더 인프라 변경 시 공식 라이선스 정책을 다시 확인합니다."
                : "Applicable terms can depend on organization size, production mode and automated rendering volume, so the official policy is rechecked when authoring or render infrastructure changes."
              }
            </p>
            <a
            ...
            >
              {ko ? "Remotion 공식 라이선스 확인" : "Review Remotion licensing"}
            </a>
          </article>
          ...
          <article className="rounded-[1.75rem] border border-accent/25 bg-accent-soft/25 p-6">
            ...
            <h3 className="mt-4 text-lg font-black text-fg">
              {ko ? "출시 전 반드시 사람이 확인할 항목" : "Human review required before release"}
            </h3>
            <ul className="mt-4 space-y-2 text-sm leading-7 text-fg-2">
              {[
                ko ? "WASM·정적 링크와 copyleft 결합 방식" : "WASM, static linking and copyleft combination",
                ko ? "상업 폰트·이미지·브러시·3D·사운드의 프로젝트별 사용권" : "Project-specific rights for commercial fonts, images, brushes, 3D and sound",
                ko ? "AI 모델·API 약관, 입력 권리와 생성 결과의 배포 범위" : "AI model/API terms, input rights and generated-output distribution",
                ko ? "외부 서비스의 로고·브랜딩·심사·데이터 보존 조건" : "External-service logo, branding, review and data-retention conditions",
              ].map((item) => (
      ...
      <aside className="mt-12 flex items-start gap-4 rounded-3xl border border-line/70 bg-raised/70 p-5" role="note">
        ...
        <div>
          <p className="font-black text-fg">{ko ? "검토 기준" : "Review standard"}</p>
          <p className="mt-2 text-xs leading-6 text-fg-2">
            {ko
              ? "페이지의 예시는 기술 교육을 위한 분류이며 법률 의견이 아닙니다. 실제 출시에서는 라이선스 원문, 계약, 배포 국가와 조직 정책에 따라 담당자 또는 법률 전문가의 검토를 거칩니다."
              : "Examples are engineering education categories, not legal opinions. A real release is reviewed against original license text, contracts, distribution jurisdictions and organizational policy by accountable owners or legal counsel."
            }
          </p>
        </div>
      </aside>
