      ...
      <ol className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        {steps.map((step, index) => (
          <li key={step.en} className="relative rounded-2xl border border-line/65 bg-panel/70 p-4">
            ...
            <p className="mt-3 text-xs leading-6 text-fg-2">{step[locale]}</p>
...
export function EngineeringFieldNotesPage() {
  ...
  const notes = useMemo(() => {
    ...
    return candidates.filter((note) => [
      ...
      note.problem[locale],
      note.pattern[locale],
      note.boundary[locale],
      ENGINEERING_FIELD_CATEGORY_META[note.category].label[locale],
      ...note.technologies,
    ].join(" ").toLocaleLowerCase(locale === "ko" ? "ko-KR" : "en-US").includes(normalizedQuery));
  }, [filter, locale, normalizedQuery]);
  const inventoryCards = [
    { icon: Workflow, value: ENGINEERING_IMPLEMENTATION_INVENTORY.workerEntries, label: ko ? "전용 Worker 엔트리" : "Dedicated worker entries" },
    { icon: Code2, value: ENGINEERING_IMPLEMENTATION_INVENTORY.workerClients, label: ko ? "Worker 클라이언트" : "Worker clients" },
    { icon: RefreshCw, value: ENGINEERING_IMPLEMENTATION_INVENTORY.serviceWorkerRuntimeFiles, label: ko ? "Service Worker 런타임 모듈" : "Service-worker runtime modules" },
    { icon: Bot, value: ENGINEERING_IMPLEMENTATION_INVENTORY.localInferenceRuntimes.length, label: ko ? "브라우저 로컬 AI 런타임" : "Browser-local AI runtimes" },
    { icon: Boxes, value: ENGINEERING_IMPLEMENTATION_INVENTORY.blenderMcpCommands, label: ko ? "Blender MCP 허용 명령" : "Allowlisted Blender MCP commands" },
    { icon: PlugZap, value: ENGINEERING_IMPLEMENTATION_INVENTORY.openApiProviders, label: ko ? "검증된 Open API adapter" : "Reviewed Open API adapters" },
  ] as const;

  useDocumentTitle(
    ko
      ? "ToonStudio 기술 심화 노트 · Worker, PWA, AI, Blender, 3D와 문제 해결"
      : "ToonStudio engineering field notes · Workers, PWA, AI, Blender, 3D and troubleshooting",
  );
  ...
  return (
    <Container size="wide" className="py-7 sm:py-10 lg:py-12">
      ...
      <EngineeringPageIntro
        ...
        title={
          ko
            ? "실제 구현에서 남은 기술 판단과 실패 복구 과정을 더 깊게 공개합니다."
            : "Deeper implementation decisions and recovery lessons from the real product."
        }
        description={
          ko
            ? "Worker와 PWA, 무료 우선 AI·인프라, Blender MCP, 멀티 3D 엔진, Open API와 트러블슈팅을 다른 개발에서도 재사용할 수 있는 경계와 절차로 정리했습니다. 운영 중·설정 필요·실험·참고 상태를 구분하고 모든 주장을 저장소 근거와 연결합니다."
            : "Workers, PWA, free-first AI and infrastructure, Blender MCP, multi-engine 3D, Open APIs and troubleshooting are documented as reusable boundaries and procedures. Live, configured, experimental and reference states remain distinct and evidence-backed."
        }
        aside={
          <div className="rounded-3xl border border-accent/25 bg-accent-soft/30 p-5">
            ...
            <p className="mt-4 text-sm font-black text-fg">
              {ko ? "패키지보다 실패 경계와 복구 계약" : "Failure boundaries over package lists"}
            </p>
            <p className="mt-2 text-xs leading-6 text-fg-3">
              {ko
                ? "같은 기술을 쓰지 않아도 입력·출력·권위·예산·취소·복구를 유지하면 설계를 재사용할 수 있습니다."
                : "The design remains reusable when inputs, outputs, authority, budgets, cancellation and recovery survive a technology change."
              }
            </p>
          </div>
        }
      ...
      <section className="mb-8" aria-labelledby="implementation-inventory-title">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 id="implementation-inventory-title" className="mt-3 text-2xl font-black tracking-tight text-fg sm:text-3xl">
              {ko ? "설명에 사용한 구현 수치를 저장소와 함께 검증합니다." : "Implementation figures are verified against the repository."}
            </h2>
          </div>
          <p className="text-xs text-fg-3">
            {ko ? "검토일" : "Reviewed"} · {ENGINEERING_IMPLEMENTATION_INVENTORY.reviewedAt}
          </p>
        </div>
        ...
        <p className="mt-3 text-xs leading-6 text-fg-3">
          {ko
            ? "Worker 수치는 apps/web/src의 전용 worker와 worker-client 파일을 구분해 계산하며, 변경 시 콘텐츠 계약 테스트가 실패합니다."
            : "Worker figures count dedicated worker and worker-client files separately under apps/web/src; content-contract tests fail when they drift."
          }
        </p>
      </section>
      ...
      <section aria-labelledby="field-note-filter-title">
        <h2 id="field-note-filter-title" className="sr-only">
          {ko ? "기술 노트 검색과 분야 필터" : "Engineering field-note search and filters"}
        </h2>
        <div className="grid gap-4 rounded-3xl border border-line/70 bg-panel/55 p-3 lg:grid-cols-[minmax(16rem,1fr)_auto] lg:items-end">
          <div>
            <label htmlFor="engineering-field-note-search" className="px-1 text-xs font-black text-fg">
              {ko ? "기술 노트 검색" : "Search field notes"}
            </label>
            <div className="relative mt-2">
              ...
              <input
                ...
                placeholder={ko ? "예: Worker, MediaPipe, Blender, OPFS" : "e.g. Worker, MediaPipe, Blender, OPFS"}
              ...
              {ko ? `${notes.length}개 / 전체 ${ENGINEERING_FIELD_NOTES.length}개 노트` : `${notes.length} of ${ENGINEERING_FIELD_NOTES.length} notes`}
            </p>
          </div>
      ...
      <section className="mt-8 grid gap-5" aria-live="polite" aria-label={ko ? "심화 기술 노트" : "Engineering field notes"}>
        {notes.length === 0 ? (
          <div className="rounded-[2rem] border border-dashed border-line-strong bg-card/45 p-8 text-center" role="status">
            ...
            <p className="mt-4 font-black text-fg">{ko ? "검색 조건과 일치하는 기술 노트가 없습니다." : "No field notes match the current search."}</p>
            <p className="mt-2 text-sm leading-7 text-fg-3">{ko ? "검색어를 줄이거나 전체 분야를 선택해 보세요." : "Shorten the query or select all categories."}</p>
          </div>
        ) : notes.map((note) => {
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-fg-3">{category.label[locale]}</p>
                  </div>
                </div>
              ...
              <dl className="mt-7 grid gap-3 lg:grid-cols-3">
                <div className="rounded-3xl border border-line/65 bg-card/65 p-5">
                  <dt className="text-[0.66rem] font-black uppercase tracking-[0.14em] text-danger">{ko ? "문제" : "Problem"}</dt>
                  <dd className="mt-3 text-sm leading-7 text-fg-2">{note.problem[locale]}</dd>
                </div>
                <div className="rounded-3xl border border-line/65 bg-card/65 p-5">
                  <dt className="text-[0.66rem] font-black uppercase tracking-[0.14em] text-accent">{ko ? "적용 패턴" : "Pattern"}</dt>
                  <dd className="mt-3 text-sm leading-7 text-fg-2">{note.pattern[locale]}</dd>
                </div>
                <div className="rounded-3xl border border-line/65 bg-card/65 p-5">
                  <dt className="text-[0.66rem] font-black uppercase tracking-[0.14em] text-warning">{ko ? "경계와 한계" : "Boundary"}</dt>
                  <dd className="mt-3 text-sm leading-7 text-fg-2">{note.boundary[locale]}</dd>
                </div>
              </dl>
              ...
              <div className="mt-7 grid gap-5 xl:grid-cols-[1fr_1fr]">
                <section aria-labelledby={`${note.id}-reuse-title`} className="rounded-3xl border border-line/70 bg-card/50 p-5">
                  <h3 id={`${note.id}-reuse-title`} className="flex items-center gap-2 text-sm font-black text-fg">
                    ...
                    {ko ? "다른 프로젝트 적용 순서" : "Reuse sequence"}
                  </h3>
                ...
                <section aria-labelledby={`${note.id}-evidence-title`} className="rounded-3xl border border-line/70 bg-card/50 p-5">
                  <h3 id={`${note.id}-evidence-title`} className="flex items-center gap-2 text-sm font-black text-fg">
                    ...
                    {ko ? "코드·테스트·공식 자료" : "Code, tests and official sources"}
                  </h3>
                    ...
                    {note.references.map((reference) => (
                      <li key={reference.id}>
                        ...
                        >
                          <span>
                            {reference.title}
                            <span className="mt-1 block text-[0.62rem] font-normal leading-5 text-fg-3">{reference.note[locale]}</span>
                          </span>
      ...
      <section className="py-14 sm:py-20" aria-labelledby="worker-pwa-pattern-title">
        ...
        <h2 id="worker-pwa-pattern-title" className="mt-3 text-balance text-2xl font-black tracking-tight text-fg sm:text-3xl">
          {ko ? "Worker와 PWA는 서로 다른 실패를 격리하는 두 개의 실행 계층입니다." : "Workers and PWA isolate two different classes of failure."}
        </h2>
        <div className="mt-7 grid gap-5 xl:grid-cols-2">
          <ProcessRail
            title={ko ? "무거운 계산의 Worker 경계" : "Worker boundary for heavy compute"}
            description={ko ? "메인 스레드의 포인터·캔버스 응답성을 지키면서도 계산 결과의 문서 commit 권위는 넘기지 않습니다." : "Protect main-thread pointer and canvas responsiveness without handing document commit authority to workers."}
            steps={WORKER_PIPELINE}
            icon={Workflow}
          />
          <ProcessRail
            title={ko ? "PWA 설치·오프라인·업데이트 경계" : "PWA install, offline and update boundary"}
            description={ko ? "설치 가능성과 캐시 편의보다 오래된 실행 코드가 작업 데이터를 손상시키지 않는 것을 우선합니다." : "Preventing stale runtime code from damaging work takes priority over install and cache convenience."}
            steps={PWA_PIPELINE}
            icon={RefreshCw}
          />
        </div>
      ...
      <section aria-labelledby="open-api-title">
        ...
        <h2 id="open-api-title" className="mt-3 text-2xl font-black tracking-tight text-fg sm:text-3xl">
          {ko ? "Open API마다 데이터보다 먼저 권리·출처·실패 규칙을 정의했습니다." : "Rights, provenance and failure rules precede data for every Open API."}
        </h2>
        <p className="mt-4 max-w-4xl text-sm leading-7 text-fg-2">
          {ko
            ? "API가 공개되어 있다는 사실과 결과물을 다시 배포할 수 있다는 사실은 다릅니다. 아래 adapter는 제공처마다 다른 공개 이용 flag, 상세 조회, image host와 오류를 독립적으로 검증합니다."
            : "Public API access does not automatically grant redistribution rights. Each adapter validates provider-specific rights flags, detail data, image hosts and errors independently."
          }
        </p>
            ...
            <article key={api.id} className="rounded-[1.75rem] border border-line/70 bg-panel/60 p-5 shadow-sm sm:p-6">
              ...
              <dl className="mt-5 space-y-3 text-xs leading-6">
                <div className="rounded-2xl border border-line/65 bg-card/60 p-4">
                  <dt className="font-black text-accent">{ko ? "활용" : "Purpose"}</dt>
                  <dd className="mt-1 text-fg-2">{api.purpose[locale]}</dd>
                </div>
                <div className="rounded-2xl border border-line/65 bg-card/60 p-4">
                  <dt className="font-black text-fg">{ko ? "접근 계약" : "Access contract"}</dt>
                  <dd className="mt-1 text-fg-2">{api.access[locale]}</dd>
                </div>
                <div className="rounded-2xl border border-line/65 bg-card/60 p-4">
                  <dt className="font-black text-success">{ko ? "권리 gate" : "Rights gate"}</dt>
                  <dd className="mt-1 text-fg-2">{api.rightsGate[locale]}</dd>
                </div>
                <div className="rounded-2xl border border-line/65 bg-card/60 p-4">
                  <dt className="font-black text-warning">{ko ? "실패·복구" : "Failure and recovery"}</dt>
                  <dd className="mt-1 text-fg-2">{api.resilience[locale]}</dd>
                </div>
              </dl>
              <div className="mt-4 flex flex-col gap-2">
                ...
                <a href={api.officialUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center gap-2 self-start rounded-xl border border-line-strong bg-card px-3 py-2 text-xs font-bold text-fg-2 hover:border-accent/40 hover:text-accent">
                  {ko ? "공식 API 문서" : "Official API documentation"}
      ...
      <section className="py-14 sm:py-20" aria-labelledby="troubleshooting-title">
        ...
        <h2 id="troubleshooting-title" className="mt-3 text-balance text-2xl font-black tracking-tight text-fg sm:text-3xl">
          {ko ? "실패를 숨기지 않고 재현 가능한 회귀 계약으로 바꾼 사례" : "Failures converted into reproducible regression contracts"}
        </h2>
        <div className="mt-7 grid gap-5 xl:grid-cols-2">
          {ENGINEERING_TROUBLESHOOTING_CASES.map((item, index) => (
            <article key={item.id} className="rounded-[2rem] border border-line/70 bg-card/65 p-5 shadow-sm sm:p-7">
              ...
              <h3 className="mt-5 text-balance text-xl font-black tracking-tight text-fg">{item.title[locale]}</h3>
              <dl className="mt-5 space-y-3">
                <div className="rounded-2xl border border-danger/20 bg-danger-soft/10 p-4">
                  <dt className="flex items-center gap-2 text-xs font-black text-danger"><AlertTriangle size={14} aria-hidden="true" />{ko ? "증상" : "Symptom"}</dt>
                  <dd className="mt-2 text-xs leading-6 text-fg-2">{item.symptom[locale]}</dd>
                </div>
                <div className="rounded-2xl border border-line/65 bg-panel/65 p-4">
                  <dt className="flex items-center gap-2 text-xs font-black text-fg"><Bug size={14} className="text-accent" aria-hidden="true" />{ko ? "근본 원인" : "Root cause"}</dt>
                  <dd className="mt-2 text-xs leading-6 text-fg-2">{item.rootCause[locale]}</dd>
                </div>
                <div className="rounded-2xl border border-success/25 bg-success-soft/15 p-4">
                  <dt className="flex items-center gap-2 text-xs font-black text-success"><Hammer size={14} aria-hidden="true" />{ko ? "수정" : "Fix"}</dt>
                  <dd className="mt-2 text-xs leading-6 text-fg-2">{item.fix[locale]}</dd>
                </div>
                <div className="rounded-2xl border border-accent/25 bg-accent-soft/15 p-4">
                  <dt className="flex items-center gap-2 text-xs font-black text-accent"><ShieldCheck size={14} aria-hidden="true" />{ko ? "재발 방지" : "Prevention"}</dt>
                  <dd className="mt-2 text-xs leading-6 text-fg-2">{item.prevention[locale]}</dd>
                </div>
              </dl>
              <ul className="mt-4 space-y-2">
                {item.evidence.map((evidence) => (
                  <li key={evidence.path} className="rounded-xl border border-line/60 bg-panel/60 px-3 py-2">
                    <p className="text-[0.65rem] font-bold text-fg">{evidence.label[locale]}</p>
      ...
      <section aria-labelledby="reference-products-title">
        ...
        <h2 id="reference-products-title" className="mt-3 text-balance text-2xl font-black tracking-tight text-fg sm:text-3xl">
          {ko ? "참고한 제품과 실제로 채택한 패턴, 채택하지 않은 이유를 구분합니다." : "Reference products are separated from applied patterns and rejected adoption choices."}
        </h2>
            ...
            return (
              <article key={product.id} className="flex flex-col rounded-[1.75rem] border border-line/70 bg-panel/60 p-5 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="rounded-full border border-line bg-card px-3 py-1.5 text-[0.64rem] font-black text-fg-2" title={role.description[locale]}>
                    {role.label[locale]}
                  </span>
                </div>
                ...
                <dl className="mt-4 flex-1 space-y-3 text-xs leading-6">
                  <div>
                    <dt className="font-black text-accent">{ko ? "참고한 점" : "Lesson"}</dt>
                    <dd className="mt-1 text-fg-2">{product.lesson[locale]}</dd>
                  </div>
                  <div>
                    <dt className="font-black text-success">{ko ? "적용" : "Applied"}</dt>
                    <dd className="mt-1 text-fg-2">{product.applied[locale]}</dd>
                  </div>
                  <div>
                    <dt className="font-black text-warning">{ko ? "경계" : "Boundary"}</dt>
                    <dd className="mt-1 text-fg-2">{product.boundary[locale]}</dd>
                  </div>
                </dl>
      ...
      <aside className="mt-12 flex items-start gap-4 rounded-[2rem] border border-accent/25 bg-accent-soft/25 p-6" role="note">
        ...
        <div>
          <p className="font-black text-fg">{ko ? "검토일과 상태는 계속 갱신해야 합니다." : "Review dates and statuses must keep moving."}</p>
          <p className="mt-2 max-w-4xl text-sm leading-7 text-fg-2">
            {ko
              ? "무료 티어, API 약관, 브라우저 지원과 엔진 기능은 바뀔 수 있습니다. 이 페이지는 2026년 9월 17일 저장소 구현과 공식 자료를 기준으로 하며, 실제 도입 시 공식 문서·라이선스·가격·브라우저 호환성을 다시 확인해야 합니다."
              : "Free tiers, API terms, browser support and engine capability can change. This page reflects repository implementation and official material reviewed on September 17, 2026; adoption requires rechecking current docs, licenses, pricing and compatibility."
            }
          </p>
        </div>
      </aside>
