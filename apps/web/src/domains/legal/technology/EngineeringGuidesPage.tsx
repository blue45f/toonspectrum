...
export function EngineeringGuidesPage() {
  ...
  useDocumentTitle(
    ko
      ? "ToonStudio 기술 적용 가이드 · 다른 프로젝트에서 재사용하기"
      : "ToonStudio implementation guides · Reuse the engineering patterns",
  );
  ...
  return (
    <Container size="wide" className="py-7 sm:py-10 lg:py-12">
      ...
      <EngineeringPageIntro
        ...
        title={
          ko
            ? "설명에서 끝내지 않고, 다른 서비스에 옮길 수 있는 단계로 정리했습니다."
            : "The story is translated into steps another service can actually adopt."
        }
        description={
          ko
            ? "각 가이드는 결과, 구현 순서와 완료 체크리스트를 함께 제공합니다. 코드 예시는 비밀값과 실제 계정 정보를 포함하지 않으며, 공급자 정책과 라이선스는 적용 시점에 다시 검토해야 합니다."
            : "Each guide includes the intended outcome, implementation order and completion checklist. Examples contain no secrets or real account data, and provider policies and licenses must be reviewed at adoption time."
        }
        aside={
          <div className="rounded-3xl border border-accent/25 bg-accent-soft/35 p-5">
            ...
            <p className="mt-4 text-sm font-black text-fg">
              {ko ? "가져가야 하는 것은 패키지가 아니라 경계입니다." : "Reuse boundaries, not just packages."}
            </p>
            <p className="mt-2 text-xs leading-6 text-fg-2">
              {ko
                ? "입력·출력·권위·실패·대체 경로를 유지하면 기술을 바꿔도 제품 계약을 지킬 수 있습니다."
                : "Preserve inputs, outputs, authority, failure and fallback so the product contract survives technology changes."
              }
            </p>
          </div>
        }
      ...
      <section aria-labelledby="guide-filter-title">
        <h2 id="guide-filter-title" className="sr-only">
          {ko ? "가이드 상태 필터" : "Guide status filters"}
        </h2>
          ...
          <article key={guide.id} id={guide.id} className="scroll-mt-28 rounded-[2rem] border border-line/70 bg-panel/55 p-5 shadow-sm sm:p-7 lg:p-8">
            ...
            <div className="mt-6 grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
              <div>
                ...
                <p className="mt-4 text-sm leading-7 text-fg-2">{guide.summary[locale]}</p>
                ...
                <div className="mt-6 rounded-3xl border border-success/30 bg-success-soft/20 p-5">
                  ...
                  <p className="mt-3 text-sm leading-7 text-fg-2">{guide.outcome[locale]}</p>
                </div>
              ...
              <div className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
                <section aria-labelledby={`${guide.id}-steps-title`}>
                  <h3 id={`${guide.id}-steps-title`} className="text-sm font-black text-fg">
                    {ko ? "구현 순서" : "Implementation sequence"}
                  </h3>
                  <ol className="mt-4 space-y-3">
                    {guide.steps.map((step, index) => (
                      <li key={step.ko} className="flex items-start gap-3 rounded-2xl border border-line/65 bg-card/65 p-4">
                        ...
                        <p className="pt-1 text-xs leading-6 text-fg-2">{step[locale]}</p>
                      </li>
                    ))}
                  </ol>
                </section>
                  ...
                  <ul className="mt-4 space-y-3">
                    {guide.checklist.map((item) => (
                      <li key={item.ko} className="flex items-start gap-3 rounded-2xl border border-line/65 bg-card/65 p-4">
                        ...
                        <p className="text-xs leading-6 text-fg-2">{item[locale]}</p>
                      </li>
                    ))}
                  </ul>
                </section>
      ...
      <section className="mt-10 grid gap-4 lg:grid-cols-2" aria-label={ko ? "중요한 적용 경계" : "Important implementation boundaries"}>
        <article className="rounded-[2rem] border border-warning/30 bg-warning-soft/15 p-6 sm:p-7">
          ...
          <h2 className="mt-4 text-xl font-black text-fg">
            {ko ? "Toss 인증을 단순 유료 제외로 설명하지 않습니다." : "Toss authentication is not reduced to a pricing decision."}
          </h2>
          <p className="mt-3 text-sm leading-7 text-fg-2">
            {ko
              ? "현재 허용 공급자는 Google, Kakao, Naver와 GitHub입니다. Toss는 일반 웹 OAuth 어댑터와 적용 범위·심사·보안 운영 조건이 달라 현재 제품 범위에서 제외하며, 가격 하나만을 이유로 주장하지 않습니다."
              : "The current allowlist is Google, Kakao, Naver and GitHub. Toss remains outside the present scope because product, review and security operations differ from a general web OAuth adapter; price alone is not presented as the reason."
            }
          </p>
        </article>
        ...
        <article className="rounded-[2rem] border border-accent/25 bg-accent-soft/25 p-6 sm:p-7">
          ...
          <h2 className="mt-4 text-xl font-black text-fg">
            {ko ? "Testifly는 CI를 대신하지 않습니다." : "Testifly does not replace CI."}
          </h2>
          <p className="mt-3 text-sm leading-7 text-fg-2">
            {ko
              ? "Vitest, Playwright, 성능·보안·라이선스 검사가 merge 판단의 정본입니다. Testifly를 연결할 때는 기능 카탈로그와 수동 피드백을 이해하기 쉽게 보여주는 선택형 포털로만 사용합니다."
              : "Vitest, Playwright, performance, security and license checks remain authoritative for merge decisions. When connected, Testifly is only an optional portal for understandable feature catalogues and manual feedback."
            }
          </p>
        </article>
      </section>
      ...
      <section className="mt-10 rounded-[2rem] border border-line/70 bg-card/65 p-6 sm:p-8" aria-labelledby="guides-next-title">
        ...
        <h2 id="guides-next-title" className="mt-4 text-2xl font-black text-fg">
          {ko ? "공개 예제에는 비밀값을 넣지 않습니다." : "Public examples never contain secrets."}
        </h2>
        <p className="mt-3 max-w-3xl text-sm leading-7 text-fg-2">
          {ko
            ? "실제 client secret, API key, 내부 endpoint, 사용자 식별자와 운영 계정 정보는 환경 비밀 저장소에만 보관합니다. 발표 자료에는 변수 이름, 책임 경계와 실패 처리만 남깁니다."
            : "Real client secrets, API keys, private endpoints, user identifiers and operations accounts stay in environment secret stores. Presentation material contains only variable names, responsibility boundaries and failure handling."
          }
        </p>
        <Link
        ...
        >
          {ko ? "라이선스와 권리 점검" : "Review licenses and rights"}
