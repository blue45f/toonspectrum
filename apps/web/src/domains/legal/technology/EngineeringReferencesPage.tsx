...
export function EngineeringReferencesPage() {
  ...
  useDocumentTitle(
    ko
      ? "ToonStudio 기술 참고 자료와 트러블슈팅"
      : "ToonStudio technical references and troubleshooting",
  );
  ...
  const references = useMemo(
    () => ENGINEERING_REFERENCES.filter((reference) => {
      ...
      return normalized([
        ...
        reference.summary[locale],
        reference.applied[locale],
        reference.caution[locale],
        ...reference.evidence.map((item) => `${item.label[locale]} ${item.path}`),
  ...
  const troubleshooting = useMemo(
    () => ENGINEERING_TROUBLESHOOTING_CASES.filter((item) => {
      ...
      return normalized([
        ...
        item.title[locale],
        item.symptom[locale],
        item.wrongTurn[locale],
        item.rootCause[locale],
        item.resolution[locale],
        item.regression[locale],
        item.lesson[locale],
        ...item.evidence.map((entry) => `${entry.label[locale]} ${entry.path}`),
      ].join(" ")).includes(search);
    }),
    [locale, search],
  );
  ...
  return (
    <Container size="wide" className="py-7 sm:py-10 lg:py-12">
      ...
      <EngineeringPageIntro
        ...
        title={
          ko
            ? "사용한 기술, 참고한 제품과 실패에서 얻은 교훈을 분리해 공개합니다."
            : "Used technology, product references and lessons from failure are published separately."
        }
        description={
          ko
            ? "설치된 기술과 검토만 한 후보를 같은 말로 소개하지 않습니다. 트러블슈팅 기록은 증상, 잘못된 접근, 실제 원인, 해결, 회귀 검사와 다른 프로젝트에 옮길 교훈까지 연결합니다."
            : "Installed technology and evaluated candidates are never presented as the same thing. Troubleshooting records connect symptoms, rejected approaches, root causes, fixes, regressions and lessons reusable elsewhere."
        }
        aside={
          <div className="rounded-3xl border border-warning/30 bg-warning-soft/15 p-5">
            ...
            <p className="mt-4 text-sm font-black text-fg">
              {ko ? "비교는 영감이지 동등성 주장이 아닙니다." : "A comparison is inspiration, not a parity claim."}
            </p>
            <p className="mt-2 text-xs leading-6 text-fg-2">
              {ko
                ? "기능 동등성은 실제 benchmark, 파일 round-trip과 사용자 흐름 검증이 있을 때만 별도로 주장합니다."
                : "Feature parity is claimed separately only with benchmarks, file round trips and verified user journeys."
              }
            </p>
          </div>
        }
      ...
      <section aria-labelledby="reference-filter-title">
        <h2 id="reference-filter-title" className="sr-only">
          {ko ? "참고 자료 검색과 분류" : "Search and classify references"}
        </h2>
        <div className="grid gap-3 rounded-3xl border border-line/70 bg-panel/55 p-3 lg:grid-cols-[minmax(16rem,1fr)_auto] lg:items-center">
          <label className="relative block">
            <Search
              ...
              aria-hidden="true"
            />
            <span className="sr-only">{ko ? "기술·제품·오류 검색" : "Search technology, products and failures"}</span>
            <input
              type="search"
              value={query}
              onChange={(event) => setQuery(event.currentTarget.value)}
              placeholder={ko ? "Worker, PWA, Blender, API, 오류 경로 검색" : "Search Worker, PWA, Blender, APIs or failure paths"}
          ...
          <div className="flex flex-wrap gap-2" role="group" aria-label={ko ? "참고 관계 필터" : "Reference relationship filter"}>
      ...
      <section className="mt-9" aria-labelledby="reference-products-title">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 id="reference-products-title" className="mt-3 text-2xl font-black tracking-tight text-fg sm:text-3xl">
              {ko ? "기술·제품·자료 참고 지도" : "Technology, product and source-reference map"}
            </h2>
          </div>
          <p className="rounded-full border border-line bg-card px-3 py-1.5 text-xs font-bold text-fg-3" role="status">
            {ko ? `${references.length}개 결과` : `${references.length} results`}
          </p>
        </div>
        ...
        {references.length ? (
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <span className={cx(
                    ...
                    {ENGINEERING_REFERENCE_RELATION_META[reference.relation].label[locale]}
                  </span>
                ...
                <p className="mt-3 text-sm leading-7 text-fg-2">{reference.summary[locale]}</p>
                ...
                <dl className="mt-5 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-success/25 bg-success-soft/15 p-4">
                    <dt className="text-[0.66rem] font-black uppercase tracking-[0.13em] text-success">
                      {ko ? "가져온 원칙" : "Applied lesson"}
                    </dt>
                    <dd className="mt-2 text-xs leading-6 text-fg-2">{reference.applied[locale]}</dd>
                  </div>
                  <div className="rounded-2xl border border-warning/25 bg-warning-soft/10 p-4">
                    ...
                    <dd className="mt-2 text-xs leading-6 text-fg-2">{reference.caution[locale]}</dd>
                  </div>
                </dl>
                ...
                <details className="group mt-5 rounded-2xl border border-line/70 bg-card/55 open:bg-card/75">
                  <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-3 px-4 py-2.5 text-xs font-black text-fg marker:hidden">
                    <span>{ko ? "저장소 근거" : "Repository evidence"}</span>
          ...
          <div className="mt-6 rounded-3xl border border-dashed border-line-strong bg-card/45 p-8 text-center">
            ...
            <p className="mt-3 text-sm font-bold text-fg">{ko ? "일치하는 참고 자료가 없습니다." : "No matching references."}</p>
          </div>
        )}
      </section>
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 id="troubleshooting-title" className="mt-3 text-2xl font-black tracking-tight text-fg sm:text-3xl">
              {ko ? "재현 가능한 트러블슈팅 아카이브" : "Reproducible troubleshooting archive"}
            </h2>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-fg-2">
              {ko
                ? "단순 해결 팁이 아니라 같은 장애가 다시 생기지 않게 만든 코드·테스트까지 기록합니다. 검색어는 아래 장애 기록에도 적용됩니다."
                : "These are not isolated tips: each record includes the code or test that prevents the same incident from returning. Search applies here as well."
              }
            </p>
          </div>
          <span className="rounded-full border border-line bg-card px-3 py-1.5 text-xs font-bold text-fg-3">
            {ko ? `${troubleshooting.length}개 사건` : `${troubleshooting.length} incidents`}
          </span>
        </div>
            ...
            >
              <summary className="grid min-h-20 cursor-pointer list-none gap-3 p-5 marker:hidden sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center sm:p-6">
                ...
                <span className="min-w-0">
                  <span className="block font-display text-[0.66rem] font-black uppercase tracking-[0.15em] text-fg-3">{item.area[locale]}</span>
                  <span className="mt-1 block text-lg font-black text-fg sm:text-xl">{item.title[locale]}</span>
                </span>
              ...
              <div className="border-t border-line/70 p-5 sm:p-6">
                <dl className="grid gap-3 lg:grid-cols-2 xl:grid-cols-3">
                  ...
                  <div className="rounded-2xl border border-warning/25 bg-warning-soft/10 p-4">
                    <dt className="text-[0.66rem] font-black uppercase tracking-[0.13em] text-warning">{ko ? "잘못된 접근" : "Wrong turn"}</dt>
                    <dd className="mt-2 text-xs leading-6 text-fg-2">{item.wrongTurn[locale]}</dd>
                  </div>
                  <div className="rounded-2xl border border-line bg-card/65 p-4">
                    <dt className="text-[0.66rem] font-black uppercase tracking-[0.13em] text-fg-3">{ko ? "실제 원인" : "Root cause"}</dt>
                  ...
                  <div className="rounded-2xl border border-accent/25 bg-accent-soft/20 p-4">
                    <dt className="text-[0.66rem] font-black uppercase tracking-[0.13em] text-accent">{ko ? "해결" : "Resolution"}</dt>
                    <dd className="mt-2 text-xs leading-6 text-fg-2">{item.resolution[locale]}</dd>
                  </div>
                  <div className="rounded-2xl border border-success/25 bg-success-soft/15 p-4">
                    ...
                    <dd className="mt-2 text-xs leading-6 text-fg-2">{item.regression[locale]}</dd>
                  </div>
                  <div className="rounded-2xl border border-info/25 bg-info-soft/15 p-4">
                    <dt className="text-[0.66rem] font-black uppercase tracking-[0.13em] text-info">{ko ? "재사용 교훈" : "Transferable lesson"}</dt>
                    <dd className="mt-2 text-xs leading-6 text-fg-2">{item.lesson[locale]}</dd>
                  </div>
                </dl>
                  ...
                  {item.evidence.map((entry) => {
                    ...
                    return (
                      <li key={`${entry.kind}-${entry.path}`} className="flex min-w-0 items-start gap-3 rounded-2xl border border-line/70 bg-card/65 p-4">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-fg">{entry.label[locale]}</p>
