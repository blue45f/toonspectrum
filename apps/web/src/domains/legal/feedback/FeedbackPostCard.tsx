...
function ProgressEditor({ post, readOnly, onUpdated }: { post: FeedbackEntry; readOnly: boolean; onUpdated: (patch: Partial<FeedbackEntry>) => void }) {
  ...
  return <details className="fb-management">
    <summary><ShieldCheck size={16} aria-hidden="true" /> {translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "운영자 처리 상태 관리")}</summary>
    <form onSubmit={save} className="fb-form" aria-busy={saving}>
      <label htmlFor={formatI18nTemplate(translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "en", "{v0}-progress"), { v0: String(id) })}>{translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "변경할 처리 상태")}</label><select id={formatI18nTemplate(translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "en", "{v0}-progress"), { v0: String(id) })} value={progress} onChange={(event) => setProgress(event.target.value as FeedbackProgress)} disabled={saving}>{FEEDBACK_PROGRESS.map((value) => <option key={value} value={value}>{FEEDBACK_PROGRESS_LABELS[value]}</option>)}</select>
      <label htmlFor={formatI18nTemplate(translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "en", "{v0}-note"), { v0: String(id) })}>{translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "공개 처리 안내")}</label><textarea id={formatI18nTemplate(translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "en", "{v0}-note"), { v0: String(id) })} value={note} onChange={(event) => setNote(event.target.value)} rows={3} maxLength={1000} disabled={saving} placeholder={translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "검토 결과, 반영 범위 또는 보류 이유를 알려주세요.")} />
      <p className="fb-caption">{translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "상태와 안내는 공개 운영자 댓글로 함께 기록됩니다. 답변만으로 ‘반영 완료’가 되지는 않습니다.")}</p>
      <button type="submit" disabled={saving || readOnly || progress === post.progress || note.trim().length < 2} className="fb-button">{saving ? translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "저장 중…") : translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "처리 상태 저장")}</button>
  ...
  return <li className="fb-post" data-expanded={expanded || undefined}>
    <article>
      ...
      <div className="fb-post-bottom">
        <div className="fb-meta"><span>{post.author.name}</span><span className="fb-comment-count"><MessageSquare size={13} aria-hidden="true" />{translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "댓글 ")}{post.replyCount}</span>{post.status === "answered" && <span className="fb-official"><ShieldCheck size={13} aria-hidden="true" />{translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "운영자 답변")}</span>}</div>
        <button type="button" className="fb-vote" onClick={vote} aria-pressed={post.viewerVoted} aria-label={formatI18nTemplate(translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "{v0} 공감 {v1}개{v2}"), { v0: String(post.title), v1: String(post.voteCount), v2: String(post.viewerVoted ? " 취소" : "") })} disabled={!userId || voting || readOnly} title={!userId ? translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "로그인 후 공감할 수 있어요") : undefined}><ThumbsUp size={15} aria-hidden="true" /><span>{voting ? "…" : post.voteCount}</span><span>{translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "공감")}</span></button>
      </div>
      ...
      <div id={detailId} hidden={!expanded}>
        {expanded && <>
          ...
          {post.metadata?.area && <p className="fb-caption">{translateCurrentStaticSourceText("domains.legal.feedback.FeedbackPostCard", "ko", "관련 기능: ")}{FEEDBACK_AREA_LABELS[post.metadata.area]}</p>}
