...
export function TechnologyPage() {
  ...
  useDocumentTitle(
    ko
      ? "ToonStudio Engineering Story · 브라우저 제작실을 만든 과정"
      : "ToonStudio Engineering Story · How the browser studio was built",
  );
  ...
  return (
    <Container size="wide" className="py-7 sm:py-10 lg:py-12">
      <PublicStoryHero
        ...
        title={
          ko
            ? "브라우저에서 웹툰 제작 스튜디오를 만들기까지."
            : "How we built a webtoon production studio in the browser."
        }
        description={
          ko
            ? "어떤 기술을 사용했는지뿐 아니라 왜 선택했는지, 실제로 어디까지 동작하는지, 실패와 대체 경로는 무엇인지, 다른 서비스에는 어떻게 적용할 수 있는지까지 공개합니다."
            : "Not only what we used, but why, how far it really works, what fails, which fallback remains and how to reuse the approach elsewhere."
        }
        image="materials"
        imageAlt={
          ko
        ...
        caption={
          ko
            ? "WEB · CANVAS · STORAGE · COLLABORATION · AI · DELIVERY"
            : "WEB · CANVAS · STORAGE · COLLABORATION · AI · DELIVERY"
        }
      >
        <div className="flex flex-wrap gap-3">
          ...
          >
            {ko ? "전체 제작 과정 보기" : "Read the full story"}
      ...
      <section className="py-14 sm:py-20" aria-labelledby="engineering-status-title">
        <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:gap-14">
          <div>
            ...
            >
              {ko
                ? "코드가 있다는 이유만으로 운영 기능이라고 부르지 않습니다."
                : "Code existence alone does not make a capability live."}
            </h2>
            <p className="mt-4 max-w-lg text-sm leading-7 text-fg-2">
              {ko
                ? "모든 공개 항목을 운영 경로, 설정 완료, 실험, 문서화와 설계 단계로 구분하고 코드·테스트·워크플로·문서를 근거로 연결합니다."
                : "Every public claim is labelled as live, configured, experimental, documented or planned and connected to code, tests, workflows or documents."
              }
            </p>
          </div>
      ...
      <section aria-labelledby="engineering-architecture-title">
        <div className="flex items-end justify-between gap-5">
          <div>
            ...
            >
              {ko
                ? "한 엔진이 아니라, 실패 범위를 제한하는 다섯 개 층."
                : "Not one engine, but five layers that limit failure scope."}
            </h2>
          </div>
        ...
        <div className="mt-8 overflow-hidden rounded-[2rem] border border-line/70 bg-panel/65 p-4 shadow-sm sm:p-6">
          <ol className="grid gap-3 lg:grid-cols-5" aria-label={ko ? "기술 아키텍처 계층" : "Engineering architecture layers"}>
            {ARCHITECTURE_LAYERS.map((layer, index) => {
              ...
              return (
                <li key={layer.title.ko} className="relative rounded-3xl border border-line/70 bg-card/75 p-5">
                  ...
                  <h3 className="mt-5 text-base font-bold text-fg">{layer.title[locale]}</h3>
                  <p className="mt-2 text-xs leading-6 text-fg-3">{layer.body[locale]}</p>
          ...
          {HUB_LINKS.map((item) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="mt-6 text-lg font-bold text-fg">{item.title[locale]}</h3>
                <p className="mt-3 flex-1 text-sm leading-7 text-fg-3">{item.body[locale]}</p>
      ...
      <section aria-labelledby="engineering-featured-title">
        ...
        <h2 id="engineering-featured-title" className="mt-3 text-2xl font-bold tracking-tight text-fg sm:text-3xl">
          {ko ? "ToonStudio를 지탱하는 핵심 기술 의사결정" : "Core engineering decisions behind ToonStudio"}
        </h2>
        <div className="mt-7 grid gap-4 lg:grid-cols-2">
          {featuredChapters.map((chapter) => (
            <article key={chapter.id} className="rounded-[2rem] border border-line/70 bg-panel/55 p-6 shadow-sm sm:p-7">
              ...
              <h3 className="mt-5 text-balance text-xl font-bold tracking-tight text-fg">{chapter.title[locale]}</h3>
              <p className="mt-3 text-sm leading-7 text-fg-2">{chapter.thesis[locale]}</p>
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            ...
            <h2 id="engineering-transparency-title" className="mt-3 max-w-3xl text-balance text-2xl font-bold tracking-tight text-fg sm:text-3xl">
              {ko
                ? "판단에 필요한 근거는 공개하고, 공격에 도움이 되는 운영 비밀은 보호합니다."
                : "Publish evidence needed for judgment while protecting operational secrets."}
            </h2>
