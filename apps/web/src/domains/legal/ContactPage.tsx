  ...
  return (
    <Container size="wide" className="py-8 sm:py-12 lg:py-16">
      <PublicStoryHero
        ...
        title={translateCurrentStaticSourceText("domains.legal.ContactPage", "ko", "웹툰을 만드는 더 나은 환경, 함께.")}
        description={translateCurrentStaticSourceText("domains.legal.ContactPage", "ko", "창작 도구와 교육, 리소스 공유, 콘텐츠와 플랫폼의 연결. 웹툰을 그리는 사람에게 도움이 되는 협업을 제안해 주세요. 서비스 이용 문제는 지원 센터에서, 투자·제휴·스폰서십은 비공개 비즈니스 센터에서, 개인 서포터와 후원 경계는 응원 센터에서 확인할 수 있습니다.")}
        image="materials"
        imageAlt={translateCurrentStaticSourceText("domains.legal.ContactPage", "ko", "브러시와 재료가 놓인 웹툰 작업실 콘셉트 아트")}
        caption={translateCurrentStaticSourceText("domains.legal.ContactPage", "ko", "LET’S MAKE ROOM FOR IDEAS · 드로잉 재료 콘셉트 아트")}
      >
        <Link
          href="/business"
        ...
        >
          {translateCurrentStaticSourceText("domains.legal.ContactPage", "ko", "비즈니스 문의")}<Handshake size={16} aria-hidden="true" />
        </Link>
        <Link
          href="/help"
          className="ml-4 inline-flex min-h-12 items-center text-sm font-semibold text-fg-2 hover:text-accent"
        >
          {translateCurrentStaticSourceText("domains.legal.ContactPage", "ko", "사용법과 도움말")}</Link>
      </PublicStoryHero>

      <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
        <div>
          <h2 className="mb-3 text-lg font-bold text-fg">{translateCurrentStaticSourceText("domains.legal.ContactPage", "ko", "이런 문의를 받습니다")}</h2>
          ...
          <h2 className="mb-3 mt-8 text-lg font-bold text-fg">
            {translateCurrentStaticSourceText("domains.legal.ContactPage", "ko", "문의 성격에 맞는 전용 경로")}</h2>
        ...
        <aside>
          <div className="sticky top-[var(--site-header-sticky-offset,5rem)] rounded-2xl border border-line bg-panel/40 p-5">
            <h2 className="text-sm font-semibold text-fg">{translateCurrentStaticSourceText("domains.legal.ContactPage", "ko", "공개와 비공개를 구분합니다")}</h2>
            <p className="mt-2 text-xs leading-relaxed text-fg-3">
              {translateCurrentStaticSourceText("domains.legal.ContactPage", "ko", "사용 중 발생한 문제는 지원 센터에서 해결 방법과 진단 정보를 확인하세요. 공개해도 되는 제안과 오류는 피드백 보드에서 논의하고, 연락처·사업 정보가 포함되는 투자·제휴 문의는 비공개 센터를 이용해 주세요.")}</p>
            <Link
              href="/business"
            ...
            >
              {translateCurrentStaticSourceText("domains.legal.ContactPage", "ko", "비공개 비즈니스 센터 열기")}</Link>
          </div>
        </aside>
      </div>
    </Container>
