...
export const BrowserCompatModalContent: React.FC<BrowserCompatModalProps> = ({
  isOpen,
  onClose,
  reason,
  missingFeatures = [],
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
      ...
      <div
        ...
        tabIndex={isBlocking ? undefined : 0}
        aria-label={isBlocking ? undefined : "모달 닫기"}
        className="fixed inset-0 bg-canvas/80 backdrop-blur-md transition-opacity"
      ...
      {/* 모달 팝업 컨테이너 */}
      <div role="dialog" aria-modal="true" aria-labelledby="browser-compat-title" aria-describedby="browser-compat-description" data-stable-contrast="true" className="relative w-full max-w-lg overflow-hidden rounded-3xl border border-line-strong bg-panel/95 p-6 sm:p-8 text-fg shadow-2xl backdrop-blur-xl transition-all">
        ...
        {!isBlocking && (
          <button
            ...
            onClick={onClose}
            aria-label="닫기"
            className="absolute top-4 right-4 inline-flex h-11 w-11 items-center justify-center rounded-full bg-raised/70 text-fg-2 hover:bg-raised hover:text-fg transition-colors"
          >
            <X size={18} />
          </button>
        )}
        ...
        <div className="flex items-start gap-4 mb-6">
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-warning-soft border border-warning/50 text-warning shadow-inner">
            <Monitor size={28} />
          </div>
          <div>
            <span className="inline-flex items-center gap-1.5 rounded-full bg-warning-soft px-3 py-1 text-xs font-semibold text-warning border border-warning/50">
              <AlertCircle size={12} /> 브라우저 호환성 안내
            </span>
            <h3 id="browser-compat-title" className="mt-2 text-xl font-bold tracking-tight text-fg sm:text-2xl">
              최신 브라우저 업데이트가 필요합니다
            </h3>
          </div>
        </div>
        ...
        <div id="browser-compat-description" className="space-y-4 text-sm text-fg-2 leading-relaxed">
          ...
          {reason && (
            <div className="rounded-xl border border-bad/30 bg-bad/10 p-3.5 text-xs text-bad font-mono break-all flex items-start gap-2">
          ...
          {actualMissing.length > 0 && (
            <div className="rounded-xl border border-line bg-card p-3.5 text-xs">
        ...
        <div className="mt-6 grid grid-cols-2 gap-2.5 sm:grid-cols-4">
          <a
            ...
            className="flex flex-col items-center justify-center rounded-2xl border border-line bg-card p-3 text-center transition-all hover:border-accent/50 hover:bg-accent/10 hover:scale-[1.02] group"
          >
        ...
        {/* 푸터 하단 버튼 */}
        <div className="mt-7 flex flex-wrap items-center justify-end gap-2.5 pt-4 border-t border-line-strong">
          <button
            type="button"
            onClick={handleCopyLink}
            className="inline-flex min-h-11 max-w-full items-center justify-center rounded-xl border border-line-strong bg-card px-4 py-2.5 text-center text-xs font-medium leading-tight text-fg hover:bg-raised transition-colors"
          >
            {copyState === "copied"
              ? "주소 복사됨!"
              : copyState === "failed"
          ...
          <button
            ...
            className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-xl border border-accent bg-accent px-4 py-2.5 text-xs font-semibold text-on-accent hover:bg-accent-2 transition-colors"
          >
            <RefreshCw size={14} />
            새로고침
          </button>
