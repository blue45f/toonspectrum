  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
      ...
      <div
        role="button"
        tabIndex={0}
        aria-label={translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "모달 닫기")}
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-3xl border border-white/15 bg-[#1a1410]/95 p-6 sm:p-8 text-fg shadow-2xl backdrop-blur-xl transition-all">
        ...
        {!isBlocking && (
          <button
            type="button"
            onClick={onClose}
            aria-label={translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "닫기")}
        ...
        <div className="flex items-start gap-4 mb-6">
          ...
          <div>
            <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-500/10 px-3 py-1 text-xs font-semibold text-amber-400 border border-amber-500/20">
              <AlertCircle size={12} /> {translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "브라우저 호환성 안내")}</span>
            <h3 className="mt-2 text-xl font-bold tracking-tight text-fg sm:text-2xl">
              {translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "최신 브라우저 업데이트가 필요합니다")}</h3>
          </div>
        </div>
        ...
        <div className="space-y-4 text-sm text-fg-2 leading-relaxed">
          <p>
            {translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "현재 사용 중인 브라우저(")}<strong className="text-fg font-semibold">{browser.name} {browser.version}</strong>{translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", ")는 툰스펙트럼의 3D 리더스튜디오 및 최신 웹 표준 기능을 완벽히 지원하지 않을 수 있습니다.")}</p>
          ...
          {actualMissing.length > 0 && (
            <div className="rounded-xl border border-white/10 bg-white/5 p-3.5 text-xs">
              <span className="font-semibold text-fg block mb-1.5">{translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "미지원 감지 항목:")}</span>
          ...
          <p className="text-xs text-fg-3">
            {translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "더 빠르고 안전하며 쾌적한 이용을 위해 아래 추천 브라우저의 최신 버전으로 업데이트 후 접속해 주세요.")}</p>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-2.5 sm:grid-cols-4">
          ...
          >
            ...
            <span className="text-[10px] text-fg-3">{translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "권장")}</span>
          </a>
          ...
          >
            ...
            <span className="text-[10px] text-fg-3">{translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "최신")}</span>
          </a>
        ...
        <div className="mt-7 flex flex-wrap items-center justify-end gap-2.5 pt-4 border-t border-white/10">
          ...
          >
            {copyState === "copied"
              ? translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "주소 복사됨!")
              : copyState === "failed"
                ? translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "복사할 수 없어요 — 주소창에서 직접 복사해 주세요")
                : translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "접속 주소 복사")}
          </button>
          ...
          >
            <RefreshCw size={14} />
            {translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "새로고침")}</button>
          ...
          {!isBlocking && (
            ...
            >
              {translateCurrentStaticSourceText("components.browser.compat.modal.content", "ko", "호환 모드로 계속하기")}</button>
          )}
        </div>
      </div>
    </div>
