...
export async function inspectStudioReviewTaskCompletion(client: PoolClient, actor: string, workId: string, taskId: string, requiredAccess: "view" | "edit" = "edit") {
  ...
  if (!access[requiredAccess]) return fail("forbidden");
...
export class StudioReviewTaskCompletionRepository {
  ...
  read(actor: string, workId: string, taskId: string): Promise<StudioReviewTaskCompletionContext> {
    return this.transaction(async (client) => (await inspectStudioReviewTaskCompletion(client, actor, workId, taskId)).context);
  }
