...
describe("PostgresStudioAiAdmissionRepository", () => {
  ...
  it("persists sent before provider I/O and updates only the exact hashed receipt fence", async () => {
    ...
    expect(sql).toContain('"attemptCount" < 5');
  ...
  it("expands receipts and usage rows for the five-provider shared free pool", () => {
    ...
    expect(usageChecks.get("studio_ai_usage_attempt_count_check")).toContain(
      '"attemptCount" between 1 and 5'
    );

    const migration = readFileSync(
      new URL(
        "../../db/migrations/0057_studio_ai_free_provider_expansion.sql",
        import.meta.url
      ),
      "utf8"
    );
    expect(migration).toContain('CHECK ("attemptCount" BETWEEN 0 AND 5)');
    expect(migration).toContain("'gemini', 'groq', 'sambanova', 'mistral', 'openrouter', 'zai', 'deepseek'");
    expect(migration).toContain('CHECK ("attemptCount" BETWEEN 1 AND 5)');
