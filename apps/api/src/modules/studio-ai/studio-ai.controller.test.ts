...
const IDEMPOTENCY_KEY = "studio-ai-controller-operation-0001";
...
describe("StudioAiController", () => {
  it("이미 중단된 HTTP 요청 상태를 서비스 AbortSignal로 전달하고 리스너를 정리한다", async () => {
    ...
    const complete = vi.fn((_userId, _body, _idempotencyKey, signal?: AbortSignal) =>
  ...
  it("응답 전 연결 종료를 서비스 신호로 전파하고 성공/실패 뒤 리스너를 정리한다", async () => {
    ...
    const result = controller.chat("studio-user", IDEMPOTENCY_KEY, input, request, response);
  ...
  it("정상 완료 시 신호를 중단하지 않고 연결 리스너를 정리한다", async () => {
    ...
    await expect(
      controller.chat("studio-user", IDEMPOTENCY_KEY, input, request, response)
