...
function setup(){
  const repository={door:vi.fn().mockResolvedValue({open:false}),changeDoor:vi.fn(),open:vi.fn(),current:vi.fn(),revoke:vi.fn()};
...
describe("acoustic HTTP authority boundary",()=>{
  it("requires a verified cookie session rather than caller actor headers or a public room ticket",async()=>{
    ...
    for(const method of [f.controller.door,f.controller.changeDoor,f.controller.open,f.controller.current,f.controller.renew,f.controller.revoke]) {
