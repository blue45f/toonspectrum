...
export const studioAiUsageLedger = pgTable(
  ...
  (t) => [
    ...
    check(
      ...
      sql`${t.provider} in ('gemini', 'groq', 'sambanova', 'cloudflare', 'mistral', 'openrouter', 'zai', 'deepseek')`
    ),
    check("studio_ai_usage_model_check", sql`char_length(${t.model}) between 1 and 200`),
    check("studio_ai_usage_attempt_count_check", sql`${t.attemptCount} between 1 and 6`),
    check(
