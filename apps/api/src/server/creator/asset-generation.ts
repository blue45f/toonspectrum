...
// GPT Image는 더 긴 프롬프트를 허용한다. 제품 레벨에서는 과도한 요청 본문을 막으면서도
// 캐릭터·배경·조명·구도 지시를 함께 담을 수 있도록 8K로 제한한다.
...
/**
 * 대부분의 생성은 지연이 낮은 Flare가 소유한다. 사용자가 최고 정밀도(max)를 명시한 경우에만
 * 장시간 정밀 렌더러인 Sunburst로 승격한다. 이 계약은 UI에 모델명을 강요하지 않으면서도
 * 사용자가 선택한 품질 의도를 실제 모델 선택까지 보존한다.
 */
