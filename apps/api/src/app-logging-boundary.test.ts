  ...
  it("allowlists request method and query-free path only", () => {
    const serialized = serializeSafeHttpRequest({
      ...
      headers: {
        ...
        "x-provider-oidc-token": "never-log-this",
        "x-provider-proxy-signature": "never-log-this",
      ...
      const response = await fetch(
        `http://127.0.0.1:${address.port}/api/health/ready?ticket=query-secret`,
        {
          headers: {
            ...
            "x-provider-oidc-token": "oidc-secret",
            "x-provider-proxy-signature": "proxy-signature-secret",
          },
        },
      );
