 ...
 * 재실행 안전(고정 id + onConflictDoNothing). 실행: `pnpm db:seed` (또는 DATABASE_URL=... tsx apps/api/src/platform/database/seed.ts)
 *
...
async function main() {
  // apps/api/src/platform/database 는 모듈 로드 시 DATABASE_URL 을 읽으므로 import 전에 .env.local 주입.
