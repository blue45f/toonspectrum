...
Use Node 24 and install Korean system fonts (`fonts-noto-cjk` on Ubuntu). Run `npm --prefix tools/media/brand-film ci`, then `npm --prefix tools/media/brand-film run typecheck` and `npm --prefix tools/media/brand-film run render -- all`. The first render may download Chrome Headless Shell through Remotion. `landscape`, `portrait` or `square` may be selected instead of `all`.
...
`npm --prefix tools/media/brand-film run studio` opens the Remotion editing preview. The website does not import this package or Remotion runtime. System fonts are build dependencies and are not shared as font files.
