...
기본은 warm-ink 다크이며, 사용자 선택으로 잉크·페이퍼·그래파이트·미드나이트·세피아·고대비와 시스템 테마를 지원한다. 사이트와 스튜디오는 별도로 선택할 수 있고 스튜디오 기본값은 사이트 테마 상속이다. 테마는 도구·패널·메뉴 등 UI에만 적용하며 원고, 브러시 색상, 렌더러 및 내보내기 데이터에는 적용하지 않는다. `apps/web/src/shared/lib/theme-presets.ts`와 `apps/web/src/app/styles/design-themes.css`가 테마 정의다.
...
**Warm-ink 축 강제(필독).** 아래 규칙은 기본 잉크 브랜드 테마에 적용한다. 명시적으로 선택한 디자인 테마의 UI 토큰은 예외이며, 컴포넌트에 색을 하드코딩하지 말고 의미 토큰을 사용한다. 모든 중립 표면·그라디언트·그림자·장식 글로는 **hue ≈ 64–70** 축에만 둔다(예: 헤더/푸터 그라디언트, 카드 hover 그림자, hero 글로). hue 23x–27x 계열 쿨-블루/퍼플은 중립·장식 맥락에서 금지 — 따뜻한 잉크 명제를 정면으로 위반한다. 쿨톤은 오직 시맨틱 `--color-cool`(info/보조 신호, hue 232)로만, 데이터 맥락에서 의도적으로 사용한다. 색은 정보지 장식이 아니며, raw `rgb()/rgba()`/hex 대신 항상 OKLCH를 쓴다(`#000`/`#fff` 절대 금지).
...
- **스위치/토글(재발 방지 규칙)**: 독립 클릭 타깃은 `@/shared/components/ui/switch`의 `Switch`, 행 전체가 이진 제어이거나 `aria-pressed`/checkbox를 스위치 모양으로 표현하는 경우에는 같은 모듈의 `SwitchIndicator`로 시각 geometry를 재사용한다. 각 화면에서 thumb/track을 다시 그리지 않는다. 기준 geometry는 44×24px 트랙, 20px thumb, `left/top` 2px의 **명시적 절대 위치 기준점**, ON 이동 20px, 독립 스위치는 최소 44×44px hit target이다. `position:absolute` thumb에 `left`/`right` 없이 브라우저 static-position에 기대거나, 트랙 폭과 맞지 않는 임의 `translate-x-*`, `bg-white/#fff` 하드코딩을 사용하지 않는다. 의미론은 제어 목적을 유지한다(`Switch`는 `role="switch"` + `aria-checked`, 네이티브 checkbox는 checkbox, 토글 버튼은 `aria-pressed`). 모든 경우 접근 가능한 이름, focus ring, `prefers-reduced-motion`을 필수로 제공한다.
...
홈, 리서치, 학습, 마켓, 쇼케이스와 제품 소개는 **웹툰 드로잉 전문 도구**를 중심으로 연결한다. 핵심 여정은 자료 관찰 → 선화·채색 → 컷·말풍선·페이지 구성 → 작품 발표다. 전문 작업실 진입을 주 행동으로 두고 심플 모드는 입문 경로로 제공한다.

- 기존 warm-ink 중립, persimmon 행동 신호, Pretendard 본문과 Space Grotesk 영문 표기를 유지한다. 작품 자체의 색은 제한하지 않는다.
- 실제 기능을 설명하는 큰 작품 이미지, 제작 과정 이미지, 컷의 크기와 시선을 보여주는 편집을 사용한다. 브랜드 콘셉트 아트는 실제 사용자 게시물이나 편집기 결과물로 표시하지 않는다.
- 공개 소개 영역은 사용자가 조절할 수 있는 이미지 비교, 컷 구성 전환, 미세한 작품 움직임을 허용한다. 반복 모션에는 정지 조작을 제공하고 모션 감소 설정에서 정지한다. 소개 영상은 사용자 재생 전 내려받지 않는다.
- 자료 검색, 마켓 필터, 학습 진도, 작품 게시 등 기존 기능을 유지하면서 다음 작업 화면을 연결한다. 편집기 내부의 레이아웃·렌더러·입력 동작에는 이 공개 페이지 스타일을 적용하지 않는다.
