...
기본은 warm-ink 다크이며, 사용자 선택으로 잉크·페이퍼·그래파이트·미드나이트·세피아·고대비와 시스템 테마를 지원한다. 사이트와 스튜디오는 별도로 선택할 수 있고 스튜디오 기본값은 사이트 테마 상속이다. 테마는 도구·패널·메뉴 등 UI에만 적용하며 원고, 브러시 색상, 렌더러 및 내보내기 데이터에는 적용하지 않는다. `shared/lib/theme-presets.ts`와 `styles/design-themes.css`가 테마 정의다.
