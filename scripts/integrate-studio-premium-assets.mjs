...
async function patch(name, edits) {
  ...
  for (const [before, after] of edits) {
    ...
    if (text.split(before).length !== 2) {
      throw new Error(`Expected one unmodified integration anchor in ${fileURLToPath(file)}: ${before.slice(0, 80)}`);
    }
...
await patch('StudioCc0AssetLibraryPanel.tsx', [
  ...
  ['placeholder="가구, 음식, 나무, chair, tree…"',
   'placeholder="골목, 정원, 실내, 의자, 조명, chair…"'],
]);
console.log('Integrated 2D background and transparent-prop kinds with legacy URLs preserved.');
