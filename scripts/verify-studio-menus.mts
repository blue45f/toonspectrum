...
function pinnedTitleDrift(): string[] {
  ...
  if (matches) return [];
  return [
    `제시 제목 9종 계약 위반 — 기대: [${PINNED_PRESENTED_TITLES.join(" ")}] / 실제: [${presented.join(" ")}]`,
  ];
}
...
/**
 * Menubar title that currently owns a catalogue group (예: "ai" → 효과). An unmapped group
...
async function assertMainMenus(page: Page): Promise<string[]> {
  const failures: string[] = [];
  // The pinned nine come first: if the presentation re-folded, every derived assertion
...
async function main() {
  ...
  try {
    ...
    if (failures.length === 0) {
      log("PASS: optimized menus exposed (9 titles + sections + rail + popovers + draw options + export)");
      exitCode = 0;
    } else {
