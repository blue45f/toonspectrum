...
async function main(): Promise<void> {
  ...
  try {
    ...
    const desktop = await runDesktopBrushMatrix(browser, studioUrl);
    invariant(desktop.ok, "desktop 35-brush matrix failed");
    const mobile = await runMobileTouchAudit(browser, studioUrl);
    invariant(mobile.ok, "mobile catalogue touch audit failed");
    const durability = await runDeferredDurabilityAudit(browser, origin, studioUrl);
    invariant(durability.ok, "deferred stroke durability audit failed");

    await browser.close();
    browser = null;
    log("ALL BRUSH BROWSER GATES OK");
    console.log(JSON.stringify({ scratch: SCRATCH, desktop, mobile, durability }, null, 2));
  } finally {
