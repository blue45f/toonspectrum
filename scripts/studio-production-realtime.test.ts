...
describe("production realtime configuration", () => {
  it("commits only the three public browser settings, never a server credential", () => {
    expect(Object.keys(configuration).sort()).toEqual([
      "VITE_STUDIO_LIVE_ORIGIN", "VITE_STUDIO_REALTIME_ORIGIN", "VITE_STUDIO_REALTIME_PROVIDER_ID",
    ]);
