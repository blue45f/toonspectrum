...
test("manifest lists every numbered SQL migration exactly once in order", () => {
  ...
  expect(manifest.at(-1).id).toBe(
    "0053_personal_cloud_readiness_marker",
  );
...
test("personal cloud readiness marker is a forward-only verified repair", () => {
  const migration = loadMigrationManifest().find(
    ({ id }) => id === "0053_personal_cloud_readiness_marker",
  );
  expect(migration?.id).toBe("0053_personal_cloud_readiness_marker");
  const sql = migration?.contents ?? "";
