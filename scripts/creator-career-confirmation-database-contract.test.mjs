...
const sql = read('../apps/api/src/db/migrations/pending/0080_creator_career_confirmation.sql');
test('pending 0080 is atomic and leaves original exact78 and original0078 unchanged', () => {
  expect(sql).toMatch(/\nBEGIN;[\s\S]*COMMIT;\s*$/u);
  expect(readdirSync(new URL('../apps/api/src/db/migrations/', import.meta.url)).filter((f) => /^\d{4}_.+\.sql$/u.test(f))).toHaveLength(78);
