...
test('managed 0080 is atomic with exact91 inventory and immutable original0078', () => {
  expect(sql).toMatch(/\nBEGIN;[\s\S]*COMMIT;\s*$/u);
  expect(readdirSync(new URL('../apps/api/src/platform/database/migrations/', import.meta.url)).filter((f) => /^\d{4}_.+\.sql$/u.test(f))).toHaveLength(92);
