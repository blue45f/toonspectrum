...
import { createCpuSurfaceFactory, type CpuCanvasKit } from './cpu-surface';
import { resolveExperimentOutput } from './output';

import { normalizeStudioBrushDynamicsSettings, STUDIO_DYNAMIC_BRUSH_DEPOSIT_PIPELINE_CAUSAL_V3 as V3, STUDIO_DYNAMIC_BRUSH_DEPOSIT_PIPELINE_CAUSAL_V4 as V4 } from '@/domains/creator/brush/studio-brush-dynamics';
import { studioCoreBrushCatalogSelection } from '@/domains/creator/brush/studio-brush-selection';
import { clearStudioBrushTextureStampCache } from '@/domains/creator/brush/studio-brush-textured-stamp';
...
import { planStudioDynamicBrushCoverageMarks, renderStudioDynamicBrushCoverageMark, renderStudioDynamicBrushCoverage } from '@/domains/creator/studio-dynamic-brush-coverage-renderer';
...
function raster(marks: ReturnType<typeof plan>['marks'],tiled:boolean,phase:number){
 const surfaces=createCpuSurfaceFactory(kit);
 try {
  const canvas=surfaces.create(1100,820),ctx=canvas.getContext('2d');if(!ctx)throw new Error('Missing CPU destination context.');ctx.fillStyle='#fff';ctx.fillRect(0,0,1100,820);ctx.setTransform(1048/720,0,0,1048/720,phase,phase);
  const result=tiled?renderStudioDynamicBrushCoverage(ctx,marks,{activeDraft:false,opacity:1,surfaceFactory:surfaces.create}):marks.forEach(m=>renderStudioDynamicBrushCoverageMark(ctx,m,1,surfaces.create));
  if(tiled) expect(result?.status).toBe('rendered');
  return {bytes:new Uint8Array(ctx.getImageData(0,0,1100,820).data),result};
 } finally { clearStudioBrushTextureStampCache(); surfaces.dispose(); }
}
...
for(const [label,pipeline] of [['V3',V3],['V4',V4]])it(label+' compares prefix marks and direct/tile pixels',()=>{
 ...
 for(const phase of [0,.25,.5]){
  const live=raster(half.marks,false,phase),committed=raster(full.marks,true,phase),direct=raster(full.marks,false,phase);
