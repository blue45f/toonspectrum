...
try {
  ...
  const result = spawnSync(process.execPath, [tsc, "--strict", "--skipLibCheck", "--target", "es2022", "--module", "commonjs", "--lib", "es2023,dom,dom.iterable", "--outDir", output, "tests/creator-resources-cases.ts"], { cwd: root, stdio: "inherit" });
  ...
  for (const testCase of creatorResourceCases) {
    try { await testCase.run(); console.log(`PASS ${testCase.name}`); }
    catch (error) { failed++; console.error(`FAIL ${testCase.name}`, error); }
  }
  console.log(`${creatorResourceCases.length - failed}/${creatorResourceCases.length} creator-resource cases passed.`);
