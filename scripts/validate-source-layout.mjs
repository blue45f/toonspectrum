...
export function measureSourceLayout(files) {
  return {
    ...
    apiHttpFiles: filesUnder(files, "apps/api/src/platform/http"),
    apiAdapterFiles: filesUnder(files, "apps/api/src/platform/adapters"),
    apiDatabaseFiles: filesUnder(files, "apps/api/src/platform/database"),
    ...
    rootViteConfigFiles: files.includes("vite.config.ts") ? 1 : 0,
    rootDrizzleConfigFiles: files.includes("drizzle.config.ts") ? 1 : 0,
  };
}
