/** Managed first-party feedback, inquiries, supporter payments, and creator-support grants. */
...
export function buildFeedbackCapabilitySql(role) {
  ...
  const required = privileges.map(([table, privilege]) =>
    `('${table}', '${privilege}')`
  ).join(",\n    ");
  return `
...
BEGIN
  ...
  PERFORM id, "orderId", amount, "balanceAmount", currency, "orderName", "supporterName",
    message, visibility, "showAmount", "showMessage", "termsVersion", mode, "providerStatus",
