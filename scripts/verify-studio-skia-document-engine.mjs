...
const gpuMode = process.argv[3] ?? 'default';
if (!['default', 'metal'].includes(gpuMode) || (gpuMode === 'metal' && process.platform !== 'darwin')) throw new Error('Use default GPU selection, or explicit metal on macOS');
const report = { gpuMode, checks: [], measurements: [], errors: [] };
function passed(message) { report.checks.push(message); console.log(message); }
...
async function verify(type, name) {
  console.log(`${name}: starting GPU renderer checks`);
  const browser = await type.launch({ headless: true, args: name === 'chromium' && gpuMode === 'metal' ? ['--use-gl=angle', '--use-angle=metal', '--enable-gpu'] : [] });
  const deadline = setTimeout(() => { report.errors.push(`${name}: browser check exceeded 180 seconds`); void browser.close(); }, 180_000);
  try {
    ...
    report.measurements.push({ engine: name, renderer: driver });
    ...
    passed(`${name}: incremental append preserves old pixels; GPU snapshot undo is exact; finalized eraser matches canonical alpha`);
    ...
    passed(`${name}: real WebGL2 pixels, canonical pen parity, rotation/reflection reuse`);
    ...
    passed(`${name}: original marker nib, pressure and stroke opacity match within antialiasing tolerance`);
    ...
    passed(`${name}: independent GPU contexts and DPR resize preserve source geometry and return pixels`);
    ...
    passed(`${name}: 3000 actual strokes, append compiles one, undo releases, 50 warm frames reuse stable picture bytes`);
    ...
    passed(`${name}: 10000 retained strokes, one affected append batch, repeated camera frames keep bounded caches`);
    ...
    passed(`${name}: real GPU context loss stays unavailable until explicit recovery of the same engine`);
    await page.evaluate(() => window.skiaEngineQA.load(10001));
    const soak = [];
    for (let i = 0; i < 60; i++) {
      const next = await page.evaluate(() => window.skiaEngineQA.append());
      assert.equal(next.stats.compiledItems, 1); assert.equal(next.stats.compiledBatches, 1);
      const back = await page.evaluate(() => window.skiaEngineQA.undo());
      assert.equal(back.status, 'presented'); assert(back.stats.pictureBytes <= 128 * 1024 * 1024);
      assert(back.stats.retainedSnapshotBytes <= 32 * 1024 * 1024);
      soak.push(back.stats.pictureBytes);
    }
    assert.equal(Math.max(...soak), Math.min(...soak));
    passed(`${name}: 60 append/undo cycles at 10001 strokes keep picture bytes stable and snapshots bounded`);
    const cycleBase = await page.evaluate(() => window.skiaEngineQA.load(10000));
    assert.equal(cycleBase.status, 'presented'); assert.equal(cycleBase.stats.cachedItems, 10000);
    const cycles = [];
    for (let cycle = 0; cycle < 100; cycle++) {
      const extra = await page.evaluate(() => window.skiaEngineQA.append());
      assert.equal(extra.status, 'presented'); assert.equal(extra.stats.compiledItems, 1);
      assert.equal(extra.stats.compiledBatches, 1); assert.equal(extra.stats.paintedItems, 1);
      const back = await page.evaluate(() => window.skiaEngineQA.undo());
      assert.equal(back.status, 'presented'); assert.equal(back.stats.cachedItems, 10000);
      assert.equal(back.stats.pictureBytes, cycleBase.stats.pictureBytes);
      assert(back.stats.retainedSnapshotBytes <= 32 * 1024 * 1024);
      cycles.push(extra.stats.frameMs);
    }
    cycles.sort((a, b) => a - b);
    report.measurements.push({ engine: name, strokes: 10000, appendUndoCycles: cycles.length,
      appendSubmissionP95Ms: cycles[Math.floor(cycles.length * 0.95)], worstSubmissionMs: cycles.at(-1),
      pictureBytes: cycleBase.stats.pictureBytes, cachedBatches: cycleBase.stats.cachedBatches });
    passed(`${name}: 10000 actual strokes and 100 append/undo cycles; one batch recompiled; retained picture bytes stay bounded`);
