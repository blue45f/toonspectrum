...
const checkpoint = (text) => { report.checks.push(text); console.log(text); };
...
async function verify(type, name) {
  ...
  let timedOut = false;
  const deadline = setTimeout(() => { timedOut = true; void browser.close(); }, 180000);
  try {
    console.log(`${name}: starting direct GPU verification`);
    ...
    const driver = await page.evaluate(() => {
      ...
      return { version: gl?.getParameter(gl.VERSION) ?? null,
        renderer: debug ? gl.getParameter(debug.UNMASKED_RENDERER_WEBGL) : gl?.getParameter(gl.RENDERER) ?? null };
    });
    console.log(`${name}: driver ${JSON.stringify(driver)}`);
    report.measurements.push({ engine: name, browser: browser.version(), driver });
    ...
    const transformStart = await page.evaluate(() => window.skiaEngineQA.beginTransform());
    assert.equal(transformStart.result.status, 'presented');
    assert.equal(transformStart.result.stats.cachedItems, 79);
    assert.match(transformStart.token, /^draft:/);
    const transformMove = await page.evaluate(() => window.skiaEngineQA.moveTransform(12));
    assert.equal(transformMove.token, transformStart.token);
    assert.equal(transformMove.submitted, false);
    const transformMoveAgain = await page.evaluate(() => window.skiaEngineQA.moveTransform(16));
    assert.equal(transformMoveAgain.token, transformStart.token);
    assert.equal(transformMoveAgain.submitted, false);
    const transformHandoff = await page.evaluate(() => window.skiaEngineQA.handoffTransform());
    assert.equal(transformHandoff.result.status, 'presented');
    assert.equal(transformHandoff.result.stats.cachedItems, 80);
    assert.equal(transformHandoff.result.stats.compiledItems, 1);
    assert.equal(transformHandoff.token, 'base');
    assert.equal(transformHandoff.acknowledged, true);
    checkpoint(`${name}: exact transform hides one retained source once, skips per-pointer GPU submissions, and recompiles only the committed source at handoff`);
    ...
    const steady = await page.evaluate(() => window.skiaEngineQA.load(10000));
    assert.equal(steady.status, 'presented'); assert.equal(steady.stats.cachedItems, 10000);
    const cycles = [];
    for (let cycle = 0; cycle < 100; cycle++) {
      ...
      assert.equal(back.stats.pictureBytes, steady.stats.pictureBytes);
    ...
    report.measurements.push({ engine: name, strokes: 10000, appendUndoCycles: cycles.length,
      ...
      pictureBytes: steady.stats.pictureBytes, cachedBatches: steady.stats.cachedBatches });
    checkpoint(`${name}: 10000 actual strokes and 100 append/undo cycles; one batch recompiled; retained picture bytes stay bounded`);
  ...
  } finally { clearTimeout(deadline); await browser.close(); if (timedOut) report.errors.push(`${name}: GPU verification exceeded 180 seconds`); }
}
const selected = (process.argv[3] ?? 'chromium,firefox').split(',');
if (selected.some((name) => !['chromium', 'firefox'].includes(name))) throw new Error('Unsupported verification browser');
for (const [name, type] of [['chromium', chromium], ['firefox', firefox]].filter(([name]) => selected.includes(name))) {
