...
if (STUDIO_LISTED_ALL_BRUSH_CATALOG_ITEMS.map((item) => item.id).join("\0") !== ids.join("\0")) {
  failures.push("default catalogue order differs from the quality portfolio");
}
...
for (const [alias, representative] of aliases) {
  ...
  if (catalog && !isStudioBrushQuarantinedPresetId(alias)
...
const report = {
  ...
  counts: {
    ...
    hiddenFromDefault: STUDIO_ALL_BRUSH_CATALOG_ITEMS.length - rows.length,
    reductionRatio: 1 - rows.length / STUDIO_ALL_BRUSH_CATALOG_ITEMS.length,
