...
test("0079 closes its atomic transaction and exactly matches the canonical job/receipt contract", () => {
  ...
  const manifest = loadMigrationManifest(); expect(manifest).toHaveLength(83);
  expect(manifest.slice(-5).map((m) => m.id)).toEqual(["0079_creator_hiring_automation", "0080_creator_career_confirmation", "0081_fortune_public_snapshot", "0082_studio_review_policy", "0083_studio_review_vote_epoch"]);
