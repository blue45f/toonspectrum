...
test("0079 closes its atomic transaction and exactly matches the canonical job/receipt contract", () => {
  ...
  const manifest = loadMigrationManifest(); expect(manifest).toHaveLength(80);
  expect(manifest.slice(-2).map((m) => m.id)).toEqual(["0079_creator_hiring_automation", "0080_creator_career_confirmation"]);
