...
test("0079 closes its atomic transaction and exactly matches the canonical job/receipt contract", () => {
  ...
  const manifest = loadMigrationManifest(); expect(manifest).toHaveLength(81);
  expect(manifest.slice(-3).map((m) => m.id)).toEqual(["0079_creator_hiring_automation", "0080_creator_career_confirmation", "0081_fortune_public_snapshot"]);
