...
const sql = read("../apps/api/src/db/migrations/0079_creator_hiring_automation.sql");
test("0079 closes its atomic transaction and exactly matches the canonical job/receipt contract", () => {
  ...
  const manifest = loadMigrationManifest(); expect(manifest).toHaveLength(86);
  expect(manifest.slice(-8).map((m) => m.id)).toEqual(["0079_creator_hiring_automation", "0080_creator_career_confirmation", "0081_fortune_public_snapshot", "0082_studio_review_policy", "0083_studio_review_vote_epoch", "0084_production_model_v2_compatibility", "0085_production_team_workspace", "0086_production_operation_policy"]);
