/**
 * Close-up evidence from the actual Character Shaper UI, not substitute meshes or DOM-only tests.
 * Screenshots are review evidence, never an automatic claim of artistic quality. The JSON separates
 * rendered, unavailable and failed cases so a missing capability cannot count as a visual pass.
 */
...
async function camera(page: Page, label: string) {
  const button = page.locator(ROOT).getByRole("group", { name: "카메라 프리셋", exact: true })
...
async function capture(page: Page, name: string, full = false) {
  const surface = full ? page.locator(ROOT) : page.locator(VIEWPORT);
  const file = `${safeName(name)}.png`;
  await surface.screenshot({ path: join(OUTPUT, file), timeout: 45_000 });
...
async function slot(page: Page, kind: string) {
  ...
  catalog[kind] = await page.locator(`${ROOT} [data-character-slot-card]`).evaluateAll((elements) => elements.map((element) => ({
    id: element.getAttribute("data-character-slot-card"),
    availability: element.getAttribute("data-character-slot-card-availability"),
...
async function commit(page: Page, id: string): Promise<boolean> {
  ...
  if (await card.count() === 0 || await card.getAttribute("aria-disabled") === "true") {
    records.push({ id, status: "unavailable", reason: await card.count() ? await card.getAttribute("title") : "Not present in current catalog" });
    return false;
  }
  ...
  if (selected !== "true") throw new Error(`Card ${id} did not become selected (aria-pressed=${selected})`);
  return true;
}
...
async function main() {
  ...
  const context = await browser.newContext({ viewport: { width: 1600, height: 1000 }, deviceScaleFactor: 1.5, locale: "ko-KR" });
  await context.addInitScript(() => {
    ...
    localStorage.setItem("toonspectrum-auth-session-v1", JSON.stringify({
      user: { id: "11111111-2222-4333-8444-555555555555", name: "테스트 크리에이터", email: "creator-test@toonspectrum.dev", image: null, role: "creator" },
  ...
  try {
    ...
    // Capture the visible catalog before narrowing the cases. No entries are silently counted as checked.
    ...
    for (const [kind, items] of [
      ["bottom", ["jeans", "pants", "wide", "shorts", "pleated", "longskirt"]],
      ["shoes", ["sneakers", "boots", "longboots", "heels", "sandals"]],
    ] as const) {
    ...
    for (const item of ["glasses", "sunglasses", "cap", "headphones", "faceMask", "backpack", "shoulderbag", "scarf", "belt"]) {
      try {
        ...
        // Multi-slot accessories toggle independently; remove each before inspecting the next one.
    ...
    await page.screenshot({ path: join(OUTPUT, "fatal-page.png"), fullPage: true }).catch(() => undefined);
  } finally {
    writeFileSync(join(OUTPUT, "page-text.txt"), await page.locator("body").innerText().catch(() => "unavailable"));
    writeFileSync(join(OUTPUT, "evidence.json"), JSON.stringify({ source: process.env.GITHUB_SHA ?? "local", capturedAt: new Date().toISOString(), renderer: "Chromium SwiftShader WebGL2", visualApproval: "pending-human-image-review", catalog, records, errors }, null, 2));
    await browser.close();
  }
