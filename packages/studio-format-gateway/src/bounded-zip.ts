/**
 * Bounded, dependency-free ZIP32 reader used by external creative-format adapters.
 *
 * Kept package-local so the engine-neutral FormatGateway never imports the Studio UI domain.
 *
...
export const FORMAT_ZIP_READER_LIMITS = Object.freeze({
...
export interface FormatZipReaderLimits {
...
export type FormatZipReaderSource = Blob | Uint8Array | ArrayBuffer;

export interface FormatZipInflateContext {
  path: string;
  expectedBytes: number;
  signal?: AbortSignal;
}

export type FormatZipInflateRawAdapter = (
  compressed: Uint8Array,
  context: FormatZipInflateContext
) => Promise<Uint8Array>;

export interface FormatZipReaderOptions {
  /** Callers may lower, but never raise, the fixed browser-safety limits. */
  limits?: Partial<FormatZipReaderLimits>;
  /** Raw-DEFLATE adapter for runtimes without DecompressionStream("deflate-raw"). */
  inflateRaw?: FormatZipInflateRawAdapter;
  signal?: AbortSignal;
}

export interface FormatZipReadEntryOptions {
  signal?: AbortSignal;
}

export interface FormatZipEntry {
  path: string;
  directory: boolean;
...
export interface FormatZipArchive {
  /** Entries in physical local-record order, not attacker-controlled central-directory order. */
  entries: readonly FormatZipEntry[];
  comment: string;
  getEntry(path: string): FormatZipEntry | undefined;
  readEntry(
    entry: FormatZipEntry | string,
    options?: FormatZipReadEntryOptions
  ): Promise<Uint8Array>;
  readEntryBlob(
    entry: FormatZipEntry | string,
    mimeType?: string,
...
export type FormatZipReaderErrorCode =
  | "ABORTED"
  | "ARCHIVE_SIZE_LIMIT"
...
export class FormatZipReaderError extends Error {
  readonly code: FormatZipReaderErrorCode;
  readonly path?: string;

  constructor(code: FormatZipReaderErrorCode, message: string, path?: string) {
    super(message);
    this.name = "FormatZipReaderError";
    this.code = code;
...
interface CentralEntry extends FormatZipEntry {
  flags: number;
...
function zipError(
  code: FormatZipReaderErrorCode,
  message: string,
  path?: string
): FormatZipReaderError {
  return new FormatZipReaderError(code, message, path);
}
...
function resolveIntegerLimit(
  ...
  maximum: number,
  key: keyof FormatZipReaderLimits
): number {
...
function resolveRatioLimit(value: number | undefined): number {
  if (value === undefined) return FORMAT_ZIP_READER_LIMITS.maxCompressionRatio;
  if (
    !Number.isFinite(value) ||
    value < 1 ||
    value > FORMAT_ZIP_READER_LIMITS.maxCompressionRatio
  ) {
    throw zipError(
      "LIMIT_INVALID",
      `maxCompressionRatio는 1 이상 ${FORMAT_ZIP_READER_LIMITS.maxCompressionRatio} 이하여야 합니다.`
    );
  }
  return value;
}

function resolveLimits(value?: Partial<FormatZipReaderLimits>): FormatZipReaderLimits {
  return {
    maxArchiveBytes: resolveIntegerLimit(
      value?.maxArchiveBytes,
      FORMAT_ZIP_READER_LIMITS.maxArchiveBytes,
      "maxArchiveBytes"
    ),
    maxEntries: resolveIntegerLimit(
      value?.maxEntries,
      FORMAT_ZIP_READER_LIMITS.maxEntries,
      "maxEntries"
    ),
    maxEntryCompressedBytes: resolveIntegerLimit(
      value?.maxEntryCompressedBytes,
      FORMAT_ZIP_READER_LIMITS.maxEntryCompressedBytes,
      "maxEntryCompressedBytes"
    ),
    maxEntryUncompressedBytes: resolveIntegerLimit(
      value?.maxEntryUncompressedBytes,
      FORMAT_ZIP_READER_LIMITS.maxEntryUncompressedBytes,
      "maxEntryUncompressedBytes"
    ),
    maxTotalUncompressedBytes: resolveIntegerLimit(
      value?.maxTotalUncompressedBytes,
      FORMAT_ZIP_READER_LIMITS.maxTotalUncompressedBytes,
      "maxTotalUncompressedBytes"
    ),
    maxCentralDirectoryBytes: resolveIntegerLimit(
      value?.maxCentralDirectoryBytes,
      FORMAT_ZIP_READER_LIMITS.maxCentralDirectoryBytes,
      "maxCentralDirectoryBytes"
    ),
    maxPathBytes: resolveIntegerLimit(
      value?.maxPathBytes,
      FORMAT_ZIP_READER_LIMITS.maxPathBytes,
      "maxPathBytes"
    ),
    maxCompressionRatio: resolveRatioLimit(value?.maxCompressionRatio),
    maxCommentBytes: resolveIntegerLimit(
      value?.maxCommentBytes,
      FORMAT_ZIP_READER_LIMITS.maxCommentBytes,
      "maxCommentBytes"
    ),
  };
}
...
function validateFlags(flags: number, method: number, path?: string): void {
  ...
  // ZIP permits the UTF-8 flag to be absent. decodeAndValidatePath still
  // rejects every non-UTF-8 byte sequence, so flagless canonical ASCII paths
  // (common in KoStore-generated Krita bundles) are safe while CP437 ambiguity
  // remains fail-closed.
...
async function inflateWithDecompressionStream(
  ...
  try {
    ...
    if (cause instanceof FormatZipReaderError) throw cause;
    const detail = cause instanceof Error ? `: ${cause.message}` : "";
...
function resolveArchiveEntry(
  entriesByPath: ReadonlyMap<string, FormatZipEntry>,
  value: FormatZipEntry | string
): FormatZipEntry {
...
export async function readFormatZipArchive(
  source: FormatZipReaderSource,
  options: FormatZipReaderOptions = {}
): Promise<FormatZipArchive> {
  ...
  async function readEntry(
    value: FormatZipEntry | string,
    readOptions: FormatZipReadEntryOptions = {}
  ): Promise<Uint8Array> {
