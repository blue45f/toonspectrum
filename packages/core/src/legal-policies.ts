// The existing bundled policy copies, shared by API and browser.
// These are NOT asserted to be the latest remotely published policy versions.
...
/** Preserve fallback provenance; never relabel a bundled copy as a published original. */
export function normalizePolicyDocument(payload: unknown, slug: string): PolicyDocument {
  ...
  if (
    ...
    typeof value.versionLabel !== "string" || !value.versionLabel.trim() ||
    (value.policySlug !== undefined && value.policySlug !== slug) ||
    (value.source !== undefined && value.source !== "termsdesk" && value.source !== "static")
  ) {
    throw new Error("policy_payload_malformed");
  }
  return {
    ...
    source: value.source === "static" ? "static" : "termsdesk",
  };
}
