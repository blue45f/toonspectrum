// wgsl-variant wgslfx-v1-hsl.bc-4ebf4514
// structure: v1:hsl[hue,luminance,saturation]|brightness-contrast[brightness,contrast]
struct Params {
  pixel_count : u32,
  _pad0 : u32,
  _pad1 : u32,
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  // stage 0: hsl (nativeHSL matrix rows via uniform)
  {
    let p = vec4<f32>(f32(r), f32(g), f32(b), 1.0);
    let hr = studio_quantize_byte(dot(p, params.s0_row_r));
    let hg = studio_quantize_byte(dot(p, params.s0_row_g));
    let hb = studio_quantize_byte(dot(p, params.s0_row_b));
    r = hr;
    g = hg;
    b = hb;
  }
  // stage 1: brightness-contrast (lut base 0)
  r = lut[0u + r] & 0xffu;
  g = lut[256u + g] & 0xffu;
  b = lut[512u + b] & 0xffu;
