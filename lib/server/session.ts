...
// 토큰 형식(v2): "v2.<userId>.<sessionVersion>.<expiresAtMs>.<base64url(HMAC-SHA256(payload))>".
// 비밀 없이는 위조할 수 없고, sessionVersion/계정 상태를 DB에서 확인해 서버 로그아웃·정지·탈퇴를 반영한다.
...
export function signSession(userId: string, sessionVersion: number = 1, now: number = Date.now()): string {
  ...
  const expiresAt = Math.floor(now + SESSION_TOKEN_TTL_MS);
  const payload = `v2.${userId}.${safeVersion}.${expiresAt}`;
  return `${payload}.${sign(payload)}`;
}

export function verifySessionToken(token: string | null | undefined, now: number = Date.now()): VerifiedSessionToken | null {
