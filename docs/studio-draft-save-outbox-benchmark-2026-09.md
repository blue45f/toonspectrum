...
재생 직전 레코드를 제거해 effect 중복 실행을 막고, 저장 Promise가 실패하면 같은 문서의 최신 revision 좌표로 다시 예약한다. 성공 후에는 한 번 더 정리해 잔여 레코드를 없앤다.
...
- 오프라인 예약 기록과 온라인 1회 재생
- 새로고침 복구 레코드의 follower 대기 및 leader 승격 후 재생
- 재연결 저장 실패 시 outbox 재예약
- 사용자 취소 후 레코드 제거
