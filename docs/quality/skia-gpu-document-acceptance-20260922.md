...
Yjs, immutable source strokes, editor commands, Undo and SQLite/OPFS remain authoritative. Automerge is enabled only behind an optional offline-proposal feature flag and cannot directly become canonical without the existing authorization and Yjs promotion boundary. CanvasKit 0.41.1 uses WebGL2 for this surface; this is not a completed WebGPU conversion.
...
Canonical input and committed-stroke fences remain separate from the retained display surface. Receipt-gated transform handoff, bounded text shaping, filtered/masked/current-frame specialist rasters, settled natural-media final pixels and detached exact-revision Skia export are now admitted where their source contracts are exact. Unsupported page composites, gradients, paper grain, animation timelines, colour proofing or backing-store budgets explicitly retain the canonical compatibility renderer/export; they are not approximated or omitted. Full removal of every Konva/Canvas2D interaction owner is not a correctness goal while those explicit compatibility contracts remain. Physical 30/120-minute device operation, pen latency, thermal/battery and OS memory-pressure certification remain separate operational acceptance gates. No tests, image thresholds, CI protections or bundle ratchets were relaxed. No production deployment was performed.
...
The real `/studio/canvas` verifier was updated to require that an exact single-draw selection keeps the Skia surface active and the Konva paint tree unmounted. That runner could not be completed in the original isolated transform worktree because its local Nest API required a `DATABASE_URL` that was not available; it stopped before editor mount rather than bypassing API, authorization or persistence. This remains environment-limited negative evidence, not a product success. No test threshold, bundle budget, permission check or fallback policy was weakened.
...
## Final software migration closure — 2026-09-23

The retained document boundary now covers exact causal ink, admitted vector/path content, static panels, bounded horizontal text, rich static images, and specialist final-pixel projections for filters, masks and browser-owned current animation frames. The specialist projection is bounded by decode, pixel, byte, concurrency and ref-counted cache limits. It consumes the established worker/mask/final-surface result; it does not replace original strokes, image sources, adjustment programs, Undo or persistence with a flattened canonical asset.

Raster export no longer has to read the visible viewport for supported pages. After the existing capture-readiness fence commits pending ink, fonts and assets, an independent offscreen Skia document renderer presents an exact revision at export DPR, verifies the matching receipt, creates an encoded PNG snapshot and copies it to the existing grade/watermark/format pipeline. Master-page elements preserve their established lower z-order. Hidden groups and hidden masters remain absent. Transparent export omits the authored solid background.

This export path is deliberately fail-closed. Gradient backgrounds, paper grain, page animation timelines, colour-proof projection, unsupported elements, receipt mismatch, decoder/resource failure or backing surfaces above the admission budget return an explicit unsupported/unavailable result. The existing full-document compatibility export then remains authoritative. No viewport screenshot, omitted effect or visually different “best effort” output is substituted.

Yjs remains the only canonical collaborative document and save authority. The Automerge runtime stores and exchanges bounded offline proposal operations; promotion still crosses the existing role, lock and Yjs reconciliation boundary. P2P-only rooms exchange cooperative mutation locks with role validation and fail-closed partial-fanout handling, while a solo editor remains available when no peer claim exists. The production feature flag is managed independently from this renderer migration.

Immutable pinned-review sharing and public showcase routes consume server-issued immutable share identities. Secret tokens are accepted from the URL fragment only and are not persisted or logged. This UI closure does not grant publication or mutation authority.

A synthetic five-hour/pressure-aware harness, direct browser engine scenarios, same-browser multi-tab isolation and touch-event mobile emulation are automated software evidence. They do not certify a physical iOS/Android stylus, palm rejection, device thermals, battery draw or OS memory pressure. Those checks require named hardware and measured sessions and remain labelled operational acceptance rather than an unimplemented renderer feature. No production deployment is part of this migration closure.
