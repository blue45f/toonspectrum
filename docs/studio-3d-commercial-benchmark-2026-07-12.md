...
- ToonSpectrum 현재 강점: 실시간 선화 미리보기, 투명 PNG 삽입
- 진행 중: 엔진 중립 문서에 선화·텍스처 선·톤 출력 설정과 프리셋을 저장
- 남은 격차: 선화/톤 별도 레이어 삽입, vector 선 결과, 깊이 기반 선 굵기, 텍스처 선, 출력 프리셋 관리
...
| 영역 | 상용 기준 | 상태 | 합격 증거 |
| --- | --- | --- | --- |
| 장면 문서 | 카메라·렌더·배경·조명·품질·출력·예산·첨부·노드의 버전 문서 | 검증 코어 완료 · UI 연결 대기 | strict/lenient API 분리, UTF-8 256 KiB, 버전 우회·저장 키 유출·이모지 경계 테스트 |
| GLB 반입 | 실제 바이트·해시·GLB 구조·외부 URI·복잡도 검증 | 검증 코어 완료 · 로더 연결 대기 | 방어 복사, SHA-256, GLB 2.0, embedded-only, 모바일/데스크톱 예산, 디코드 메모리 폭탄 테스트 |
| 논리 첨부 ID | 저장 키와 장면 참조 분리 | 순수 어댑터 완료 · 라이브러리 연결 대기 | 양방향 explicit mapping, identity/conflict 차단, 저장 키 비영속·legacy mapping 테스트 |
| 품질 프로필 | desktop/mobile DPR·그림자·텍스처·LOD·FPS 목표 | 문서 모델 완료 · 런타임 연결 대기 | 375×812 및 대표 Android/iPhone에서 프레임·메모리 측정 |
| LT 출력 | 선화·텍스처 선·톤 설정, 분리 결과 | 문서 모델 완료 · 렌더러 연결 대기 | 선/톤 별도 레이어 삽입과 골든 이미지 |
| 객체 목록 | 표시·잠금·복제·삭제·검색 | 일부 구현 | 상태 왕복, 잠긴 객체 기즈모 차단, 44px 모바일 조작 |
| 다중 선택 | Shift/범위 선택, 함께 이동, 개별/중앙 피벗 | 미구현 | 다중 변형 undo 한 단계, 선택 접근성 |
| 계층 | 부모-자식 연결과 함께 이동 | 미구현 | cycle 거부, 삭제/분리 정책, round-trip |
| 접지·스냅 | 바닥 접지, 이동/회전 스텝, 객체 스냅 | 미구현 | 결정적 순수 함수와 기즈모 QA |
| 카메라 | position/target/FOV, preset, 초점, 평행 투영 | 일부 구현 | 사용자 카메라 왕복, 재열기 픽셀 근사 일치 |
| All Sides View | 원근+정면+측면+상단 동기화 | 미구현 | 선택 중심/카메라 프러스텀/모바일 단일뷰 전환 |
| 소재 라이브러리 | 장면·객체 재사용, 권리/출처, 검색/즐겨찾기 | 일부 구현 | 권리 경고, hash 중복 제거, 내보내기 포함 정책 |
