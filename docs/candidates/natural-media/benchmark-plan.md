...
- **Hokusai 주력 승격**: G1~G6 통과 + libmypaint 대비 p95 지연·peak memory 비열위 + 커버리지 리포트상 주요 프리셋군 손실 없음.
- **libmypaint 주력 유지**: Hokusai가 품질 또는 커버리지에서 미달하는 프리셋군에 대해 프리셋 단위로 유지(프리셋별 provider pin — 전역 단일 승자를 강제하지 않는다).
- 차이가 없으면 유지보수 비용이 낮은 쪽을 선택한다(V11 §6.3 — "차이가 없으면 검증 엔진을 선택한다"의 취지를 프리셋 단위로 적용).
