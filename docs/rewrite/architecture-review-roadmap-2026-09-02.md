...
| P1 | 호스트에 `EditorClient` 배선, 툴 레일 → selector+command 전환 | **완료(툴 레일)** | 레일 raw React setter 16 → 0. `StudioLeftToolRail`은 persistent `EditorClientRuntime`의 client 하나만 받고 모든 클릭을 CommandRegistry 리시트 경로로 전달 |
| P1 | 런타임 소유자 분해(`StudioDocumentRuntime`, `ToolRuntime`, `ViewportRuntime`, `DurabilityRuntime`, `CollaborationRuntime`) | **부분(툴 레일 런타임)** | `EditorClientRuntime`과 `StudioLeftToolRailRuntime`이 client identity·구독·명령 수명을 소유하고 snapshot/action context만 갱신. 호스트 500–1,000행, feature 공개 타입 `any` 0, `ViewSessionCore/Rest` 삭제는 잔여 |
