...
| P2-12 | 텍스트·말풍선 속성 세분화 | **적용(텍스트).** 감사가 실측한 타이포그래피 15개를 글꼴(5)·외형(9)·고급 조판(2)로 다시 나누고 자간·행간은 문단으로 옮겼다(문단 3→5, 같은 커밋 446d86fe). 왼쪽 15 와 오른쪽 합이 다른 것은 산술 오류가 아니라 재분할이다. 고급 조판은 이후 main 6ddf0406 의 원형 텍스트가 §4 대로 편입되며 2→3 이 됐고, 이 칸의 수는 잎이 아니라 자체 패널 개수다(근거는 밀도 표 주석). 문단(정렬·세로 쓰기·자간·행간·맞춤)은 한 곳. 정렬 이중 노출 제거. 말풍선 순서 재배치(대사 편집 최상단, 꼬리·글자·변형 순)는 보류. | `StudioInspectorTypographySection.tsx`, `StudioInspectorSelectionSection.tsx` |
| P2-13 | 용어 통합 | **부분 적용.** 불투명/투명도 → 불투명도(canonical 상수 사용), 탭 페이지 → 문서, 검색 경로 `인스펙터 › 속성` → `인스펙터 › 대상`, 문서 하위 탭 라벨 "문서 설정" 은 적용. **속성 패널 → 작업 패널은 두 번에 걸쳐 끝냈다.** 최초 반영 때는 패널 자신(헤더·aria 라벨)만 바뀌고 그 패널을 여닫는 명령·메뉴 행이 옛 이름에 남아, 통합 검색이 화면에 보이는 이름으로 이 패널을 찾지 못했다("작업 패널" 0건 / "속성 패널" 2건). 2026-09-04 1차로 명령 `window.right-panel` 라벨과 Window 메뉴 행을 "작업 패널"로 옮기고 옛 이름을 검색 별칭(`ours("속성 패널")`)으로 남겼다. 같은 날 2차로 **패널 자신의 chrome** 까지 맞췄다 — 접힌 패널을 되돌리는 엣지 레일(`label="작업 패널"`, 접근 이름 "작업 패널 펼치기"), 너비 스플리터(`label="작업 패널 너비 조절"`), 모바일 로딩 폴백 `aria-label`, 선택 섹션 Suspense 라벨. 이 이름들은 카피가 아니라 셀렉터이기도 해서 같은 변경에서 `scripts/verify-studio-inspector-walkthrough.mts` 의 `button[title="속성 패널 펼치기"]` 와 `tests/benchmarks/harness/heavy-feature-perf.ts` 의 `[title="속성 패널 접기"]`·`[title="속성 패널 펼치기"]` 두 셀렉터(앞의 것은 이미 어느 요소와도 매칭되지 않던 죽은 셀렉터였다)를 함께 옮겼다. 회귀 방지는 색인 질의(`studio-command-search.test.ts`)와 chrome·셀렉터 동기 계약(`StudioInspectorAsideShell.accessibility-contract.test.ts`)이 맡는다. **남은 것(재집계, `src/`·`scripts/`·`tests/` 기준):** "속성 패널" 리터럴은 31곳/19파일이다. 그중 11곳/5파일은 옛 이름을 일부러 보존하는 줄(카탈로그 검색 별칭, 이름이 갈라졌던 경위를 적은 pin 주석, 이번 회차 회귀 테스트의 질의·부정 단언)이라 지울 대상이 아니다. 실제 미정리 카피는 20곳/14파일 — 사용자에게 보이는 문자열 9곳(도구 힌트 `StudioLeftToolRail`·`studio-brush-behavior-ui`, 툴벨트 설명 2, 튜토리얼 본문 3, 밀도 설명 1, 모바일 힌트 1)과 주석·테스트 픽스처·검증기 진단 문자열 11곳. 이 중 툴벨트 2곳과 `studio-inspector-density` 1곳은 좌우 두 패널을 한꺼번에 가리키므로 단순 치환이 아니라 문장 재작성이 필요하다(별도 카피 회차). "빠른 수정 → 빠른 이미지" 는 보류(검색 색인·튜토리얼·검증기 전반에 걸친 이름이라 별도 회차). | `studio-command-catalog.ts`, `studio-main-menu-items-workspace.ts`, `StudioCuttoonEditorInspectorColumn.tsx`, `verify-studio-inspector-walkthrough.mts` 외 여러 파일 |
...
| 주요 용어 | 같은 개념 동일 명칭 | 부분 — 불투명도·문서·대상 통일. "작업 패널"은 패널 자신과 이를 여닫는 명령·메뉴 행, 그리고 접기/펼치기 엣지 레일·너비 스플리터·로딩 폴백까지 한 이름이다(2026-09-04). 이 패널의 chrome·명령·메뉴에서 옛 이름 "속성 패널"은 검색 별칭으로만 남았다. 다만 이 패널을 문장 안에서 가리키는 카피는 20곳/14파일(사용자에게 보이는 문자열 9곳 — 도구 힌트·툴벨트 설명·튜토리얼 본문·밀도 설명)로 미정리다(P2-13) |
...
| `StudioDrawNode` 펜슬 3건 | e90aadbe 가 리본 셀을 알파 버킷별 복합 경로로 배치 → 스텁이 fill 당 마지막 서브패스만 기록 | 스텁에 서브패스 기록(`fillSubpathPolygons` + 병렬 알파 배열 `fillSubpathAlphas`) 추가, 셀 단언을 서브패스 기준으로. `fillPolygons`/`fillCompoundPaths`/`fillAlphas` 는 fill 당 한 항목을 유지한다 |
