...
| 장면 이미지 | 장면별 이미지 생성, 재생성, 후보 선택, 다운로드 | 기존 코믹 디렉터 후보 2개, 선택, 재생성, 부분 수리, 픽셀 품질 분석 | `StudioAiComicDirectorPanel.tsx` | 기존 기능 재사용 |
...
| 계정·사용량 | 로그인, 토큰/사용량, 결제·추천 | 제작 문서에 중복 구현하지 않고 기존 인증, membership wallet, commerce, AI admission에 유지 | `apps/api/src/modules/membership-wallet`, `commerce`, `studio-ai` | 기존 플랫폼 재사용 |
