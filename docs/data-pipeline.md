...
WEBDEX의 작품 데이터는 **파일 seed가 아니라 서버 DB 스냅샷**을 운영 소스로 사용합니다.
...
flowchart TD
    ...
    D --> DB[("libSQL(로컬 data/webdex.db) / Turso(원격)<br/>catalog_snapshot · isCurrent=1")]
...
② INGEST  pnpm ingest │ cron(CATALOG_INGEST_MODE=fixed) │ POST /api/catalog/ingest/run
   품질 게이트(총건수 급감·주요소스 붕괴 시 승격 거부) → catalog_snapshot INSERT(isCurrent=1)
        │  libSQL(로컬 data/webdex.db) / Turso(원격, TURSO_DATABASE_URL)
        ▼
...
- **단일 DB**: `lib/db`가 cwd가 아니라 **레포 루트 `data/webdex.db`**(또는 `TURSO_DATABASE_URL`)로 고정 →
  크롤/ingest/API/drizzle-kit이 동일 DB를 사용(프로세스 실행 위치에 따라 갈리던 split-brain 방지).
...
> 원격(Turso) 적재: `TURSO_DATABASE_URL` + `TURSO_AUTH_TOKEN`을 환경에 주입하면 ②③④가 모두 원격 DB를 사용합니다.
