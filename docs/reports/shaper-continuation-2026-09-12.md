...
PSD 편집 레이어 재합성·비대칭 카메라·의상 접촉·장시간 출력의 다음 개선은
[2026-09-13 후속 품질 기록](shaper-remaining-quality-2026-09-13.md)에 이어진다.
...
기본 Worker API는 caller buffer를 복제한다. 제품 캡처가 독점 소유한 패스만 명시적으로 transfer하며, 부분 view와 중복 backing buffer는 이 경로에서 거부한다. 버전·요청 ID·패스·동일 크기·바이트 예산·응답 형식과 PSD v1/RGB8 헤더를 검사한다. 가로 최소 3px, 최대 14패스·2048²·입력 224MiB·PSD 출력 256MiB이며, 조립 중간 버퍼의 추가 메모리를 포함한 총 프로세스 예산이라는 뜻은 아니다.
...
- 최종 집중 회귀 15개 파일 306개 통과. 최종 CI 실행 계약 103개 통과. 추가 회귀 8개 경로를 필수 core 검사에 연결했다.
...
- 전체 캐릭터 제품 UI의 native 검사 통과: 1926×2048 및 3852×4096 투명 PNG, 4K 렌더 6%에서 취소/다운로드 0/권한 반납/재시도, 실제 PSD Worker 실행 1회와 파일 다운로드, 390px·320px 모바일 버튼·overflow 검사, page 오류 0. 원본: `/private/tmp/shaper-next-qa/character-boundary-native/character-shaper-evidence.json`.
...
`ag-psd` 31.0.1은 폭 1px RGB/RGBA 및 폭 2px RGBA 합성 이미지의 임시 RLE 저장소 부족으로 픽셀을 잘라내는 문제가 재현됐다. 손상된 파일을 성공으로 반환하지 않도록 공통 입력·Worker 응답·PSD 헤더 검증에 최소 폭 3px를 적용했다. 폭 1·2px는 alpha 여부와 관계없이 명시적으로 거부하고, 폭 3px 및 일반 제품 해상도에서 실제 파일을 다시 읽어 확인한다. 정상 UI의 긴 변 1024/2048/4096px 선택 범위는 유지한다.
