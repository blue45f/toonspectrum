...
정렬된 두 캔버스의 획 중심 행 평균 알파를 열별로 추출(12열 샘플): 커밋은 균일 스케일이
아니라 **공간적 패치 소실** — 일부 열은 보존(6.0→6.4, 5.8→6.1)되고 일부 열은 붕괴
(4.7→0.4, 8.2→0.4). 총 에너지 2,987→1,706(0.57×). 패치가 종이 결 위상과 공간 상관을
보이므로, 커밋 체인 어딘가에서 라이브보다 강한 종이 변조가 한 번 더 걸리거나, 같은
마크라도 다른 응답 상수(직렬화 왕복 등)로 계획되고 있다.

다음 세션 실험(우선순위 1): 검증기 페이지에서 릴리스 직후 요소의 brushDynamics를
