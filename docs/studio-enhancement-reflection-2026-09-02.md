...
| 적색 main에서도 배포되는 구조 | **참**, 단 2026-08-14 의도적 결정 | 소유자 지시로 main 브랜치 보호(`core` 필수) 적용 — 관리자 우회(`enforce_admins=false`)만 예외 (§3.3) |
...
효과: PR 경로로는 적색 커밋이 main에 들어갈 수 없으므로 "적색 main 배포"는 구조적으로 사라진다. 유일한 예외는
`enforce_admins=false`가 허용하는 관리자 직접 push·`--admin` 머지이며, 이는 게이트가 깨졌을 때의 탈출구다 — 우회 커밋은
그대로 배포되므로 사유를 남기고 다음 PR에서 core를 복구해야 한다. 배포 경로(Vercel) 자체는 그대로다.
