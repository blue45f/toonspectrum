...
| Auto-color hints panel + scribble/apply | fill-tab; worker `onRun`; selected `imageSrc`; **scribble seed palette + rec chips**; **Advanced Fill batch apply** via `onApplyResult` → `patchEl`; **새 채색 레이어** multi-layer target via `onApplyNewLayer` (transparent paint layer) |
...
| Auto-color freehand canvas scribble | Seed chips + multi-layer paint target shipped; freehand paint-on-canvas seed strokes still deferred |
