...
- KTX2/BasisU: decoder-free 구조 검증 경계와 pinned decoder release gate를 구현했다. KTX2 식별자·header, 안전한 uint64와 level
  ...
  `KHR_texture_basisu` source/fallback 참조를 먼저 검증한다. optional Basis는 검증된 PNG/JPEG fallback을
  요구하고, 필수 Basis 확장은 renderer allowlist, 실행 realm이 직접 발급한 transcoder capability,
  실제 payload preflight를 모두 요구한다. 선언 mip 전체의
  ...
  0.184.0의 JS/WASM 크기·SHA-256을 고정했다. validation Worker는 같은 realm에서 raw JS와 bounded-fetch
