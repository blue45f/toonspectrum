...
The W3C Pointer Events specification defines pressure, tangential pressure, tilt, twist, altitude/azimuth,
contact geometry, coalesced events, and predicted events as browser-exposed input channels. It also permits
user agents to coalesce pointer movement for performance and intentionally leaves prediction algorithms to
the browser. The Input Center therefore treats capability detection as runtime evidence rather than a
promise, uses the latest coalesced event as the current observed sample, and never treats a predicted event
...
- The repository's protected `core` gate remains responsible for lint, typecheck, build, and the complete
  test matrix on the pull request.

## Real-device acceptance matrix

Source-level and synthetic-event tests cannot substitute for hardware validation. Before release, run this
matrix on the production build:
...
- Adobe Fresco, “Pixel brushes” (last updated 2026-05-07):
  https://helpx.adobe.com/fresco/desktop/draw-paint-animate-and-share/pixel-brushes.html
- Concepts, official product overview:
  https://www.concepts.app/en/
