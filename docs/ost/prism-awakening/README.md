...
Dependency safety: the worktree reuses the original checkout's installed dependencies; package.json, pnpm-lock.yaml and pnpm-workspace.yaml were byte-identical when checked. pnpm 11 attempted an automatic reinstall because installed allowBuilds metadata differed. For local hook runs, the process-local `pnpm_config_verify_deps_before_run=warn` retains the dependency warning without purging shared node_modules. Git hooks, lockfile validation, lint, secret checks, architecture checks and typechecks are not disabled by this setting.
