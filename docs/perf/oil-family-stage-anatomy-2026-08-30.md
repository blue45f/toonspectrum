...
판정했는데, **이 카운트는 캡에서 4096 에 고정된다**. 그런데 캡에서는 `sampleStations` 가 전체
호에 격자를 다시 깔기 때문에 새 표본 하나가 **모든 스테이션을 움직인다** — 즉 카운트가 근거가
되기를 멈추는 바로 그 지점에서 근거로 쓰이고 있었다.

결과: **4096 대브를 넘어선 오일 획은 드래그가 끝날 때까지 라이브 프리뷰가 커서를 따라오지
않았다.** `appendFrom` 의 반환 상태도 같은 포화 카운터로 계산돼 실제로 칠한 프레임을 `noop`
으로 보고했다.

바깥에 있던 `paintedDabs >= FX_OIL_DAB_CAP` 조기 반환은 이 동작을 의도적으로 굳힌 것이었고,
주석의 근거는 "이동당 전체 refit ~70ms". 그 근거는 8절로 이미 약해졌지만, 매 표본마다 전면
재계획하면 여전히 이동당 35–55 ms(캐리어 지배)를 메인 스레드가 잡는다.

**그래서 얼리지도, 매번 칠하지도 않는다**: 캡에서는 소스가
`OIL_CAP_REPAINT_SAMPLE_STRIDE`(32) 표본만큼 자랐을 때 다시 칠한다. 캡 베드는 자라는 게 아니라
재분배되므로 표본 하나가 만드는 이동은 서브픽셀이고, 32개를 합쳐 칠해도 프리뷰는 커서를 따라온다.
이동당 상각 비용은 약 1 ms 대다. 판정도 실제 칠 횟수(`paintedOilPasses`)로 옮겨 `noop` 오보가
사라졌다. 캡 아래 동작은 그대로다(표본이 하나라도 늘면 칠한다).

세 가지를 테스트로 고정했다 — 캡에서 자란 획은 다시 칠한다(고치기 전 적색), 32 미만은 합친다,
새 표본이 없으면 여전히 건너뛴다.
