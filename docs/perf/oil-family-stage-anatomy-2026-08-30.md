...
여섯 성질을 각각 반증적으로 고정했다(해당 부분만 되돌리면 그 테스트만 적색): 캡을 넘긴 획이
자라면 다시 칠한다 / 비싼 재칠은 비례해서 유예된다 / 쿨다운은 끝난 시각부터다 / 유예하면 스스로
깨어나 칠한다 / 같은 길이의 대체 초안도 다시 칠한다 / 이미 일치하는 베드는 `end()` 에서 다시
세우지 않는다. 예산의 타이밍과 깨우기는 **주입한 시계·타이머**로 검증해 기기 속도나 실제 타이머에
좌우되지 않는다.
