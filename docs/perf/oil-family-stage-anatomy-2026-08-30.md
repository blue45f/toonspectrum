...
캡 베드의 대브 수는 4096 → 3492~3793 (**밀도 85~93%**). 사다리가 한 칸 오른 직후가 가장 성기고
(1/1.19 = 84%) 다음 칸까지 자라며 100% 로 회복하는 톱니다. 오일 대브는 반경이 간격의 몇 배라
겹침이 크고, 이 정도 변화는 텍스처의 알갱이를 조금 굵게 만든다.

### 오일에만 적용한 이유

`sampleStations` 는 글리터·파스텔도 쓴다. 그쪽은 입자를 훨씬 싸게 계획하고 캐리어 파이프라인이
없어 재계산이 아프지 않다. 이득 없이 대브 분포만 바꿀 이유가 없어 `capMode` 를 옵트인으로 두고
오일 두 호출부만 켰다 — 나머지는 기존 경로 그대로다.
