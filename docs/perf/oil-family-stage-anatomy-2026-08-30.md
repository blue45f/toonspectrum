...
- 가드가 대브 카운트 대신 **소스 표본 수 + 마지막 점**을 본다. 표본 수만으로는 예측 초안을 같은
  길이의 authoritative 초안이 **대체**한 경우를 못 봐서 회수된 픽셀이 화면에 남는다.
- 가드가 **계획 전에** 돈다 — 기존에는 계획한 뒤 버렸다.
- 포인터 업은 유예된 꼬리를 반드시 흘려보내되(`finalize`), 이미 일치하는 베드를 공짜로 다시
  세우지는 않는다.
- 반환 상태가 실제 칠 횟수(`paintedOilPasses`)를 읽어 `noop` 오보가 사라졌다.

**캡 아래 동작은 그대로다** — 표본이 하나라도 늘면 칠한다.

네 성질을 각각 반증적으로 고정했다(해당 수정을 되돌리면 그 테스트만 적색): 캡을 넘긴 획이 자라면
다시 칠한다 / 비싼 재칠은 비례해서 유예된다 / 같은 길이의 대체 초안도 다시 칠한다 / 이미 일치하는
베드는 `end()` 에서 다시 세우지 않는다. 예산의 타이밍은 주입한 시계로 검증해 기기 속도에
좌우되지 않는다.
...
| 항목 | 이득(캡 기준) | 재개 조건 |
| --- | --- | --- |
| 캡에서의 캐리어 전면 재계획 | ~30 ms/이동 | `sampleStations` 가 접두를 보존해야 한다 = 격자를 바꾸는 결정. 예컨대 스텝을 2의 거듭제곱 사다리로 양자화하면 접두는 안정해지지만 캡에서 대브 밀도가 최대 2배까지 성기어진다 — **픽셀이 바뀐다** |
