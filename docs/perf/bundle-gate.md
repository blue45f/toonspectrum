...
### 한시적 Studio route gzip 여유 (2026-09-14 승인)

소유자의 임시 상한 상향 요청에 따라 **`static / Studio route gzip`만 +1 KiB** 허용한다.
기존 1,966,337 B (1,920.3 KiB) → 임시 1,967,361 B (1,921.3 KiB).
**2026-09-21 00:00 한국시간부터 자동으로 기존 상한을 적용**한다. 만료는 빌드 실행 시 평가되며,
이미 배포된 앱을 중단하거나 파일을 자동 변경하지 않는다.

`scripts/lib/studio-bundle-temporary-allowance.mjs`가 이 단일 예외를 소유한다. 기준선 JSON의
실측치·기록 날짜·+2% 정책은 변경하지 않는다. 다른 바이트 지표·청크 수·구조·런타임 검사는 그대로다.
예외 대상의 기준선이나 기본 상한이 바뀌면 적용하지 않으며, 임시 상한까지 초과하면 계속 실패한다.
기존 상한을 넘고 예외 안에만 드는 결과는 `temporarily allowed`로 표시해 최적화로 오인하지 않는다.

승인 시 확인한 main `63bbce91`의 실제 빌드는 1,918.6 KiB로 기존 상한에도 통과했다.
이 변경은 과거 약 0.3 KiB 초과를 다시 막지 않도록 요청한 여유이며, 기능 오류 수정이나 성능 개선은 아니다.
최적화 완료 시 이 예외와 게이트 연결·관련 테스트를 함께 제거한다. 전체 기준선 재기록이나 자동 연장은 하지 않는다.
