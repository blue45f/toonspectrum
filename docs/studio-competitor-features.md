...
>
> 모바일 집중 셸의 후속 입력·초기 로딩 계측, 공식 제스처 비교와 번들 회귀 예산은
> `docs/studio-mobile-canvas-performance-benchmark-2026-07-13.md`에 기록했다.
...
- **컷툰 편집기 전용 앱 셸**: 1024px 미만의 `/studio` 컷툰 편집기는 기본적으로 사이트 헤더·푸터·
  전역 플로팅 컨트롤·맨 위로 버튼을 실제 React 트리에서 언마운트한다. 단순 z-index 가림이 아니므로
  숨은 사이트 링크가 Tab 순서와 스크린리더 트리에 남지 않는다. 토스트·호환성 경고·커맨드 팔레트 같은
  필수 전역 오버레이는 유지하며, `/studio?mode=upload` 업로드 편집기는 영향을 받지 않는다.
- **상태를 보존하는 임시 전환**: 항상 보이는 44px `앱 모드 종료`/`집중 드로잉` 버튼으로 같은 편집기
  인스턴스 안에서 왕복한다. 현재 페이지, 선택, undo/redo, zoom, 캔버스 스크롤을 초기화하지 않는다.
  사용자의 명시적 종료만 탭 수명의 `sessionStorage`에 기억하며 작품·계정·작업공간 문서에는 넣지 않는다.
  storage가 차단되면 메모리 상태로 동작하고 다음 진입은 안전한 기본값인 집중 모드다.
- **고정 높이 차감 제거**: 기존 `max-height: calc(100dvh - 26rem)` 추정값을 몰입 모드에서 제거했다.
  셸·중앙 작업면·캔버스를 `min-height: 0` flex 체인으로 연결해 상단 44px 작업 바와 한 줄 도구막대,
  실제 112px 하단 도크를 제외한 잔여 높이를 캔버스가 전부 사용한다. 긴 원고는 페이지가 아니라 캔버스
  viewport 내부에서만 스크롤하고 `overscroll-contain`으로 문서 바운스를 막는다.
- **safe area·키보드·도크 경계**: `100dvh`와 상/좌/우 safe area, 기존 `visualViewport` 키보드 inset을
  유지한다. 하단 도크 예약값과 페이지/속성/브러시 시트를 실제 7rem 높이에 통일해 캔버스와 4px 간격을
  확보하고, 배경 문서는 앱 모드 동안 `overflow: hidden`이다. 전역 토스트도 도크와 홈 인디케이터 위로
  이동하며 프로젝트 작업 메뉴의 마지막 명령은 하단 safe area 안쪽에 남는다.
- **저화면 안전성·접근성**: 다운로드·내보내기 옵션·저장·게시까지 모바일 핵심 액션을 최소 44px로
  맞췄다. 오류·공동 잠금·복구·다중 선택·고급 채우기 알림은 숨기지 않고 높이가 낮을 때 독립 스크롤되는
  bounded status rail에 담아 캔버스와 안전 조작이 함께 잘리지 않게 한다. 시각적으로 압축된 앱 바에는
  스크린리더용 H1을 유지하고, `/studio` 진입은 장식 와이프 없이 즉시 안정된 작업면을 표시한다.
- **실측 결과**: 375×812 깨끗한 원고에서 일반 모드 캔버스 343×396px가 집중 모드 367×590px로
  확대됐다(폭 +7.0%, 높이 +49.0%). 390×844는 382×622px, 430×932는 422×710px, 768×1024는
  760×802px, 1023×800은 1015×578px였다. 844×390 가로 화면에서도 836×168px 캔버스와 112px
  도크 사이 간격을 유지했다. 전 구간 문서 가로 overflow 0, 사이트 크롬 DOM 0, body 스크롤 잠금,
  1024px 경계의 일반 데스크톱 복귀, 업로드 모드 미영향, 콘솔 error/warning 0을 실제 브라우저로 확인했다.

375×812 최종 상태를 실제 브라우저 캡처로 함께 검수했다.
...
- **서버 owner-only revision**: 작품 생성·수정·복원을 snapshot과 같은 PostgreSQL transaction으로 처리하고
  ...
  Studio의 버전 패널에서 로컬 이름 있는 복구 지점과 서버 자동 버전을 분리한다. 서버 버전 복원 전에는
  현재 편집본→선택 revision과 현재 서버→현재 편집본을 별도로 의미 비교하며, 대형 원고는 전용 Worker에서
  계산하고 세부 결과를 240개로 제한하되 합계는 정확히 유지한다. 변경 페이지·요소로 바로 이동할 수 있고,
  비교 API는 렌더 cover/pages를 제외하고 data/blob 리소스를 SHA-256 식별자로 투영한다. 복원 직전
  IndexedDB 로컬 checkpoint 저장 실패·비교 후 로컬 변경·stale base revision이면 mutation을 시작하지
  않으며, 공개 상태·연재 연결 변경과 호환되지 않는 upload-format 복원을 명시적으로 차단·경고한다.
  미래 `doc` 확장도 원문 값을 결과에 복사하지 않은 채 변경 키를 감지하고, AI 비교는 실제 의미 필드만
  allowlist로 투영한다. 공개 응답에는 revision/snapshot과 재귀적 private note 필드를 노출하지 않는다.
...
- **모바일 우선 조작** — `추가` 빠른 실행에 3D 배경 카드를 노출하고, 320/390px에서도 물리 transport의
  ...
  글자가 세로로 깨지지 않게 한다.
- **모바일 필터 작업** — 이미지 선택 문맥 바에서 가우시안·모션 블러, 색조/채도/밝기, 명도/대비,
  RGB 커브를 네이티브 터치 선택기로 바로 연다. 320px 대화상자는 2열 숫자 레이아웃과 44px 조작 대상을
  사용하고, 캔버스 미리보기는 문서/CRDT 밖에서 수행한 뒤 `적용` 성공 시에만 단일 undo 단계로 확정한다.
...
- **모바일 하단 도구막대 겹침 수정** — 전역 사이트 헤더의 모바일 하단 탭바와 스튜디오 자체 도구막대가
  ...
  `hideBottomTabs`(경로가 `/studio`로 시작하면 전역 하단 탭바 숨김) 추가 + 당시 캔버스 max-height를
  실측 기반 13rem→26rem 차감으로 보정했다. 이 고정 차감은 2026-07-13 모바일 전용 집중 드로잉 셸에서
  잔여 높이 flex 구조로 대체됐고, 일반 모바일 사이트 모드의 하위 호환값으로만 남는다. 커밋 `dfeb2c1`.
...
- 함정: 개발 서버가 여러 세션이 동시에 접속하는 공유 브라우저/포트일 수 있다 — 이번 검증 중에도 무관한
  다른 세션의 `src/app/errors/error-boundary.tsx` 동시 수정이 Vite HMR 풀 리로드를 유발해 팝오버 상태
