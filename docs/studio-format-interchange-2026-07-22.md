...
| CBZ | 미지원 | 사용 가능 | 없음 | 전체 페이지를 평탄화해 `ComicInfo.xml`과 묶는 독자 배포 패키지. import codec core는 있으나 UI와 decoded-image 사전 검증이 남음 |
| OpenRaster `.ora` | 엔진 준비 | 사용 가능 | 부분 | import codec은 ZIP·PNG IHDR·개별/누적 decoded-pixel 예산을 검사하지만 아직 UI 미연결. 현재 메뉴의 export는 화면 합성 1레이어 |
...
### 1. 남은 포맷 허브 연결과 손실 미리보기

ASE/ACO/ACT/JASC-PAL/CSS/JSON 팔레트 양방향 UI와 CBZ/ORA export는 연결됐다. 다음 단계는
안전 검증된 ORA/CBZ import를 파일 메뉴에 연결하고, 공개 래스터의 1,280px WebP 프록시 변환처럼
해상도·알파·색공간·레이어가 달라지는 작업을 실행 전에 요약하는 공통 손실 미리보기다. 초기 Studio
bundle에는 codec을 넣지 않고 파일 선택/메뉴 hover에서 dynamic import로 예열하는 원칙은 유지한다.

### 2. OpenRaster 고도화

- 중첩 group, opacity, visibility, blend mode mapping과 `mergedimage.png`/`Thumbnails/thumbnail.png`
- 알 수 없는 XML extension은 보존할 수 없음을 import summary에 명시
- layer PNG 총 decoded pixel/byte budget, ZIP bomb/zip-slip/duplicate-path 차단
- Studio 전용 벡터·말풍선·필터는 원본 편집 데이터를 프로젝트 archive에 유지하고 ORA에는
  렌더 레이어를 내보내는 dual-save workflow

### 3. CBZ·이미지 시퀀스

- 페이지 번호 zero-padding, Unicode-safe filename, natural order 고정
- PNG/JPEG/WebP 선택과 전체 예상 용량 preflight
- `ComicInfo.xml`은 개인정보·AI provenance 공개 정책이 확정된 필드만 opt-in
