...
| BG3D 실행취소/다시실행 | 동작 |
| BG3D 씬 템플릿 | "교실"(오브젝트 39개) 적용 시 칠판·창문·책상 3×3이 실제로 렌더 |
| BG3D 조명 프리셋 | 시네마틱 3점·선화 균일이 같은 장면의 휘도를 실제로 바꿈 |
| BG3D 물리 미리보기 | 재생 0→4.0초 완주, 다시 재생·초기화·현재 자세 적용 제공 |
| BG3D 줄자·추론 가이드 | 측정 모드 진입·종료, 단위(mm/cm/m) 전환 |
| BG3D 공유 3D 스테이지 | 문서에 캐릭터가 있으면 "캐릭터 연결"이 열리고 1명 연결 예정으로 표시 |
| BG3D 내 템플릿 저장 | 현재 장면 저장 동작 |
| 3D 캐릭터(VRM) | 캐릭터 렌더, `.vrm` 업로드, "이 포즈로 추가"가 캔버스 레이어 생성 |
| 3D 데생 인형 | 렌더, 포즈 라이브러리, "캔버스로 캡처"가 캔버스에 삽입 |

WebXR(VR·AR 미리보기)은 이 환경에서 `지원 안 함`으로 비활성화되고 그 이유를 함께 보여준다 —
기기가 WebXR을 주지 않을 때의 의도된 degradation이며, 결함이 아니다.
