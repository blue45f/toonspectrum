...
Status: **implemented integration candidate; product cutover validation pending**.
...
Composite records are retained in 128-item batches: a normal append rebuilds its one affected batch, while unchanged geometry is reused. Snapshot identity descriptors use weakly assigned numeric revision tokens rather than holding old raw point arrays. Imperative clip scrolling follows the same live Stage camera as input, without waiting for the settled React snapshot.
...
See `docs/quality/skia-gpu-resume-20260922.md` for measured GPU backends, canonical pixel checks, 10000-stroke stress results, the failed SwiftShader stress run, and the still-blocked unauthenticated full-editor probe. None is replaced by a fabricated full-editor pass.
