...
| 2 | Vello GPU WebGPU wasm | **빌드 게이트 통과** | 2026-08-08 `cargo build --release --target wasm32-unknown-unknown --features gpu` 성공(21.28s), 산출물 `studio_engine_vello.wasm` 4,406,493B(≈4.4MB), vello 0.9.0+wgpu 29.0.4 wasm32 컴파일 확인 | 실브라우저 WebGPU 구동: 디바이스 획득·렌더·레인 1과 동일 퍼지 패리티·성능 실측. 통과 시 `toon-vello` fork 2트랙 개설(`docs/engines/vello-baseline.md` §3) | vello_cpu wasm(기본 wasm-pack 산출물, gpu 피처 제외) |
...
| 9 | 8h/24h soak | **진행 중** | 8h 런 2026-08-08 KST 00:31 기동 실측(`SOAK_MINUTES=480 pnpm run soak:studio-engine`, `tests/benchmarks/harness/soak.ts` 프로세스 가동 확인; 지시 기점 00:29) — 완료 시 `tests/benchmarks/results/soak.json`에 기록. 현재 동 파일은 직전 스모크 런(0.25분, 356 cycles, 14,240 commands, errors 0) 결과 | 8h 런 errors 0 + RSS 추이 판정 통과 → 24h 런 착수(§13 Reliability) | 스모크 soak(분 단위)로 회귀 방어 유지 |
