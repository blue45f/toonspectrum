...
| 14 | OCIO + LittleCMS deltaE 색관리 랩 | **조사 단계** | 스튜디오에 고비트/P3 파이프 기출하(2026-07-24 웨이브). 게이트 매트릭스의 "OCIO/LCMS reference deltaE 허용치" 대조 랩은 미구축 — 현재 색변환은 canvaskit skcms+자체 구현으로 시각 회귀만 방어 | 참조 매트릭스(sRGB↔Display-P3↔선형) deltaE 랩 하니스 구축·허용치 실측 → wide gamut 레인 승격. 인쇄 CMYK는 별도 파이프(매트릭스 명기) | 현행 skcms 경로(시각 회귀 테스트로 방어) |
