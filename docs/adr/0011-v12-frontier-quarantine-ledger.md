...
| 11 | libmypaint WASM 자연매체 기준 | **격리** | 레인 3과 동일 실측: emcc/emsdk 부재(2026-08-08). C 소스 WASM 빌드 파이프가 리포·로컬에 없음. 대신 Rust 챌린저 Hokusai가 이미 실측 레인: fullFrame sha256 골든 6변형(`tests/visual/hokusai-fidelity.test.ts`) + MYB 컴파일 브리지(`packages/studio-brush-platform/src/raster-compile.ts`, 필압 상관 0.963/0.940) 출하 | emsdk 재현 빌드 구축 후 MyPaint reference parity + 1000px 브러시 처리량(매트릭스 행 게이트). Hokusai가 "품질 동률+20% 처리량" 게이트를 먼저 통과하면 libmypaint는 참조 전용으로 축소 | Hokusai(현행 실측 레인) / CanvasKit textured dab |
