...
를 갖는다 — 둘 다 커스텀 업로드 모델에는 의미가 없다(모델은 자체 머티리얼/텍스처를 가져오거나,
`.obj`인 경우 무광 중립색 하나로 통일된다 — §14 마지막 항목 참고). 대신 `BgCustomModelInstance`는
...
## 14. 범위 밖(의도적 축소) — 후속 패스가 넓힐 수 있는 지점

- **SketchUp `.skp` 자체는 파싱하지 않는다.** `.skp`는 비공개 바이너리 포맷이라 브라우저에서
  직접 열 수 없다 — 대신 SketchUp의 표준 내보내기 대상인 `.glb`/`.gltf`/`.obj`만 지원한다
  (사용자가 SketchUp에서 "내보내기"로 만든 파일을 업로드하는 흐름). "스케치업 대응"이라는
  이 스튜디오의 3D 배경 도구 취지(도형 블록아웃)와 일관된 범위 설정.
- **멀티파일 `.gltf`(외부 `.bin`/텍스처 참조)는 지원하지 않는다** — 업로드 UI가 파일 하나만
  받는 구조라(VRM 업로드와 동일 패턴), 텍스처/버퍼가 별도 파일로 분리된 `.gltf`는 로드 시
  참조가 깨진다. `.glb`(바이너리 단일 파일)를 권장 포맷으로 안내하는 문구를 업로드 UI에 넣는
  것을 권장.
...
- **라이브러리 썸네일 없음(v1)** — 그리드에는 아이콘 폴백만 표시한다(포맷 뱃지 정도는 가능:
  ...
  필요 — 이번 패스 범위 밖.
- **`.obj`는 `.mtl`을 지원하지 않는다** — 업로드 UI가 파일 하나만 받으므로 `.mtl`+텍스처를
  별도로 받을 경로가 없다. 대신 `applyBg3dFallbackMaterial`이 모든 `.obj` 모델에 무광 중립색
  하나를 강제해, 재질이 깨진 채(검은색 등) 렌더링되는 것을 막는다.
- **모델 원점 재중심화 없음** — `measureBg3dObjectSize`/`computeAutoFitScale`은 크기만
  정규화하고 로컬 원점(피벗)은 내보내기 원본 그대로 둔다. 내보낸 툴이 오브젝트를 월드 원점이
  아닌 곳에 두고 내보냈다면(원본 씬의 위치를 그대로 반영한 파일) `TransformControls` 기즈모가
  눈에 보이는 메시에서 떨어진 곳에 나타날 수 있다 — 실사용에서 문제가 되면 `THREE.Box3.getCenter`
  로 구한 오프셋만큼 로드 직후 지오메트리를 이동시키는 재중심화를 후속 패스에서 추가한다.
