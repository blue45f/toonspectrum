...
describe("auth session import graph", () => {
  it("keeps the shared API client below the React auth facade without an ineffective dynamic edge", () => {
    const authStore = readFileSync(join(process.cwd(), "src/compat/auth-session-store.ts"), "utf8");
    const apiClient = readFileSync(join(process.cwd(), "src/infrastructure/api.ts"), "utf8");
