 ...
 *
 * A bearer-like signed token cannot be made safe by putting it in a different
 * Web Storage bucket: any same-origin XSS can still read it.  `sessionStorage`
 * does, however, prevent the token from surviving a browser restart or being
 * copied into newly opened top-level tabs.  This module deliberately keeps the
 * token out of `localStorage` while retaining an in-memory fallback for locked
 * down / private browser contexts where Web Storage is unavailable.
 */
...
function parseStoredSession(raw: string | null, now: number): Session {
  if (!raw || raw.length > CLIENT_SESSION_MAX_SERIALIZED_BYTES) return null;
  try {
...
export function readClientSession(now: number = Date.now()): Session {
  ...
  const legacySession = parseStoredSession(legacyRaw, now);
  // Never leave a bearer-like token behind in persistent localStorage, even
  // when sessionStorage is unavailable. The in-memory caller can still use a
  // valid migrated value for the current tab.
...
/** Writes only sessionStorage; blocked storage degrades to caller-owned memory. */
export function persistClientSession(value: unknown, now: number = Date.now()): Session {
  const session = normalizeClientSession(value, now);
  if (session) writeStorage("session", SESSION_KEY, JSON.stringify(session));
