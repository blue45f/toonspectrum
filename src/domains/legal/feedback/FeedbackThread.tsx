...
export function FeedbackThread({ postId, userId, revision, onAdded }: { postId: string; userId: string | null; revision: number; onAdded: (reply: FeedbackComment) => void }) {
  const id = useId();
  ...
  return <section className="fb-thread" aria-label="댓글과 운영자 답변">
    ...
    {userId ? <form onSubmit={send} className="fb-form fb-reply-form" aria-busy={sending}>
      ...
      <div className="fb-row"><span className="fb-caption">개인정보는 남기지 마세요.</span><button type="submit" className="fb-button" disabled={sending || !text.trim()}>{sending ? "등록 중…" : "댓글 등록"}</button></div>
