...
function ProgressEditor({ post, onUpdated }: { post: FeedbackEntry; onUpdated: (patch: Partial<FeedbackEntry>) => void }) {
  const id = useId();
  ...
  const save = async (event: React.FormEvent) => {
    ...
    if (busy.current || progress === post.progress) return;
  ...
  return <details className="fb-management">
    ...
    <form onSubmit={save} className="fb-form" aria-busy={saving}>
      ...
      <button type="submit" disabled={saving || progress === post.progress || note.trim().length < 2} className="fb-button">{saving ? "저장 중…" : "처리 상태 저장"}</button>
...
export function FeedbackPostCard({ post, expanded, onToggle, userId, canManage, onUpdated, onTag }: Props) {
  ...
  return <li className="fb-post" data-expanded={expanded || undefined}>
    <article>
      ...
      <div className="fb-post-bottom">
        ...
        <button type="button" className="fb-vote" onClick={vote} aria-pressed={post.viewerVoted} aria-label={`${post.title} 공감 ${post.voteCount}개${post.viewerVoted ? " 취소" : ""}`} disabled={!userId || voting} title={!userId ? "로그인 후 공감할 수 있어요" : undefined}><ThumbsUp size={15} aria-hidden="true" /><span>{voting ? "…" : post.voteCount}</span><span>공감</span></button>
      </div>
      ...
      <div id={detailId} hidden={!expanded}>
        {expanded && <>
          ...
          {canManage && <ProgressEditor post={post} onUpdated={onUpdated} />}
          <FeedbackThread key={`${post.id}:${userId ?? "guest"}`} postId={post.id} userId={userId} revision={post.replyCount} onAdded={(reply) => onUpdated({ replyCount: post.replyCount + 1, ...(reply.isOfficial ? { status: "answered", answeredAt: reply.createdAt } : {}) })} />
        </>}
      </div>
    </article>
  </li>;
