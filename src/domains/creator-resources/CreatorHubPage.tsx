...
export function CreatorHubPage() {
  const { workspace, update, restore, error } = useCreatorWorkspace();
  ...
  const importBackup = async (file: File | undefined) => {
    ...
    try {
      ...
      parseWorkspace(raw);
      const question = restoreMode === "merge"
        ? "현재 자료와 작성한 기획서는 유지하고, 백업의 새 자료·빈 기획 항목·체크 항목을 합칠까요?"
        : "현재 창작 보드·기획서·체크리스트를 모두 이 백업으로 대체할까요? 먼저 현재 보드를 백업하는 것을 권장합니다.";
      if (!window.confirm(question)) return;
      if (restore(raw, restoreMode)) setImportNotice(restoreMode === "merge" ? "현재 작업을 유지하고 백업을 합쳤습니다." : "백업으로 보드를 대체했습니다.");
  ...
  return <ResourceLayout title="아이디어를 다음 작업으로" intro="소재를 모으고, 이야기를 설계하고, 한 장면을 완성하세요. 외부 자료는 출처와 이용조건을 함께 확인합니다.">
    ...
    <section className="space-y-4 rounded-2xl border border-line p-6" aria-labelledby="hub-saved-title">
      ...
      <label htmlFor="creator-board-replace" className="flex min-h-11 items-center gap-3 text-sm">
        <input id="creator-board-replace" type="checkbox" className="size-5" checked={restoreMode === "replace"} onChange={(event) => setRestoreMode(event.target.checked ? "replace" : "merge")} />
    ...
    <SavedBoard items={workspace.saved} onRemove={(id) => update((value) => ({ ...value, saved: value.saved.filter((item) => item.id !== id) }))} />
