...
/**
 * Records which viewer the store is serving and reports whether that changed.
 *
 * The write lives out here rather than in the effect because these stores are module-scoped and
 * long-lived — mutating one is the whole point of an external store — but inside a component the
 * React Compiler cannot tell that apart from mutating a value it owns, and flags it.
 */
function adoptStoreViewer(store: MarketSocialStore, viewerKey: string): boolean {
...
export function useMarketSocial(
  ...
  useEffect(() => {
    void loadStore(store, adoptStoreViewer(store, normalizedViewerKey));
