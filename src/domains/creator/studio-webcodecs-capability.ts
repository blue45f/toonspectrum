/**
 * Studio WebCodecs — 코덱 탐지(capability probing)와 폴백 계약.
 *
 ...
 *
 * 이 모듈이 지키는 계약 두 가지:
 *  1) **하드 실패 금지.** WebCodecs가 없거나 코덱이 하나도 안 되면 기존 경로(MediaRecorder WebM,
 *     순수 TS GIF/APNG 인코더)로 조용히 내려간다. `selectExportPipeline`이 항상 결정을 돌려준다.
 ...
 *
 * 순수 부분(코덱 문자열 산출·비트레이트 권장·폴백 선택)은 전부 DOM 무의존이고, 브라우저 접촉은
...
/** 실브라우저 기본 프로브 — WebCodecs가 없으면 null(호출부가 폴백 경로를 고른다). */
...
export interface StudioCodecProbeResult {
  ...
  webmCodecId: WebmVideoCodecId;
  bitrate: number;
  /** prefer-hardware 설정으로 지원 판정을 받았는가. */
...
/**
 * 최종 코덱 선택 — 하드웨어 가속 후보가 있으면 그중 hardwareRank 최우선, 없으면 소프트웨어
 * softwareRank 최우선. 지원 코덱이 없으면 null(폴백 계약이 받는다).
 */
export async function selectStudioVideoCodec(
  ...
  const rankOf = (result: StudioCodecProbeResult): number => {
    ...
    return result.hardwareAccelerated ? candidate.hardwareRank : candidate.softwareRank;
  };
  const hardware = results.filter((r) => r.hardwareAccelerated);
  const pool = hardware.length > 0 ? hardware : results;
...
export interface StudioExportPipelineInput {
  ...
  /** 순수 인코더까지 내려갔을 때 어떤 이미지 포맷을 쓸지(기본 gif). */
...
/**
 * 사용 가능한 최선의 내보내기 경로를 고른다. **절대 throw하지 않는다** — 아무 경로도 없으면
 * available=false인 결정을 돌려주고, 호출부는 그걸 UI 안내로 바꾼다.
 */
export function selectExportPipeline(input: StudioExportPipelineInput): StudioExportPipelineDecision {
  if (!input.disableWebCodecs && input.webCodecsCodec) {
    ...
    return {
      ...
      available: true,
      degraded: false,
      reason: `${codec.label} ${codec.hardwareAccelerated ? "하드웨어" : "소프트웨어"} 인코더로 빠르게 내보내요.`,
    };
  }
  const webCodecsReason = input.disableWebCodecs
    ? "빠른 인코딩(WebCodecs)이 설정에서 꺼져 있어요."
    : "이 브라우저는 빠른 인코딩(WebCodecs)을 지원하지 않아요.";
  if (input.mediaRecorderSupported) {
    return {
      ...
      available: true,
      degraded: true,
      reason: `${webCodecsReason} 기존 실시간 녹화 방식으로 내보내요(영상 길이만큼 시간이 걸려요).`,
    };
  }
  if (input.pureEncoderSupported) {
    const format = input.preferredImageFormat ?? "gif";
    return {
      pipeline: format,
      codec: null,
      available: true,
      degraded: true,
      reason: `${webCodecsReason} 영상 녹화도 지원되지 않아 ${format.toUpperCase()}로 내보내요.`,
    };
  }
  return {
    pipeline: input.preferredImageFormat ?? "gif",
    codec: null,
    available: false,
    degraded: true,
    reason: `${webCodecsReason} 이 브라우저에서는 애니메이션 내보내기를 쓸 수 없어요.`,
  };
}
