...
export const __probe: {
  trace: number[] | null;
  film: Float32Array | null;
  ridge: Float32Array | null;
  grid: ImpastoReliefGrid | null;
} = { trace: null, film: null, ridge: null, grid: null };
...
function stampImpastoFilm(
  ...
  for (let index = from; index < to; index += 1) {
    ...
    cursor.travelled = 0;
    if (__probe.trace) __probe.trace.push(index);
...
function shadeImpastoRelief(
  ...
  __probe.film = new Float32Array(film);
  __probe.ridge = new Float32Array(ridge);
  __probe.grid = grid;
