...
describe("quality-first Studio brush portfolio", () => {
  it("reduces the default picker to 48 materially distinct representatives", () => {
    ...
    expect(STUDIO_LISTED_ALL_BRUSH_CATALOG_ITEMS.map((item) => item.id))
    ...
    expect(STUDIO_LISTED_ALL_BRUSH_CATALOG_ITEMS.length / STUDIO_ALL_BRUSH_CATALOG_ITEMS.length)
  ...
  it("keeps absorbed variants out of the default list but searchable and resolvable", () => {
    const defaultIds = new Set(STUDIO_LISTED_ALL_BRUSH_CATALOG_ITEMS.map((item) => item.id));
  ...
  it("preserves explicit hidden favorites and recents on user-owned lanes", () => {
