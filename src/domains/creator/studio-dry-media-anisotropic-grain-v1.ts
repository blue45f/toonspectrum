...
export const STUDIO_DRY_MEDIA_ANISOTROPIC_PRESETS_V1 = Object.freeze({
  crayon: freezePreset({
    ...
    // Product dry media is transported as connected pigment lanes. Keep each lane broad enough
    // to overlap its neighbours, while reserving paper tooth for the union carrier's subtractive
    // micro grain instead of exposing isolated wax lozenges.
    halfLengthRatio: { minimum: 0.5, maximum: 0.7 },
    halfThicknessRatio: { minimum: 0.11, maximum: 0.16 },
    minimumAspectRatio: 3.05,
    lateralSpread: 0.34,
    longitudinalJitter: 0.08,
    orientationJitterRadians: 0.045,
