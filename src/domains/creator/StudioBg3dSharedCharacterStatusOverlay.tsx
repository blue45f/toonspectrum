...
export function StudioBg3dSharedCharacterStatusOverlay({
  ...
  const headline = unavailableCount > 0
  ...
  return (
    <div
      ...
      className="pointer-events-none absolute left-2 top-2 z-30 max-w-[min(88%,24rem)] rounded-lg border border-line/80 bg-panel/92 px-2.5 py-2 text-[0.68rem] leading-relaxed text-fg-2 shadow-lg backdrop-blur sm:left-3 sm:top-3"
    >
      <p className="font-bold text-fg">{headline}</p>
      <p>배경 카메라·조명을 함께 사용하며, 포즈 원본은 각 캐릭터 레이어에 그대로 보존돼요.</p>
      {previewOmissionCount > 0 ? (
