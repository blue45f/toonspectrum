...
export function generateBrushLabVariants(
  ...
  const mutable = base.slots.filter((slot) => !slot.locked);
  ...
  for (let attempt = 0; attempt < count * 48 && result.length < count; attempt++) {
    const available = mutable.map((slot) => slot.id);
    ...
    for (let index = 0; index < mutations && available.length; index++) {
      ...
      replacements.set(selected, donors[Math.floor(random() * donors.length)]!);
    }
...
export function commitBrushLabHistory<T>(history: BrushLabHistory<T>, next: T): BrushLabHistory<T> {
  ...
  return { past: [...history.past, history.present].slice(-BRUSH_LAB_HISTORY_LIMIT), present: next, future: [] };
}
export function moveBrushLabHistory<T>(history: BrushLabHistory<T>, direction: "undo" | "redo"): BrushLabHistory<T> {
  if (direction === "undo") {
    if (!history.past.length) return history;
    return {
      past: history.past.slice(0, -1), present: history.past[history.past.length - 1]!,
      future: [history.present, ...history.future].slice(0, BRUSH_LAB_HISTORY_LIMIT),
    };
  }
  if (!history.future.length) return history;
  return {
    past: [...history.past, history.present].slice(-BRUSH_LAB_HISTORY_LIMIT),
