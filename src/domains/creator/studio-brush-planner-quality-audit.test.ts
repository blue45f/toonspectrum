...
describe("studio brush planner quality audit", () => {
  it("deterministically audits all 214 shipped presets without planner failures", () => {
    const replay = auditStudioBrushPlannerQualityCatalogue(SHIPPED_CANDIDATES);

    expect(SHIPPED_CANDIDATES).toHaveLength(214);
    expect(SHIPPED_REPORT.results).toHaveLength(214);
    expect(new Set(SHIPPED_REPORT.results.map((result) => result.catalogId)).size).toBe(214);
