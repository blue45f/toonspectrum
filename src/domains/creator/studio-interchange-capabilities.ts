...
const STUDIO_INTERCHANGE_CAPABILITY_DEFINITIONS: readonly StudioInterchangeCapabilityDefinition[] = [
  ...
  },
  {
    ...
    lossModel: [
      ...
      "text·image·pattern·mask·filter·marker·use/symbol·외부 참조·objectBoundingBox 또는 중첩 clip은 native 경로에서 절대 생략하지 않고 SceneIR/resvg/신뢰된 원본 SVG로 명시적으로 우회",
    ...
    runtimeRequirement: [
      ...
      "CanvasKit SceneIR candidate with explicit resvg fallback",
      "resvg-wasm visual reference",
    ],
    sizeBudget: {
      maxFileBytes: 2 * MiB,
    ...
    recommendedBridge: ["완전한 SVG 편집은 원본을 보관하고, 지원 부분집합만 SceneIR로 편집하며 나머지는 resvg/reference raster로 표시"],
    technicalLayers: {
    ...
    implementation: {
      ...
      export: "implemented",
      notes: [
        "vello_svg strict audit + vello_cpu render + per-asset resvg visual gate가 제품 라우터에 연결됨",
        "FormatGateway SceneIR warning/unsupported가 0일 때만 editable 후보이며 CanvasKit 실패는 resvg로 표면화하여 우회",
      ],
    },
    uiWiring: {
      import: "partial",
