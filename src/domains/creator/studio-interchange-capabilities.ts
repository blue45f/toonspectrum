...
export const STUDIO_INTERCHANGE_CAPABILITIES: readonly StudioInterchangeCapability[] = Object.freeze([
  ...
  },
  {
    ...
    sizeBudget: { notes: "문서/캔버스 안전 한도와 브라우저 메모리 한도를 함께 적용" },
    status: "partial",
    notes: ["레이어 기반 교환은 가능하지만 Photoshop/CSP와 완전한 편집 왕복은 아닙니다."],
  },
  {
    id: "svg",
    label: "SVG",
  ...
  })),
  {
    ...
    export: "available",
    roundTrip: "partial",
    lossModel: ["OpenRaster가 표현하지 못하는 Studio 전용 요소/효과는 PNG 레이어로 렌더"],
    runtimeRequirement: ["bounded ZIP32 reader/writer", "strict XML parser", "PNG codec"],
    sizeBudget: {
      maxFileBytes: 520_000_000,
      maxDecodedBytes: 128 * MiB,
      maxDimensionPx: 32_768,
      maxFiles: 516,
      notes: "PNG 한 장당 최대 16,777,216픽셀, 전체 디코딩 RGBA 최대 128MiB를 적용",
    },
    status: "partial",
    notes: [
      "현재 내보내기 메뉴는 화면과 같은 합성 1레이어 ORA를 저장하며, 다층 import/export 코어는 UI 연결 전입니다.",
      "가져오기와 내보내기 모두 PNG IHDR를 선검증하고 보수적인 RGBA 메모리 예산을 초과하면 디코딩 전에 중단합니다.",
    ],
  },
  {
    id: "cbz",
    ...
    export: "available",
    roundTrip: "none",
    lossModel: ["페이지가 순서 지정 PNG/JPEG로 평탄화됨"],
    runtimeRequirement: ["ZIP writer", "Canvas image encoder"],
    sizeBudget: { notes: "CBZ codec의 항목/총 바이트 예산 적용" },
    status: "partial",
    notes: ["전체 페이지와 ComicInfo.xml을 실제 내보내기 메뉴에서 저장합니다. 가져오기 코어는 UI 연결 전입니다."],
  },
  {
