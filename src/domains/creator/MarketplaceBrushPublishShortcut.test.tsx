...
import {
  findStoredBrushSnapshotForMarketplace,
...
describe("MarketplaceBrushPublishShortcut", () => {
  ...
  it("ignores malformed and non-brush cache entries", () => {
    const storage = createStorage({
  ...
  it("returns null when an in-app browser blocks storage metadata", () => {
    const storage = {
      get length() {
  ...
  it("skips one inaccessible cache entry and keeps scanning", () => {
    ...
    const storage = {
      ...entries,
      getItem(key: string) {
        if (key === "studio-brush-blocked") throw new DOMException("Blocked", "SecurityError");
  ...
  it("exposes a stable accessible publishing action and recovers after a provider error", async () => {
    ...
    const button = screen.getByTestId("brush-studio-marketplace-publish");
    expect(button).toHaveAttribute("aria-busy", "false");
    expect(screen.getByRole("status")).toHaveTextContent("현재 브러시 원본을 보존해 등록합니다.");

    fireEvent.click(button);
    await waitFor(() => {
      expect(screen.getByRole("status")).toHaveTextContent("현재 브러시를 읽지 못했습니다.");
      expect(button).toHaveAttribute("aria-busy", "false");
