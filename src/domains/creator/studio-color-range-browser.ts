 ...
 * — 그대로 applyColorRangeMaskToSelection / PixelSelection 에 먹이면 된다.
 */
import {
  buildColorRangeMask,
  flipColorRangeMask,
 ...
 * 스캔은 마술봉과 같은 다운스케일 상한(MAGIC_WAND_TRACE_MAX_DIM)에서 돌므로 최악 비용이 유계다
 * (전 픽셀 1패스 + region 캡 — 마술봉 Worker 폴백의 동기 비용과 같은 수준이라 메인 스레드 유지).
 */
