...
/**
 * Non-React lifecycle owner for remote CRDT asset bodies. It deduplicates same-reference loads,
 * exposes an immediate placeholder, aborts requests removed from the frontier, and never lets a
 * late response from a previous work/scope replace current state. Object URLs are revoked on
 * removal, retry, work switch, and disposal.
 */
export class StudioWorkAssetHydrator {
  ...
  readySources(): ReadonlyMap<string, StudioWorkAssetHydratedSource> {
    const sources = new Map<string, StudioWorkAssetHydratedSource>();
  ...
  private start(reference: StudioWorkAssetReference): void {
    ...
    void this.download(workId, reference, controller.signal)
      .then(({ manifest, blob }) => {
        ...
        try {
          residentByteCost = studioWorkAssetResidentByteCost(manifest, blob.size);
        } catch (error) {
