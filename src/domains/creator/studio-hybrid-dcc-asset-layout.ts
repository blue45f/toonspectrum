/**
 * Deterministic, renderer-free inspection layout for Hybrid DCC authority meshes.
 *
...
export function deriveStudioHybridDccAssetLayout(
  ...
  if (valid.length === 0) {
    return {
      ...
      radius: 2,
      gridSize: 12,
      signature: sorted.map((item) => `${item.assetId}:${item.meshHash}`).join("|"),
    };
  }
  ...
  return {
    ...
    signature: items.map((item) => `${item.assetId}:${item.meshHash}`).join("|"),
  };
}
