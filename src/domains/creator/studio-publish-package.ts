...
function normalizeArtifactFileName(value: unknown, role: StudioPublishArtifactRole, index: number): string {
...
function normalizeManifestArtifact(value: unknown, index: number): StudioPublishPackagePublicArtifact | null {
  ...
  return {
    ...
    state,
    ...(mimeType ? { mimeType } : {}),
    ...(safeOptionalPositiveInteger(value.width, STUDIO_PUBLISH_PACKAGE_LIMITS.maxCanvasDimension) !== undefined
      ? { width: value.width as number }
      : {}),
    ...(safeOptionalPositiveInteger(value.height, STUDIO_PUBLISH_PACKAGE_LIMITS.maxCanvasDimension) !== undefined
      ? { height: value.height as number }
      : {}),
    ...(safeOptionalPositiveInteger(value.byteSize ?? value.bytes, STUDIO_PUBLISH_PACKAGE_LIMITS.maxByteSize) !== undefined
...
export function parseStudioPublishPackageManifest(value: unknown): StudioPublishPackageManifest {
  ...
  const usage = normalizeAiUsage(aiValue.usage ?? aiValue.aiUsage);
  ...
  return {
    ...
    preset: {
      ...
      revision: normalizeText(
        isRecord(decoded.preset) ? decoded.preset.revision : undefined,
        120
      ) || preset.revision,
      policySnapshotDate:
        normalizeText(
