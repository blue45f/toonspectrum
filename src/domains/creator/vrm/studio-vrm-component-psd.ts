...
function containsControlCharacter(value: string): boolean {
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (code <= MAX_CONTROL_CHARACTER_CODE || code === DELETE_CHARACTER_CODE) return true;
  }
  return false;
}
...
function safeIdentifier(value: string, label: string): string {
  ...
  if (!resolved || resolved.length > 160 || containsControlCharacter(resolved)) {
...
function preparePasses(
  ...
  return Object.freeze(passes.map((pass) => {
    ...
    if (
      !(pass.rgba instanceof Uint8Array || pass.rgba instanceof Uint8ClampedArray)
      || pass.rgba.byteLength !== expectedBytes
    ) {
...
function validateScene(raw: StudioVrmLinkedSceneDescriptor): StudioVrmLinkedSceneDescriptor {
...
export function studioVrmLinkedRenderNeedsRefresh(
  ...
  const scene = validateScene(next);
  return (
