...
import { buildStudioVrmGarmentWeaveTile, STUDIO_VRM_GARMENT_WEAVE_SIZE, studioVrmGarmentWeaveReliefM } from "./studio-vrm-garment-weave";
 ...
 *
 * 좌표계는 나머지 치수와 같은 spine 로컬 rest 공간이다. 지배 본은 몸통 영역 분류에만
 * 사용하며 위치는 실제 렌더러처럼 전체 스킨 가중치와 활성 모프를 적용한다. 체형을 바꾼 뒤
 * 화면에 보이는 몸과 재단 기준이 같은 형상을 유지한다.
 */
function measureTorsoSilhouette(vrm: VRM, anchors: {
  ...
  vrm.scene.traverse((object) => {
    ...
    // Bone identity classifies the region; the renderer owns the actual weighted deformation.
...
function createGarmentWeaveTexture(fabricId: WardrobeEquip["fabricId"]): THREE.DataTexture | null {
  const data = buildStudioVrmGarmentWeaveTile(fabricId);
  if (!data) return null;
...
function applyGarmentMaterialStyle(
  ...
  material.bumpScale = fabricSurface ? studioVrmGarmentWeaveReliefM(fabricId) : 0;
