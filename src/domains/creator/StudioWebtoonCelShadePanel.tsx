 ...
 * - Algorithmic 5-step Comic Hue-Shift Shadow: Prevents dirty muddy gray shadows by rotating hue toward cool indigo.
...
export function StudioWebtoonCelShadePanel({
  ...
  const celShadeSteps = [
    { label: "하이라이트", hex: celShades.highlight, desc: "따뜻한 광원 (-12°)" },
    { label: "밑색 (현재)", hex: celShades.base, desc: "기본 지정색" },
    { label: "1차 음영", hex: celShades.celShadow1, desc: "쿨톤 쉬프트 (+25°)" },
    { label: "2차 딥음영", hex: celShades.celShadow2, desc: "깊은 음영 (+42°)" },
    { label: "생기 틴트", hex: celShades.blushTint, desc: "홍조·틴트" },
    { label: "림라이트", hex: celShades.rimLight, desc: "외곽 역광" },
  ];
    ...
    <div className="flex flex-col gap-2.5">
      ...
      >
        <button
          ...
          className={`flex-1 rounded py-1 text-[0.65rem] font-medium transition-all ${
            subTab === "hue-shift"
      ...
      {subTab === "hue-shift" ? (
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <span className="text-[0.64rem] font-medium text-fg-2">
              색상환 회전 기반 맑은 음영 (탁한 회색 방지)
            </span>
            <span className="rounded bg-accent/15 px-1.5 py-0.5 text-[0.58rem] font-semibold text-accent">
              Anti-Muddy
            </span>
          </div>
            ...
            {celShadeSteps.map((step) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-1 rounded-lg border p-1.5 transition-all hover:scale-[1.03] active:scale-[0.98] ${
                    isSelected
                      ? "border-accent bg-accent-soft/30 ring-1 ring-accent"
                      : "border-line/70 bg-card/60 hover:bg-card hover:border-line-strong"
                  }`}
                >
                  <span className="text-[0.58rem] font-medium text-fg-3">{step.label}</span>
                  <div
                    className="relative size-7 rounded-md border border-line shadow-inner"
                    style={{ backgroundColor: step.hex }}
                  >
                    {isSelected && (
                      <span className="absolute inset-0 flex items-center justify-center">
                        <Check className="size-3 text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.9)]" />
                      </span>
                    )}
                  </div>
                  <span className="font-mono text-[0.58rem] text-fg-2">{step.hex}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="flex flex-col gap-2">
          <p className="text-[0.64rem] text-fg-3">
          ...
          <div className="space-y-1.5">
            {WEBTOON_CHARACTER_SKIN_PRESETS.map((preset) => (
              <div
                key={preset.id}
                className="flex flex-col gap-1 rounded-lg border border-line bg-card/50 p-2"
              >
                <div className="flex items-center justify-between">
                  <span className="text-[0.66rem] font-semibold text-fg-1">{preset.name}</span>
                  <span className="text-[0.58rem] text-fg-3">{preset.role}</span>
                </div>
                <div className="flex items-center gap-1.5" role="radiogroup" aria-label={`${preset.name} 피부톤 색상`}>
                  {preset.colors.map((hex, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`size-6 rounded-md border border-line transition-all hover:scale-110 ${
